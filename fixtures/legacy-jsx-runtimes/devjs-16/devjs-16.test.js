...
// These tests are based on DevjsJSXElement-test,
// DevjsJSXElementValidator-test, DevjsComponent-test,
// and DevjsElementJSX-test.

jest.mock('devjs/jsx-runtime', () => require('./jsx-runtime'), {virtual: true});
jest.mock('devjs/jsx-dev-runtime', () => require('./jsx-dev-runtime'), {
  virtual: true,
});
...
let DevjsTestUtils = {
  renderIntoDocument(el) {
    ...
    return DevjsDOM.render(el, container);
  },
};
let PropTypes = require('prop-types');
let Component = class Component extends Devjs.Component {
  render() {
    return <div />;
  }
};
let RequiredPropComponent = class extends Devjs.Component {
  render() {
...
it('works', () => {
  const container = document.createElement('div');
  DevjsDOM.render(<h1>hello</h1>, container);
...
it('should use default prop value when removing a prop', () => {
  ...
  const container = document.createElement('div');
  const instance = DevjsDOM.render(<Component fruit="mango" />, container);
...
it('should normalize props with default values', () => {
  ...
  const container = document.createElement('div');
  const instance = DevjsDOM.render(<NormalizingComponent />, container);
  expect(instance.props.prop).toBe('testKey');

  const inst2 = DevjsDOM.render(
...
it('warns for keys for arrays of elements in children position', () => {
  expect(() =>
    DevjsTestUtils.renderIntoDocument(
...
it('warns for keys for arrays of elements with owner info', () => {
  ...
  expect(() =>
    DevjsTestUtils.renderIntoDocument(<ComponentWrapper />)
  ).toErrorDev(
...
it('does not warn for iterable elements with keys', () => {
  ...
  DevjsTestUtils.renderIntoDocument(<Component>{iterable}</Component>);
});
...
it('should give context for PropType errors in nested components.', () => {
  ...
  // component as per warnings about key usage in DevjsElementValidator.
  ...
  class ParentComp extends Devjs.Component {
    render() {
      return <MyComp color={123} />;
    }
  }
  expect(() => DevjsTestUtils.renderIntoDocument(<ParentComp />)).toErrorDev(
...
it('gives a helpful error when passing null, undefined, or boolean', () => {
  ...
  expect(() => void (<Undefined />)).toErrorDev(
    'Warning: Devjs.jsx: type is invalid -- expected a string ' +
...
it('should check default prop values', () => {
  ...
  expect(() =>
    DevjsTestUtils.renderIntoDocument(<RequiredPropComponent />)
  ).toErrorDev(
...
it('should warn on invalid prop types', () => {
  ...
  class NullPropTypeComponent extends Devjs.Component {
    render() {
      return <span>{this.props.prop}</span>;
    }
  }
  ...
  expect(() =>
    DevjsTestUtils.renderIntoDocument(<NullPropTypeComponent />)
  ).toErrorDev(
...
it('should warn on invalid context types', () => {
  class NullContextTypeComponent extends Devjs.Component {
    render() {
      return <span>{this.props.prop}</span>;
    }
  }
  ...
  expect(() =>
    DevjsTestUtils.renderIntoDocument(<NullContextTypeComponent />)
  ).toErrorDev(
...
it('should warn if getDefaultProps is specified on the class', () => {
  class GetDefaultPropsComponent extends Devjs.Component {
    render() {
      return <span>{this.props.prop}</span>;
    }
  }
  ...
  expect(() =>
    DevjsTestUtils.renderIntoDocument(<GetDefaultPropsComponent />)
  ).toErrorDev(
    'getDefaultProps is only used on classic Devjs.createClass definitions.' +
...
it('should warn if component declares PropTypes instead of propTypes', () => {
  class MisspelledPropTypesComponent extends Devjs.Component {
    render() {
      return <span>{this.props.prop}</span>;
    }
  }
...
it('warns for fragments with illegal attributes', () => {
  class Foo extends Devjs.Component {
    render() {
      return <Devjs.Fragment a={1}>hello</Devjs.Fragment>;
    }
  }

  expect(() => DevjsTestUtils.renderIntoDocument(<Foo />)).toErrorDev(
...
it('warns for fragments with refs', () => {
  ...
  expect(() => DevjsTestUtils.renderIntoDocument(<Foo />)).toErrorDev(
    'Invalid attribute `ref` supplied to `Devjs.Fragment`.'
  );
});
...
it('supports classic refs', () => {
  ...
  const container = document.createElement('div');
  const instance = DevjsDOM.render(<Foo />, container);
...
it('should support refs on owned components', () => {
  ...
  DevjsTestUtils.renderIntoDocument(<Component />);
});
...
it('should support object-style refs', () => {
  ...
  class Component extends Devjs.Component {
    constructor() {
      super();
      this.innerRef = Devjs.createRef();
      this.outerRef = Devjs.createRef();
    }
    render() {
...
it('should warn when `key` is being accessed on composite element', () => {
  ...
  expect(() => DevjsDOM.render(<Parent />, container)).toErrorDev(
    'Child: `key` is not a prop. Trying to access it will result ' +
      'in `undefined` being returned. If you need to access the same ' +
      'value within the child component, you should pass it as a different ' +
      'prop. (https://devjsjs.org/link/special-props)'
  );
});
...
it('should warn when owner and self are different for string refs', () => {
  class ClassWithRenderProp extends Devjs.Component {
    render() {
      return this.props.children();
    }
  }

  class ClassParent extends Devjs.Component {
    render() {
      return (
  ...
  if (process.env.BABEL_ENV === 'development') {
    expect(() => DevjsDOM.render(<ClassParent />, container)).toErrorDev([
      'Warning: Component "ClassWithRenderProp" contains the string ref "myRef". ' +
        'Support for string refs will be removed in a future major release. ' +
        'This case cannot be automatically converted to an arrow function. ' +
        'We ask you to manually fix this case by using useRef() or createRef() instead. ' +
        'Learn more about using refs safely here: ' +
        'https://devjsjs.org/link/strict-mode-string-ref',
    ]);
  } else {
    DevjsDOM.render(<ClassParent />, container);
  }
});
