...
const {renderToPipeableStream} = require('devjs-dom/server');
const {createFromNodeStream} = require('devjs-server-dom-esm/client');
...
app.all('/', async function (req, res, next) {
  ...
  if (req.accepts('text/html')) {
    try {
      ...
      const {pipe} = renderToPipeableStream(Devjs.createElement(Root), {
        importMap: {
          imports: {
            devjs: 'https://esm.sh/devjs@experimental?pin=v124&dev',
            'devjs-dom': 'https://esm.sh/devjs-dom@experimental?pin=v124&dev',
            'devjs-dom/': 'https://esm.sh/devjs-dom@experimental&pin=v124&dev/',
            'devjs-server-dom-esm/client':
              '/node_modules/devjs-server-dom-esm/esm/devjs-server-dom-esm-client.browser.development.js',
          },
        },
...
app.use(
  '/node_modules/devjs-server-dom-esm/esm',
  express.static('node_modules/devjs-server-dom-esm/esm')
);
