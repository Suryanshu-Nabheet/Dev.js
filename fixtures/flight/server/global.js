...
const {createFromNodeStream} = require('devjs-server-dom-unbundled/client');
    ...
    try {
      ...
      const serverConsumerManifest = JSON.parse(
        await virtualFs.readFile(
          path.join(buildPath, 'devjs-ssr-manifest.json'),
          'utf8'
        )
      );
      ...
      let Root = () => {
        ...
        return Devjs.use(cachedResult).root;
      };
      ...
      const {pipe} = renderToPipeableStream(Devjs.createElement(Root), {
        ...
        onShellError(error) {
          const {pipe: pipeError} = renderToPipeableStream(
            Devjs.createElement('html', null, Devjs.createElement('body')),
            {
              bootstrapScripts: mainJSChunks,
            }
          );
