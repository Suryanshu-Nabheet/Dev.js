...
const register = require('devjs-server-dom-unbundled/node-register');
...
async function renderApp(res, returnValue, formState, noCache, debugChannel) {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    moduleMap = await (
      await fetch('http://localhost:3000/devjs-client-manifest.json')
    ).json();
    mainCSSChunks = (
      await (
    ...
    moduleMap = JSON.parse(
      await readFile(
        path.resolve(__dirname, `../build/devjs-client-manifest.json`),
        'utf8'
      )
    );
  ...
  const root = Devjs.createElement(
    ...
    Devjs.createElement(App, {noCache})
  );
...
async function prerenderApp(res, returnValue, formState, noCache) {
  ...
  const root = Devjs.createElement(
    ...
    Devjs.createElement(App, {prerender: true, noCache})
  );
  // For client-invoked server actions we refresh the tree and return a return value.
  const payload = {root, returnValue, formState};
...
app.post('/', bodyParser.text(), async function (req, res) {
  ...
  const {decodeReply, decodeReplyFromBusboy, decodeAction, decodeFormState} =
    await import('devjs-server-dom-unbundled/server');
  ...
  if (serverReference) {
    ...
    if (action.$$typeof !== Symbol.for('devjs.server.reference')) {
