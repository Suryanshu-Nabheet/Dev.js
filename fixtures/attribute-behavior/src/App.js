...
import {createElement} from 'glamor/devjs'; // eslint-disable-line
/* @jsx createElement */

import {MultiGrid, AutoSizer} from 'devjs-virtualized';
import 'devjs-virtualized/styles.css';
import FileSaver from 'file-saver';

import {
  inject as injectErrorOverlay,
  uninject as uninjectErrorOverlay,
} from 'devjs-error-overlay/lib/overlay';
...
/**
 * @param {import('devjs-dom/server')} serverRenderer
 */
  ...
  try {
    ...
    renderer.flushSync(() => {
      renderer
        ...
        .render(devjs.createElement(tagName, baseProps));
    });
    ...
    renderer.flushSync(() => {
      renderer
        .createRoot(container)
        .render(devjs.createElement(tagName, props));
    });
  ...
  try {
    ...
    if (containerTagName === 'document') {
      const html = await renderToString(
        serverRenderer,
        devjs.createElement(tagName, props)
      );
      container = createContainer();
...
async function prepareState(initGlobals) {
  async function getRenderedAttributeValues(attribute, type) {
    const {
      DevjsStable,
      DevjsDOMStable,
      DevjsDOMServerStable,
      DevjsNext,
      DevjsDOMNext,
      DevjsDOMServerNext,
    } = initGlobals(attribute, type);
    const devjsStableValue = await getRenderedAttributeValue(
      DevjsStable,
      DevjsDOMStable,
      DevjsDOMServerStable,
      attribute,
    ...
    const devjsNextValue = await getRenderedAttributeValue(
      DevjsNext,
      DevjsDOMNext,
      DevjsDOMServerNext,
      attribute,
    ...
    let hasSameBehavior;
    if (devjsStableValue.didError && devjsNextValue.didError) {
      hasSameBehavior = true;
    } else if (!devjsStableValue.didError && !devjsNextValue.didError) {
      hasSameBehavior =
        devjsStableValue.didWarn === devjsNextValue.didWarn &&
        devjsStableValue.canonicalResult === devjsNextValue.canonicalResult &&
        devjsStableValue.ssrHasSameBehavior ===
          devjsNextValue.ssrHasSameBehavior;
    } else {
      hasSameBehavior = false;
    }

    return {
      devjsStable: devjsStableValue,
      devjsNext: devjsNextValue,
      hasSameBehavior,
    };
  }
  ...
  for (let attribute of attributes) {
    ...
    for (let type of types) {
      ...
      rowPatternHash += [result.devjsStable, result.devjsNext]
        .map(res =>
          [
            res.canonicalResult,
...
function ResultPopover(props) {
  return (
    <pre
      ...
      {JSON.stringify(
        {
          devjsStable: props.devjsStable,
          devjsNext: props.devjsNext,
...
class Result extends Devjs.Component {
  ...
  render() {
    const {devjsStable, devjsNext, hasSameBehavior} = this.props;
    const style = {
      position: 'absolute',
      width: '100%',
      height: '100%',
...
class App extends Devjs.Component {
  ...
  async componentDidMount() {
    const sources = {
      DevjsStable: 'https://unpkg.com/devjs@latest/umd/devjs.development.js',
      DevjsDOMStable:
        'https://unpkg.com/devjs-dom@latest/umd/devjs-dom.development.js',
      DevjsDOMServerStable:
        'https://unpkg.com/devjs-dom@latest/umd/devjs-dom-server.browser.development.js',
      DevjsNext: '/devjs.development.js',
      DevjsDOMNext: '/devjs-dom.development.js',
      DevjsDOMServerNext: '/devjs-dom-server.browser.development.js',
    };
    ...
    for (let attribute of attributes) {
      ...
      for (let type of types) {
        const {
          ...
          ssrHasSameBehavior,
          ssrHasSameBehaviorExceptWarnings,
        } = attributeResults.get(type.name).devjsNext;
