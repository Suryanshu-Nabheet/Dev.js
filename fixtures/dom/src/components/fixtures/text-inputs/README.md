...
**Devjs detaches all inputs**. This prevents `value` from accidentally updating if
`defaultValue` changes.

## Form reset events

Devjs does not support `form.reset()` for controlled inputs. This is a feature,
not a bug. `form.reset()` works by reverting an input's `value` _property_ to
that of the current `defaultValue`. Since Devjs assigns the value `attribute`
...
https://github.com/Suryanshu-Nabheet/dev.js/pull/7359#issuecomment-256499693
