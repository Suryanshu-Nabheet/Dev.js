...
// app. This allows rendering for versions of Devjs older than the DOM
// test fixtures can support.
const initialVersion = qs.parse(window.location.search).version || 'local';

class Hydration extends Devjs.Component {
  state = {
    error: null,
    code: SAMPLE_CODE,
    hydrate: true,
  ...
  render() {
    ...
    const src =
      '/renderer.html?' + qs.stringify({hydrate, ...devjsPaths(version)});
