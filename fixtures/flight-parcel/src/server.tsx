...
import {
  ...
  decodeReply,
  decodeAction,
} from 'devjs-server-dom-parcel/server.edge';
import {injectRSCPayload} from 'rsc-html-stream/server';

// Client dependencies, used for SSR.
// These must run in the same environment as client components (e.g. same instance of Devjs).
import {createFromReadableStream} from 'devjs-server-dom-parcel/client.edge' with {env: 'devjs-client'};
import {renderToReadableStream as renderHTMLToReadableStream} from 'devjs-dom/server.edge' with {env: 'devjs-client'};
import DevjsClient, {DevjsElement} from 'devjs' with {env: 'devjs-client'};
...
async function render(
  ...
  if (req.accepts('text/html')) {
    ...
    // Use client devjs to render the RSC payload to HTML.
    let [s1, s2] = stream.tee();
    let data: Promise<DevjsElement>;
    function Content() {
      data ??= createFromReadableStream<DevjsElement>(s1);
      return DevjsClient.use(data);
    }
