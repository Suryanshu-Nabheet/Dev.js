...
import {
  ...
  encodeReply,
  setServerCallback,
} from 'devjs-server-dom-parcel/client';
import {rscStream} from 'rsc-html-stream/client';

// Stream in initial RSC payload embedded in the HTML.
let initialRSCPayload = createFromReadableStream<DevjsElement>(rscStream);
let updateRoot:
  | ((root: DevjsElement, cb?: (() => void) | null) => void)
  | null = null;

function Content() {
  // Store the current root element in state, along with a callback
  // to call once rendering is complete.
  let [[root, cb], setRoot] = useState<[DevjsElement, (() => void) | null]>([
...
startTransition(() => {
  DevjsDOM.hydrateRoot(document, <Content />);
});

// A very simple router. When we navigate, we'll fetch a new RSC payload from the server,
// and in a Devjs transition, stream in the new page. Once complete, we'll pushState to
...
async function navigate(pathname: string, push = false) {
  ...
  let root = await createFromFetch<DevjsElement>(res);
  startTransition(() => {
    updateRoot!(root, () => {
      if (push) {
