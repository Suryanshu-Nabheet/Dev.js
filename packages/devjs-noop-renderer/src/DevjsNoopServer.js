...
import DevjsFizzServer from 'devjs-server';
...
const DevjsNoopServer = DevjsFizzServer({
...
function render(children: Devjs$Element<any>, options?: Options): Destination {
  const destination: Destination = {
    ...
    stack: [],
    abort() {
      DevjsNoopServer.abort(request);
    },
  };
  const request = DevjsNoopServer.createRequest(
    children,
    null,
    null,
    null,
  ...
  DevjsNoopServer.startWork(request);
  DevjsNoopServer.startFlowing(request, destination);
  return destination;
}
