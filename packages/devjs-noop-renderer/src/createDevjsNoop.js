...
import type {UpdateQueue} from 'devjs-reconciler/src/DevjsFiberClassUpdateQueue';
import type {DevjsNodeList} from 'shared/DevjsTypes';
import type {RootTag} from 'devjs-reconciler/src/DevjsRootTags';
...
import {devjs_FRAGMENT_TYPE, devjs_ELEMENT_TYPE} from 'shared/DevjsSymbols';
...
function createDevjsNoop(reconciler: Function, useMutation: boolean) {
  ...
  const sharedHostConfig = {
    rendererVersion: DevjsVersion,
    rendererPackageName: 'devjs-noop',
    ...
    mayResourceSuspendCommit(resource: mixed): boolean {
      throw new Error(
        'Resources are not implemented for Devjs Noop yet. This method should not be called',
      );
    },
    ...
    preloadInstance(instance: Instance, type: string, props: Props): boolean {
      ...
      // instance is ready to be committed. If it's not, Devjs may yield to the
  ...
  function getChildrenAsJSX(root) {
    ...
    if (isArray(children)) {
      return createJSXElementForTestComparison(devjs_FRAGMENT_TYPE, {children});
    }
    return children;
  }
  ...
  const DevjsNoop = {
    ...
    getChildren(rootID: string = DEFAULT_ROOT_ID) {
      throw new Error(
        'No longer supported due to bad performance when used with `expect()`. ' +
          'Use `DevjsNoop.getChildrenAsJSX()` instead or, if you really need to, `dangerouslyGetChildren` after you carefully considered the warning in its JSDOC.',
      );
    },
    ...
    getPendingChildren(rootID: string = DEFAULT_ROOT_ID) {
      throw new Error(
        'No longer supported due to bad performance when used with `expect()`. ' +
          'Use `DevjsNoop.getPendingChildrenAsJSX()` instead or, if you really need to, `dangerouslyGetPendingChildren` after you carefully considered the warning in its JSDOC.',
      );
    },
    ...
    // TODO: Replace DevjsNoop.render with createRoot + root.render
    ...
    render(element: Devjs$Element<any>, callback: ?Function) {
      DevjsNoop.renderToRootWithID(element, DEFAULT_ROOT_ID, callback);
    },

    renderLegacySyncRoot(element: Devjs$Element<any>, callback: ?Function) {
      if (disableLegacyMode) {
        throw new Error('createLegacyRoot: Unsupported Legacy Mode API.');
      }
      const rootID = DEFAULT_ROOT_ID;
      const container = DevjsNoop.getOrCreateRootContainer(rootID, LegacyRoot);
    ...
    renderToRootWithID(
      ...
      const container = DevjsNoop.getOrCreateRootContainer(
        rootID,
        ConcurrentRoot,
      );
      const root = roots.get(container.rootID);
...
export default createDevjsNoop;
