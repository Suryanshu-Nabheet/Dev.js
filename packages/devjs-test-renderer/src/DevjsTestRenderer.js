...
import {findCurrentFiberUsingSlowPath} from 'devjs-reconciler/src/DevjsFiberTreeReflection';
import {
  Fragment,
  FunctionComponent,
  ClassComponent,
...
import {getPublicInstance} from './DevjsFiberConfigTestHost';
import {ConcurrentRoot, LegacyRoot} from 'devjs-reconciler/src/DevjsRootTags';
import {
  enableDevjsTestRendererWarning,
  disableLegacyMode,
} from 'shared/DevjsFeatureFlags';

import noop from 'shared/noop';
import type {WorkTag} from 'devjs-reconciler/src/DevjsWorkTags';
...
type DevjsTestRendererJSON = {
  ...
  children: null | Array<DevjsTestRendererNode>,
  $$typeof?: symbol, // Optional because we add it with defineProperty().
};
type DevjsTestRendererNode = DevjsTestRendererJSON | string;
...
export type Predicate = (node: DevjsTestInstance) => ?boolean;
...
function toJSON(inst: Instance | TextInstance): DevjsTestRendererNode | null {
  if (inst.isHidden) {
  ...
  switch (inst.tag) {
    ...
    case 'INSTANCE': {
      ...
      const json: DevjsTestRendererJSON = {
        type: inst.type,
        props: props,
        children: renderedChildren,
      };
      Object.defineProperty(json, '$$typeof', {
        value: Symbol.for('devjs.test.json'),
      });
      return json;
    }
    default:
...
class DevjsTestInstance {
  ...
  constructor(fiber: Fiber) {
    if (!validWrapperTypes.has(fiber.tag)) {
      throw new Error(
        `Unexpected object passed to DevjsTestInstance constructor (tag: ${fiber.tag}). ` +
  ...
  get children(): Array<DevjsTestInstance | string> {
    return getChildren(this._currentFiber());
  }

  // Custom search functions
  find(predicate: Predicate): DevjsTestInstance {
    return expectOne(
  ...
  findByType(type: any): DevjsTestInstance {
    return expectOne(
  ...
  findByProps(props: Object): DevjsTestInstance {
    return expectOne(
...
function create(
  ...
  root: void,
  toJSON(): Array<DevjsTestRendererNode> | DevjsTestRendererNode | null,
  toTree(): mixed,
  update(newElement: Devjs$Element<any>): any,
  unmount(): void,
  ...
  if (__DEV__) {
    if (
      enableDevjsTestRendererWarning === true &&
  ...
  const entry = {
    ...
    update(newElement: Devjs$Element<any>): number | void {
      if (root == null || root.current == null) {
        return;
      }
...
const fiberToWrapper = new WeakMap<Fiber, DevjsTestInstance>();
function wrapFiber(fiber: Fiber): DevjsTestInstance {
  let wrapper = fiberToWrapper.get(fiber);
  if (wrapper === undefined && fiber.alternate !== null) {
    wrapper = fiberToWrapper.get(fiber.alternate);
  }
  if (wrapper === undefined) {
    wrapper = new DevjsTestInstance(fiber);
    fiberToWrapper.set(fiber, wrapper);
  }
  return wrapper;
}

// Enable DevjsTestRenderer to be used to test DevTools integration.
injectIntoDevTools();
