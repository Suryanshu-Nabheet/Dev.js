...
describe('DevjsTestRendererAsync', () => {
  beforeEach(() => {
    jest.resetModules();
  ...
  it('flushAll flushes all work', async () => {
    ...
    const renderer = DevjsTestRenderer.create(<Foo>Hi</Foo>, {
  ...
  it('flushAll returns array of yielded values', async () => {
    ...
    const renderer = DevjsTestRenderer.create(<Parent step={1} />, {
      unstable_isConcurrent: true,
    });
  ...
  it('flushThrough flushes until the expected values is yielded', async () => {
    ...
    Devjs.startTransition(() => {
      renderer = DevjsTestRenderer.create(<Parent step={1} />, {
        unstable_isConcurrent: true,
      });
    });
  ...
  it('supports high priority interruptions', async () => {
    ...
    let renderer;
    Devjs.startTransition(() => {
      renderer = DevjsTestRenderer.create(<Example step={1} />, {
        unstable_isConcurrent: true,
      });
    });
