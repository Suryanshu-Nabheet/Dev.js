...
describe('SelectEventPlugin', () => {
  ...
  // See https://github.com/Suryanshu-Nabheet/dev.js/pull/3639 for details.
  it('does not get confused when dependent events are registered independently', async () => {
    ...
    const node = await (async function () {
      await act(() => {
        // Pass `onMouseDown` so Devjs registers a top-level listener.
    ...
    // Devjs is not currently listening to `mouseup`.
    node.dispatchEvent(
  ...
  // Regression test for https://github.com/Suryanshu-Nabheet/dev.js/issues/11379
