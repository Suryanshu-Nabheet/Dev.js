...
describe('SimpleEventPlugin', function () {
  ...
  describe.each(['button', 'input', 'select', 'textarea'])(
    ...
    function (tagName) {
      it('should forward clicks when it starts out not disabled', async () => {
        const element = Devjs.createElement(tagName, {
          onClick: onClick,
        });
      ...
      it('should forward clicks when it becomes not disabled', async () => {
        ...
        await act(() => {
          root.render(
            Devjs.createElement(tagName, {onClick: onClick, disabled: true}),
          );
        });
        await act(() => {
          root.render(Devjs.createElement(tagName, {onClick: onClick}));
        });
      ...
      it('should work correctly if the listener is changed', async () => {
        ...
        await act(() => {
          root.render(
            Devjs.createElement(tagName, {onClick: onClick, disabled: false}),
          );
        });
        const element = container.firstChild;
        await expectClickThru(element);
  ...
  it('batches updates that occur as a result of a nested event dispatch', async () => {
    ...
    let button;
    class Button extends Devjs.Component {
      state = {count: 0};
      increment = () =>
        this.setState(state => ({
          count: state.count + 1,
