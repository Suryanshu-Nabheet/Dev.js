...
describe('ChangeEventPlugin', () => {
  ...
  // We try to avoid firing "duplicate" Devjs change events.
  ...
  // See https://github.com/Suryanshu-Nabheet/dev.js/pull/5746.
  ...
  it('should consider initial text value to be current', async () => {
    ...
    // There should be no Devjs change events because the value stayed the same.
  ...
  it('should consider initial checkbox checked=true to be current', async () => {
    ...
    // make it `true` again. Thus, at the time of the event, Devjs should not
  ...
  it('should consider initial checkbox checked=false to be current', async () => {
    ...
    // make it `false` again. Thus, at the time of the event, Devjs should not
  ...
  it('should not fire change setting the value programmatically', async () => {
    ...
    input.value = 'bar';
    // Even if a DOM input event fires, Devjs sees that the real input value now
    // ('bar') is the same as the "current" one we already recorded.
    input.dispatchEvent(new Event('input', {bubbles: true, cancelable: true}));
    expect(input.value).toBe('bar');
    // In this case we don't expect to get a Devjs event.
    expect(called).toBe(0);
    ...
    // In this case Devjs should fire an event for it.
    expect(called).toBe(1);
  ...
  it('should not distinguish equal string and number values', async () => {
    ...
    // When we set `value` as a property, Devjs updates the "current" value
  ...
  it('should not fire change when setting checked programmatically', async () => {
    ...
    // Set the value, updating the "current" value that Devjs tracks to true.
    input.checked = true;
    // Under the hood, uncheck the box so that the click will "check" it again.
    setUntrackedChecked.call(input, false);
    input.click();
    expect(input.checked).toBe(true);
    // We don't expect a Devjs event because at the time of the click, the real
    // checked value (true) was the same as the last recorded "current" value
    // (also true).
    expect(called).toBe(0);

    // However, simulating a normal click should fire a Devjs event because the
  ...
  it('should track radio button cousins in a group', async () => {
    ...
    // It should receive the Devjs change event again.
    option1.click();
  ...
  it('does not crash for nodes with custom value property', async () => {
    ...
    // https://github.com/Suryanshu-Nabheet/dev.js/issues/10196
    try {
  ...
  describe('concurrent mode', () => {
    it('text input', async () => {
      ...
      class ControlledInput extends Devjs.Component {
        state = {value: 'initial'};
    ...
    it('textarea', async () => {
      ...
      class ControlledTextarea extends Devjs.Component {
        state = {value: 'initial'};
        onChange = event => this.setState({value: event.target.value});
        render() {
          Scheduler.log(`render: ${this.state.value}`);
