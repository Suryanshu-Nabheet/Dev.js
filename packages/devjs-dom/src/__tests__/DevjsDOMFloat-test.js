...
describe('DevjsDOMFloat', () => {
  ...
  async function act(callback) {
    ...
    if (streamingContainer === null) {
      ...
      if (
        ...
        throw new Error(
          'Recieved <html> without a <!DOCTYPE html> which is almost certainly a bug in Devjs',
        );
      }
  ...
  function renderToPipeableStream(jsx, options) {
    ...
    return DevjsDOMFizzServer.renderToPipeableStream(
      jsx,
  ...
  it('can render resources before singletons', async () => {
    const root = DevjsDOMClient.createRoot(document);
    root.render(
      <>
        <title>foo</title>
        <html>
  ...
  it('warns if you render resource-like elements above <head> or <body>', async () => {
    ...
    assertConsoleErrorDev([
      'Cannot render a <style> outside the main document without knowing its precedence ' +
        'and a unique href key. Devjs can hoist and deduplicate <style> tags if you provide a ' +
    ...
    expect(chunks).toEqual([
      '<!DOCTYPE html><html><head><script async="" src="foo"></script>' +
        (gate(flags => flags.shouldUseFizzExternalRuntime)
          ? '<script src="devjs-dom/unstable_server-external-runtime" async=""></script>'
          : '') +
  ...
  it('dedupes if the external runtime is explicitly loaded using preinit', async () => {
    ...
    function App() {
      DevjsDOM.preinit(unstable_externalRuntimeSrc, {as: 'script'});
      return (
        <div>
...
body {
  ...
  it('client renders a boundary if a style Resource dependency fails to load', async () => {
    ...
    const errors = [];
    DevjsDOMClient.hydrateRoot(document, <App />, {
  ...
  it('hoists late stylesheets the correct precedence', async () => {
    function PresetPrecedence() {
      DevjsDOM.preinit('preset', {as: 'style', precedence: 'preset'});
    }
    await act(() => {
      const {pipe} = renderToPipeableStream(
        <html>
  ...
  it('stylesheet resources are inserted according to precedence order on the client', async () => {
    ...
    const root = DevjsDOMClient.hydrateRoot(
      document,
      <html>
        <head />
        <body>
  ...
  it('encodes attributes consistently whether resources are flushed in shell or in late boundaries', async () => {
    ...
    assertConsoleErrorDev([
      'Devjs does not recognize the `nonStandardAttr` prop on a DOM element. ' +
      ...
      'Invalid values for props `shouldnotincludefunctions`, `norsymbols` on <link> tag. ' +
        ...
        'For details, see https://devjs.dev/link/attribute-behavior \n' +
        '    in link (at **)\n' +
        '    in App (at **)',
    ]);
  ...
  it('always enforces crossOrigin "anonymous" for font preloads', async () => {
    function App() {
      DevjsDOM.preload('foo', {as: 'font', type: 'font/woff2'});
      DevjsDOM.preload('bar', {as: 'font', crossOrigin: 'foo'});
      DevjsDOM.preload('baz', {as: 'font', crossOrigin: 'use-credentials'});
      DevjsDOM.preload('qux', {as: 'font', crossOrigin: 'anonymous'});
      return (
        <html>
          <head />
          <body />
  ...
  it('warns if you render <meta> tag with itemProp outside <body> or <head>', async () => {
    ...
    assertConsoleErrorDev([
      'Cannot render a <meta> outside the main document if it has an `itemProp` prop. ' +
        ...
        'If you were intending for Devjs to hoist this <meta> remove the `itemProp` prop. ' +
  ...
  it('warns if you render a <title> tag with itemProp outside <body> or <head>', async () => {
    ...
    assertConsoleErrorDev([
      'Cannot render a <title> outside the main document if it has an `itemProp` prop. ' +
        '`itemProp` suggests the tag belongs to an `itemScope` which can appear anywhere in the DOM. ' +
        'If you were intending for Devjs to hoist this <title> remove the `itemProp` prop. ' +
        'Otherwise, try moving this tag into the <head> or <body> of the Document.\n' +
        '    in html (at **)',
  ...
  it('warns if you render a <style> tag with itemProp outside <body> or <head>', async () => {
    ...
    assertConsoleErrorDev([
      'Cannot render a <style> outside the main document if it has an `itemProp` prop. ' +
        '`itemProp` suggests the tag belongs to an `itemScope` which can appear anywhere in the DOM. ' +
        'If you were intending for Devjs to hoist this <style> remove the `itemProp` prop. ' +
        'Otherwise, try moving this tag into the <head> or <body> of the Document.\n' +
        '    in html (at **)',
      'In HTML, <style> cannot be a child of <html>.\n' +
        'This will cause a hydration error.\n' +
  ...
  it('warns if you render a <link> tag with itemProp outside <body> or <head>', async () => {
    ...
    assertConsoleErrorDev([
      'Cannot render a <link> outside the main document if it has an `itemProp` prop. ' +
        '`itemProp` suggests the tag belongs to an `itemScope` which can appear anywhere in the DOM. ' +
        'If you were intending for Devjs to hoist this <link> remove the `itemProp` prop. ' +
        'Otherwise, try moving this tag into the <head> or <body> of the Document.\n' +
        '    in html (at **)',
  ...
  it('warns if you render a <script> tag with itemProp outside <body> or <head>', async () => {
    ...
    assertConsoleErrorDev([
      'Cannot render a <script> outside the main document if it has an `itemProp` prop. ' +
        '`itemProp` suggests the tag belongs to an `itemScope` which can appear anywhere in the DOM. ' +
        'If you were intending for Devjs to hoist this <script> remove the `itemProp` prop. ' +
        'Otherwise, try moving this tag into the <head> or <body> of the Document.\n' +
        '    in html (at **)',
      'In HTML, <script> cannot be a child of <html>.\n' +
        'This will cause a hydration error.\n' +
  ...
  it('can hydrate resources and components in the head and body even if a browser or 3rd party script injects extra html nodes', async () => {
    ...
    const root = DevjsDOMClient.hydrateRoot(document, <App />);
    await waitForAll([]);
    expect(getMeaningfulChildren(document)).toEqual(
      <html itemscope="">
        <head>
    ...
    // 3. Nodes that Devjs skipped over during hydration
    root.unmount();
    expect(getMeaningfulChildren(document)).toEqual(
      <html>
        <head>
  ...
  // https://github.com/Suryanshu-Nabheet/dev.js/issues/27585
  it('does not reinsert already inserted stylesheets during a delayed commit', async () => {
    ...
    const root = DevjsDOMClient.createRoot(document.body);
    expect(getMeaningfulChildren(container)).toBe(undefined);
    root.render(
      <>
        <link rel="stylesheet" href="first" precedence="default" />
    ...
    await waitForAll([]);
    // In https://github.com/Suryanshu-Nabheet/dev.js/issues/27585 the order updated
  ...
  it('assumes stylesheets that load in the shell loaded already', async () => {
    ...
    let root;
    Devjs.startTransition(() => {
      root = DevjsDOMClient.hydrateRoot(
        document,
        <html>
          <body>
            <link rel="stylesheet" href="foo" precedence="default" />
  ...
  it('will assume stylesheets already in the document have loaded if it cannot confirm it is not yet loaded', async () => {
    ...
    const root = DevjsDOMClient.createRoot(document.querySelector('#foo'));
  ...
  it('will not insert a preload if the underlying resource already exists in the Document', async () => {
    ...
    function ClientApp() {
      DevjsDOM.preload('foo', {as: 'style'});
      DevjsDOM.preload('bar', {as: 'script'});
      DevjsDOM.preload('baz', {as: 'font'});
      return 'foo';
    }
  ...
  it('uses imageSrcSet and imageSizes when keying image preloads', async () => {
    function App({isClient}) {
      ...
      DevjsDOM.preload('foo', {as: 'image'});
      DevjsDOM.preload('foo', {as: 'image'});

      // Will key off imageSrcSet + imageSizes
      DevjsDOM.preload('foo', {as: 'image', imageSrcSet: 'fooset'});
      DevjsDOM.preload('foo2', {as: 'image', imageSrcSet: 'fooset'});

      // Will key off imageSrcSet + imageSizes
      DevjsDOM.preload('foo', {
        as: 'image',
        imageSrcSet: 'fooset',
        imageSizes: 'foosizes',
      });
      DevjsDOM.preload('foo2', {
        as: 'image',
        imageSrcSet: 'fooset',
        imageSizes: 'foosizes',
      });
      ...
      DevjsDOM.preload('foo', {as: 'image', imageSizes: 'foosizes'});
      DevjsDOM.preload('foo', {as: 'image', imageSizes: 'foosizes'});

      // These preloads are for something that isn't an image
      // They should all key off the href
      DevjsDOM.preload('bar', {as: 'somethingelse'});
      DevjsDOM.preload('bar', {
        as: 'somethingelse',
      ...
      if (isClient) {
        // Will key off href in absense of imageSrcSet
        DevjsDOM.preload('client', {as: 'image'});
        DevjsDOM.preload('client', {as: 'image'});

        // Will key off imageSrcSet + imageSizes
        DevjsDOM.preload('client', {as: 'image', imageSrcSet: 'clientset'});
        DevjsDOM.preload('client2', {as: 'image', imageSrcSet: 'clientset'});

        // Will key off imageSrcSet + imageSizes
        DevjsDOM.preload('client', {
          as: 'image',
          imageSrcSet: 'clientset',
          imageSizes: 'clientsizes',
        });
        DevjsDOM.preload('client2', {
          as: 'image',
          imageSrcSet: 'clientset',
          imageSizes: 'clientsizes',
        });
        ...
        // Will key off href in absense of imageSrcSet, imageSizes is ignored. these should match the
        // first preloads not not emit a new preload tag
        DevjsDOM.preload('client', {as: 'image', imageSizes: 'clientsizes'});
        DevjsDOM.preload('client', {as: 'image', imageSizes: 'clientsizes'});
      }
  ...
  it('can emit preloads for non-lazy images that are rendered', async () => {
    function App() {
      DevjsDOM.preload('script', {as: 'script'});
      DevjsDOM.preload('a', {as: 'image'});
      DevjsDOM.preload('b', {as: 'image'});
      return (
        <html>
          <body>
            <img src="a" />
  ...
  it('preloads up to 10 suspensey images as high priority when fetchPriority is not specified', async () => {
    function App() {
      DevjsDOM.preload('1', {as: 'image', fetchPriority: 'high'});
      DevjsDOM.preload('auto', {as: 'image'});
      DevjsDOM.preload('low', {as: 'image', fetchPriority: 'low'});
      DevjsDOM.preload('9', {as: 'image', fetchPriority: 'high'});
      DevjsDOM.preload('10', {as: 'image', fetchPriority: 'high'});
      return (
        <html>
          <body>
            {/* skipping 1 */}
  ...
  it('preloads from rendered images properly use srcSet and sizes', async () => {
    function App() {
      DevjsDOM.preload('1', {as: 'image', imageSrcSet: 'ss1'});
      DevjsDOM.preload('2', {
        as: 'image',
        imageSrcSet: 'ss2',
        imageSizes: 's2',
  ...
  // Fixes: https://github.com/Suryanshu-Nabheet/dev.js/issues/27910
  ...
  it('should warn if you preload a stylesheet and then render a style tag with the same href', async () => {
    ...
    assertConsoleErrorDev([
      'Devjs encountered a hoistable style tag for the same href as a preload: "foo". ' +
  ...
  it('should preload only once even if you discover a stylesheet, script, or moduleScript late', async () => {
    function App() {
      ...
      DevjsDOM.preinit('shell preinit/shell', {as: 'style'});
      DevjsDOM.preinit('shell preinit/shell', {as: 'script'});
      DevjsDOM.preinitModule('shell preinit/shell', {as: 'script'});

      // We initiate all the shell preloads
      DevjsDOM.preload('shell preinit/shell', {as: 'style'});
      DevjsDOM.preload('shell preinit/shell', {as: 'script'});
      DevjsDOM.preloadModule('shell preinit/shell', {as: 'script'});

      DevjsDOM.preload('shell/shell preinit', {as: 'style'});
      DevjsDOM.preload('shell/shell preinit', {as: 'script'});
      DevjsDOM.preloadModule('shell/shell preinit', {as: 'script'});

      DevjsDOM.preload('shell/shell render', {as: 'style'});
      DevjsDOM.preload('shell/shell render', {as: 'script'});
      DevjsDOM.preloadModule('shell/shell render');

      DevjsDOM.preload('shell/late preinit', {as: 'style'});
      DevjsDOM.preload('shell/late preinit', {as: 'script'});
      DevjsDOM.preloadModule('shell/late preinit');

      DevjsDOM.preload('shell/late render', {as: 'style'});
      DevjsDOM.preload('shell/late render', {as: 'script'});
      DevjsDOM.preloadModule('shell/late render');

      // we preinit later ones that should be created by
      DevjsDOM.preinit('shell/shell preinit', {as: 'style'});
      DevjsDOM.preinit('shell/shell preinit', {as: 'script'});
      DevjsDOM.preinitModule('shell/shell preinit');

      DevjsDOM.preinit('late/shell preinit', {as: 'style'});
      DevjsDOM.preinit('late/shell preinit', {as: 'script'});
      DevjsDOM.preinitModule('late/shell preinit');
      return (
        <html>
          <body>
            <link
    ...
    function Late() {
      DevjsDOM.preload('late/later preinit', {as: 'style'});
      DevjsDOM.preload('late/later preinit', {as: 'script'});
      DevjsDOM.preloadModule('late/later preinit');

      DevjsDOM.preload('late/later render', {as: 'style'});
      DevjsDOM.preload('late/later render', {as: 'script'});
      DevjsDOM.preloadModule('late/later render');

      DevjsDOM.preload('late/shell preinit', {as: 'style'});
      DevjsDOM.preload('late/shell preinit', {as: 'script'});
      DevjsDOM.preloadModule('late/shell preinit');

      DevjsDOM.preload('late/shell render', {as: 'style'});
      DevjsDOM.preload('late/shell render', {as: 'script'});
      DevjsDOM.preloadModule('late/shell render');
      ...
      DevjsDOM.preinit('shell/late preinit', {as: 'style'});
      DevjsDOM.preinit('shell/late preinit', {as: 'script'});
      DevjsDOM.preinitModule('shell/late preinit');
      return (
        <>
          Late
          <link
    ...
    function Later() {
      // late preinits don't actually flush so we won't see this in the DOM as a stylesehet but we should see
      // the preload for this resource
      DevjsDOM.preinit('late/later preinit', {as: 'style'});
      DevjsDOM.preinit('late/later preinit', {as: 'script'});
      DevjsDOM.preinitModule('late/later preinit');
      return (
        <>
          Later
          <link
  ...
  it('should never flush hoistables before the preamble', async () => {
    ...
    function App() {
      DevjsDOM.preinit('foo', {as: 'script'});
      Devjs.use(promise);
      return (
        <html>
          <body>hello</body>
  ...
  describe('DevjsDOM.prefetchDNS(href)', () => {
    it('creates a dns-prefetch resource when called', async () => {
      function App({url}) {
        DevjsDOM.prefetchDNS(url);
        DevjsDOM.prefetchDNS(url);
        DevjsDOM.prefetchDNS(url, {});
        DevjsDOM.prefetchDNS(url, {crossOrigin: 'use-credentials'});
        return (
          <html>
            <body>hello world</body>
          </html>
      ...
      assertConsoleErrorDev([
        'DevjsDOM.prefetchDNS(): Expected only one argument, `href`, ' +
          ...
          'Try calling DevjsDOM.prefetchDNS() with just a single string argument, `href`.\n' +
          '    in App (at **)',
        'DevjsDOM.prefetchDNS(): Expected only one argument, `href`, ' +
          'but encountered something with type "object" as a second argument instead. ' +
          'This argument is reserved for future options and is currently disallowed. ' +
          ...
          'on the resource hint has no effect. Try calling DevjsDOM.prefetchDNS() with just ' +
      ...
      const root = DevjsDOMClient.hydrateRoot(document, <App url="foo" />);
      await waitForAll([]);
      assertConsoleErrorDev([
        'DevjsDOM.prefetchDNS(): Expected only one argument, `href`, ' +
          'but encountered something with type "object" as a second argument instead. ' +
  ...
  describe('DevjsDOM.preconnect(href, { crossOrigin })', () => {
    it('creates a preconnect resource when called', async () => {
      function App({url}) {
        DevjsDOM.preconnect(url);
        DevjsDOM.preconnect(url);
        DevjsDOM.preconnect(url, {crossOrigin: true});
        DevjsDOM.preconnect(url, {crossOrigin: ''});
        DevjsDOM.preconnect(url, {crossOrigin: 'anonymous'});
        DevjsDOM.preconnect(url, {crossOrigin: 'use-credentials'});
        return (
          <html>
            <body>hello world</body>
          </html>
      ...
      assertConsoleErrorDev([
        'DevjsDOM.preconnect(): Expected the `crossOrigin` option (second argument) ' +
  ...
  describe('DevjsDOM.preload(href, { as: ... })', () => {
    it('creates a preload resource when called', async () => {
      ...
      function ClientApp() {
        DevjsDOM.preload('foo', {as: 'style'});
        DevjsDOM.preload('font', {as: 'font', type: 'font/woff2'});
        Devjs.useInsertionEffect(() => DevjsDOM.preload('bar', {as: 'script'}));
        Devjs.useLayoutEffect(() => DevjsDOM.preload('baz', {as: 'font'}));
        Devjs.useEffect(() => DevjsDOM.preload('qux', {as: 'style'}));
        return (
          <html>
            <body>
              <Suspense fallback="loading...">
      ...
      DevjsDOMClient.hydrateRoot(document, <ClientApp />);
      await waitForAll([]);
      expect(getMeaningfulChildren(document)).toEqual(
        <html>
          <head>
    ...
    it('warns if you do not pass in a valid href argument or options argument', async () => {
      function App() {
        DevjsDOM.preload();
        DevjsDOM.preload('');
        DevjsDOM.preload('foo', null);
        DevjsDOM.preload('foo', {});
        DevjsDOM.preload('foo', {as: 'foo'});
        return <div>foo</div>;
      }
      ...
      assertConsoleErrorDev([
        'DevjsDOM.preload(): Expected two arguments, a non-empty `href` string and an ' +
    ...
    it('supports fetchPriority', async () => {
      function Component({isServer}) {
        DevjsDOM.preload(isServer ? 'highserver' : 'highclient', {
          as: 'script',
          fetchPriority: 'high',
        });
        DevjsDOM.preload(isServer ? 'lowserver' : 'lowclient', {
          as: 'style',
          fetchPriority: 'low',
        });
        DevjsDOM.preload(isServer ? 'autoserver' : 'autoclient', {
          as: 'style',
          fetchPriority: 'auto',
        });
        return 'hello';
    ...
    it('supports nonce', async () => {
      function App({url}) {
        DevjsDOM.preload(url, {as: 'script', nonce: 'abc'});
        return 'hello';
      }
      ...
      DevjsDOMClient.hydrateRoot(container, <App url="client" />);
      await waitForAll([]);
      expect(getMeaningfulChildren(document)).toEqual(
        <html>
          <head>
  ...
  describe('DevjsDOM.preloadModule(href, options)', () => {
    it('preloads scripts as modules', async () => {
      function App({ssr}) {
        const prefix = ssr ? 'ssr ' : 'browser ';
        DevjsDOM.preloadModule(prefix + 'plain');
        DevjsDOM.preloadModule(prefix + 'default', {as: 'script'});
        DevjsDOM.preloadModule(prefix + 'cors', {
          crossOrigin: 'use-credentials',
        });
        DevjsDOM.preloadModule(prefix + 'integrity', {integrity: 'some hash'});
        DevjsDOM.preloadModule(prefix + 'serviceworker', {as: 'serviceworker'});
        return <div>hello</div>;
      }
      await act(() => {
    ...
    it('warns if you provide invalid arguments', async () => {
      function App() {
        DevjsDOM.preloadModule();
        DevjsDOM.preloadModule(() => {});
        DevjsDOM.preloadModule('');
        DevjsDOM.preloadModule('1', true);
        DevjsDOM.preloadModule('2', {as: true});
        return <div>hello</div>;
      }
      await act(() => {
        renderToPipeableStream(<App />).pipe(writable);
      ...
      assertConsoleErrorDev([
        'DevjsDOM.preloadModule(): Expected two arguments, a non-empty `href` string and, optionally, ' +
      ...
      assertConsoleErrorDev([
        ...
        'DevjsDOM.preloadModule(): Expected two arguments, a non-empty `href` string and, optionally, ' +
          ...
          'The `href` argument encountered was something with type "function".\n' +
          '    in App (at **)',
        'DevjsDOM.preloadModule(): Expected two arguments, a non-empty `href` string and, optionally, an ' +
  ...
  describe('DevjsDOM.preinit(href, { as: ... })', () => {
    it('creates a stylesheet resource when DevjsDOM.preinit(..., {as: "style" }) is called', async () => {
      function App() {
        DevjsDOM.preinit('foo', {as: 'style'});
        return (
          <html>
      ...
      // The reason we do not see the "bar" stylesheet here is that DevjsDOM.preinit is not about
    ...
    it('creates a stylesheet resource in the ownerDocument when DevjsDOM.preinit(..., {as: "style" }) is called outside of render on the client', async () => {
      function App() {
        Devjs.useEffect(() => {
          DevjsDOM.preinit('foo', {as: 'style'});
        }, []);
    ...
    it('creates a stylesheet resource in the ownerDocument when DevjsDOM.preinit(..., {as: "style" }) is called in shadowRoot', async () => {
    ...
    it('creates a script resource when DevjsDOM.preinit(..., {as: "script" }) is called', async () => {
      function App() {
        DevjsDOM.preinit('foo', {as: 'script'});
        return (
          <html>
      ...
      function Component() {
        DevjsDOM.preinit('bar', {as: 'script'});
        return <div>hello</div>;
      }
    ...
    it('creates a script resource when DevjsDOM.preinit(..., {as: "script" }) is called outside of render on the client', async () => {
      function App() {
        Devjs.useEffect(() => {
          DevjsDOM.preinit('foo', {as: 'script'});
        }, []);
    ...
    it('warns if you do not pass in a valid href argument or options argument', async () => {
      function App() {
        DevjsDOM.preinit();
        DevjsDOM.preinit('');
        DevjsDOM.preinit('foo', null);
        DevjsDOM.preinit('foo', {});
        DevjsDOM.preinit('foo', {as: 'foo'});
        return <div>foo</div>;
      }
      ...
      assertConsoleErrorDev([
        'DevjsDOM.preinit(): Expected the `href` argument (first) to be a non-empty string ' +
          'but encountered `undefined` instead.\n' +
          '    in App (at **)',
        'DevjsDOM.preinit(): Expected the `href` argument (first) to be a non-empty string ' +
          'but encountered an empty string instead.\n' +
          '    in App (at **)',
        'DevjsDOM.preinit(): Expected the `options` argument (second) to be an object with an `as` ' +
          'property describing the type of resource to be preinitialized but encountered `null` instead.\n' +
          '    in App (at **)',
        'DevjsDOM.preinit(): Expected the `as` property in the `options` argument (second) to contain ' +
    ...
    it('accepts a `nonce` option for `as: "script"`', async () => {
      function Component({src}) {
        DevjsDOM.preinit(src, {as: 'script', nonce: 'R4nD0m'});
        return 'hello';
      }
    ...
    it('accepts an `integrity` option for `as: "script"`', async () => {
      function Component({src, hash}) {
        DevjsDOM.preinit(src, {as: 'script', integrity: hash});
        return 'hello';
      }
    ...
    it('accepts an `integrity` option for `as: "style"`', async () => {
      function Component({src, hash}) {
        DevjsDOM.preinit(src, {as: 'style', integrity: hash});
        return 'hello';
      }
    ...
    it('supports fetchPriority', async () => {
      function Component({isServer}) {
        DevjsDOM.preinit(isServer ? 'highserver' : 'highclient', {
          as: 'script',
          fetchPriority: 'high',
        });
        DevjsDOM.preinit(isServer ? 'lowserver' : 'lowclient', {
          as: 'style',
          fetchPriority: 'low',
        });
        DevjsDOM.preinit(isServer ? 'autoserver' : 'autoclient', {
          as: 'style',
          fetchPriority: 'auto',
        });
        return 'hello';
  ...
  describe('DevjsDOM.preinitModule(href, options)', () => {
    it('creates a script module resources', async () => {
      function App({ssr}) {
        const prefix = ssr ? 'ssr ' : 'browser ';
        DevjsDOM.preinitModule(prefix + 'plain');
        DevjsDOM.preinitModule(prefix + 'default', {as: 'script'});
        DevjsDOM.preinitModule(prefix + 'cors', {
          crossOrigin: 'use-credentials',
        });
        DevjsDOM.preinitModule(prefix + 'integrity', {integrity: 'some hash'});
        DevjsDOM.preinitModule(prefix + 'warning', {as: 'style'});
        return <div>hello</div>;
      }
      await act(() => {
        renderToPipeableStream(<App ssr={true} />).pipe(writable);
      ...
      assertConsoleErrorDev([
        'DevjsDOM.preinitModule(): Expected up to two arguments, a non-empty `href` ' +
    ...
    it('warns if you provide invalid arguments', async () => {
      function App() {
        DevjsDOM.preinitModule();
        DevjsDOM.preinitModule(() => {});
        DevjsDOM.preinitModule('');
        DevjsDOM.preinitModule('1', true);
        DevjsDOM.preinitModule('2', {as: true});
        return <div>hello</div>;
      }
      await act(() => {
        renderToPipeableStream(<App />).pipe(writable);
  ...
  describe('Stylesheet Resources', () => {
    ...
    it('escapes hrefs when selecting matching elements in the document when rendering Resources', async () => {
      function App() {
        DevjsDOM.preload('preload', {as: 'style'});
        DevjsDOM.preload('with\nnewline', {as: 'style'});
        return (
          <html>
            <head />
            <body>
    ...
    it('escapes hrefs when selecting matching elements in the document when using preload and preinit', async () => {
      ...
      function App() {
        DevjsDOM.preload('preload"][rel="preload', {as: 'style'});
        DevjsDOM.preinit('style"][rel="stylesheet', {
          as: 'style',
          precedence: 'style',
        });
        DevjsDOM.preinit('with\\slashes', {
          as: 'style',
          precedence: 'style',
        });
        DevjsDOM.preload('with\nnewline', {as: 'style'});
    ...
    it('warns if you provide a `precedence` prop with other props that invalidate the creation of a stylesheet resource', async () => {
      ...
      assertConsoleErrorDev(
        [
          'Devjs encountered a `<link rel="stylesheet" .../>` with a `precedence` prop and ' +
            'expected the `href` prop to be a non-empty string but ecountered `undefined` instead. ' +
            'If your intent was to have Devjs hoist and deduplciate this stylesheet using the ' +
          ...
          'Devjs encountered a `<link rel="stylesheet" .../>` with a `precedence` prop and ' +
            ...
            'and Devjs will not hoist or deduplicate this stylesheet. ' +
            'If your intent was to have Devjs hoist and deduplciate this stylesheet using the ' +
          ...
          'Devjs encountered a `<link rel="stylesheet" .../>` with a `precedence` prop and ' +
            ...
            'Devjs will not hoist or deduplicate this stylesheet. ' +
            'If your intent was to have Devjs hoist and deduplciate this stylesheet using the ' +
            '`precedence` prop remove the `onLoad` prop, otherwise remove the `precedence` prop.\n' +
            '    in link (at **)',
          'Devjs encountered a `<link rel="stylesheet" .../>` with a `precedence` prop and `onError` prop. ' +
            'The presence of loading and error handlers indicates an intent to manage the stylesheet loading state ' +
            'from your from your Component code and Devjs will not hoist or deduplicate this stylesheet. ' +
            'If your intent was to have Devjs hoist and deduplciate this stylesheet using the `precedence` ' +
            'prop remove the `onError` prop, otherwise remove the `precedence` prop.\n' +
            '    in link (at **)',
          'Devjs encountered a `<link rel="stylesheet" .../>` with a `precedence` prop and a `disabled` prop. ' +
            'The presence of the `disabled` prop indicates an intent to manage the stylesheet active state from ' +
            'your from your Component code and Devjs will not hoist or deduplicate this stylesheet. ' +
            'If your intent was to have Devjs hoist and deduplciate this stylesheet using the `precedence` ' +
      ...
      await waitForAll([]);
      assertConsoleErrorDev([
        'Devjs encountered a <link rel="stylesheet" href="foo" ... /> with a `precedence` ' +
          ...
          'loading state from your from your Component code and Devjs will not hoist or deduplicate this stylesheet. ' +
          'If your intent was to have Devjs hoist and deduplciate this stylesheet using the `precedence` ' +
...
background-color: green;
    ...
    it('warns if you render a <style> with an href with a space on the server', async () => {
      ...
      assertConsoleErrorDev([
        'Devjs expected the `href` prop for a <style> tag opting into hoisting semantics ' +
  ...
  it('still outlines a boundary with a suspensey image inside a ViewTransition when flushing the shell', async () => {
    ...
    const ViewTransition = Devjs.ViewTransition;
