...
describe('DevjsDOMOption', () => {
  ...
  it('should throw on object children', async () => {
    ...
    ).rejects.toThrow('Objects are not valid as a Devjs child');
    await expect(async () => {
  ...
  it('should support element-ish child', async () => {
    ...
    const obj = {
      $$typeof: Symbol.for('devjs.transitional.element'),
  ...
  it('should be able to use dangerouslySetInnerHTML on option', async () => {
    ...
    const container = await renderIntoDocument(stub);
    assertConsoleErrorDev([
      'Pass a `value` prop if you set dangerouslyInnerHTML so Devjs knows which value should be selected.\n' +
        '    in option (at **)',
    ]);
