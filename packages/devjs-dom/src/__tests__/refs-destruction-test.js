...
describe('refs-destruction', () => {
  beforeEach(() => {
    ...
    TestComponent = class extends Devjs.Component {
      constructor(props) {
        super(props);
        theInnerDivRef = Devjs.createRef();
        theInnerClassComponentRef = Devjs.createRef();
      }
  ...
  it('should not error when destroying child with ref asynchronously', async () => {
    let nestedRoot;
    class Modal extends Devjs.Component {
      componentDidMount() {
        this.div = document.createElement('div');
        nestedRoot = DevjsDOMClient.createRoot(this.div);
    ...
    class AppModal extends Devjs.Component {
      render() {
        return (
          <Modal>
            <a ref={Devjs.createRef()} />
          </Modal>
        );
      }
    }
