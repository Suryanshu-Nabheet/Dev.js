...
describe('DevjsDOMEventListener', () => {
  ...
  describe('Propagation', () => {
    ...
    it('should propagate events two levels down', async () => {
      ...
      const childRoot = DevjsDOMClient.createRoot(childContainer);
      const parentRoot = DevjsDOMClient.createRoot(parentContainer);
      const grandParentRoot = DevjsDOMClient.createRoot(grandParentContainer);
    ...
    // Regression test for https://github.com/Suryanshu-Nabheet/dev.js/issues/1105
    ...
    it('should batch between handlers from different roots (discrete)', async () => {
      ...
      function Parent() {
        ...
        const [state, _] = Devjs.useState('Parent');
        const handleClick = () => {
          childSetState(2);
      ...
      function Child() {
        const [state, setState] = Devjs.useState('Child');
  ...
  it('should not fire duplicate events for a Devjs DOM tree', async () => {
    const mouseOut = jest.fn();
  ...
  // Regression test for https://github.com/Suryanshu-Nabheet/dev.js/pull/12877
  ...
  it('should not receive submit events if native, interim DOM handler prevents it', async () => {
    ...
    try {
      const formRef = Devjs.createRef();
      const interimRef = Devjs.createRef();
  ...
  it('should delegate media events even without a direct listener', async () => {
    ...
    try {
      ...
      // Regression test: ensure Devjs tree delegation still works
  ...
  it('should subscribe to scroll during hydration', async () => {
    ...
    document.body.appendChild(container);
    try {
      container.innerHTML = DevjsDOMServer.renderToString(tree);
      let root;
      await act(() => {
        root = DevjsDOMClient.hydrateRoot(container, tree);
      });
      await act(() => {
        ref.current.dispatchEvent(
          new Event('scroll', {
  ...
  it('should not subscribe to selectionchange twice', async () => {
    ...
    try {
      const rootOne = DevjsDOMClient.createRoot(document.createElement('div'));
      const rootTwo = DevjsDOMClient.createRoot(document.createElement('div'));
