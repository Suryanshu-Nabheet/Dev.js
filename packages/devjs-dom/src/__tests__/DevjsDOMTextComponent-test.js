...
describe('DevjsDOMTextComponent', () => {
  beforeEach(() => {
    Devjs = require('devjs');
   ...
   * See https://github.com/Suryanshu-Nabheet/dev.js/issues/9836 tracking whether we'll need to fix this or if it's unnecessary.
   */
  // @gate TODO
  ...
  it('can reconcile text from pre-rendered markup', async () => {
    ...
    container.innerHTML = DevjsDOMServer.renderToString(children);

    const root = await act(() => {
      return DevjsDOMClient.hydrateRoot(container, children);
    });
    ...
    await act(() => {
      DevjsDOMClient.hydrateRoot(container, children);
    });
