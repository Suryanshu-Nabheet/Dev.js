...
describe('DevjsDOMComponentTree', () => {
  ...
  it('finds closest instance for node when an event happens', async () => {
    const nonDevjsElemID = 'aID';
    const innerHTML = {__html: `<div id="${nonDevjsElemID}"></div>`};
    ...
    expect(currentTargetID).toBe(null);
    simulateClick(document.getElementById(nonDevjsElemID));
  ...
  it('updates event handlers from fiber props', async () => {
    ...
    function HandlerFlipper() {
      const [flipVal, setFlipVal] = Devjs.useState(false);
  ...
  it('finds a controlled instance from node and gets its current fiber props', async () => {
    ...
    function Controlled() {
      const [state, setState] = Devjs.useState(startValue);
