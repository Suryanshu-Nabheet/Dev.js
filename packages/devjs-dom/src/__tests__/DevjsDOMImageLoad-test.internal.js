...
describe('DevjsDOMImageLoad', () => {
  beforeEach(() => {
    ...
    onLoadSpy = jest.fn(devjsEvent => {
      const src = devjsEvent.target.getAttribute('src');
  ...
  it('replays the last load event when more than one fire before the end of the layout phase completes', async function () {
    ...
    function Base() {
      const [src, setSrc] = Devjs.useState('a');
      return (
        <PhaseMarkers>
    ...
    Devjs.startTransition(() => root.render(<Base />));
  ...
  it('captures and suppresses the load event if it happens before passive effects and a cascading update causes the img to be removed', async function () {
    ...
    function ChildSuppressing({children}) {
      const [showChildren, update] = Devjs.useState(true);
      Devjs.useLayoutEffect(() => {
        if (showChildren) {
          update(false);
        }
  ...
  it('captures and suppresses the load event if it happens before passive effects and a cascading update causes the img to be removed, alternate', async function () {
    ...
    function Switch({children}) {
      const [shouldShow, updateShow] = Devjs.useState(true);
  ...
  //   Devjs.startTransition(() => root.render(<SuspendingWithImage />));
  ...
  it('correctly replays the last img load even when a yield + update causes the host element to change', async function () {
    ...
    function Base() {
      const [src, setSrc] = Devjs.useState(null);
      const [srcAlt, setSrcAlt] = Devjs.useState(null);
    ...
    Devjs.startTransition(() => externalSetSrc('a'));
    ...
    DevjsDOM.flushSync(() => externalSetSrcAlt('b'));
