...
describe('DevjsDOMSVG', () => {
  ...
  it('creates elements with SVG namespace inside SVG tag during mount', async () => {
    ...
    let div,
      ...
      svg3,
      svg4;
    const root = DevjsDOMClient.createRoot(node);
    await act(() => {
      root.render(
        <div>
          <svg ref={el => (svg = el)}>
  ...
  it('can render SVG into a non-Devjs SVG tree', async () => {
  ...
  it('can render HTML into a foreignObject in non-Devjs SVG tree', async () => {
    const outerSVGRoot = document.createElementNS(
      'http://www.w3.org/2000/svg',
      'svg',
    );
