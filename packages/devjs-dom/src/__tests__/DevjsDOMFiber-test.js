...
describe('DevjsDOMFiber', () => {
  ...
  const assertNamespacesMatch = async function (tree) {
    ...
    const testRoot = DevjsDOMClient.createRoot(testContainer);
    await act(async () => {
  ...
  it('should reconcile portal children', async () => {
    ...
    await act(() => {
      root.render(
        <div>
          {DevjsDOM.createPortal(<div>portal:1</div>, portalContainer)}
        </div>,
      );
    });
    ...
    await act(() => {
      root.render(
        <div>
          {DevjsDOM.createPortal(<div>portal:2</div>, portalContainer)}
        </div>,
      );
    });
    ...
    await act(() => {
      root.render(<div>{DevjsDOM.createPortal(null, portalContainer)}</div>);
    });
    expect(portalContainer.innerHTML).toBe('');
    expect(container.innerHTML).toBe('<div></div>');
  });
  ...
  it('should not warn when doing an update to a container manually updated outside of Devjs', async () => {
    // when not messing with the DOM outside of Devjs
    await act(() => {
      root.render(<div>foo</div>);
    });
  ...
  it('should not warn when doing an update to a container manually cleared outside of Devjs', async () => {
    // when not messing with the DOM outside of Devjs
    await act(() => {
      root.render(<div>foo</div>);
    });
  ...
  it('should render a text component with a text DOM node on the same document as the container', async () => {
    ...
    const iFrameRoot = DevjsDOMClient.createRoot(iframeContainer);
    await act(() => {
  ...
  it('should mount into a document fragment', async () => {
    ...
    const fragmentRoot = DevjsDOMClient.createRoot(fragment);
    await act(() => {
  ...
  it('should restore selection in the correct window', async () => {
    ...
    root = DevjsDOMClient.createRoot(newContainer);

    const Test = () => {
      const [reverse, setReverse] = Devjs.useState(false);
      const [items] = Devjs.useState(() => ['a', 'b', 'c']);
      const onClick = () => {
        setReverse(true);
      };

      // shuffle the items so that the devjs commit needs to restore focus
