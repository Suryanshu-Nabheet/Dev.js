...
describe('DevjsDOMLegacyFiber', () => {
  ...
  it('should render strings as children', () => {
    ...
    DevjsDOM.render(<Box value="foo" />, container);
  ...
  it('should render numbers as children', () => {
    ...
    DevjsDOM.render(<Box value={10} />, container);
  ...
  it('should be called a callback argument', () => {
    // mounting phase
    let called = false;
    DevjsDOM.render(<div>Foo</div>, container, () => (called = true));
  ...
  it('should call a callback argument when the same element is re-rendered', () => {
    ...
    // mounting phase
    let called = false;
    DevjsDOM.render(element, container, () => (called = true));
    expect(called).toEqual(true);
  ...
  it('should render a component returning strings directly from render', () => {
    ...
    DevjsDOM.render(<Text value="foo" />, container);
    expect(container.textContent).toEqual('foo');
  });
  ...
  it('should render a component returning numbers directly from render', () => {
    ...
    DevjsDOM.render(<Text value={10} />, container);
  ...
  it('finds the DOM Text node of a string child', () => {
    class Text extends Devjs.Component {
      render() {
  ...
  it('finds the first child when a component returns a fragment', () => {
    class Fragment extends Devjs.Component {
      render() {
    ...
    let instance = null;
    DevjsDOM.render(<Fragment ref={ref => (instance = ref)} />, container);
  ...
  it('renders an empty fragment', () => {
    ...
    DevjsDOM.render(<EmptyFragment />, container);
    expect(container.firstChild).toBe(null);

    DevjsDOM.render(<NonEmptyFragment />, container);
  ...
  const usePortal = function (tree) {
    return DevjsDOM.createPortal(tree, document.createElement('div'));
  };
  ...
  const assertNamespacesMatch = function (tree) {
    ...
    DevjsDOM.render(tree, testContainer);
    svgEls.forEach(el => {
    ...
    DevjsDOM.unmountComponentAtNode(testContainer);
  ...
  it('should render one portal', () => {
    ...
    DevjsDOM.render(
      <div>{DevjsDOM.createPortal(<div>portal</div>, portalContainer)}</div>,
      container,
    );
  ...
  it('should render many portals', () => {
    ...
    DevjsDOM.render(<Parent step="a" />, container);
    ...
    ops.length = 0;
    DevjsDOM.render(<Parent step="b" />, container);
  ...
  it('should reconcile portal children', () => {
    ...
    DevjsDOM.render(
      <div>{DevjsDOM.createPortal(<div>portal:1</div>, portalContainer)}</div>,
      container,
    );
    ...
    DevjsDOM.render(
      <div>{DevjsDOM.createPortal(<div>portal:2</div>, portalContainer)}</div>,
      container,
    );
    ...
    DevjsDOM.render(
      <div>{DevjsDOM.createPortal(<p>portal:3</p>, portalContainer)}</div>,
      container,
    );
    ...
    DevjsDOM.render(
      <div>{DevjsDOM.createPortal(['Hi', 'Bye'], portalContainer)}</div>,
      container,
    );
    ...
    DevjsDOM.render(
      <div>{DevjsDOM.createPortal(['Bye', 'Hi'], portalContainer)}</div>,
      container,
    );
    ...
    DevjsDOM.render(
      <div>{DevjsDOM.createPortal(null, portalContainer)}</div>,
      container,
    );
    expect(portalContainer.innerHTML).toBe('');
    expect(container.innerHTML).toBe('<div></div>');
  ...
  it('should unmount empty portal component wherever it appears', () => {
      ...
      render() {
        return (
          <div>
            {this.state.show && (
              <>
                {DevjsDOM.createPortal(null, portalContainer)}
                <div>child</div>
              </>
            )}
            <div>parent</div>
    ...
    const instance = DevjsDOM.render(<Wrapper />, container);
    expect(container.innerHTML).toBe(
  ...
  it('should pass portal context when rendering subtree elsewhere', () => {
    ...
    class Parent extends Devjs.Component {
      ...
      render() {
        return DevjsDOM.createPortal(<Component />, portalContainer);
      }
    }

    DevjsDOM.render(<Parent />, container);
    assertConsoleErrorDev([
      'Parent uses the legacy childContextTypes API which will soon be removed. Use Devjs.createContext() instead.',
      'Component uses the legacy contextTypes API which will soon be removed. Use Devjs.createContext() with static contextType instead.',
    ]);
    expect(container.innerHTML).toBe('');
  ...
  it('should update portal context if it changes due to setState', () => {
    ...
    const instance = DevjsDOM.render(<Parent />, container);
    assertConsoleErrorDev([
      'Parent uses the legacy childContextTypes API which will soon be removed. Use Devjs.createContext() instead.',
      'Component uses the legacy contextTypes API which will soon be removed. Use Devjs.createContext() with static contextType instead.',
    ]);
  ...
  it('should update portal context if it changes due to re-render', () => {
    ...
    DevjsDOM.render(<Parent bar="initial" />, container);
    assertConsoleErrorDev([
      'Parent uses the legacy childContextTypes API which will soon be removed. Use Devjs.createContext() instead.',
      'Component uses the legacy contextTypes API which will soon be removed. Use Devjs.createContext() with static contextType instead.',
    ]);
    expect(portalContainer.innerHTML).toBe('<div>initial-initial</div>');
    expect(container.innerHTML).toBe('');
    DevjsDOM.render(<Parent bar="changed" />, container);
    expect(portalContainer.innerHTML).toBe('<div>changed-changed</div>');
    expect(container.innerHTML).toBe('');
  });
  ...
  // Regression test for https://github.com/Suryanshu-Nabheet/dev.js/issues/19562
  // @gate !disableLegacyMode
  ...
  it('listens to events that do not exist in the Portal subtree', () => {
    ...
    DevjsDOM.render(
      <div onClick={onClick}>
        {DevjsDOM.createPortal(<button ref={ref}>click</button>, document.body)}
      </div>,
      container,
    );
  ...
  it('should throw on bad createPortal argument', () => {
    expect(() => {
      DevjsDOM.createPortal(<div>portal</div>, null);
    }).toThrow('Target container is not a DOM element.');
    expect(() => {
      DevjsDOM.createPortal(<div>portal</div>, document.createTextNode('hi'));
    }).toThrow('Target container is not a DOM element.');
  });
  ...
  it('should warn for non-functional event listeners', () => {
    ...
    DevjsDOM.render(<Example />, container);
    assertConsoleErrorDev([
  ...
  it('should not update event handlers until commit', () => {
    ...
    class Click extends Devjs.Component {
      constructor() {
        super();
        node.click();
      }
    ...
    let inst;
    DevjsDOM.render([<Example key="a" ref={n => (inst = n)} />], container);
  ...
  it('should not warn when rendering into an empty container', () => {
    DevjsDOM.render(<div>foo</div>, container);
    expect(container.innerHTML).toBe('<div>foo</div>');
    DevjsDOM.render(null, container);
    expect(container.innerHTML).toBe('');
    DevjsDOM.render(<div>bar</div>, container);
    expect(container.innerHTML).toBe('<div>bar</div>');
  });

  // @gate !disableLegacyMode
  it('should warn when replacing a container which was manually updated outside of Devjs', async () => {
    // when not messing with the DOM outside of Devjs
    DevjsDOM.render(<div key="1">foo</div>, container);
    DevjsDOM.render(<div key="1">bar</div>, container);
    expect(container.innerHTML).toBe('<div>bar</div>');
    ...
    await expect(async () => {
      await act(() => {
        DevjsDOM.render(<div key="2">baz</div>, container);
      });
    }).rejects.toThrow('The node to be removed is not a child of this node.');
    assertConsoleErrorDev([
      '' +
        'It looks like the Devjs-rendered content of this container was ' +
        'removed without using Devjs. This is not supported and will ' +
        'cause errors. Instead, call DevjsDOM.unmountComponentAtNode ' +
        'to empty a container.',
    ]);
  });

  // @gate !disableLegacyMode
  it('should warn when doing an update to a container manually updated outside of Devjs', () => {
    // when not messing with the DOM outside of Devjs
    DevjsDOM.render(<div>foo</div>, container);
    DevjsDOM.render(<div>bar</div>, container);
    expect(container.innerHTML).toBe('<div>bar</div>');
    // then we mess with the DOM before an update
    container.innerHTML = '<div>MEOW.</div>';
    DevjsDOM.render(<div>baz</div>, container);
    assertConsoleErrorDev([
      '' +
        'It looks like the Devjs-rendered content of this container was ' +
        'removed without using Devjs. This is not supported and will ' +
  ...
  // @gate !disableLegacyMode
  it('should warn when doing an update to a container manually cleared outside of Devjs', () => {
    // when not messing with the DOM outside of Devjs
    DevjsDOM.render(<div>foo</div>, container);
    DevjsDOM.render(<div>bar</div>, container);
    expect(container.innerHTML).toBe('<div>bar</div>');
  ...
  it('should render a text component with a text DOM node on the same document as the container', () => {
    ...
    DevjsDOM.render(textContent, iframeContainer);
  ...
  it('should mount into a document fragment', () => {
    ...
    DevjsDOM.render(<div>foo</div>, fragment);
    expect(container.innerHTML).toBe('');
    container.appendChild(fragment);
    expect(container.innerHTML).toBe('<div>foo</div>');
  });

  // Regression test for https://github.com/Suryanshu-Nabheet/dev.js/issues/12643#issuecomment-413727104
  // @gate !disableLegacyMode
  it('should not diff memoized host components', () => {
    const inputRef = Devjs.createRef();
    ...
    class Child extends Devjs.Component {
      ...
      update = () => {
        ...
        // doesn't cause Devjs to commit updates
  ...
  it('unmounted legacy roots should never clear newer root content from a container', () => {
    ...
    DevjsDOM.render(<OldApp />, container);
    ref.current.focus();

    DevjsDOM.render(<NewApp />, container);
