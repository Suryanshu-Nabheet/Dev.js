...
describe('DevjsDOMFizzForm', () => {
  beforeEach(() => {
    ...
    useFormStatus = require('devjs-dom').useFormStatus;
    useOptimistic = require('devjs').useOptimistic;
  ...
  it('should restore the form fields even if they were incorrectly set', async () => {
    ...
    assertConsoleErrorDev([
      'Cannot specify a encType or method for a form that specifies a function as the action. ' +
        'Devjs provides those automatically. They will get overridden.\n' +
        '    in form (at **)\n' +
        '    in App (at **)',
  ...
  it('can provide a custom action on buttons the server for actions', async () => {
    const hiddenRef = Devjs.createRef();
    const inputRef = Devjs.createRef();
    const buttonRef = Devjs.createRef();
    let foo;
