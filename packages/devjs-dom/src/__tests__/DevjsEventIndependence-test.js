...
describe('DevjsEventIndependence', () => {
  beforeEach(() => {
    jest.resetModules();
  ...
  it('does not crash with other devjs inside', async () => {
    let clicks = 0;
        ...
        root.render(
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: '<button data-devjsid=".z">click me</div>',
            }}
          />,
        );
      });
  ...
  it('does not crash with other devjs outside', async () => {
    let clicks = 0;
    const outer = document.createElement('div');
    document.body.appendChild(outer);
    const root = DevjsDOMClient.createRoot(outer);
    try {
      outer.setAttribute('data-devjsid', '.z');
      await act(() => {
  ...
  it('does not when event fired on unmounted tree', async () => {
    ...
    try {
      ...
      // Now we unmount the component, as if caused by a non-Devjs event handler
