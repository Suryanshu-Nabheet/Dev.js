...
describe('DevjsDOMForm', () => {
  ...
  beforeEach(() => {
    ...
    use = Devjs.use;
    useFormStatus = DevjsDOM.useFormStatus;
    requestFormReset = DevjsDOM.requestFormReset;
  ...
  it('should submit once if one root is nested inside the other', async () => {
    ...
    const innerContainerRef = Devjs.createRef();
    const outerRoot = DevjsDOMClient.createRoot(container);
    await act(async () => {
      outerRoot.render(
    ...
    const innerRoot = DevjsDOMClient.createRoot(innerContainerRef.current);
    await act(async () => {
      innerRoot.render(
        <form action={innerAction} ref={ref}>
          <input type="text" name="data" defaultValue="inner" />
  ...
  it('can read the clicked button in the formdata event', async () => {
    const inputRef = Devjs.createRef();
    const buttonRef = Devjs.createRef();
    const outsideButtonRef = Devjs.createRef();
    const imageButtonRef = Devjs.createRef();
    let button;
    let buttonX;
    let buttonY;
    let title;
  ...
  it('excludes the submitter name when the submitter is a function action', async () => {
    ...
    assertConsoleErrorDev([
      'Cannot specify a "name" prop for a button that specifies a function as a formAction. ' +
        'Devjs needs it to encode which action should be invoked. ' +
  ...
  it('allows a non-devjs html formaction to be invoked', async () => {
    let actionCalled = false;
  ...
  it('should error if submitting a form manually', async () => {
    ...
    expect(error.message).toContain(
      'A Devjs form was unexpectedly submitted. If you called form.submit()',
    );
  });
  ...
  it('useActionState updates state asynchronously and queues multiple actions', async () => {
    ...
    // NOTE: Devjs does not render in between these actions because they all
  ...
  it('reset multiple forms in the same transition', async () => {
    const formRefA = Devjs.createRef();
    const formRefB = Devjs.createRef();
  ...
  it('requestFormReset throws if the form is not managed by Devjs', async () => {
    container.innerHTML = `
      <form id="myform">
  ...
  it("regression: submitter's formAction prop is coerced correctly before checking if it exists", async () => {
    ...
    await act(() => root.render(<App submitterAction="https://devjs.dev/" />));
    await expect(submit(buttonRef.current)).rejects.toThrow(
      'Navigate to: https://devjs.dev/',
    );
  });
  ...
  it(
    ...
    async () => {
      ...
      const formRef = Devjs.createRef();
      const inputRef = Devjs.createRef();
      const outputRef = Devjs.createRef();
      const root = DevjsDOMClient.createRoot(container);
  ...
  it('useFormStatus coerces the value of the "action" prop', async () => {
    ...
    await act(() => root.render(<Form action="https://devjs.dev" />));
    await submit(formRef.current);
    assertLog(['https://devjs.dev']);
