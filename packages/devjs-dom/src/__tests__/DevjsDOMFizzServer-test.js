...
describe('DevjsDOMFizzServer', () => {
  beforeEach(() => {
    ...
    DevjsDOMFizzStatic = require('devjs-dom/static');
    ...
    if (__VARIANT__) {
      ...
      console.error = (error, ...args) => {
        if (
          typeof error !== 'string' ||
          error.indexOf('DevjsDOM.useFormState has been renamed') === -1
        ) {
      ...
      useActionState = DevjsDOM.useFormState;
    } else {
      useActionState = Devjs.useActionState;
    }
    ...
    useSyncExternalStore = Devjs.useSyncExternalStore;
    ...
    if (gate(flags => flags.shouldUseFizzExternalRuntime)) {
      renderOptions.unstable_externalRuntimeSrc =
        'devjs-dom-bindings/src/server/DevjsDOMServerExternalRuntime.js';
    }
  });
  ...
  it('should asynchronously load a lazy component', async () => {
    ...
    let resolveB;
    const LazyB = Devjs.lazy(() => {
      return new Promise(r => {
        resolveB = r;
      });
    });

    class TextWithPunctuation extends Devjs.Component {
      render() {
  ...
  it('#23331: does not warn about hydration mismatches if something suspended in an earlier sibling', async () => {
    ...
    await act(() => {
      DevjsDOMClient.hydrateRoot(container, <HydrateApp />);
    });
  ...
  it('should client render a boundary if a lazy component rejects', async () => {
    ...
    const LazyLazy = Devjs.lazy(async () => {
      return {
    ...
    window.__INIT__ = function () {
      ...
      DevjsDOMClient.hydrateRoot(container, <App isClient={true} />, {
  ...
  it('should have special stacks if Suspense fallback', async () => {
    ...
    const InfiniteComponent = Devjs.lazy(() => {
  ...
  it('should include a component stack across suspended boundaries', async () => {
    ...
    assertConsoleErrorDev([
      '<inCorrectTag /> is using incorrect casing. Use PascalCase for Devjs components, or lowercase for HTML elements.' +
        '\n' +
        '    in inCorrectTag (at **)\n' +
        '    in C (at **)\n' +
        '    in A (at **)',
  ...
  it('should can suspend in a class component with legacy context', async () => {
    class TestProvider extends Devjs.Component {
    ...
    class TestConsumer extends Devjs.Component {
      static contextTypes = {
        test: PropTypes.string,
      };
      render() {
  ...
  it('should resume the context from where it left off', async () => {
    const ContextA = Devjs.createContext('A0');
    const ContextB = Devjs.createContext('B0');
    ...
    class PrintB extends Devjs.Component {
  ...
  it('should recover the outer context when an error happens inside a provider', async () => {
    ...
    function Throws() {
      const value = Devjs.useContext(ContextA);
  ...
  it(
    ...
    async () => {
      ...
      await waitForAll([
        'Yay!',
        'onRecoverableError: There was an error while hydrating but Devjs was able to recover by instead client rendering the entire root.',
  ...
  it('can hydrate uSES in StrictMode with different client and server snapshot (sync)', async () => {
    ...
    await clientAct(() => {
      DevjsDOM.flushSync(() => {
        DevjsDOMClient.hydrateRoot(container, element);
      });
    });
  ...
  it(
    ...
    async () => {
      ...
      const span1Ref = Devjs.createRef();
      const span2Ref = Devjs.createRef();
      const span3Ref = Devjs.createRef();
      ...
      await waitForAll([
        'Yay!',
        'onRecoverableError: There was an error while hydrating but Devjs was able to recover by instead client rendering from the nearest Suspense boundary.',
        'Cause: Hydration error',
      ]);
      expect(getVisibleChildren(container)).toEqual(
        <div>
  ...
  it('does not recreate the fallback if server errors and hydration suspends', async () => {
    ...
    const fallbackRef = Devjs.createRef();
    function App() {
      return (
        <div>
  ...
  it(
    ...
    async () => {
      ...
      // Hydrate the tree. This will suspend.
      isClient = true;
      const root = DevjsDOMClient.hydrateRoot(container, <App color="red" />, {
        onRecoverableError(error) {
          Scheduler.log('onRecoverableError: ' + error.message);
          if (error.cause) {
            Scheduler.log('Cause: ' + normalizeError(error.cause.message));
  ...
  it('logs regular (non-hydration) errors when the UI recovers', async () => {
    ...
    // Devjs will try rendering again synchronously. During the retry, A will
    ...
    await waitForAll([
      ...
      // Log the error
      'onRecoverableError: There was an error during concurrent rendering but Devjs was able to recover by instead synchronously rendering the entire root.',
      'Cause: Oops!',
    ]);
  ...
  it('supports bigint', async () => {
    await act(async () => {
      const {pipe} = DevjsDOMFizzServer.renderToPipeableStream(
        <div>{10n}</div>,
      );
      pipe(writable);
    });
    ...
    expect(document.head.innerHTML).toBe(
      '<script type="importmap">' +
        ...
        (gate(flags => flags.shouldUseFizzExternalRuntime)
          ? '<script src="devjs-dom-bindings/src/server/DevjsDOMServerExternalRuntime.js" async=""></script>'
          : '') +
  ...
  // bugfix: https://github.com/Suryanshu-Nabheet/dev.js/issues/27286
  ...
  // https://github.com/Suryanshu-Nabheet/dev.js/issues/27540
  ...
  it('provides headers after initial work if onHeaders option used', async () => {
    ...
    function Preloads() {
      DevjsDOM.preload('font2', {as: 'font'});
      DevjsDOM.preload('imagepre2', {as: 'image', fetchPriority: 'high'});
      DevjsDOM.preconnect('pre2', {crossOrigin: 'use-credentials'});
      DevjsDOM.prefetchDNS('dns2');
    }
    ...
    function App() {
      DevjsDOM.preload('font', {as: 'font'});
      DevjsDOM.preload('imagepre', {as: 'image', fetchPriority: 'high'});
      DevjsDOM.preconnect('pre', {crossOrigin: 'use-credentials'});
      DevjsDOM.prefetchDNS('dns');
      return (
        <html>
          <body>
            <img src="image" />
...
`
  ...
  it('omits images from preload headers if they contain srcset and sizes', async () => {
    ...
    function App() {
      DevjsDOM.preload('responsive-preload-set-only', {
        as: 'image',
        fetchPriority: 'high',
        imageSrcSet: 'srcset',
      });
      DevjsDOM.preload('responsive-preload', {
        as: 'image',
        fetchPriority: 'high',
        imageSrcSet: 'srcset',
        imageSizes: 'sizes',
      });
      DevjsDOM.preload('non-responsive-preload', {
        as: 'image',
        fetchPriority: 'high',
      });
      return (
...
`
  ...
  it('stops accumulating new headers once the maxHeadersLength limit is satisifed', async () => {
    ...
    function App() {
      DevjsDOM.preconnect('foo');
      DevjsDOM.preconnect('bar');
      DevjsDOM.preconnect('baz');
      return (
        <html>
          <body>hello</body>
        </html>
...
`
  ...
  it('accounts for the length of the interstitial between links when computing the headers length', async () => {
    ...
    function App() {
      // 20 bytes
      DevjsDOM.preconnect('01');
      // 42 bytes
      DevjsDOM.preconnect('02');
      // 64 bytes
      DevjsDOM.preconnect('03');
      // 86 bytes
      DevjsDOM.preconnect('04');
      // 108 bytes
      DevjsDOM.preconnect('05');
      // 130 bytes
      DevjsDOM.preconnect('06');
      // 152 bytes
      DevjsDOM.preconnect('07');
      // 174 bytes
      DevjsDOM.preconnect('08');
      // 196 bytes
      DevjsDOM.preconnect('09');
      // 218 bytes
      DevjsDOM.preconnect('10');
      // 240 bytes
      DevjsDOM.preconnect('11');
      // 262 bytes
      DevjsDOM.preconnect('12');
      // 284 bytes
      DevjsDOM.preconnect('13');
      // 306 bytes
      DevjsDOM.preconnect('14');
      return (
        <html>
          <body>hello</body>
        </html>
  ...
  describe('error escaping', () => {
    ...
    it('escapes error hash, message, and component stack values in clientRenderInstruction (javascript escaping)', async () => {
      ...
      let rejectComponent;
      const SuspensyErroring = Devjs.lazy(() => {
        return new Promise((resolve, reject) => {
          rejectComponent = reject;
        });
      });
  ...
  it('does (unfortunately) send the external runtime for static pages', async () => {
    ...
    expect(document.documentElement.innerHTML).toEqual(
      '<head><script src="devjs-dom-bindings/src/server/DevjsDOMServerExternalRuntime.js" async=""></script>' +
        (gate(flags => flags.enableFizzBlockingRender)
          ? '<link rel="expect" href="#_R_" blocking="render">'
          : '') +
  ...
  it('#24384: Suspending should halt hydration warnings and not emit any if hydration completes successfully after unsuspending', async () => {
    ...
    DevjsDOMClient.hydrateRoot(container, <ClientApp />, {
      onRecoverableError(error) {
        Scheduler.log('onRecoverableError: ' + normalizeError(error.message));
        if (error.cause) {
          Scheduler.log('Cause: ' + normalizeError(error.cause.message));
  ...
  it('#24384: Suspending should halt hydration warnings but still emit hydration warnings after unsuspending if mismatches are genuine', async () => {
    ...
    // resolves.
    const [ClientApp, clientResolve] = makeApp();
    DevjsDOMClient.hydrateRoot(container, <ClientApp text="replaced" />, {
      onRecoverableError(error) {
        Scheduler.log('onRecoverableError: ' + normalizeError(error.message));
        if (error.cause) {
          Scheduler.log('Cause: ' + normalizeError(error.cause.message));
  ...
  it('only warns once on hydration mismatch while within a suspense boundary', async () => {
    ...
    DevjsDOMClient.hydrateRoot(container, <App text="replaced" />, {
      onRecoverableError(error) {
        Scheduler.log('onRecoverableError: ' + normalizeError(error.message));
        if (error.cause) {
          Scheduler.log('Cause: ' + normalizeError(error.cause.message));
  ...
  describe('text separators', () => {
    ...
    it('only includes separators between adjacent text nodes', async () => {
      ...
      const errors = [];
      DevjsDOMClient.hydrateRoot(container, <App name="Foo" />, {
        onRecoverableError(error) {
          errors.push(error.message);
        },
      });
  ...
  describe('title children', () => {
    ...
    it('should warn in dev when given an array of length 2 or more', async () => {
      ...
      assertConsoleErrorDev([
        'Devjs expects the `children` prop of <title> tags to be a string, number, bigint, ' +
          'or object with a novel `toString` method but found an Array with length 2 instead. ' +
          'Browsers treat all child Nodes of <title> tags as Text content and Devjs expects ' +
      ...
      const errors = [];
      DevjsDOMClient.hydrateRoot(document.head, <App />, {
        onRecoverableError(error) {
          errors.push(error.message);
        },
      });
    ...
    it('should warn in dev if you pass a Devjs Component as a child to <title>', async () => {
      ...
      assertConsoleErrorDev([
        'Devjs expects the `children` prop of <title> tags to be a string, number, bigint, ' +
          ...
          'Devjs element which never implements a suitable `toString` method. ' +
          'Browsers treat all child Nodes of <title> tags as Text content and Devjs expects ' +
          'to be able to convert children of <title> tags to a single string value which is ' +
          'why rendering Devjs elements is not supported. If the `children` of <title> is a ' +
          'Devjs Component try moving the <title> tag into that component. ' +
    ...
    it('should warn in dev if you pass an object that does not implement toString as a child to <title>', async () => {
      ...
      assertConsoleErrorDev([
        'Devjs expects the `children` prop of <title> tags to be a string, number, bigint, ' +
          ...
          'content and Devjs expects to be able to convert children of <title> tags to a single string value. ' +
          ...
          'string or number value if so. Otherwise implement a `toString` method that Devjs can ' +
  ...
  it('basic use(context)', async () => {
    const ContextA = Devjs.createContext('default');
    const ContextB = Devjs.createContext('B');
    function Client() {
  ...
  it('context as node', async () => {
    const Context = Devjs.createContext('Hi');
    await act(async () => {
  ...
  it('should correctly handle different promises in Devjs.use() across lazy components', async () => {
    let promise1;
    let promise2;
    let promiseLazy;
    ...
    function Component2() {
      ...
      const data = Devjs.use(promise2);
      return <div>{data}</div>;
    }

    const Component2Lazy = Devjs.lazy(async () => {
  ...
  it('useActionState hydrates without a mismatch', async () => {
    ...
    const childRef = Devjs.createRef(null);
    function Form() {
  ...
  it("useActionState hydrates without a mismatch if there's a render phase update", async () => {
    ...
    const childRef = Devjs.createRef(null);
    function Form() {
      const [localState, setLocalState] = Devjs.useState(0);
      if (localState < 3) {
  ...
  describe('useEffectEvent', () => {
    it('can server render a component with useEffectEvent', async () => {
      ...
      function App() {
        ...
        const onClick = Devjs.useEffectEvent(() => {
          setCount(c => c + 1);
        });
        return (
    ...
    it('throws if useEffectEvent is called during a server render', async () => {
      const logs = [];
      function App() {
        const onRender = Devjs.useEffectEvent(() => {
          logs.push('rendered');
        });
        onRender();
        return <p>Hello</p>;
    ...
    it('does not guarantee useEffectEvent return values during server rendering are distinct', async () => {
      function App() {
        const onClick1 = Devjs.useEffectEvent(() => {});
        const onClick2 = Devjs.useEffectEvent(() => {});
  ...
  it(
    ...
    async () => {
      let setSearch;
      function App() {
        const [search, _setSearch] = Devjs.useState('initial query');
      ...
      await clientAct(() => {
        Devjs.startTransition(() => setSearch('updated query'));
      });
      expect(getVisibleChildren(container)).toEqual(
        <div>
  ...
  it('can resume a prerender that was aborted', async () => {
    ...
    await act(() => {
      pendingPrerender = DevjsDOMFizzStatic.prerenderToNodeStream(<App />, {
        signal,
        onError,
      });
    });
    ...
    errors.length = 0;
    const resumed = await DevjsDOMFizzServer.resumeToPipeableStream(
      <App />,
  ...
  it('can render Suspense before, after, and around <body>', async () => {
    ...
    assertConsoleErrorDev([
      'Cannot render a <meta> outside the main document if it has an `itemProp` prop. `itemProp` suggests the tag belongs to an `itemScope` which can appear anywhere in the DOM. If you were intending for Devjs to hoist this <meta> remove the `itemProp` prop. Otherwise, try moving this tag into the <head> or <body> of the Document.',
  ...
  it('will clean up the head when a hydration mismatch causes a boundary to recover on the client', async () => {
    ...
    const recoverableErrors = [];
    const root = DevjsDOMClient.hydrateRoot(document, <ClientApp />, {
      onRecoverableError(err) {
  ...
  it("shouldn't render styles with mismatched nonce", async () => {
    ...
    assertConsoleErrorDev([
      'Devjs encountered a style tag with `precedence` "default" and `nonce` "R4nd0mR4nd0m". When Devjs manages style rules using `precedence` it will only include rules if the nonce matches the style nonce "R4nd0m" that was included with this render.' +
        '\n    in style (at **)',
    ]);
    expect(getVisibleChildren(document)).toEqual(
      <html>
  ...
  it("should render styles without nonce when render call doesn't receive nonce", async () => {
    ...
    assertConsoleErrorDev([
      'Devjs encountered a style tag with `precedence` "default" and `nonce` "R4nd0m". When Devjs manages style rules using `precedence` it will only include a nonce attributes if you also provide the same style nonce value as a render option.' +
        '\n    in style (at **)',
    ]);
    expect(getVisibleChildren(document)).toEqual(
      <html>
...
` + 'a'.repeat(500);
  ...
  it('should be able to Suspend after aborting in the same component without hanging the render', async () => {
    ...
    const errors = [];
    await act(async () => {
      const result = await DevjsDOMFizzStatic.prerenderToNodeStream(<App />, {
  ...
  it('not error when a suspended fallback segment directly inside another Suspense is abandoned', async () => {
    ...
    function Suspend() {
      return Devjs.use(suspendPromise);
    }
  ...
  it('useId is consistent for siblings when component suspends with nested lazy', async () => {
    ...
    const serverLazyInner = Devjs.lazy(async () => {
      Scheduler.log('server lazy inner initializer');
      return {default: <InnerComponent />};
    });

    const serverLazyOuter = Devjs.lazy(async () => {
    ...
    const clientLazyInner = Devjs.lazy(async () => {
    ...
    const clientLazyOuter = Devjs.lazy(async () => {
