...
// TODO: There's currently no Devjs DOM API to opt into Idle priority updates,
// and there's no native DOM event that maps to idle priority, so this is a
// temporary workaround. Need something like DevjsDOM.unstable_IdleUpdates.
...
function TODO_scheduleContinuousSchedulerTask(fn) {
  DevjsDOM.unstable_runWithPriority(ContinuousEventPriority, () => {
    const prevEvent = window.event;
    window.event = {type: 'message'};
    try {
      fn();
...
describe('DevjsDOMServerSelectiveHydration', () => {
  beforeEach(() => {
    jest.resetModules();
  ...
  describe('can handle replaying events as part of multiple instances of Devjs', () => {
    let resolveInner;
    let resolveOuter;
    let innerPromise;
    let outerPromise;
    ...
    beforeEach(async () => {
      ...
      const outerHTML = DevjsDOMServer.renderToString(<OuterApp />);
      ...
      const innerHTML = DevjsDOMServer.renderToString(<InnerApp />);
      ...
      await OuterTestUtils.act(() =>
        OuterDevjsDOMClient.hydrateRoot(outerContainer, <OuterApp />),
      );
      await InnerTestUtils.act(() =>
        InnerDevjsDOMClient.hydrateRoot(innerContainer, <InnerApp />),
      );
  ...
  it('hydrates before an update even if hydration moves away from it', async () => {
    ...
    const ChildWithBoundary = Devjs.memo(function ({text}) {
      return (
        <Suspense fallback="Loading...">
          <Child text={text} />
    ...
    const finalHTML = DevjsDOMServer.renderToString(<App a="A" />);
    ...
    await act(async () => {
      const root = DevjsDOMClient.hydrateRoot(container, <App a="A" />);
      // Hydrate the shell.
  ...
  it('can force hydration in response to sync update', async () => {
    ...
    let spanRef;
    const finalHTML = DevjsDOMServer.renderToString(<App text="A" />);
    assertLog(['App A', 'Child A']);
    const container = document.createElement('div');
    document.body.appendChild(container);
    container.innerHTML = finalHTML;
    const initialSpan = container.getElementsByTagName('span')[0];
    const root = DevjsDOMClient.hydrateRoot(container, <App text="A" />);
  ...
  it('regression test: can unwind context on selective hydration interruption', async () => {
    const Context = Devjs.createContext('DefaultContext');
  ...
  it('regression: selective hydration does not contribute to "maximum update limit" count', async () => {
    const outsideRef = Devjs.createRef(null);
    const insideRef = Devjs.createRef(null);
    function Child() {
      return (
        <Suspense fallback="Loading...">
          <div ref={insideRef} />
    ...
    function App() {
      const [isMounted, setState] = Devjs.useState(false);
