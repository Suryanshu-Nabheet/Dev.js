...
module.exports = function (initModules) {
  ...
  function resetModules() {
    ({DevjsDOM, DevjsDOMClient, DevjsDOMServer} = initModules());
    act = require('internal-test-utils').act;
  }

  function shouldUseDocument(devjsElement) {
    // Used for whole document tests.
    return devjsElement && devjsElement.type === 'html';
  }

  function getContainerFromMarkup(devjsElement, markup) {
    if (shouldUseDocument(devjsElement)) {
  ...
  // promisified version of DevjsDOM.render()
  async function asyncDevjsDOMRender(devjsElement, domElement, forceHydrate) {
    if (forceHydrate) {
      await act(() => {
      ...
      await act(() => {
        if (DevjsDOMClient) {
          const root = DevjsDOMClient.createRoot(domElement);
          root.render(devjsElement);
        } else {
          DevjsDOM.render(devjsElement, domElement);
        }
      });
    }
  }
  ...
  // renders the devjsElement into domElement, and expects a certain number of errors.
  ...
  function renderIntoDom(
    ...
    return expectErrors(async () => {
      await asyncDevjsDOMRender(devjsElement, domElement, forceHydrate);
      return domElement.firstChild;
    }, errorCount);
  }

  async function renderIntoString(devjsElement, errorCount = 0) {
    return await expectErrors(
      () =>
        new Promise(resolve =>
          resolve(DevjsDOMServer.renderToString(devjsElement)),
        ),
      errorCount,
    );
  }
  ...
  // element that corresponds with the devjsElement.
  // Does not render on client or perform client-side revival.
  async function serverRender(devjsElement, errorCount = 0) {
    const markup = await renderIntoString(devjsElement, errorCount);
    return getContainerFromMarkup(devjsElement, markup).firstChild;
  }
  ...
  async function renderIntoStream(devjsElement, errorCount = 0) {
    return await expectErrors(
      () =>
        new Promise((resolve, reject) => {
          const writable = new DrainWritable();
          const s = DevjsDOMServer.renderToPipeableStream(devjsElement, {
            onShellError(e) {
              reject(e);
            },
          });
  ...
  // returns the DOM element that corresponds with the devjsElement.
  // Does not render on client or perform client-side revival.
  async function streamRender(devjsElement, errorCount = 0) {
    const markup = await renderIntoStream(devjsElement, errorCount);
    let firstNode = getContainerFromMarkup(devjsElement, markup).firstChild;
  ...
  const clientRenderOnBadMarkup = async (element, errorCount = 0) => {
    ...
    await asyncDevjsDOMRender(element, cleanContainer, true);
  ...
  // function takes in a DevjsElement and an optional expected error count and
