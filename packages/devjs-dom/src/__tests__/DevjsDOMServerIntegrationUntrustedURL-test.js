...
const EXPECTED_SAFE_URL =
  "javascript:throw new Error('Devjs has blocked a javascript: URL as a security precaution.')";

describe('DevjsDOMServerIntegration - Untrusted URLs', () => {
  ...
  itRenders('a javascript protocol frame src', async render => {
    if (render === clientCleanRender || render === clientRenderOnServerString) {
      // Devjs does not hydrate framesets properly because the default hydration scope
      // is the body
      return;
    }
    const e = await render(
  ...
  itRenders('only the first invocation of toString', async render => {
    ...
    expect(e.href).toBe('https://devjsjs.org/');
  });
