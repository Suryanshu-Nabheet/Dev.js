...
describe('DevjsDOM', () => {
  ...
  it('should allow children to be passed as an argument', async () => {
    ...
    const root = DevjsDOMClient.createRoot(container);
    await act(() => {
      root.render(Devjs.createElement('div', null, 'child'));
    });
  ...
  it('should overwrite props.children with children argument', async () => {
    ...
    const root = DevjsDOMClient.createRoot(container);
    await act(() => {
      root.render(Devjs.createElement('div', {children: 'fakechild'}, 'child'));
    });
  ...
  it('throws in render() if the mount callback in legacy roots is not a function', async () => {
    ...
    await expect(async () => {
      await act(() => {
        DevjsDOM.render(<A />, myDiv, {foo: 'bar'});
      });
    }).rejects.toThrowError(
      'Invalid argument passed as callback. Expected a function. Instead ' +
    ...
    await expect(async () => {
      await act(() => {
        DevjsDOM.render(<A />, myDiv, new Foo());
      });
    }).rejects.toThrowError(
      'Invalid argument passed as callback. Expected a function. Instead ' +
        'received: [object Object]',
  ...
  it('throws in render() if the update callback in legacy roots is not a function', async () => {
    ...
    DevjsDOM.render(<A />, myDiv); // Re-mount
    await expect(async () => {
      await act(() => {
        DevjsDOM.render(<A />, myDiv, {foo: 'bar'});
      });
  ...
  it('preserves focus', async () => {
    ...
    try {
      ...
      // deterministically force without relying intensely on Devjs DOM
  ...
  it('should not crash calling findDOMNode inside a function component', async () => {
    ...
    if (__DEV__) {
      root = DevjsDOMClient.createRoot(document.createElement('div'));
      await act(() => {
        root.render(<App />);
      });
    }
  ...
  it('reports stacks with re-entrant renderToString() calls on the client', async () => {
    ...
    function ServerEntry() {
      return DevjsDOMServer.renderToString(<Child />);
    }
    ...
    assertConsoleErrorDev([
      // DevjsDOM(App > div > span)
      'Invalid ARIA attribute `ariaTypo`. ARIA attributes follow the pattern aria-* and must be lowercase.\n' +
        '    in span (at **)\n' +
        '    in App (at **)',
      // DevjsDOM(App > div > ServerEntry) >>> DevjsDOMServer(Child) >>> DevjsDOMServer(App2) >>> DevjsDOMServer(blink)
      'Invalid ARIA attribute `ariaTypo2`. ARIA attributes follow the pattern aria-* and must be lowercase.\n' +
        '    in blink (at **)\n' +
        '    in App2 (at **)\n' +
        '    in Child (at **)\n' +
        '    in ServerEntry (at **)',
      // DevjsDOM(App > div > ServerEntry) >>> DevjsDOMServer(Child) >>> DevjsDOMServer(App2 > Child2 > span)
      'Invalid ARIA attribute `ariaTypo3`. ARIA attributes follow the pattern aria-* and must be lowercase.\n' +
        ...
        '    in Child (at **)\n' +
        '    in ServerEntry (at **)',
      // DevjsDOM(App > div > ServerEntry) >>> DevjsDOMServer(Child > span)
      'Invalid ARIA attribute `ariaTypo4`. ARIA attributes follow the pattern aria-* and must be lowercase.\n' +
        '    in span (at **)\n' +
        '    in Child (at **)\n' +
        '    in ServerEntry (at **)',

      // DevjsDOM(App > div > font)
  ...
  it('should render root host components into body scope when the container is a the <html> tag', async () => {
    ...
    const root = DevjsDOMClient.createRoot(document.documentElement);
    await act(() => {
      root.render(<App phase={0} />);
    });
    expect(document.documentElement.outerHTML).toBe(
  ...
  it('can render a Suspense boundary above the <html> tag', async () => {
    ...
    createNewPromise();
    function Comp() {
      const [promise, setPromise] = Devjs.useState(currentPromise);
      suspendOnNewPromise = () => {
    ...
    function Message() {
      const [pendingMessage, setPendingMessage] =
        Devjs.useState(currentMessage);
      suspendOnNewMessage = () => {
        setPendingMessage(createNewMessage());
      };
      return Devjs.use(pendingMessage);
    }
