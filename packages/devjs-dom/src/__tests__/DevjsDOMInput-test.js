...
describe('DevjsDOMInput', () => {
  ...
  it('should control a value in reentrant events', async () => {
    class ControlledInputs extends Devjs.Component {
      state = {value: 'lion'};
      a = null;
      b = null;
      switchedFocus = false;
  ...
  describe('switching text inputs between numeric and string numbers', () => {
    ...
    it('changes the number 2 to "2.0" using a change handler', async () => {
      class Stub extends Devjs.Component {
        state = {
          value: 2,
        };
        onChange = event => {
  ...
  it('should render value for SSR', () => {
    ...
    const markup = DevjsDOMServer.renderToString(element);
    const div = document.createElement('div');
    div.innerHTML = markup;
    expect(div.firstChild.getAttribute('value')).toBe('1');
    expect(div.firstChild.getAttribute('defaultValue')).toBe(null);
  ...
  it('should restore uncontrolled inputs to last defaultValue upon reset', async () => {
    ...
    // https://github.com/Suryanshu-Nabheet/dev.js/issues/4618
  ...
  it('should control radio buttons', async () => {
    class RadioGroup extends Devjs.Component {
      aRef = Devjs.createRef();
      bRef = Devjs.createRef();
      cRef = Devjs.createRef();
    ...
    // Now let's run the actual DevjsDOMInput change event handler
    await act(() => {
  ...
  it('should hydrate controlled radio buttons', async () => {
    function App() {
      const [current, setCurrent] = Devjs.useState('a');
      return (
        <>
          <input
            type="radio"
  ...
  it('should control radio buttons if the tree updates during render in legacy mode', async () => {
    ...
    class ComponentA extends Devjs.Component {
      ...
      componentDidMount() {
        DevjsDOM.render(<ComponentB />, container2);
      }
      render() {
        return (
          <div>
    ...
    DevjsDOM.render(<ComponentA />, container1);
  ...
  it('should control radio buttons if the tree updates during render (case 2; #26876)', async () => {
    let thunk = null;
    function App() {
      const [disabled, setDisabled] = Devjs.useState(false);
      const [value, setValue] = Devjs.useState('one');
  ...
  it('does not raise a validation warning when it switches types', async () => {
    class Input extends Devjs.Component {
  ...
  it('resets value of date/time input to fix bugs in iOS Safari', async () => {
    ...
    if (disableInputAttributeSyncing) {
      ...
      expect(log).toEqual([
        ...
        // https://github.com/Suryanshu-Nabheet/dev.js/issues/7233
  ...
  describe('assigning the value attribute on controlled inputs', function () {
    function getTestInput() {
      return class extends Devjs.Component {
        state = {
