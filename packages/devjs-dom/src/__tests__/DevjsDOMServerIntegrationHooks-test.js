...
describe('DevjsDOMServerHooks', () => {
  ...
  describe('useState', () => {
    ...
    itThrowsWhenRendering(
      ...
      async render => {
        class Counter extends Devjs.Component {
          render() {
            const [count] = useState(0);
  ...
  itRenders(
    ...
    async render => {
      const Context = Devjs.createContext({foo: 0, bar: 0, baz: 0});
  ...
  describe('readContext', () => {
    ...
    itRenders('with a warning inside useMemo and useReducer', async render => {
      const Context = Devjs.createContext(42);

      function ReadInMemo(props) {
        const count = Devjs.useMemo(() => readContext(Context), []);
        return <Text text={count} />;
      }

      function ReadInReducer(props) {
        const [count, dispatch] = Devjs.useReducer(() => readContext(Context));
        if (count !== 42) {
          dispatch();
        }
        return <Text text={count} />;
  ...
  it('renders successfully after a component using hooks throws an error', () => {
    ...
    // by a dispatch call.
    expect(() => DevjsDOMServer.renderToString(<ThrowingComponent />)).toThrow(
