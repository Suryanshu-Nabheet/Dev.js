...
describe('DevjsDOMFizzStaticNode', () => {
  beforeEach(() => {
    jest.resetModules();
    Devjs = require('devjs');
  ...
  it('should call prerenderToNodeStream', async () => {
    const result = await DevjsDOMFizzStatic.prerenderToNodeStream(
      <div>hello world</div>,
    );
  ...
  it('should suppport web streams', async () => {
    const result = await DevjsDOMFizzStatic.prerender(<div>hello world</div>);
  ...
  it('emits all HTML as one unit', async () => {
    ...
    const resultPromise = DevjsDOMFizzStatic.prerenderToNodeStream(
      <div>
  ...
  it('should reject the promise when an error is thrown at the root', async () => {
    ...
    try {
      await DevjsDOMFizzStatic.prerenderToNodeStream(
        <div>
          <Throw />
        </div>,
        {
  ...
  it('should resolve an empty shell if aborting before the shell is complete', async () => {
    const errors = [];
    const controller = new AbortController();
    const promise = DevjsDOMFizzStatic.prerenderToNodeStream(
      <div>
        <InfiniteSuspend />
      </div>,
      {
  ...
  it('should be able to abort before something suspends', async () => {
    ...
    const streamPromise = DevjsDOMFizzStatic.prerenderToNodeStream(
      <div>
        <App />
      </div>,
      {
  ...
  it('supports custom abort reasons with a string', async () => {
    ...
    const errors = [];
    const controller = new AbortController();
    const resultPromise = DevjsDOMFizzStatic.prerenderToNodeStream(<App />, {
      signal: controller.signal,
      onError(x) {
        errors.push(x);
        return 'a digest';
