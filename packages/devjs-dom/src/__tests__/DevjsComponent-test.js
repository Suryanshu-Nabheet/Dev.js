...
describe('DevjsComponent', () => {
  ...
  it('fires the callback after a component is rendered in legacy roots', () => {
    ...
    const container = document.createElement('div');
    DevjsDOM.render(<div />, container, callback);
    expect(callback).toHaveBeenCalledTimes(1);
    DevjsDOM.render(<div className="foo" />, container, callback);
    expect(callback).toHaveBeenCalledTimes(2);
    DevjsDOM.render(<span />, container, callback);
  ...
  it('throws if a plain object is used as a child', async () => {
    ...
    await expect(
      ...
      'Objects are not valid as a Devjs child (found: object with keys {x, y, z}). ' +
  ...
  it('throws if a plain object even if it is in an owner', async () => {
    ...
    await expect(
      ...
      'Objects are not valid as a Devjs child (found: object with keys {a, b, c}).' +
  ...
  it('throws if a plain object is used as a child when using SSR', async () => {
    ...
    const element = <div>{[children]}</div>;
    expect(() => {
      DevjsDOMServer.renderToString(element);
    }).toThrowError(
      'Objects are not valid as a Devjs child (found: object with keys {x, y, z}). ' +
  ...
  it('throws if a plain object even if it is in an owner when using SSR', async () => {
    ...
    const container = document.createElement('div');
    expect(() => {
      DevjsDOMServer.renderToString(<Foo />, container);
    }).toThrowError(
      'Objects are not valid as a Devjs child (found: object with keys {a, b, c}). ' +
        'If you meant to render a collection of children, use ' +
        'an array instead.',
