...
describe('Devjs.act()', () => {
  ...
  const renderConcurrent = (el, dom) => {
    root = DevjsDOMClient.createRoot(dom);
    if (__DEV__) {
      act(() => root.render(el));
    } else {
      root.render(el);
  ...
  describe('concurrent render', () => {
    ...
    describe('sync', () => {
      ...
      it('warns if a setState is called outside of act(...)', () => {
        let setValue = null;
        function App() {
          const [value, _setValue] = Devjs.useState(0);
          setValue = _setValue;
          return value;
        }
        ...
        assertConsoleErrorDev([
          'An update to App inside a test was not wrapped in act(...).\n' +
            '\n' +
            'When testing, code that causes Devjs state updates should be wrapped into act(...):\n' +
            '\n' +
            'act(() => {\n' +
      ...
      it('warns if a setState is called outside of act(...) after a component threw', () => {
        ...
        function App({defaultValue}) {
          ...
          const [value, _setValue] = Devjs.useState(defaultValue);
          setValue = _setValue;
          return value;
        }
      ...
      // @gate __DEV__
      it('does not warn if IS_devjs_ACT_ENVIRONMENT is set to false', () => {
        let setState;
        function App() {
        ...
        const prevIsActEnvironment = global.IS_devjs_ACT_ENVIRONMENT;
        global.IS_devjs_ACT_ENVIRONMENT = false;
        try {
          setState(2);
        } finally {
          global.IS_devjs_ACT_ENVIRONMENT = prevIsActEnvironment;
        }

        // When the flag is restored to its previous value, it should start
        // warning again. This shows that Devjs reads the flag each time.
        setState(3);
        assertConsoleErrorDev([
          'An update to App inside a test was not wrapped in act(...).\n' +
            '\n' +
      ...
      describe('fake timers', () => {
        ...
        it('lets a ticker update', () => {
          function App() {
            const [toggle, setToggle] = Devjs.useState(0);
            Devjs.useEffect(() => {
