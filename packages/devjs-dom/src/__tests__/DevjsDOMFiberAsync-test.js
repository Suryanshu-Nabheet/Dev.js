...
describe('DevjsDOMFiberAsync', () => {
  ...
  describe('concurrent mode', () => {
    it('does not perform deferred updates synchronously', async () => {
      ...
      const asyncValueRef = Devjs.createRef();
      const syncValueRef = Devjs.createRef();
  ...
  it('unmounted roots should never clear newer root content from a container', async () => {
    ...
    function OldApp() {
      const [value, setValue] = Devjs.useState('old');
      function hideOnClick() {
        // Schedule a discrete update.
        setValue('update');
        // Synchronously unmount this root.
        DevjsDOM.flushSync(() => oldRoot.unmount());
      }
      return (
    ...
    const oldRoot = DevjsDOMClient.createRoot(container);
    await act(() => {
    ...
    const newRoot = DevjsDOMClient.createRoot(container);
    DevjsDOM.flushSync(() => {
  ...
  it('should synchronously render the transition lane scheduled in a popState', async () => {
    function App() {
      const [syncState, setSyncState] = Devjs.useState(false);
      const [hasNavigated, setHasNavigated] = Devjs.useState(false);
      function onPopstate() {
  ...
  it('Should not flush transition lanes if there is no transition scheduled in popState', async () => {
    ...
    function App() {
      const [syncState, setSyncState] = Devjs.useState(false);
      const [hasNavigated, _setHasNavigated] = Devjs.useState(false);
  ...
  it('transition lane in popState should be allowed to suspend', async () => {
    ...
    function App() {
      const [pathname, setPathname] = Devjs.useState('/path/a');
  ...
  it('regression: useDeferredValue in popState leads to infinite deferral loop', async () => {
    ...
    function App({initialPathname}) {
      const [pathname, _setPathname] = Devjs.useState('/path/a');
      setPathname = _setPathname;

      const deferredPathname = Devjs.useDeferredValue(pathname);
    ...
    // of transition lanes would cause Devjs to fall into an infinite deferral
  ...
  it('regression: infinite deferral loop caused by unstable useDeferredValue input', async () => {
    ...
    function App() {
      ...
      const {value: deferredPathname} = Devjs.useDeferredValue({
        value: pathname,
      });
      if (i++ > 100) {
