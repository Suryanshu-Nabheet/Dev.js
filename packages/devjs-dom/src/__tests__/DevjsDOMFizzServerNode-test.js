...
describe('DevjsDOMFizzServerNode', () => {
  beforeEach(() => {
    jest.resetModules();
    Devjs = require('devjs');
  ...
  it('should support web streams', async () => {
    const stream = await act(() =>
      DevjsDOMFizzServer.renderToReadableStream(<div>hello world</div>),
    );
  ...
  it('should start writing after pipe', async () => {
    ...
    let pipe;
    await act(() => {
      pipe = DevjsDOMFizzServer.renderToPipeableStream(
        <div>hello world</div>,
      ).pipe;
    });
  ...
  it('should be able to complete by aborting even if the promise never resolves', async () => {
    ...
    let abort;
    await act(() => {
      const pipeable = DevjsDOMFizzServer.renderToPipeableStream(
        <div>
          <Suspense fallback={<div>Loading</div>}>
            <InfiniteSuspend />
          </Suspense>
  ...
  it('should be able to get context value when promise resolves', async () => {
    ...
    const DelayContext = Devjs.createContext(undefined);
    const Component = () => {
      const client = Devjs.useContext(DelayContext);
      if (!client) {
