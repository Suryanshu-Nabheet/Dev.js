...
// Copy of DevjsUpdates using DevjsDOM.render and DevjsDOM.unstable_batchedUpdates.
// Can be deleted when we remove both.
describe('DevjsLegacyUpdates', () => {
  beforeEach(() => {
    jest.resetModules();
    Devjs = require('devjs');
  ...
  it('should batch state and props together', () => {
    ...
    const container = document.createElement('div');
    const instance = DevjsDOM.render(<Component x={0} />, container);
    expect(instance.props.x).toBe(0);
    expect(instance.state.y).toBe(0);

    DevjsDOM.unstable_batchedUpdates(function () {
      DevjsDOM.render(<Component x={1} />, container);
      instance.setState({y: 2});
      expect(instance.props.x).toBe(0);
      expect(instance.state.y).toBe(0);
      expect(updateCount).toBe(0);
  ...
  it('should not reconcile children passed via props', () => {
    ...
    class Top extends Devjs.Component {
      render() {
        return (
          <Middle>
            <Bottom />
    ...
    class Middle extends Devjs.Component {
      componentDidMount() {
        this.forceUpdate();
      }
      ...
      render() {
        numMiddleRenders++;
        return Devjs.Children.only(this.props.children);
      }
    }

    class Bottom extends Devjs.Component {
      render() {
        numBottomRenders++;
        return null;
      }
  ...
  it('should flow updates correctly', () => {
    ...
    class Box extends Devjs.Component {
      boxDivRef = Devjs.createRef();
    ...
    class Switcher extends Devjs.Component {
      state = {tabKey: 'hello'};
      boxRef = Devjs.createRef();
      switcherDivRef = Devjs.createRef();
      render() {
    ...
    const container = document.createElement('div');
    const root = DevjsDOM.render(<App />, container);
  ...
  it('should queue mount-ready handlers across different roots', () => {
    ...
    class A extends Devjs.Component {
      state = {x: 0};
      ...
      render() {
        ...
        portal = DevjsDOM.createPortal(<B ref={n => (b = n)} />, bContainer);
        return (
          <div>
            A{this.state.x}
            {portal}
    ...
    class B extends Devjs.Component {
      state = {x: 0};
    ...
    const container = document.createElement('div');
    const a = DevjsDOM.render(<A />, container);
    DevjsDOM.unstable_batchedUpdates(function () {
      a.setState({x: 1});
      b.setState({x: 1});
    });
  ...
  it('should flush updates in the correct order', () => {
    ...
    const container = document.createElement('div');
    const instance = DevjsDOM.render(<Outer />, container);
  ...
  it('should flush updates in the correct order across roots', () => {
    ...
    class MockComponent extends Devjs.Component {
      render() {
    ...
    const container = document.createElement('div');
    DevjsDOM.render(<MockComponent depth={0} count={2} />, container);
  ...
  it('should queue nested updates', () => {
    // See https://github.com/Suryanshu-Nabheet/dev.js/issues/1147
    ...
    class Y extends Devjs.Component {
      render() {
        return (
          <div>
            <Z />
    ...
    class Z extends Devjs.Component {
      render() {
        return <div />;
      }
    ...
    const x = DevjsDOM.render(<X />, container);
    container = document.createElement('div');
    const y = DevjsDOM.render(<Y />, container);
  ...
  it('should queue updates from during mount', () => {
    // See https://github.com/Suryanshu-Nabheet/dev.js/issues/1353
    let a;
  ...
  it('calls componentWillReceiveProps setState callback properly', () => {
    ...
    const container = document.createElement('div');
    DevjsDOM.render(<A x={1} />, container);
    DevjsDOM.render(<A x={2} />, container);
  ...
  it('does not call render after a component as been deleted', () => {
    ...
    const container = document.createElement('div');
    const component = DevjsDOM.render(<A />, container);
  ...
  it('throws in setState if the update callback is not a function', async () => {
    ...
    let container = document.createElement('div');
    let component = DevjsDOM.render(<A />, container);
    ...
    container = document.createElement('div');
    component = DevjsDOM.render(<A />, container);
    await expect(async () => {
      await act(() => {
  ...
  it('does not update one component twice in a batch (#2410)', () => {
    ...
    const container = document.createElement('div');
    const parent = DevjsDOM.render(<Parent />, container);
  ...
  it('does not update one component twice in a batch (#6371)', () => {
    ...
    class EmitsChangeOnUnmount extends Devjs.Component {
    ...
    class ForceUpdatesOnChange extends Devjs.Component {
      componentDidMount() {
    ...
    DevjsDOM.render(<App />, document.createElement('div'));
  });

  it('unstable_batchedUpdates should return value from a callback', () => {
    const result = DevjsDOM.unstable_batchedUpdates(function () {
      return 42;
    });
  ...
  it('unmounts and remounts a root in the same batch', () => {
    const container = document.createElement('div');
    DevjsDOM.render(<span>a</span>, container);
    DevjsDOM.unstable_batchedUpdates(function () {
      DevjsDOM.unmountComponentAtNode(container);
      DevjsDOM.render(<span>b</span>, container);
    });
  ...
  it('handles reentrant mounting in synchronous mode', () => {
    let mounts = 0;
    class Editor extends Devjs.Component {
      render() {
  ...
  it('mounts and unmounts are sync even in a batch', () => {
    ...
    DevjsDOM.unstable_batchedUpdates(() => {
      DevjsDOM.render(<div>Hello</div>, container);
    ...
    class Foo extends Devjs.Component {
      componentDidMount() {
        ...
        for (let i = 0; i < limit; i++) {
          if (i < limit - 1) {
            DevjsDOM.render(<div />, document.createElement('div'));
          } else {
            DevjsDOM.render(<div />, document.createElement('div'), () => {
  ...
  it('resets the update counter for unrelated updates', async () => {
    ...
    class EventuallyTerminating extends Devjs.Component {
      state = {step: 0};
      componentDidMount() {
    ...
    await expect(async () => {
      await act(() => {
        DevjsDOM.render(<EventuallyTerminating ref={ref} />, container);
      });
  ...
  it('does not fall into an infinite update loop', async () => {
    class NonTerminating extends Devjs.Component {
      state = {step: 0};
      componentDidMount() {
        this.setState({step: 1});
      }
    ...
    await expect(async () => {
      await act(() => {
        DevjsDOM.render(<NonTerminating />, container);
      });
    }).rejects.toThrow('Maximum');
  });
  ...
  it('does not fall into an infinite update loop with useLayoutEffect', async () => {
    function NonTerminating() {
      const [step, setStep] = Devjs.useState(0);
  ...
  it('can recover after falling into an infinite update loop', async () => {
    ...
    class Terminating extends Devjs.Component {
      state = {step: 0};
      componentDidMount() {
        this.setState({step: 1});
      }
    ...
    DevjsDOM.render(<Terminating />, container);
