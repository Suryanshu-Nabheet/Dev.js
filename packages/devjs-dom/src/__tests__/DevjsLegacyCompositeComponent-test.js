...
describe('DevjsLegacyCompositeComponent', () => {
  beforeEach(() => {
    jest.resetModules();
    Devjs = require('devjs');
  ...
  it('should warn about `setState` in render in legacy mode', () => {
    ...
    const instance2 = DevjsDOM.render(<Component prop={123} />, container);
    ...
    DevjsDOM.render(<Component prop={123} />, container);
  });
  ...
  it('should pass context to children when not owner', async () => {
    ...
    class Grandchild extends Devjs.Component {
      static contextTypes = {
        foo: PropTypes.string,
      };
  ...
  it('unmasked context propagates through updates', () => {
    class Leaf extends Devjs.Component {
      static contextTypes = {
    ...
    class Intermediary extends Devjs.Component {
      UNSAFE_componentWillReceiveProps(nextProps, nextContext) {
    ...
    DevjsDOM.render(<Parent cntxt="noise" />, div);
    assertConsoleErrorDev([
      'Parent uses the legacy childContextTypes API which will soon be removed. ' +
        'Use Devjs.createContext() instead. (https://devjs.dev/link/legacy-context)\n' +
        '    in Parent (at **)',
    ...
    DevjsDOM.render(<Parent cntxt="bar" />, div);
  ...
  it('should trigger componentWillReceiveProps for context changes', () => {
    ...
    class ChildWithContext extends Devjs.Component {
      static contextTypes = {
        foo: PropTypes.string.isRequired,
      };
    ...
    class ChildWithoutContext extends Devjs.Component {
      UNSAFE_componentWillReceiveProps(nextProps, nextContext) {
        expect('foo' in nextContext).toBe(false);
  ...
  it('only renders once if updated in componentWillReceiveProps in legacy mode', () => {
    ...
    const container = document.createElement('div');
    const instance = DevjsDOM.render(<Component update={0} />, container);
    expect(renders).toBe(1);
    expect(instance.state.updated).toBe(false);
    DevjsDOM.render(<Component update={1} />, container);
  ...
  it('should update refs if shouldComponentUpdate gives false in legacy mode', () => {
    class Static extends Devjs.Component {
    ...
    class Component extends Devjs.Component {
      static0Ref = Devjs.createRef();
      static1Ref = Devjs.createRef();
    ...
    const container = document.createElement('div');
    const comp = DevjsDOM.render(<Component flipped={false} />, container);
    ...
    // contents do not
    DevjsDOM.render(<Component flipped={true} />, container);
  ...
  it('should replace state in legacy mode', async () => {
    class Moo extends Devjs.Component {
      state = {x: 1};
      render() {
        return <div />;
      }
  ...
  it('should not warn about unmounting during unmounting in legacy mode', () => {
    ...
    class Component extends Devjs.Component {
      componentDidMount() {
        DevjsDOM.render(<div />, layer);
      }

      componentWillUnmount() {
        DevjsDOM.unmountComponentAtNode(layer);
      }
    ...
    DevjsDOM.render(<Outer />, container);
  });
});
