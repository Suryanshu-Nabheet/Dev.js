...
describe('FragmentRefs', () => {
  beforeEach(() => {
    ...
    createPortal = DevjsDOM.createPortal;
  ...
  it('attaches fragment handles to nodes', async () => {
    const fragmentParentRef = Devjs.createRef();
    const fragmentRef = Devjs.createRef();
    ...
    expect(childA.unstable_devjsFragments.has(fragmentRef.current)).toBe(true);
    expect(childB.unstable_devjsFragments.has(fragmentRef.current)).toBe(true);
    expect(childC.unstable_devjsFragments.has(fragmentRef.current)).toBe(false);
    expect(childA.unstable_devjsFragments.has(fragmentParentRef.current)).toBe(
      true,
    );
    expect(childB.unstable_devjsFragments.has(fragmentParentRef.current)).toBe(
      true,
    );
    expect(childC.unstable_devjsFragments.has(fragmentParentRef.current)).toBe(
      true,
    );
    ...
    expect(childD.unstable_devjsFragments.has(fragmentRef.current)).toBe(false);
    expect(childD.unstable_devjsFragments.has(fragmentParentRef.current)).toBe(
      true,
    );
  });
  ...
  describe('focus methods', () => {
    ...
    describe('blur()', () => {
      ...
      it('does not remove focus from elements outside of the Fragment', async () => {
        const fragmentRefA = Devjs.createRef();
        const fragmentRefB = Devjs.createRef();
        const root = DevjsDOMClient.createRoot(container);
  ...
  describe('events', () => {
    describe('add/remove event listeners', () => {
      ...
      it('adds and removes event listeners from children', async () => {
        const parentRef = Devjs.createRef();
        const fragmentRef = Devjs.createRef();
        const childARef = Devjs.createRef();
        const childBRef = Devjs.createRef();
        const root = DevjsDOMClient.createRoot(container);
      ...
      it('adds and removes event listeners from children with multiple fragments', async () => {
        const fragmentRef = Devjs.createRef();
        const nestedFragmentRef = Devjs.createRef();
        const nestedFragmentRef2 = Devjs.createRef();
        const childARef = Devjs.createRef();
        const childBRef = Devjs.createRef();
        const childCRef = Devjs.createRef();
        const root = DevjsDOMClient.createRoot(container);
      ...
      it('adds an event listener to a newly added child', async () => {
        ...
        function Component() {
          const [shouldShowChild, setShouldShowChild] = Devjs.useState(false);
          showChild = () => {
      ...
      it('applies event listeners to host children nested within non-host children', async () => {
        const fragmentRef = Devjs.createRef();
        const childRef = Devjs.createRef();
        const nestedChildRef = Devjs.createRef();
        const root = DevjsDOMClient.createRoot(container);
      ...
      it('allows adding and cleaning up listeners in effects', async () => {
        ...
        function Test() {
          ...
          const [_, setState] = Devjs.useState(0);
          rerender = () => {
            setState(p => p + 1);
          };
      ...
      it('does not apply removed event listeners to new children', async () => {
        const root = DevjsDOMClient.createRoot(container);
        const fragmentRef = Devjs.createRef(null);
        function Test() {
          return (
            <Fragment ref={fragmentRef}>
              <div id="child-a" />
      ...
      describe('with activity', () => {
        ...
        it('handles Activity modes switching', async () => {
          const fragmentRef = Devjs.createRef();
          const fragmentRef2 = Devjs.createRef();
          const parentRef = Devjs.createRef();
          const root = DevjsDOMClient.createRoot(container);
  ...
  describe('observers', () => {
    ...
    it('attaches handles to observed elements to allow caching of observers', async () => {
      ...
      function createObserverIfNeeded(fragmentInstance, onIntersection) {
        ...
        const observer = new IntersectionObserver(entries => {
          entries.forEach(entry => {
            const fragmentInstances = entry.target.unstable_devjsFragments;
      ...
      function IntersectionObserverFragment({onIntersection, children}) {
        ...
        return <Devjs.Fragment ref={fragmentRef}>{children}</Devjs.Fragment>;
      }
  ...
  describe('getRootNode', () => {
    ...
    // with a future refactor. See: https://github.com/Suryanshu-Nabheet/dev.js/pull/32682#discussion_r2008313082
    // @gate enableFragmentRefs
    it('returns the topmost disconnected element if the fragment and parent are unmounted', async () => {
      const containerRef = Devjs.createRef();
      const parentRef = Devjs.createRef();
      const fragmentRef = Devjs.createRef();
      const root = DevjsDOMClient.createRoot(container);
  ...
  describe('compareDocumentPosition', () => {
    ...
    it('returns the relationship between the fragment instance and a given node', async () => {
      const fragmentRef = Devjs.createRef();
      const beforeRef = Devjs.createRef();
      const afterRef = Devjs.createRef();
      const middleChildRef = Devjs.createRef();
      const firstChildRef = Devjs.createRef();
      const lastChildRef = Devjs.createRef();
      const containerRef = Devjs.createRef();
    ...
    it('handles fragment instances with one child', async () => {
      ...
      const afterRef = Devjs.createRef();
      const containerRef = Devjs.createRef();
      const onlyChildRef = Devjs.createRef();
      const disconnectedElement = document.createElement('div');
      const root = DevjsDOMClient.createRoot(container);
    ...
    it('handles empty fragment instances', async () => {
      const fragmentRef = Devjs.createRef();
      const beforeParentRef = Devjs.createRef();
      const beforeRef = Devjs.createRef();
      const afterRef = Devjs.createRef();
      const afterParentRef = Devjs.createRef();
      const containerRef = Devjs.createRef();
      const root = DevjsDOMClient.createRoot(container);
      ...
      function Test() {
        return (
          <Devjs.Fragment ref={fragmentRef}>
            ...
            <Devjs.Fragment ref={nestedFragmentRef}>
    ...
    it('compares a root-level Fragment', async () => {
      const fragmentRef = Devjs.createRef();
      const emptyFragmentRef = Devjs.createRef();
      const childRef = Devjs.createRef();
      const siblingPrecedingRef = Devjs.createRef();
      const siblingFollowingRef = Devjs.createRef();
      const root = DevjsDOMClient.createRoot(container);
    ...
    describe('with portals', () => {
      ...
      it('handles portaled elements', async () => {
        const fragmentRef = Devjs.createRef();
        const portaledSiblingRef = Devjs.createRef();
        const portaledChildRef = Devjs.createRef();
        ...
        // The sibling is preceding in both the DOM and the Devjs tree
        expectPosition(
        ...
        // The child is contained by in the Devjs tree but not in the DOM
        expectPosition(
      ...
      it('handles multiple portals to the same element', async () => {
        ...
        const childBRef = Devjs.createRef();
        const childCRef = Devjs.createRef();
        const childDRef = Devjs.createRef();
        const childERef = Devjs.createRef();

        function Test() {
          const [c, setC] = Devjs.useState(false);
          Devjs.useEffect(() => {
            setC(true);
          });
        ...
        // Contained by in DOM, but following in Devjs tree
        expectPosition(
          fragmentRef.current.compareDocumentPosition(childBRef.current),
          {
            preceding: false,
        ...
        // Preceding DOM but following in Devjs tree
        expectPosition(
  ...
  describe('scrollIntoView', () => {
    ...
    describe('with children', () => {
      ...
      it('handles portaled elements -- different scroll container', async () => {
        const fragmentRef = Devjs.createRef();
        const headerChildRef = Devjs.createRef();
        const childARef = Devjs.createRef();
        const childBRef = Devjs.createRef();
        const childCRef = Devjs.createRef();
        const scrollContainerRef = Devjs.createRef();
        const scrollContainerNestedRef = Devjs.createRef();
        const root = DevjsDOMClient.createRoot(container);
    ...
    describe('without children', () => {
      ...
      it('calls scrollIntoView on the next sibling by default, or if alignToTop=true', async () => {
        const fragmentRef = Devjs.createRef();
        const siblingARef = Devjs.createRef();
        const siblingBRef = Devjs.createRef();
        const root = DevjsDOMClient.createRoot(container);
        await act(() => {
          root.render(
            <div>
