...
describe('DevjsDOMTestSelectors', () => {
  let Devjs;
  let createRoot;
  let act;
  ...
  beforeEach(() => {
    ...
    if (__EXPERIMENTAL__ || global.__WWW__) {
      const DevjsDOM = require('devjs-dom/unstable_testing');
      createComponentSelector = DevjsDOM.createComponentSelector;
      createHasPseudoClassSelector = DevjsDOM.createHasPseudoClassSelector;
      createRoleSelector = DevjsDOM.createRoleSelector;
      createTextSelector = DevjsDOM.createTextSelector;
      createTestNameSelector = DevjsDOM.createTestNameSelector;
      findAllNodes = DevjsDOM.findAllNodes;
      findBoundingRects = DevjsDOM.findBoundingRects;
      focusWithin = DevjsDOM.focusWithin;
      getFindAllNodesFailureDescription =
        DevjsDOM.getFindAllNodesFailureDescription;
      observeVisibleRects = DevjsDOM.observeVisibleRects;
      createRoot = DevjsDOM.createRoot;
    }
  ...
  describe('findAllNodes', () => {
    ...
    it('should not support searching from a previous match if the match did not have a data-testname', async () => {
      ...
      expect(() => {
        ...
        'Invalid host root specified. Should be either a Devjs container or a node with a testname attribute.',
      );
    });
    ...
    it('should not search within hidden subtrees', async () => {
      const ref1 = Devjs.createRef(null);
      const ref2 = Devjs.createRef(null);
    ...
    it('should throw if no container can be found', () => {
      expect(() => findAllNodes(document.body, [])).toThrow(
        'Could not find Devjs container within specified host subtree.',
      );
    });
...
No matching component was found for:
  ...
  describe('findBoundingRects', () => {
    ...
    it('should return a multiple rects for single match that returns a fragment', async () => {
      const refA = Devjs.createRef();
      const refB = Devjs.createRef();
    ...
    it('should merge some types of adjacent rects (if they are the same in one dimension)', async () => {
      ...
      const refB = Devjs.createRef();
      const refC = Devjs.createRef();
      const refD = Devjs.createRef();
      const refE = Devjs.createRef();
      const refF = Devjs.createRef();
      const refG = Devjs.createRef();
  ...
  describe('focusWithin', () => {
    ...
    it('should successfully focus the first focusable element within the tree', async () => {
      const secondRef = Devjs.createRef(null);
    ...
    it('should successfully focus the first focusable element even if application logic interferes', async () => {
      const ref = Devjs.createRef(null);
