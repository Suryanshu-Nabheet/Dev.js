...
describe('DevjsUpdates', () => {
  ...
  it('should batch state when updating state twice', async () => {
    ...
    function Component() {
      const [state, _setState] = Devjs.useState(0);
  ...
  it('should batch state when updating two different states', async () => {
    ...
    function Component() {
      const [stateA, _setStateA] = Devjs.useState(0);
      const [stateB, _setStateB] = Devjs.useState(0);
  ...
  it('should flow updates correctly', async () => {
    ...
    const container = document.createElement('div');
    await act(() => {
      DevjsDOMClient.createRoot(container).render(<App />);
    });
    ...
    class MockComponent extends Devjs.Component {
      ...
      componentDidMount() {
        ...
        if (this.props.depth < this.props.count) {
          const root = DevjsDOMClient.createRoot(findDOMNode(this));
          root.render(
            <MockComponent
  ...
  it('delays sync updates inside hidden subtrees in Concurrent Mode', async () => {
    ...
    let setCounter;
    function Bar() {
      const [counter, _setCounter] = Devjs.useState(0);
    ...
    class Foo extends Devjs.Component {
      componentDidMount() {
        ...
        for (let i = 0; i < limit; i++) {
          if (i < limit - 1) {
            DevjsDOMClient.createRoot(document.createElement('div')).render(
              <Component />,
            );
          } else {
  ...
  it("does not infinite loop if there's a synchronous render phase update on another component", async () => {
    ...
    let setState;
    function App() {
      const [, _setState] = Devjs.useState(0);
      setState = _setState;
      return <Child />;
    }
    ...
    await expect(async () => {
      await act(() => DevjsDOM.flushSync(() => root.render(<App />)));
  ...
  it("does not infinite loop if there's an async render phase update on another component", async () => {
    ...
    await expect(async () => {
      await act(() => {
        Devjs.startTransition(() => root.render(<App />));
      });
    }).rejects.toThrow('Maximum update depth exceeded');
  ...
  if (__DEV__) {
    it('warns about a deferred infinite update loop with useEffect', async () => {
      function NonTerminating() {
        const [step, setStep] = Devjs.useState(0);
        Devjs.useEffect(function myEffect() {
          setStep(x => x + 1);
        });
        return step;
      }
  ...
  it('prevents infinite update loop triggered by synchronous updates in useEffect', async () => {
    ...
    expect(errors).toEqual([
      'Maximum update depth exceeded. ' +
        ...
        'Devjs limits the number of nested updates to prevent infinite loops.' +
  ...
  it('prevents infinite update loop triggered by too many updates in ref callbacks', async () => {
    ...
    function TooManyRefUpdates() {
      const [count, _scheduleUpdate] = Devjs.useReducer(c => c + 1, 0);
