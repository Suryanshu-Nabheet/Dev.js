...
describe('DevjsDOMFizzServerBrowser', () => {
  beforeEach(() => {
    jest.resetModules();
  ...
  it('should emit DOCTYPE at the root of the document', async () => {
    const stream = await serverAct(() =>
      DevjsDOMFizzServer.renderToReadableStream(
        <html>
  ...
  it('supports custom abort reasons with a string', async () => {
    ...
    const controller = new AbortController();
    await serverAct(() =>
      DevjsDOMFizzServer.renderToReadableStream(<App />, {
        signal: controller.signal,
        onError(x) {
          errors.push(x);
          return 'a digest';
  ...
  // https://github.com/Suryanshu-Nabheet/dev.js/pull/25534/files - fix transposed escape functions
