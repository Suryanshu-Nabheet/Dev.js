...
describe('DevjsDOMFizzStaticBrowser', () => {
  beforeEach(() => {
    jest.resetModules();
    ...
    DevjsDOMFizzStatic = require('devjs-dom/static.browser');
  ...
  it('should call prerender', async () => {
    const result = await serverAct(() =>
      DevjsDOMFizzStatic.prerender(<div>hello world</div>),
    );
  ...
  it('should be able to complete by aborting even if the promise never resolves', async () => {
    ...
    let resultPromise;
    await serverAct(() => {
      resultPromise = DevjsDOMFizzStatic.prerender(
        <div>
          <Suspense fallback={<div>Loading</div>}>
            <InfiniteSuspend />
          </Suspense>
  ...
  it('supports custom abort reasons with a string', async () => {
    ...
    let resultPromise;
    await serverAct(() => {
      resultPromise = DevjsDOMFizzStatic.prerender(<App />, {
        signal: controller.signal,
        onError(x) {
          errors.push(x);
          return 'a digest';
  ...
  it('logs an error if onHeaders throws but continues the prerender', async () => {
    ...
    const prerendered = await serverAct(() =>
      DevjsDOMFizzStatic.prerender(<div>hello</div>, {
        onHeaders,
        onError,
      }),
    );
  ...
  it('can resume render of a prerender', async () => {
    ...
    await serverAct(async () => {
      pendingResult = DevjsDOMFizzStatic.prerender(<App />, {
        signal: controller.signal,
        onError(x) {
          errors.push(x.message);
        },
    ...
    await serverAct(async () => {
      pendingResult = DevjsDOMFizzStatic.resumeAndPrerender(
        <App />,
    ...
    const dynamic = await serverAct(() =>
      DevjsDOMFizzServer.resume(<App />, JSON.parse(postponedState2)),
    );
  ...
  it('will render fallback Document when erroring a boundary above the body', async () => {
    ...
    const resumeErrors = [];
    const dynamic = await serverAct(() =>
      DevjsDOMFizzServer.resume(<App />, JSON.parse(postponedState), {
        onError: e => {
  ...
  it('can resume a partially prerendered SuspenseList', async () => {
    ...
    const pendingResult = serverAct(() =>
      DevjsDOMFizzStatic.prerender(<App />, {
        signal: controller.signal,
        onError(x) {
          errors.push(x.message);
        },
