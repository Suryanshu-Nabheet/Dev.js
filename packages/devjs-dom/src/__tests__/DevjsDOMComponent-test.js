...
describe('DevjsDOMComponent', () => {
  ...
  describe('updateDOM', () => {
    ...
    it('should warn for unknown prop', async () => {
      ...
      assertConsoleErrorDev([
        'Invalid value for prop `foo` on <div> tag. Either remove it ' +
          ...
          'it in the DOM. For details, see https://devjs.dev/link/attribute-behavior ' +
          '\n    in div (at **)',
      ]);
    });
    ...
    it('should group multiple unknown prop warnings together', async () => {
      ...
      assertConsoleErrorDev([
        'Invalid values for props `foo`, `baz` on <div> tag. Either remove ' +
          ...
          'them in the DOM. For details, see https://devjs.dev/link/attribute-behavior ' +
          '\n    in div (at **)',
      ]);
    });
    ...
    it('should warn for badly cased Devjs attributes', async () => {
      const container = document.createElement('div');
      const root = DevjsDOMClient.createRoot(container);
      await act(() => {
    ...
    it('should apply Devjs-specific aliases to HTML elements', async () => {
      const container = document.createElement('div');
      const root = DevjsDOMClient.createRoot(container);
      await act(() => {
    ...
    it('should apply Devjs-specific aliases to SVG elements', async () => {
      const container = document.createElement('div');
      const root = DevjsDOMClient.createRoot(container);
      await act(() => {
    ...
    it('should not apply Devjs-specific aliases to custom elements', async () => {
      const container = document.createElement('div');
      const root = DevjsDOMClient.createRoot(container);
      await act(() => {
    ...
    it('should reject attribute key injection attack on markup for regular DOM (SSR)', () => {
      for (let i = 0; i < 3; i++) {
        const element1 = Devjs.createElement(
          'div',
          {'blah" onclick="beevil" noise="hi': 'selected'},
          null,
        );
        const element2 = Devjs.createElement(
          'div',
          {'></div><script>alert("hi")</script>': 'selected'},
          null,
        );
        const result1 = DevjsDOMServer.renderToString(element1);
        const result2 = DevjsDOMServer.renderToString(element2);
    ...
    it('should reject attribute key injection attack on update for regular DOM', async () => {
      for (let i = 0; i < 3; i++) {
        const container = document.createElement('div');
        const beforeUpdate = Devjs.createElement('div', {}, null);
        const root = DevjsDOMClient.createRoot(container);
        await act(() => {
    ...
    it('should reject attribute key injection attack on update for custom elements', async () => {
      for (let i = 0; i < 3; i++) {
        const container = document.createElement('div');
        const beforeUpdate = Devjs.createElement('x-foo-component', {}, null);
        const root = DevjsDOMClient.createRoot(container);
        await act(() => {
          root.render(beforeUpdate);
        });
    ...
    it('should not incur unnecessary DOM mutations for controlled string properties', async () => {
      ...
      assertConsoleErrorDev([
        'A component is changing a controlled input to be uncontrolled. This is likely caused by ' +
          ...
          'More info: https://devjs.dev/link/controlled-components\n' +
          '    in input (at **)',
      ]);
      expect(nodeValueSetter).toHaveBeenCalledTimes(1);
    ...
    it('handles multiple child updates without interference', async () => {
      // This test might look like it's just testing DevjsMultiChild but the
  ...
  describe('createOpenTagMarkup', () => {
    ...
    function genMarkup(props) {
      return DevjsDOMServer.renderToString(<div {...props} />);
    }
  ...
  describe('mountComponent', () => {
    ...
    it('should warn for uppercased selfclosing tags', () => {
      class Container extends Devjs.Component {
        render() {
          return Devjs.createElement('BR', null);
        }
      }

      const returnedValue = DevjsDOMServer.renderToString(<Container />);
      assertConsoleErrorDev([
    ...
    it('should warn on upper case HTML tags, not SVG nor custom tags', async () => {
      ...
      await act(() => {
        root.render(
          Devjs.createElement('svg', null, Devjs.createElement('PATH')),
        );
      });
      ...
      root = DevjsDOMClient.createRoot(container);
      await act(() => {
        root.render(Devjs.createElement('CUSTOM-TAG'));
      });
      ...
      await act(() => {
        root.render(Devjs.createElement('IMG'));
      });
      assertConsoleErrorDev([
    ...
    it('should warn on props reserved for future use', async () => {
      ...
      assertConsoleErrorDev([
        'The `aria` attribute is reserved for future use in Devjs. ' +
      ...
      try {
        ...
        assertConsoleErrorDev([
          'The tag <bar> is unrecognized in this browser. ' +
            'If you meant to render a Devjs component, start its name with an uppercase letter.\n' +
            '    in bar (at **)',
        ]);
        // Test deduplication
        await act(() => {
        ...
        assertConsoleErrorDev([
          '<hasOwnProperty /> is using incorrect casing. ' +
            'Use PascalCase for Devjs components, or lowercase for HTML elements.\n' +
    ...
    it('should treat menuitem as a void element but still create the closing tag', async () => {
      ...
      const returnedValue = DevjsDOMServer.renderToString(
        <menu>
          <menuitem />
        </menu>,
      );
    ...
    it('should validate against multiple children props', async () => {
      await expect(async () => {
        ...
        '`props.dangerouslySetInnerHTML` must be in the form `{__html: ...}`. ' +
          'Please visit https://devjs.dev/link/dangerously-set-inner-html for more information.',
      );
    });
    ...
    it('should throw for children on void elements', async () => {
      class X extends Devjs.Component {
        render() {
  ...
  describe('updateComponent', () => {
    ...
    it('should warn about contentEditable and children', async () => {
      ...
      assertConsoleErrorDev([
        'A component is `contentEditable` and contains `children` managed by Devjs. ' +
    ...
    it('should report component containing invalid styles', async () => {
      class Animal extends Devjs.Component {
        render() {
          return <div style={1} />;
        }
      }
    ...
    it('unmounts children before unsetting DOM node info', async () => {
      class Inner extends Devjs.Component {
        ...
        componentWillUnmount() {
          // Should not throw
          expect(
            DevjsDOM.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE.findDOMNode(
              this,
            ).nodeName,
          ).toBe('SPAN');
        }
  ...
  describe('tag sanitization', () => {
    it('should throw when an invalid tag name is used server-side', () => {
      const hackzor = Devjs.createElement('script tag');
      expect(() => DevjsDOMServer.renderToString(hackzor)).toThrowError(
        'Invalid tag: script tag',
      );
    });

    it('should throw when an attack vector is used server-side', () => {
      const hackzor = Devjs.createElement('div><img /><div');
      expect(() => DevjsDOMServer.renderToString(hackzor)).toThrowError(
  ...
  describe('nesting validation', () => {
    ...
    it('warns on invalid nesting at root', async () => {
      ...
      const root = DevjsDOMClient.createRoot(p);
    ...
    it('warns nicely for table rows', async () => {
      class Row extends Devjs.Component {
        render() {
          return <tr>x</tr>;
        }
      }
    ...
    it('gives useful context in warnings 2', async () => {
      ...
      class Table extends Devjs.Component {
        render() {
          return <table>{this.props.children}</table>;
        }
      }

      class FancyTable extends Devjs.Component {
        render() {
          return <Table>{this.props.children}</Table>;
        }
      }
    ...
    it('gives useful context in warnings 5', async () => {
      ...
      class Link extends Devjs.Component {
        render() {
    ...
    it('should warn about incorrect casing on properties (ssr)', () => {
      DevjsDOMServer.renderToString(
        Devjs.createElement('input', {type: 'text', tabindex: '1'}),
      );
      assertConsoleErrorDev([
    ...
    it('should warn about incorrect casing on event handlers (ssr)', () => {
      DevjsDOMServer.renderToString(
        Devjs.createElement('input', {type: 'text', oninput: '1'}),
      );
      assertConsoleErrorDev([
        'Invalid event handler property `oninput`. ' +
          'Devjs events use the camelCase naming convention, ' +
      ...
      DevjsDOMServer.renderToString(
        Devjs.createElement('input', {type: 'text', onKeydown: '1'}),
      );
    ...
    it('should warn about incorrect casing on event handlers', async () => {
      ...
      await act(() => {
        root.render(Devjs.createElement('input', {type: 'text', oninput: '1'}));
      });
      assertConsoleErrorDev([
    ...
    it('should warn about class', async () => {
      ...
      const root = DevjsDOMClient.createRoot(container);
      await act(() => {
        root.render(Devjs.createElement('div', {class: 'muffins'}));
      });
      assertConsoleErrorDev([
    ...
    it('should warn about class (ssr)', () => {
      DevjsDOMServer.renderToString(
        Devjs.createElement('div', {class: 'muffins'}),
      );
      assertConsoleErrorDev([
        'Invalid DOM property `class`. Did you mean `className`?\n' +
          '    in div (at **)',
    ...
    it('should warn about props that are no longer supported', async () => {
      ...
      assertConsoleErrorDev([
        'Devjs uses onFocus and onBlur instead of onFocusIn and onFocusOut. ' +
          'All Devjs events are normalized to bubble, so onFocusIn and onFocusOut are not needed/supported by Devjs.\n' +
          '    in div (at **)',
      ]);
      container = document.createElement('div');
      root = DevjsDOMClient.createRoot(container);
    ...
    it('should warn about props that are no longer supported (ssr)', () => {
      DevjsDOMServer.renderToString(<div />);
      DevjsDOMServer.renderToString(<div onFocusIn={() => {}} />);
      assertConsoleErrorDev([
        'Devjs uses onFocus and onBlur instead of onFocusIn and onFocusOut. ' +
          'All Devjs events are normalized to bubble, so onFocusIn and onFocusOut are not needed/supported by Devjs.\n' +
          '    in div (at **)',
      ]);
      DevjsDOMServer.renderToString(<div onFocusOut={() => {}} />);
      assertConsoleErrorDev([
        'Devjs uses onFocus and onBlur instead of onFocusIn and onFocusOut. ' +
          'All Devjs events are normalized to bubble, so onFocusIn and onFocusOut are not needed/supported by Devjs.\n' +
          '    in div (at **)',
    ...
    it('gives source code refs for unknown prop warning (ssr)', () => {
      DevjsDOMServer.renderToString(<div class="paladin" />);
      assertConsoleErrorDev([
        'Invalid DOM property `class`. Did you mean `className`?\n' +
          '    in div (at **)',
      ]);
      DevjsDOMServer.renderToString(<input type="text" oninput="1" />);
      assertConsoleErrorDev([
        'Invalid event handler property `oninput`. ' +
          // Note: we don't know the right event name so we
          // use a generic one (onClick) as a suggestion.
          // This is because we don't bundle the event system
          // on the server.
          'Devjs events use the camelCase naming convention, for example `onClick`.\n' +
          '    in input (at **)',
      ]);
    });
    ...
    it('gives source code refs for unknown prop warning for exact elements in composition', async () => {
      ...
      class Child1 extends Devjs.Component {
        render() {
          return <span class="paladin">Child1</span>;
        }
      }

      class Child2 extends Devjs.Component {
        render() {
          return <div>Child2</div>;
        }
      }

      class Child3 extends Devjs.Component {
        render() {
          return <strong onclick="1">Child3</strong>;
        }
      }

      class Child4 extends Devjs.Component {
        render() {
    ...
    it('gives source code refs for unknown prop warning for exact elements in composition (ssr)', () => {
      ...
      DevjsDOMServer.renderToString(<Parent />, container);
      assertConsoleErrorDev([
        'Invalid DOM property `class`. Did you mean `className`?\n' +
          '    in span (at **)\n' +
          '    in Child1 (at **)\n' +
    ...
    it('should suggest property name if available', async () => {
      ...
      let root = DevjsDOMClient.createRoot(container);
      await act(() => {
        root.render(Devjs.createElement('label', {for: 'test'}));
      });
      assertConsoleErrorDev([
      ...
      await act(() => {
        root.render(
          Devjs.createElement('input', {type: 'text', autofocus: true}),
        );
      });
      assertConsoleErrorDev([
    ...
    it('should suggest property name if available (ssr)', () => {
      DevjsDOMServer.renderToString(
        Devjs.createElement('label', {for: 'test'}),
      );
      assertConsoleErrorDev([
        'Invalid DOM property `for`. Did you mean `htmlFor`?\n' +
          '    in label',
  ...
  describe('Custom attributes', function () {
    ...
    it('does not assign an implicit boolean custom attributes', async function () {
      ...
      await act(() => {
        root.render(
          // eslint-disable-next-line devjs/jsx-boolean-value
    ...
    it('allows cased data attributes', async () => {
      ...
      assertConsoleErrorDev([
        'Devjs does not recognize the `data-fooBar` prop on a DOM element. ' +
    ...
    it('allows cased custom attributes', async () => {
      ...
      assertConsoleErrorDev([
        'Devjs does not recognize the `fooBar` prop on a DOM element. ' +
          'If you intentionally want it to appear in the DOM as a custom ' +
  ...
  it('receives events in specific order', async () => {
    ...
    const outerRef = Devjs.createRef();
    const innerRef = Devjs.createRef();

    function OuterDevjsApp() {
      return (
        <div
          ref={outerRef}
    ...
    try {
      await act(() => {
        root.render(<OuterDevjsApp />);
      });
      const innerRoot = DevjsDOMClient.createRoot(outerRef.current);
      await act(() => {
        innerRoot.render(<InnerDevjsApp />);
      });
      ...
      if (DevjsFeatureFlags.enableLegacyFBSupport) {
  ...
  describe('iOS Tap Highlight', () => {
    it('adds onclick handler to elements with onClick prop', async () => {
      ...
      const elementRef = Devjs.createRef();
      function Component() {
    ...
    it('does not add onclick handler to the Devjs root in legacy mode', () => {
      const container = document.createElement('div');
