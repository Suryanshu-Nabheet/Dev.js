...
describe('DevjsDOMFizzShellHydration', () => {
  beforeEach(() => {
    jest.resetModules();
  ...
  it('suspending in the shell during hydration', async () => {
    const div = Devjs.createRef(null);
    ...
    await serverAct(async () => {
      const {pipe} = DevjsDOMFizzServer.renderToPipeableStream(<App />);
      pipe(writable);
    });
    assertLog(['Shell']);
  ...
  it('client renders when an error is thrown in an error boundary', async () => {
    ...
    await serverAct(async () => {
      const {pipe} = DevjsDOMFizzServer.renderToPipeableStream(<ErroredApp />);
      pipe(writable);
    });
  ...
  it(
    ...
    async () => {
      ...
      function App() {
        ...
        if (state === 'Client') {
          return Devjs.use(clientPromise);
        }
      ...
      // During hydration, an error is thrown. Devjs attempts to recover by
      ...
      assertLog([
        'onRecoverableError: There was an error while hydrating but Devjs was ' +
