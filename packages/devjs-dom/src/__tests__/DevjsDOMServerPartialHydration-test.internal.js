...
class TestAppClass extends Devjs.Component {
  render() {
    return (
      <div>
        <>{''}</>
...
describe('DevjsDOMServerPartialHydration', () => {
  beforeEach(() => {
    jest.resetModules();

    DevjsFeatureFlags = require('shared/DevjsFeatureFlags');
    DevjsFeatureFlags.enableSuspenseCallback = true;
    ...
    IdleEventPriority = require('devjs-reconciler/constants').IdleEventPriority;
  });
  ...
  it('calls the hydration callbacks after hydration or deletion', async () => {
    ...
    suspend = true;
    suspend2 = true;
    const root = DevjsDOMClient.hydrateRoot(container, <App />, {
      onHydrated(node) {
        hydrated.push(node);
      },
      onDeleted(node) {
  ...
  it('recovers with client render when server rendered additional nodes at suspense root', async () => {
    function CheckIfHydrating({children}) {
      // This is a trick to check whether we're hydrating or not, since Devjs
    ...
    const finalHTML = DevjsDOMServer.renderToString(<App hasB={true} />);
    ...
    await act(() => {
      DevjsDOMClient.hydrateRoot(container, <App hasB={false} />, {
        onRecoverableError(error) {
          Scheduler.log('onRecoverableError: ' + normalizeError(error.message));
          if (error.cause) {
            Scheduler.log('Cause: ' + normalizeError(error.cause.message));
  ...
  it('calls the onDeleted hydration callback if the parent gets deleted', async () => {
    ...
    const root = await act(() => {
      return DevjsDOMClient.hydrateRoot(container, <App />, {
        onDeleted(node) {
          deleted.push(node);
        },
      });
  ...
  it('can insert siblings before the dehydrated boundary', async () => {
    ...
    function Sibling() {
      const [visible, setVisibilty] = Devjs.useState(false);
  ...
  it('can delete the dehydrated boundary before it is hydrated', async () => {
    ...
    function App() {
      const [visible, setVisibilty] = Devjs.useState(true);
  ...
  it('blocks updates to hydrate the content first if props have changed', async () => {
    ...
    suspend = false;
    const finalHTML = DevjsDOMServer.renderToString(
  ...
  it('blocks updates to hydrate the content first if props changed at idle priority', async () => {
    ...
    DevjsDOM.unstable_runWithPriority(IdleEventPriority, () => {
      root.render(<App text="Hi" className="hi" />);
    });
  ...
  it('hydrates first if props changed but we are able to resolve within a timeout', async () => {
    ...
    Devjs.startTransition(() => root.render(<App text="Hi" className="hi" />));
  ...
  it('warns but works if setState is called before commit in a dehydrated component', async () => {
    ...
    function Child() {
      const [state, setState] = Devjs.useState('Hello');
      updateText = setState;
      Scheduler.log('Child');
      if (suspend) {
        throw promise;
  ...
  it('blocks the update to hydrate first if context has changed', async () => {
    ...
    function Child() {
      const {text, className} = Devjs.useContext(Context);
      if (suspend) {
        throw promise;
      } else {
        return (
    ...
    const App = Devjs.memo(function App() {
      return (
        <div>
          <Suspense fallback="Loading...">
            <Child />
  ...
  it('shows inserted items in a SuspenseList before content is hydrated', async () => {
    ...
    suspend = false;
    const html = DevjsDOMServer.renderToString(<App showMore={false} />);
    ...
    suspend = true;
    const root = await act(() =>
      DevjsDOMClient.hydrateRoot(container, <App showMore={false} />),
    );
  ...
  it('can client render nested boundaries', async () => {
    ...
    // It leaves non-Devjs comments alone.
  ...
  it('regenerates if it cannot hydrate before changes to props/context expire', async () => {
    ...
    const promise = new Promise(resolvePromise => {});
    const ref = Devjs.createRef();
    const ClassName = Devjs.createContext(null);

    function Child({text}) {
      const className = Devjs.useContext(ClassName);
  ...
  it('does not invoke an event on a hydrated node until it commits', async () => {
    ...
    function Button() {
      const [clicked, setClicked] = Devjs.useState(false);
      if (clicked) {
        return null;
      }
      return (
  ...
  it('does not invoke an event on a parent tree when a subtree is dehydrated', async () => {
    ...
    await act(() => DevjsDOMClient.hydrateRoot(childContainer, <App />));
  ...
  it('regression test: does not overfire non-bubbling browser events', async () => {
    ...
    function Form() {
      const [submitted, setSubmitted] = Devjs.useState(false);
      if (submitted) {
        return null;
      }
      return (
  ...
  it('a hidden Activity component is skipped over during server rendering', async () => {
    const visibleRef = Devjs.createRef();
  ...
  it('fallback to client render on hydration mismatch at root', async () => {
    ...
    const finalHTML = DevjsDOMServer.renderToString(<App isClient={false} />);
  ...
  it("falls back to client rendering when there's a text mismatch (direct text child)", async () => {
    ...
    const container = document.createElement('div');
    container.innerHTML = DevjsDOMServer.renderToString(
      <DirectTextChild text="good" />,
    );
    await act(() => {
      DevjsDOMClient.hydrateRoot(container, <DirectTextChild text="bad" />, {
        onRecoverableError(error) {
          Scheduler.log('onRecoverableError: ' + normalizeError(error.message));
          if (error.cause) {
            Scheduler.log('Cause: ' + normalizeError(error.cause.message));
  ...
  it("falls back to client rendering when there's a text mismatch (text child with siblings)", async () => {
    ...
    container2.innerHTML = DevjsDOMServer.renderToString(
  ...
  it('hides a dehydrated suspense boundary if the parent resuspends', async () => {
    ...
    function App({text, resuspend}) {
      const memoized = Devjs.useMemo(() => <Component text={text} />, [text]);
      return (
        <div>
          <Suspense fallback="Loading...">
            {memoized}
    ...
    suspend = false;
    const finalHTML = DevjsDOMServer.renderToString(<App text="Hello" />);
    const container = document.createElement('div');
    container.innerHTML = finalHTML;
    ...
    // hydrating anyway.
    suspend = true;
    const root = DevjsDOMClient.hydrateRoot(container, <App text="Hello" />, {
      onRecoverableError(error) {
        Scheduler.log('onRecoverableError: ' + normalizeError(error.message));
        if (error.cause) {
          Scheduler.log('Cause: ' + normalizeError(error.cause.message));
  ...
  // Regression for https://github.com/Suryanshu-Nabheet/dev.js/issues/35210 and other issues where lazy elements created in flight
  ...
  it('Can hydrate even when lazy content resumes immediately inside a Suspense', async () => {
    ...
    let root;
    Devjs.startTransition(() => {
      root = DevjsDOMClient.hydrateRoot(container, <App />, {
        onRecoverableError(error) {
          console.log('[DEBUG] hydration error:', error.message);
          hydrationErrors.push(error.message);
        },
