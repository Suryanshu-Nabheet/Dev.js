...
describe('DevjsDOMFizzServerEdge', () => {
  beforeEach(() => {
    jest.resetModules();
    jest.useRealTimers();
    Devjs = require('devjs');
  ...
  it('recoverably errors and does not add rel="expect" for large shells', async () => {
    ...
    const errors2 = [];
    const stream2 = await DevjsDOMFizzServer.renderToReadableStream(
      <App suspense={true} />,
      {
        onError(error) {
          errors2.push(error);
    ...
    const errors3 = [];
    const stream3 = await DevjsDOMFizzServer.renderToReadableStream(
      <App suspense={false} />,
      {
