...
// These tests rely both on DevjsDOMServer and DevjsDOM.
// If a test only needs DevjsDOMServer, put it in DevjsServerRendering-test instead.
describe('DevjsDOMServerHydration', () => {
  beforeEach(() => {
    jest.resetModules();
    Devjs = require('devjs');
  ...
  it('should have the correct mounting behavior', async () => {
    ...
    try {
      let root = DevjsDOMClient.createRoot(element);
      await act(() => {
      ...
      root = DevjsDOMClient.createRoot(element);
      await act(() => {
        root.render(<TestComponent name="x" />);
      });
      ...
      lastMarkup = DevjsDOMServer.renderToString(<TestComponent name="x" />);
      element.innerHTML = lastMarkup;

      let instance;

      root = await act(() => {
        return DevjsDOMClient.hydrateRoot(
          element,
      ...
      // Now simulate a situation where the app is not idempotent. Devjs should
  ...
  it('should emit autofocus on the server but not focus() when hydrating', async () => {
    const element = document.createElement('div');
    element.innerHTML = DevjsDOMServer.renderToString(
    ...
    const root = await act(() =>
      DevjsDOMClient.hydrateRoot(element, <input autoFocus={true} />),
    );
  ...
  it('should not focus on either server or client with autofocus={false}', async () => {
    ...
    element.firstChild.focus = jest.fn();
    const root = await act(() =>
      DevjsDOMClient.hydrateRoot(element, <input autoFocus={false} />),
    );
  ...
  // Regression test for https://github.com/Suryanshu-Nabheet/dev.js/issues/11726
  ...
  it('should warn when the style property differs', async () => {
    ...
    assertConsoleErrorDev([
      "A tree hydrated but some attributes of the server rendered HTML didn't match the client properties. " +
        ...
        'installed which messes with the HTML before Devjs loaded.\n' +
        '\n' +
  ...
  it('should throw rendering portals on the server', () => {
    ...
    expect(() => {
      DevjsDOMServer.renderToString(
        <div>{DevjsDOM.createPortal(<div />, div)}</div>,
      );
    }).toThrow(
  ...
  it('should be able to render and hydrate Mode components', async () => {
    class ComponentWithWarning extends Devjs.Component {
      componentWillMount() {
        // Expected warning
      }
      render() {
    ...
    const element = document.createElement('div');
    element.innerHTML = DevjsDOMServer.renderToString(markup);
    ...
    await act(() => {
      DevjsDOMClient.hydrateRoot(element, markup);
    });
    assertConsoleWarnDev([
      'componentWillMount has been renamed, and is not recommended for use. ' +
        'See https://devjs.dev/link/unsafe-component-lifecycles for details.\n' +
        ...
        'In Devjs 18.x, only the UNSAFE_ name will work. ' +
        'To rename all deprecated lifecycles to their new names, ' +
        'you can run `npx devjs-codemod rename-unsafe-lifecycles` in your project source folder.\n' +
        '\n' +
        'Please update the following components: ComponentWithWarning',
    ]);
    expect(element.textContent).toBe('Hi');
  ...
  it('should be able to render and hydrate forwardRef components', async () => {
    ...
    const WrappedFunctionComponent = Devjs.forwardRef((props, ref) => (
  ...
  it('should be able to render and hydrate Profiler components', async () => {
    ...
    const markup = (
      <Devjs.Profiler id="profiler" onRender={callback}>
        <div>Hi</div>
      </Devjs.Profiler>
    );
  ...
  // Regression test for https://github.com/Suryanshu-Nabheet/dev.js/issues/11423
  ...
  it('should be able to use lazy components after hydrating', async () => {
    ...
    const element = document.createElement('div');
    element.innerHTML = DevjsDOMServer.renderToString(<HelloWorld />);
    expect(element.textContent).toBe('Hello ');

    await act(() => {
      DevjsDOMClient.hydrateRoot(element, <HelloWorld />);
    });
  ...
  it('does not re-enter hydration after committing the first one', async () => {
    const finalHTML = DevjsDOMServer.renderToString(<div />);
    const container = document.createElement('div');
    container.innerHTML = finalHTML;
    const root = await act(() =>
      DevjsDOMClient.hydrateRoot(container, <div />),
    );
  ...
  // regression test for https://github.com/Suryanshu-Nabheet/dev.js/issues/17170
  it('should not warn if dangerouslySetInnerHtml=undefined', async () => {
    ...
    const markup = DevjsDOMServer.renderToStaticMarkup(devjsElement);
    domElement.innerHTML = markup;

    await act(() => {
      DevjsDOMClient.hydrateRoot(domElement, devjsElement);
    });
  ...
  it('should warn when hydrating read-only properties', async () => {
    ...
    for (const readOnlyProperty of readOnlyProperties) {
      ...
      const jsx = Devjs.createElement('my-custom-element', props);
      const element = document.createElement('div');
      element.innerHTML = DevjsDOMServer.renderToString(jsx);
      await act(() => {
        DevjsDOMClient.hydrateRoot(element, jsx);
      });
      assertConsoleErrorDev([
  ...
  it('should not re-assign properties on hydration', async () => {
    ...
    const jsx = Devjs.createElement('my-custom-element', {
      str: 'string',
      obj: {foo: 'bar'},
    });

    container.innerHTML = DevjsDOMServer.renderToString(jsx);
    ...
    await act(() => {
      DevjsDOMClient.hydrateRoot(container, jsx);
    });
  ...
  it('refers users to apis that support Suspense when something suspends (browser)', async () => {
    ...
    const container = document.createElement('div');
    container.innerHTML = DevjsDOMServerBrowser.renderToString(
      <App isClient={false} />,
    );
  ...
  it('allows rendering extra hidden inputs in a form', async () => {
    ...
    const ref = Devjs.createRef();
    const a = Devjs.createRef();
    const b = Devjs.createRef();
    const c = Devjs.createRef();
    await act(async () => {
      DevjsDOMClient.hydrateRoot(
        element,
        <form ref={ref}>
