...
const patchConsoleMethod = (methodName, logged) => {
  const newMethod = function (format, ...args) {
    ...
    // and Devjs addendums because they're too noisy.
    if (shouldIgnoreConsoleError(format, args)) {
      return;
    }

    // Ignore certain Devjs warnings causing test failures
    ...
    if (
      ...
      // useSyncExternalStoreShared-test that tests against Devjs 17,
      // which doesn't have the captureOwnerStack method.
      if (Devjs.captureOwnerStack) {
        const stack = Devjs.captureOwnerStack();
        if (stack) {
          format += '%s';
          args.push(stack);
        }
...
export function createLogAssertion(
  ...
  return function assertConsoleLog(expectedMessages, options = {}) {
    if (__DEV__) {
      ...
      for (let index = 0; index < observedLogs.length; index++) {
        ...
        if (typeof expectedMessage === 'string') {
          if (normalizedMessage === expectedMessage) {
            ...
            // NOT for Devjs component stacks (e.g., "\n    in ComponentName (at **)").
      ...
      // not for Devjs component stacks.
          ...
          `${stackTracePlaceholderMisuses
            .map(
              item =>
                ...
                `stack traces (messages starting with "Error:"), not for Devjs component stacks.\n\n` +
