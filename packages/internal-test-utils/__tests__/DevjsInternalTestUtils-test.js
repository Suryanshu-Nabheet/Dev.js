...
const {startTransition, useDeferredValue} = Devjs;
...
} = require('../DevjsInternalTestUtils');

describe('DevjsInternalTestUtils', () => {
  it('waitFor', async () => {
    const Yield = ({id}) => {
      Scheduler.log(id);
      return id;
  ...
  it('waitForThrow', async () => {
    ...
    assertLog([
      'A',
      'B',
      // Devjs will try one more time before giving up.
      'A',
      'B',
    ]);
  });
...
describe('DevjsInternalTestUtils console mocks', () => {
  beforeEach(() => {
    jest.resetAllMocks();
...
describe('DevjsInternalTestUtils console assertions', () => {
  beforeAll(() => {
    patchConsoleMethods({includeLog: true});
  });
    ...
    describe('in <stack> placeholder', () => {
      ...
      it('fails if `in <stack>` is used for a component stack instead of an error stack', () => {
        ...
        expect(message).toMatchInlineSnapshot(`
          ...
          Incorrect use of \\n    in <stack> placeholder. The placeholder is for JavaScript Error stack traces (messages starting with "Error:"), not for Devjs component stacks.
