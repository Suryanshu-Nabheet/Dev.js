...
// those of developers using Devjs — we're testing Devjs itself, as opposed to
// building an app with Devjs.
...
export async function act<T>(scope: () => Thenable<T>): Thenable<T> {
  ...
  const previousIsActEnvironment = global.IS_devjs_ACT_ENVIRONMENT;
  const previousActingUpdatesScopeDepth = actingUpdatesScopeDepth;
  actingUpdatesScopeDepth++;
  if (actingUpdatesScopeDepth === 1) {
    // Because this is not the "real" `act`, we set this to `false` so Devjs
  ...
  try {
    ...
    do {
      ...
      // flushUntilNextPaint stops when Devjs yields execution. Allow microtasks
    ...
    if (depth === 1) {
      ...
      global.IS_devjs_ACT_ENVIRONMENT = previousIsActEnvironment;
    }
