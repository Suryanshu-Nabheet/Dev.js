...
} from 'devjs-client/src/DevjsFlightClientConfig';
...
import {ASYNC_ITERATOR} from 'shared/DevjsSymbols';
  ...
  switch (chunk.status) {
    case INITIALIZED:
      if (typeof resolve === 'function') {
        ...
        // Recursively check if the value is itself a DevjsPromise and if so if it points
        // back to itself. This helps catch recursive thenables early error.
        let cycleProtection = 0;
        const visited = new Set<typeof DevjsPromise>();
        while (inspectedValue instanceof DevjsPromise) {
          cycleProtection++;
          if (
...
function createResolvedModelChunk<T>(
  ...
  // $FlowFixMe[invalid-constructor] Flow doesn't support functions as constructors
  return new DevjsPromise(RESOLVED_MODEL, value, {
    id,
...
function createErroredChunk<T>(
  ...
  // $FlowFixMe[invalid-constructor] Flow doesn't support functions as constructors
  return new DevjsPromise(ERRORED, null, reason);
}
...
function createResolvedIteratorResultChunk<T>(
  ...
  // $FlowFixMe[invalid-constructor] Flow doesn't support functions as constructors
  return new DevjsPromise(RESOLVED_MODEL, iteratorResultJSON, {
    id: -1,
    [RESPONSE_SYMBOL]: response,
  });
}
...
function loadServerReference<A: Iterable<any>, T>(
  ...
  if (key === 'then') {
    ...
    // as "thenable" which reduces to DevjsPromise with no other fields.
    return (null: any);
  }
  ...
  // $FlowFixMe[invalid-constructor] Flow doesn't support functions as constructors
  const blockedPromise: BlockedChunk<T> = new DevjsPromise(BLOCKED, null, null);
  ...
  if (!serverReferencePromise) {
    if (bound instanceof DevjsPromise) {
  ...
  } else if (bound instanceof DevjsPromise) {
      ...
      if (
        ...
        if (isArray(value)) {
          ...
          if (typeof value === 'string') {
            ...
            // eslint-disable-next-line devjs-internal/no-primitive-constructors
