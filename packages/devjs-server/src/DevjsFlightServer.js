...
import type {Chunk, BinaryChunk, Destination} from './DevjsServerStreamConfig';

import type {TemporaryReferenceSet} from './DevjsFlightServerTemporaryReferences';
...
import {
  ...
  close,
  closeWithError,
} from './DevjsServerStreamConfig';

export type {Destination, Chunk} from './DevjsServerStreamConfig';
...
import type {ThenableState} from './DevjsFlightThenable';
import type {
  Wakeable,
  Thenable,
  PendingThenable,
  ...
  DevjsStackTrace,
  DevjsCallSite,
  DevjsFunctionLocation,
  DevjsErrorInfo,
  DevjsErrorInfoDev,
  DevjsKey,
} from 'shared/DevjsTypes';
import type {DevjsElement} from 'shared/DevjsElementType';
...
} from './DevjsFlightServerTemporaryReferences';
...
} from './DevjsFlightHooks';
import {DefaultAsyncDispatcher} from './flight/DevjsFlightAsyncDispatcher';

import {resolveOwner, setCurrentOwner} from './flight/DevjsFlightCurrentOwner';

import {getOwnerStackByComponentInfoInDev} from 'shared/DevjsComponentInfoStack';
import {resetOwnerStackLimit} from 'shared/DevjsOwnerStackReset';
...
import {
  ...
  callLazyInitInDEV,
  callIteratorInDEV,
} from './DevjsFlightCallUserSpace';

import {
  getIteratorFn,
  devjs_ELEMENT_TYPE,
  devjs_LEGACY_ELEMENT_TYPE,
...
import {
  ...
  jsxChildrenParents,
  objectName,
} from 'shared/DevjsSerializationErrors';

import DevjsSharedInternals from './DevjsSharedInternalsServer';
import isArray from 'shared/isArray';
import getPrototypeOf from 'shared/getPrototypeOf';
import hasOwnProperty from 'shared/hasOwnProperty';
import binaryToComparableString from 'shared/binaryToComparableString';

import {SuspenseException, getSuspendedThenable} from './DevjsFlightThenable';
...
function devirtualizeURL(url: string): string {
  if (url.startsWith('about://Devjs/')) {
    ...
    const envIdx = url.indexOf('/', 'about://Devjs/'.length);
...
function filterStackTrace(
  ...
  const filterStackFrame = request.filterStackFrame;
  const filteredStack: DevjsStackTrace = [];
  for (let i = 0; i < stack.length; i++) {
    const callsite = stack[i];
    const functionName = callsite[0];
    const url = devirtualizeURL(callsite[1]);
    ...
    if (filterStackFrame(url, functionName, lineNumber, columnNumber)) {
      ...
      const clone: DevjsCallSite = (callsite.slice(0): any);
      clone[1] = url;
...
function hasUnfilteredFrame(request: Request, stack: DevjsStackTrace): boolean {
  const filterStackFrame = request.filterStackFrame;
  for (let i = 0; i < stack.length; i++) {
    const callsite = stack[i];
    const functionName = callsite[0];
    ...
    const wrapperMethod = function (this: typeof console) {
      ...
      if (methodName === 'assert' && arguments[0]) {
        ...
        const stack = filterStackTrace(
          request,
          parseStackTracePrivate(new Error('devjs-stack-top-frame'), 1) || [],
        );
        request.pendingDebugChunks++;
        const owner: null | DevjsComponentInfo = resolveOwner();
...
type DevjsJSONValue =
  ...
  | number
  | null
  | $ReadOnlyArray<DevjsClientValue>
  | DevjsClientObject;

// Serializable values
export type DevjsClientValue =
  // Server Elements and Lazy Components are unwrapped on the Server
  | Devjs$Element<component(...props: any)>
  | LazyComponent<DevjsClientValue, any>
  // References are passed by their value
  | ClientReference<any>
  | ServerReference<any>
  // The rest are passed as is. Sub-types can be passed in but lose their
  // subtype, so the receiver can only accept once of these.
  | Devjs$Element<string>
  | Devjs$Element<ClientReference<any> & any>
  | DevjsComponentInfo
  | DevjsErrorInfo
  | string
  | boolean
  ...
  | bigint
  | ReadableStream
  | $AsyncIterable<DevjsClientValue, DevjsClientValue, void>
  | $AsyncIterator<DevjsClientValue, DevjsClientValue, void>
  | Iterable<DevjsClientValue>
  | Iterator<DevjsClientValue>
  | Array<DevjsClientValue>
  | Map<DevjsClientValue, DevjsClientValue>
  | Set<DevjsClientValue>
  | FormData
  | $ArrayBufferView
  | ArrayBuffer
  | Date
  | DevjsClientObject
  | Promise<DevjsClientValue>; // Thenable<DevjsClientValue>

type DevjsClientObject = {+[key: string]: DevjsClientValue};
...
type Task = {
  id: number,
  status: 0 | 1 | 3 | 4 | 5,
  model: DevjsClientValue,
  ping: () => void,
  toJSON: (key: string, value: DevjsClientValue) => DevjsJSONValue,
  keyPath: DevjsKey, // parent server component keys
  ...
  debugOwner: null | DevjsComponentInfo, // DEV-only
...
type DevjsClientReference = Reference & DevjsClientValue;

type DeferredDebugStore = {
  retained: Map<number, DevjsClientReference | string>,
  existing: Map<DevjsClientReference | string, number>,
};
...
export type Request = {
  ...
  didWarnForKey: null | WeakSet<DevjsComponentInfo>,
...
function RequestInstance(
  ...
  if (
    DevjsSharedInternals.A !== null &&
    DevjsSharedInternals.A !== DefaultAsyncDispatcher
  ) {
    throw new Error(
      'Currently Devjs only supports one RSC renderer at a time.',
    );
  }
  DevjsSharedInternals.A = DefaultAsyncDispatcher;
  if (__DEV__) {
    ...
    DevjsSharedInternals.getCurrentStack = getCurrentStackInDEV;
  }
...
function serializeReadableStream(
  ...
  function progress(entry: {done: boolean, value: DevjsClientValue, ...}) {
...
function serializeAsyncIterable(
  request: Request,
  task: Task,
  iterable: $AsyncIterable<DevjsClientValue, DevjsClientValue, void>,
  ...
  if (__DEV__) {
    const debugInfo: ?DevjsDebugInfo = (iterable: any)._debugInfo;
    if (debugInfo) {
...
function callWithDebugContextInDEV<A, T>(
  ...
  const componentDebugInfo: DevjsComponentInfo = {
    name: '',
...
function processServerComponentReturnValue(
    ...
    if (__DEV__) {
      ...
      thenable.then(resolvedValue => {
        if (
          ...
          resolvedValue !== null &&
          resolvedValue.$$typeof === devjs_ELEMENT_TYPE
        ) {
  ...
  if (__DEV__) {
    if ((result: any).$$typeof === devjs_ELEMENT_TYPE) {
  ...
  // to be rendered as a Devjs Child. However, because we have the function to recreate
...
function renderFunctionComponent<Props>(
  ...
  let componentDebugInfo: DevjsComponentInfo;
  if (__DEV__) {
    if (!canEmitDebugInfo) {
  ...
  if (key !== null) {
    ...
    if (key === devjs_OPTIMISTIC_KEY || prevKeyPath === devjs_OPTIMISTIC_KEY) {
      // The optimistic key is viral. It turns the whole key into optimistic if any part is.
      task.keyPath = devjs_OPTIMISTIC_KEY;
    } else {
    ...
    const logKeyError = () => {
      console.error(
        'Each child in a list should have a unique "key" prop.' +
          '%s%s See https://devjs.dev/link/warning-keys for more information.',
        '',
        '',
      );
    };
...
function renderFragment(
  request: Request,
  task: Task,
  children: $ReadOnlyArray<DevjsClientValue>,
): DevjsJSONValue {
  if (__DEV__) {
    for (let i = 0; i < children.length; i++) {
      ...
      if (
        child !== null &&
        typeof child === 'object' &&
        child.$$typeof === devjs_ELEMENT_TYPE
      ) {
        const element: DevjsElement = (child: any);
  ...
  if (task.keyPath !== null) {
    ...
    const fragment = __DEV__
      ...
      : [devjs_ELEMENT_TYPE, devjs_FRAGMENT_TYPE, task.keyPath, {children}];
  ...
  if (__DEV__) {
    const debugInfo: ?DevjsDebugInfo = (children: any)._debugInfo;
    if (debugInfo) {
...
function renderAsyncFragment(
  request: Request,
  task: Task,
  children: $AsyncIterable<DevjsClientValue, DevjsClientValue, void>,
...
function renderClientElement(
  ...
  if (key === null) {
    ...
    if (keyPath === devjs_OPTIMISTIC_KEY || key === devjs_OPTIMISTIC_KEY) {
      // Optimistic key is viral and turns the whole key optimistic.
      key = devjs_OPTIMISTIC_KEY;
    } else {
      key = keyPath + ',' + key;
    }
  }
  ...
  if (__DEV__) {
    ...
    if (task.debugStack !== null) {
      ...
      // We also store this in the main dedupe set so that it can be referenced by inline Devjs Elements.
      request.writtenObjects.set(debugStack, serializeByValueID(id));
    }
  }
  const element = __DEV__
    ? [devjs_ELEMENT_TYPE, type, key, props, debugOwner, debugStack, validated]
    : [devjs_ELEMENT_TYPE, type, key, props];
...
function deferTask(request: Request, task: Task): DevjsJSONValue {
...
function outlineTask(request: Request, task: Task): DevjsJSONValue {
  const newTask = createTask(
    request,
    task.model, // the currently rendering element
    task.keyPath, // unlike outlineModel this one carries along context
...
function renderElement(
  ...
  } else if (type === devjs_FRAGMENT_TYPE && key === null) {
...
function visitAsyncNodeImpl(
  ...
  switch (node.tag) {
          ...
          if (startTime < cutOff) {
            ...
            if (
              ...
              emitDebugChunk(request, task.id, {
                awaited: ((ioNode: any): DevjsIOInfo), // This is deduped by this reference.
                env: env,
                owner: node.owner,
                stack:
                  node.stack === null
    ...
    default: {
      // eslint-disable-next-line devjs-internal/prod-error-codes
      throw new Error('Unknown AsyncSequence tag. This is a bug in Devjs.');
    }
  }
  return found;
}
...
function emitAsyncSequence(
  ...
  task: Task,
  node: AsyncSequence,
  alreadyForwardedDebugInfo: ?DevjsDebugInfo,
  ...
  if (awaitedNode === undefined) {
    ...
    const debugInfo: DevjsAsyncInfo = {
      awaited: ((awaitedNode: any): DevjsIOInfo), // This is deduped by this reference.
      env: env,
    };
    if (__DEV__) {
...
function createTask(
  ...
  const task: Task = (({
    ...
    toJSON: function (
      this:
        | {+[key: string | number]: DevjsClientValue}
        | $ReadOnlyArray<DevjsClientValue>,
      parentPropertyName: string,
      value: DevjsClientValue,
    ): DevjsJSONValue {
      const parent = this;
...
function serializeDeferredObject(
  request: Request,
  value: DevjsClientReference | string,
): string {
  const deferredDebugObjects = request.deferredDebugObjects;
  if (deferredDebugObjects !== null) {
...
function serializeClientReference(
  request: Request,
  parent:
    | {+[propertyName: string | number]: DevjsClientValue}
    | $ReadOnlyArray<DevjsClientValue>,
  parentPropertyName: string,
  ...
  if (existingId !== undefined) {
    if (parent[0] === devjs_ELEMENT_TYPE && parentPropertyName === '1') {
      // If we're encoding the "type" of an element, we can refer
      // to that by a lazy reference instead of directly since Devjs
...
function outlineModel(request: Request, value: DevjsClientValue): number {
...
function serializeServerReference(
  ...
  let location: null | DevjsFunctionLocation = null;
  if (__DEV__) {
  ...
  const serverReferenceMetadata: {
    ...
    env?: string, // DEV-only
    location?: DevjsFunctionLocation, // DEV-only
  } =
...
function serializeMap(
  request: Request,
  map: Map<DevjsClientValue, DevjsClientValue>,
): string {
...
function serializeSet(request: Request, set: Set<DevjsClientValue>): string {
...
function serializeIterator(
  request: Request,
  iterator: Iterator<DevjsClientValue>,
): string {
...
let modelRoot: null | DevjsClientValue = false;
...
function renderModel(
  ...
  try {
    ...
    const wasDevjsNode =
      typeof model === 'object' &&
      model !== null &&
      ((model: any).$$typeof === devjs_ELEMENT_TYPE ||
        (model: any).$$typeof === devjs_LAZY_TYPE);
    ...
    if (request.status === ABORTING) {
      ...
      if (request.type === PRERENDER) {
        ...
        return outlineHaltedTask(request, task, wasDevjsNode);
      }
    ...
    // If we don't know if it was a Devjs Node we render a direct reference and let
  ...
  if (typeof value === 'object') {
    switch ((value: any).$$typeof) {
      case devjs_ELEMENT_TYPE: {
        ...
        const element: DevjsElement = (value: any);
        ...
        if (__DEV__) {
          const debugInfo: ?DevjsDebugInfo = (value: any)._debugInfo;
          if (debugInfo) {
            // If this came from Flight, forward any debug info into this new row.
            if (!canEmitDebugInfo) {
              // We don't have a chunk to assign debug info. We need to outline this
        ...
        if (__DEV__) {
          ...
          if (
            ...
            let key = '';
            if (element.key !== null && element.key !== devjs_OPTIMISTIC_KEY) {
              key = ' key="' + element.key + '"';
            }
        ...
        const newChild = renderElement(
          ...
          task,
          element.type,
          // $FlowFixMe[incompatible-call] the key of an element is null | string | DevjsOptimisticKey
          element.key,
          ref,
          props,
      ...
      case devjs_LAZY_TYPE: {
        ...
        if (__DEV__) {
          const debugInfo: ?DevjsDebugInfo = lazy._debugInfo;
          if (debugInfo) {
            // If this came from Flight, forward any debug info into this new row.
            if (!canEmitDebugInfo) {
              // We don't have a chunk to assign debug info. We need to outline this
      ...
      case devjs_LEGACY_ELEMENT_TYPE: {
        throw new Error(
          'A Devjs Element from an older version of Devjs was rendered. ' +
            'This is not supported. It can happen if:\n' +
            '- Multiple copies of the "devjs" package is used.\n' +
            '- A library pre-bundled an old copy of "devjs" or "devjs/jsx-runtime".\n' +
    ...
    if (existingReference !== undefined) {
      ...
      if (parentReference !== undefined) {
        ...
        if (isArray(parent) && parent[0] === devjs_ELEMENT_TYPE) {
...
function fatalError(request: Request, error: mixed): void {
  ...
  // This is called outside error handling code such as if an error happens in Devjs internals.
...
function serializeErrorValue(request: Request, error: Error): string {
  if (__DEV__) {
    ...
    let message: string;
    let stack: DevjsStackTrace;
    let env = (0, request.environmentName)();
    try {
      name = error.name;
      // eslint-disable-next-line devjs-internal/safe-string-coercion
    ...
    const errorInfo: DevjsErrorInfoDev = {name, message, stack, env};
    if ('cause' in error) {
      const cause: DevjsClientValue = (error.cause: any);
...
function emitErrorChunk(
  ...
  owner: ?DevjsComponentInfo, // DEV-only
): void {
  let errorInfo: DevjsErrorInfo;
  if (__DEV__) {
    let name: string = 'Error';
    let message: string;
    let stack: DevjsStackTrace;
    ...
    if (causeReference !== null) {
      (errorInfo: DevjsErrorInfoDev).cause = causeReference;
    }
  } else {
    errorInfo = {digest};
  }
...
function emitDebugHaltChunk(request: Request, id: number): void {
  if (!__DEV__) {
    ...
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'emitDebugHaltChunk should never be called in production mode. This is a bug in Devjs.',
    );
  }
...
function emitDebugChunk(
  request: Request,
  id: number,
  debugInfo: DevjsDebugInfoEntry,
): void {
  if (!__DEV__) {
    // These errors should never make it into a build so we don't need to encode them in codes.json
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'emitDebugChunk should never be called in production mode. This is a bug in Devjs.',
    );
  }
...
function outlineComponentInfo(
  request: Request,
  componentInfo: DevjsComponentInfo,
): string {
  if (!__DEV__) {
    // These errors should never make it into a build so we don't need to encode them in codes.json
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'outlineComponentInfo should never be called in production mode. This is a bug in Devjs.',
    );
  }
...
function emitIOInfoChunk(
  ...
  if (!__DEV__) {
    ...
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'emitIOInfoChunk should never be called in production mode. This is a bug in Devjs.',
    );
  }
  ...
  const debugIOInfo: Omit<DevjsIOInfo, 'debugTask' | 'debugStack'> = {
    name: name,
...
function outlineIOInfo(request: Request, ioInfo: DevjsIOInfo): void {
...
function emitTextChunk(
  ...
  if (byteLengthOfChunk === null) {
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'Existence of byteLengthOfChunk should have already been checked. This is a bug in Devjs.',
    );
  }
  if (debug) {
    request.pendingDebugChunks++;
...
function serializeEval(source: string): string {
  if (!__DEV__) {
    ...
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'serializeEval should never be called in production mode. This is a bug in Devjs.',
    );
  }
  return '$E' + source;
}
  ...
  if (typeof value === 'object') {
    ...
    switch ((value: any).$$typeof) {
      case devjs_ELEMENT_TYPE: {
        ...
        let debugStack: null | DevjsStackTrace = null;
      ...
      case devjs_LAZY_TYPE: {
        ...
        // just coming from Devjs.lazy helper or Flight.
        const lazy: LazyComponent<any, any> = (value: any);
        const payload = lazy._payload;

        if (payload !== null && typeof payload === 'object') {
          // Devjs.lazy constructor
...
function serializeDebugModel(
  ...
  function replacer(
    ...
    try {
      ...
      return (
        'Unknown Value: Devjs could not send it from the server.\n' + x.message
      );
    }
  }
...
function emitOutlinedDebugModelChunk(
  ...
  if (!__DEV__) {
    ...
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'emitOutlinedDebugModel should never be called in production mode. This is a bug in Devjs.',
    );
  }
...
function outlineDebugModel(
  ...
  if (!__DEV__) {
    ...
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'outlineDebugModel should never be called in production mode. This is a bug in Devjs.',
    );
  }
...
function emitConsoleChunk(
  ...
  if (!__DEV__) {
    ...
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'emitConsoleChunk should never be called in production mode. This is a bug in Devjs.',
    );
  }
...
function emitTimeOriginChunk(request: Request, timeOrigin: number): void {
  // We emit the time origin once. All DevjsTimeInfo timestamps later in the stream
...
function forwardDebugInfoFromThenable(
  ...
  task: Task,
  thenable: Thenable<any>,
  owner: null | DevjsComponentInfo, // DEV-only
  stack: null | Error, // DEV-only
): void {
  let debugInfo: ?DevjsDebugInfo;
  if (__DEV__) {
    // If this came from Flight, forward any debug info into this new row.
...
function forwardDebugInfoFromAbortedTask(request: Request, task: Task): void {
  ...
  if (enableProfilerTimer && enableAsyncDebugInfo) {
    ...
    if (typeof model.then === 'function') {
      thenable = (model: any);
    } else if (model.$$typeof === devjs_LAZY_TYPE) {
    ...
    if (thenable !== null) {
      ...
      if (sequence !== null) {
        ...
        if (node.tag === UNRESOLVED_PROMISE_NODE) {
          ...
          request.pendingChunks++;
          const env = (0, request.environmentName)();
          const asyncInfo: DevjsAsyncInfo = {
            awaited: ((node: any): DevjsIOInfo), // This is deduped by this reference.
            env: env,
          };
          // We don't have a start time for this await but in case there was no start time emitted
          // we need to include something. TODO: We should maybe ideally track the time when we
...
function performWork(request: Request): void {
  ...
  const prevDispatcher = DevjsSharedInternals.H;
  DevjsSharedInternals.H = HooksDispatcher;
  const prevRequest = currentRequest;
  ...
  try {
    ...
    DevjsSharedInternals.H = prevDispatcher;
    resetHooksForRequest();
    currentRequest = prevRequest;
  }
}
...
export function resolveDebugMessage(request: Request, message: string): void {
  if (!__DEV__) {
    ...
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'resolveDebugMessage should never be called in production mode. This is a bug in Devjs.',
    );
  }
  const deferredDebugObjects = request.deferredDebugObjects;
  if (deferredDebugObjects === null) {
    throw new Error(
      "resolveDebugMessage/closeDebugChannel should not be called for a Request that wasn't kept alive. This is a bug in Devjs.",
    );
  }
  if (message === '') {
...
export function closeDebugChannel(request: Request): void {
  if (!__DEV__) {
    ...
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'closeDebugChannel should never be called in production mode. This is a bug in Devjs.',
    );
  }
