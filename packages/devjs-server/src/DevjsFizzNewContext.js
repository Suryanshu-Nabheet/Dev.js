...
import {isPrimaryRenderer} from './DevjsFizzConfig';
...
// We assume that this runtime owns the "current" field on all DevjsContext instances.
...
function popToNearestCommonAncestor(
  ...
  if (prev === next) {
    ...
    if (parentPrev === null) {
      if (parentNext !== null) {
        throw new Error(
          'The stacks must reach the root at the same time. This is a bug in Devjs.',
        );
      }
    } else {
      if (parentNext === null) {
...
function popPreviousToCommonLevel(
  ...
  if (parentPrev === null) {
    throw new Error(
      'The depth must equal at least at zero before reaching the root. This is a bug in Devjs.',
    );
  }
...
export function popProvider<T>(context: DevjsContext<T>): ContextSnapshot {
  const prevSnapshot = currentActiveSnapshot;

  if (prevSnapshot === null) {
    throw new Error(
      'Tried to pop a Context at the root of the app. This is a bug in Devjs.',
    );
  }
  ...
  if (__DEV__) {
    if (prevSnapshot.context !== context) {
      console.error(
        'The parent context is not the expected context. This is probably a bug in Devjs.',
      );
    }
  }
  if (isPrimaryRenderer) {
