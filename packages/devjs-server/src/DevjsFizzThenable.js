...
// Corresponds to DevjsFiberThenable and DevjsFlightThenable modules. Generally,
...
import {currentTaskInDEV} from './DevjsFizzCurrentTask';
...
function captureSuspendedCallSite(): void {
  ...
  if (currentTask === null) {
    // eslint-disable-next-line devjs-internal/prod-error-codes -- not a prod error
    throw new Error(
  ...
  if (currentComponentStack === null) {
    ...
    throw new Error(
      'Expected to have a component stack on the current task when ' +
        'tracking a suspended call site. This is a bug in Devjs.',
    );
  }
  suspendedCallSiteStack = {
    ...
    stack: Error('devjs-stack-top-frame'),
  };
