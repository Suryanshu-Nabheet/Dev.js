...
import type {Request} from './DevjsFlightServer';
import type {Thenable, Usable, DevjsComponentInfo} from 'shared/DevjsTypes';
import type {ThenableState} from './DevjsFlightThenable';
import {
  devjs_MEMO_CACHE_SENTINEL,
  devjs_CONTEXT_TYPE,
} from 'shared/DevjsSymbols';
import {createThenableState, trackUsedThenable} from './DevjsFlightThenable';
import {isClientReference} from './DevjsFlightServerConfig';
...
export function prepareToUseHooksForComponent(
  ...
  componentDebugInfo: null | DevjsComponentInfo,
) {
...
function useId(): string {
  if (currentRequest === null) {
    throw new Error('useId can only be used while Devjs is rendering');
  }
...
function use<T>(usable: Usable<T>): T {
  ...
  if (isClientReference(usable)) {
    if (usable.value != null && usable.value.$$typeof === devjs_CONTEXT_TYPE) {
