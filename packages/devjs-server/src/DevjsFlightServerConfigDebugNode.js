...
import type {DevjsStackTrace} from 'shared/DevjsTypes';
...
import {resolveOwner} from './flight/DevjsFlightCurrentOwner';
import {resolveRequest, isAwaitInUserspace} from './DevjsFlightServer';
...
import {parseStackTracePrivate} from './DevjsFlightServerConfig';
...
const emptyStack: DevjsStackTrace = [];
        ...
        if (node !== undefined) {
          ...
          switch (node.tag) {
            ...
            default:
              ...
              throw new Error(
                'A Promise should never be an IO_NODE. This is a bug in Devjs.',
              );
          }
          const currentAsyncId = executionAsyncId();
...
export function markAsyncSequenceRootTask(): void {
  if (__DEV__ && enableAsyncDebugInfo) {
    // Whatever Task we're running now is spawned by Devjs itself to perform render work.
    // Don't track any cause beyond this task. We may still track I/O that was started outside
    // Devjs but just not the cause of entering the render.
