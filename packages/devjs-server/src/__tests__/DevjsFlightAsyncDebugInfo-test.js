...
describe('DevjsFlightAsyncDebugInfo', () => {
  beforeEach(() => {
    jest.resetModules();
    jest.useRealTimers();
    patchSetImmediate();
  ...
  function fetchThirdParty(Component) {
    ...
    const result = DevjsServerDOMClient.createFromNodeStream(readable, {
      moduleMap: {},
      moduleLoading: {},
    });
  ...
  it('can track async information when awaited', async () => {
    ...
    const stream = DevjsServerDOMServer.renderToPipeableStream(<Component />);
        ...
        [
          ...
          },
          {
            ...
            "stack": [
              [
                "Object.<anonymous>",
                "/packages/devjs-server/src/__tests__/DevjsFlightAsyncDebugInfo-test.js",
                130,
                109,
                109,
                50,
  ...
  it('adds a description to I/O', async () => {
    ...
    async function Component() {
      await getData('http://github.com/Suryanshu-Nabheet/dev.js/pulls');
      await getData('http://github.com/Suryanshu-Nabheet/dev.js/pulls/');
      await getData(
        'https://this-is-a-very-long-domain-name-what-happens.app/test',
      );
      await getData(
        'https://github.com/Suryanshu-Nabheet/dev.js/commit/75897c2dcd1dd3a6ca46284dd37e13d22b4b16b4',
      );
    ...
    const result = DevjsServerDOMClient.createFromNodeStream(
      readable,
      {
        moduleMap: {},
        moduleLoading: {},
  ...
  it('can track async information when use()d', async () => {
    ...
    function Component() {
      const result = DevjsServer.use(getData('hi'));
      const moreData = getData('seb');
      return <InnerComponent text={result} promise={moreData} />;
    }
    ...
    function InnerComponent({text, promise}) {
      // This async function depends on the I/O in parent components but it should not
      // include that I/O as part of its own meta data.
      return text + ', ' + DevjsServer.use(promise);
    }
  ...
  it('can track async file reads', async () => {
    ...
    const stream = DevjsServerDOMServer.renderToPipeableStream(
      DevjsServer.createElement(Component),
      {},
    );
  ...
  it('should not exponentially accumulate debug info on outlined debug chunks', async () => {
    ...
    async function Level({depth, context}) {
      ...
      return DevjsServer.createElement(Level, {
        depth: depth - 1,
        context: newContext,
      });
    }
    ...
    const stream = DevjsServerDOMServer.renderToPipeableStream(
      DevjsServer.createElement(Level, {depth: 20, context: {root: true}}),
    );
