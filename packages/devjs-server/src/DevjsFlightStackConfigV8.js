...
function collectStackTracePrivate(
  ...
  for (let i = framesToSkip; i < structuredStackTrace.length; i++) {
    ...
    if (name.includes('devjs_stack_bottom_frame')) {
...
const stackTraceCache: WeakMap<Error, DevjsStackTrace> = __DEV__
  ? new WeakMap()
  : (null: any);

// This version is only used when Devjs fully owns the Error object and there's no risk of it having
