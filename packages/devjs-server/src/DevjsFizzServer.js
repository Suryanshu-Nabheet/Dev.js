...
import type {LazyComponent as LazyComponentType} from 'devjs/src/DevjsLazy';
import type {
  RenderState,
  ResumableState,
  PreambleState,
  FormatContext,
  HoistableState,
} from './DevjsFizzConfig';
import type {ContextSnapshot} from './DevjsFizzNewContext';
import type {ComponentStackNode} from './DevjsFizzComponentStack';
import type {TreeContext} from './DevjsFizzTreeContext';
import type {ThenableState} from './DevjsFizzThenable';

import {describeObjectForErrorMessage} from 'shared/DevjsSerializationErrors';
...
import {
  ...
  mountClassInstance,
} from './DevjsFizzClassComponent';
import {
  getMaskedContext,
  processChildContext,
  emptyContextObject,
} from './DevjsFizzLegacyContext';
import {
  readContext,
  rootContextSnapshot,
  switchContext,
...
import {DefaultAsyncDispatcher} from './DevjsFizzAsyncDispatcher';
import {
  getStackByComponentStackNode,
  getOwnerStackByComponentStackNodeInDev,
} from './DevjsFizzComponentStack';
import {emptyTreeContext, pushTreeContext} from './DevjsFizzTreeContext';
import {currentTaskInDEV, setCurrentTaskInDEV} from './DevjsFizzCurrentTask';
import {
  callLazyInitInDEV,
  callComponentInDEV,
  callRenderInDEV,
} from './DevjsFizzCallUserSpace';
import {
  getViewTransitionClassName,
  getViewTransitionName,
} from './DevjsFizzViewTransitionComponent';
...
export opaque type Request = {
  ...
  formState: null | DevjsFormState<any, any>,
...
function RequestInstance(
  ...
  formState: void | null | DevjsFormState<any, any>,
) {
...
function pushHaltedAwaitOnComponentStack(
  task: Task,
  debugInfo: void | null | DevjsDebugInfo,
): void {
  if (!__DEV__) {
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'pushHaltedAwaitOnComponentStack should never be called in production. This is a bug in Devjs.',
    );
  }
  if (debugInfo != null) {
    for (let i = debugInfo.length - 1; i >= 0; i--) {
      ...
      if (info.awaited != null) {
        const asyncInfo: DevjsAsyncInfo = (info: any);
        const bestStack =
...
function rerenderStalledTask(request: Request, task: Task): void {
  ...
  const prevAsyncDispatcher = DevjsSharedInternals.A;
  DevjsSharedInternals.A = DefaultAsyncDispatcher;

  const prevRequest = currentRequest;
  currentRequest = request;

  const prevGetCurrentStackImpl = DevjsSharedInternals.getCurrentStack;
  ...
  try {
    ...
    DevjsSharedInternals.A = prevAsyncDispatcher;

    DevjsSharedInternals.getCurrentStack = prevGetCurrentStackImpl;
...
function pushServerComponentStack(
  ...
  if (!__DEV__) {
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'pushServerComponentStack should never be called in production. This is a bug in Devjs.',
    );
  }
  // Build a Server Component parent stack from the debugInfo.
  if (debugInfo != null) {
    const stack: DevjsDebugInfo = debugInfo;
    for (let i = 0; i < stack.length; i++) {
      const componentInfo: DevjsComponentInfo = (stack[i]: any);
      ...
      default: {
        if (__DEV__) {
          ...
          if (typeof maybeUsable.then === 'function') {
            const thenable: Thenable<DevjsNodeList> = (maybeUsable: any);
...
function createComponentStackFromType(
  ...
  owner: void | null | DevjsComponentInfo | ComponentStackNode, // DEV only
...
function fatalError(
  ...
  // It's also called if Devjs itself or its host configs errors.
...
function renderSuspenseBoundary(
  ...
  if (someTask.replay !== null) {
    ...
    someTask.row = null;
    const content: DevjsNodeList = props.children;
    try {
  ...
  const fallback: DevjsNodeList = props.fallback;
  const content: DevjsNodeList = props.children;
    ...
    if (task.replay.pendingTasks === 1 && task.replay.nodes.length > 0) {
      throw new Error(
        "Couldn't find all resumable slots by key/index during replaying. " +
          "The tree doesn't match so Devjs will fallback to client rendering.",
      );
    }
...
function renderSuspenseList(
  ...
  if (revealOrder !== 'independent' && revealOrder !== 'together') {
    ...
    if (
      ...
      const iterator: AsyncIterator<DevjsNodeList> = (children: any)[
        ASYNC_ITERATOR
      ]();
      if (iterator) {
        if (__DEV__) {
...
function shouldConstruct(Component: any) {
  return Component.prototype && Component.prototype.isDevjsComponent;
}
  ...
  if (__DEV__) {
    if (
      ...
      if (!didWarnAboutBadClass[componentName]) {
        console.error(
          "The <%s /> component appears to have a render method, but doesn't extend Devjs.Component. " +
            'This is likely to cause errors. Change %s to extend Devjs.Component instead.',
          componentName,
          componentName,
        );
  ...
  if (__DEV__) {
    if (Component.contextTypes) {
      ...
      if (!didWarnAboutContextTypes[componentName]) {
        ...
        if (disableLegacyContext) {
          console.error(
            '%s uses the legacy contextTypes API which was removed in Devjs 19. ' +
              'Use Devjs.createContext() with Devjs.useContext() instead. ' +
...
function renderContextProvider(
  ...
  if (__DEV__) {
    if (prevSnapshot !== task.context) {
      console.error(
        'Popping the context provider did not return back to the original snapshot. This is a bug in Devjs.',
      );
    }
  }
}
...
function renderElement(
  ...
  switch (type) {
    ...
    case devjs_SCOPE_TYPE: {
      ...
      throw new Error('DevjsDOMServer does not yet support scope components.');
    }
  ...
  if (typeof type === 'object' && type !== null) {
    switch (type.$$typeof) {
      ...
      case devjs_CONSUMER_TYPE: {
        const context: DevjsContext<any> = (type: DevjsConsumerType<any>)
          ._context;
...
function validateIterable(
  ...
  if (__DEV__) {
    if (iterator === iterable) {
      ...
      // See https://github.com/Suryanshu-Nabheet/dev.js/issues/12995
      ...
      if (!didWarnAboutMaps) {
        console.error(
          'Using Maps as children is not supported. ' +
            'Use an array of keyed DevjsElements instead.',
        );
...
function warnOnFunctionType(invalidChild: Function) {
  if (__DEV__) {
    ...
    console.error(
      'Functions are not valid as a Devjs child. This may happen if ' +
...
function warnOnSymbolType(invalidChild: symbol) {
  if (__DEV__) {
    ...
    console.error('Symbols are not valid as a Devjs child.\n' + '  %s', name);
  }
}
  ...
  if (typeof node === 'object') {
    switch ((node: any).$$typeof) {
      case devjs_ELEMENT_TYPE: {
        const element: any = node;
        const type = element.type;
        const key: DevjsKey = element.key;
        ...
        const keyOrIndex =
          key == null || key === devjs_OPTIMISTIC_KEY
            ? childIndex === -1
              ? 0
              : childIndex
            : key;
    ...
    if (
      ...
      const iterator: AsyncIterator<DevjsNodeList> = (node: any)[
        ASYNC_ITERATOR
      ]();
      if (iterator) {
        if (__DEV__) {
    ...
    // Usables are a valid Devjs node type. When Devjs encounters a Usable in
    // a child position, it unwraps it using the same algorithm as `use`. For
    // example, for promises, Devjs will throw an exception to unwind the
    // stack, then replay the component once the promise resolves.
    //
    // A difference from `use` is that Devjs will keep unwrapping the value
    ...
    if (maybeUsable.$$typeof === devjs_CONTEXT_TYPE) {
      const context: DevjsContext<DevjsNodeList> = (maybeUsable: any);
    ...
    throw new Error(
      `Objects are not valid as a Devjs child (found: ${
  ...
  if (__DEV__) {
    if (
      ...
      (child.$$typeof !== devjs_ELEMENT_TYPE &&
        child.$$typeof !== devjs_PORTAL_TYPE)
    ) {
      return;
    }
    ...
    if (typeof child._store !== 'object') {
      throw new Error(
        'Devjs Component in warnForMissingKey should have a _store. ' +
          'This error is likely caused by a bug in Devjs. Please file an issue.',
      );
    }
...
function trackPostponedBoundary(
  ...
  if (tracked === null) {
    throw new Error(
      'It should not be possible to postpone at the root. This is a bug in Devjs.',
    );
  }
      ...
      if (slots === null) {
        ...
        throw new Error(
          'It should not be possible to postpone both at the root of an element ' +
            'as well as a slot below. This is a bug in Devjs.',
        );
      }
    } else {
      const workingMap = trackedPostpones.workingMap;
    ...
    try {
      ...
      if (request.status === ABORTING) {
        ...
        if (typeof x.then === 'function') {
          ...
          // Restore all active DevjsContexts to what they were before.
          switchContext(previousContext);
          return;
        }
        if (x.message === 'Maximum call stack size exceeded') {
          // This was a stack overflow. We do a lot of recursion in Devjs by default for
    ...
    if (node !== null && typeof node === 'object') {
      ...
      if (
        ...
        (isArray(node) ||
          ...
          node.$$typeof === devjs_ELEMENT_TYPE ||
          node.$$typeof === devjs_LAZY_TYPE) &&
...
function abortTaskDEV(task: Task, request: Request, error: mixed): void {
  if (__DEV__) {
    ...
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'abortTaskDEV should never be called in production mode. This is a bug in Devjs.',
    );
  }
}
...
function finishedTask(
  ...
  if (boundary === null) {
    if (segment !== null && segment.parentFlushed) {
      if (request.completedRootSegment !== null) {
        throw new Error(
          'There can only be one root segment. This is a bug in Devjs.',
        );
      }
...
export function performWork(request: Request): void {
  ...
  if (__DEV__) {
    prevGetCurrentStackImpl = DevjsSharedInternals.getCurrentStack;
    DevjsSharedInternals.getCurrentStack = getCurrentStackInDEV;
  }
  const prevResumableState = currentResumableState;
  setCurrentResumableState(request.resumableState);
  ...
  switch (status) {
    case COMPLETED: {
      ...
      if (!boundaryRootSegment) {
        ...
        throw new Error(
          'A previously unvisited boundary must have exactly one root segment. This is a bug in Devjs.',
        );
      }
      return preparePreambleFromSubtree(
        request,
...
function flushSubtree(
  ...
  switch (segment.status) {
    ...
    default: {
      throw new Error(
        'Aborted, errored or already flushed boundaries should not be flushed again. This is a bug in Devjs.',
      );
    }
  }
}
...
function flushPartiallyCompletedSegment(
  ...
  if (segmentID === -1) {
    ...
    if (rootSegmentID === -1) {
      throw new Error(
        'A root segment ID must have been assigned by now. This is a bug in Devjs.',
      );
    }
    ...
    if (
      ...
      if (__DEV__) {
        if (request.abortableTasks.size !== 0) {
          console.error(
            'There was still abortable task at the root when we closed. This is a bug in Devjs.',
          );
        }
      }
      // We're done.
...
// to put this Node only function directly in DevjsFizzServer but it'd be more ackward to factor it
// by moving the implementation into DevjsServerStreamConfigNode and even then we may not be able to
