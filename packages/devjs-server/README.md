...
This is an experimental package for creating custom Devjs streaming server renderers.
...
`devjs-server` is a package implementing various Server Rendering capabilities. The two implementation are codenamed `Fizz` and `Flight`.

`Fizz` is a renderer for Server Side Rendering Devjs. The same code that runs in the client (browser or native) is run on the server to produce an initial view to send to the client before it has to download and run Devjs and all the user code to produce that view on the client.

`Flight` is a renderer for Devjs Server Components. These are components that never run on a client. The output of a Devjs Server Component render can be a Devjs tree that can run on the client or be SSR'd using `Fizz`.
...
To use `devjs-server` for Devjs Server Components you must set up an implementation package alongside `devjs-client`. Use an existing implementation such as `devjs-server-dom-webpack` as a guide.
...
import {
  ...
  stopFlowing,
  abort,
} from 'devjs-server/src/DevjsFlightServer'
...
`devjs-server` implements the Devjs Server Components rendering implementation. Devjs Server Components is in essence a general purpose serialization and deserialization capability with support for some built-in Devjs primitives such as Suspense and Lazy.

The renderable type is a superset of `structuredClone()`. In addition to all the cloneable types `devjs-server` can render Symbols, Promises, Iterators and Iterables, async Iterators and Iterables.
...
// objects and Arrays
createResponse({ messages: ['hello', 'devjs'] }, ...)

// Maps, Sets, and more
createResponse({ m: Map(['k', 'v'])}, ...)
```

Additionally Devjs built ins can be rendered including Function Components

Function Component are called and the return value can be any renderable type. Since `devjs-server` supports Promises, Function Components can be async functions.
...
When rendering with `devjs-server` there are two broad contexts when this might happen. Realtime when responding to a user request and ahead of time when prerendering a page that can later be used more than once.
...
The signature of this method changes as we evolve the project so this Readme will omit the specific signature but generally this function will produce a Request that represents the rendering of some Devjs application (the model) along with implementation specific bundler configuration. Typically this configuration will tell the `Flight` implementation how to encode Client References in the serialized output
