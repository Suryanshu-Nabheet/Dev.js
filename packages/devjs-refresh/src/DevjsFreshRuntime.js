...
import type {
  ...
  ScheduleRoot,
  SetRefreshHandler,
} from 'devjs-reconciler/src/DevjsFiberHotReloading';
import type {DevjsNodeList} from 'shared/DevjsTypes';

import {devjs_MEMO_TYPE, devjs_FORWARD_REF_TYPE} from 'shared/DevjsSymbols';
...
if (!__DEV__) {
  throw new Error(
    'Devjs Refresh runtime should not be included in the production bundle.',
  );
}
...
const allSignaturesByType: WeakMap<any, Signature> | Map<any, Signature> =
  new PossiblyWeakMap();
// This WeakMap is read by Devjs, so we only put families
// that have actually been edited here. This keeps checks fast.
const updatedFamiliesByType: WeakMap<any, Family> | Map<any, Family> =
  new PossiblyWeakMap();

// This is cleared on every performDevjsRefresh() call.
...
const rootElements: WeakMap<any, DevjsNodeList> | null =
...
function isDevjsClass(type: any) {
  return type.prototype && type.prototype.isDevjsComponent;
}

function canPreserveStateBetween(prevType: any, nextType: any) {
  if (isDevjsClass(prevType) || isDevjsClass(nextType)) {
    return false;
  }
...
export function performDevjsRefresh(): RefreshUpdate | null {
  if (!__DEV__) {
    throw new Error(
      'Unexpected call to Devjs Refresh in a production environment.',
    );
  }
  ...
  try {
    ...
    updates.forEach(([family, nextType]) => {
      ...
      // that will be read by the Devjs reconciler.
    ...
    failedRootsSnapshot.forEach(root => {
      ...
      if (helpers === undefined) {
        throw new Error(
          'Could not find helpers for a root. This is a bug in Devjs Refresh.',
        );
      }
...
export function register(type: any, id: string): void {
  if (__DEV__) {
    ...
    // until the first performDevjsRefresh() call above.
...
export function injectIntoGlobalHook(globalObject: any): void {
  if (__DEV__) {
    // For Devjs Native, the global hook will be set up by require('devjs-devtools-core').
    // That code will run before us. So we need to monkeypatch functions on existing hook.

    // For Devjs Web, the global hook will be set up by the extension.
    // This will also run before us.
    let hook = globalObject.__devjs_DEVTOOLS_GLOBAL_HOOK__;
    if (hook === undefined) {
      // However, if there is no DevTools extension, we'll need to set up the global hook ourselves.
      // Note that in this case it's important that renderer code runs *after* this method call.
      // Otherwise, the renderer will think that there is no global hook, and won't do the injection.
      let nextID = 0;
      globalObject.__devjs_DEVTOOLS_GLOBAL_HOOK__ = hook = {
        renderers: new Map(),
        supportsFiber: true,
    ...
    if (hook.isDisabled) {
      ...
      console['warn'](
        'Something has shimmed the Devjs DevTools global hook (__devjs_DEVTOOLS_GLOBAL_HOOK__). ' +
    ...
    hook.inject = function (this: mixed, injected) {
      ...
      if (
        ...
        // This version supports Devjs Refresh.
    ...
    // This is useful if DevjsDOM has already been initialized.
    // https://github.com/Suryanshu-Nabheet/dev.js/issues/17626
  ...
  if (__DEV__) {
    ...
    return function <T>(
      ...
      if (typeof key === 'string') {
        ...
        // problems like https://github.com/Suryanshu-Nabheet/dev.js/issues/20417.
        if (
          type != null &&
    ...
    switch (typeof type) {
      case 'function': {
        ...
        if (type.prototype != null) {
          if (type.prototype.isDevjsComponent) {
            // Devjs class.
            return true;
          }
