...
const freshPlugin = require('devjs-refresh/babel');
const ts = require('typescript');

describe('DevjsFreshIntegration', () => {
  let container;
  let root;
  let exportsObj;
  ...
  afterEach(() => {
    if (__DEV__) {
      ...
      expect(DevjsFreshRuntime._getMountedRootCount()).toBe(0);
  ...
  function $RefreshSig$() {
    return DevjsFreshRuntime.createSignatureFunctionForTransform();
  }
  ...
  describe.each([
    ...
    async function render(source) {
      ...
      expect(DevjsFreshRuntime.performDevjsRefresh()).toBe(null);
    }
    ...
    async function patch(source) {
      ...
      const didExportsChange =
        DevjsFreshRuntime.getFamilyByType(prevExports.default) !==
        DevjsFreshRuntime.getFamilyByType(nextExports.default);
      if (didExportsChange) {
      ...
      await act(() => {
        const result = DevjsFreshRuntime.performDevjsRefresh();
        if (!didExportsChange) {
      ...
      expect(DevjsFreshRuntime._getMountedRootCount()).toBe(1);
    }
  ...
  function testJavaScript(render, patch) {
    ...
    it('reloads a combination of memo and forwardRef', async () => {
      if (__DEV__) {
        await render(`
          ...
          const Parent = memo(Devjs.forwardRef(function (props, ref) {
            return <Child prop="A" ref={ref} />;
          }));

          const Child = Devjs.memo(({prop}) => {
            return <h1>{prop}1</h1>;
          });

          export default Devjs.memo(Parent);
        `);
        const el = container.firstChild;
        expect(el.textContent).toBe('A1');
        await patch(`
    ...
    it('reloads default export with named memo', async () => {
      if (__DEV__) {
        await render(`
          ...
          export default memo(Devjs.forwardRef(function Parent(props, ref) {
            return <Child prop="A" ref={ref} />;
          }));
        `);
        const el = container.firstChild;
    ...
    it('ignores ref for class component in hidden subtree', async () => {
      const code = `
        import {Activity} from 'devjs';

        // Avoid creating a new class on Fast Refresh.
        global.A = global.A ?? class A extends Devjs.Component {
          render() {
            return <div />;
          }
        }
    ...
    it('reloads HOCs if they return functions', async () => {
      if (__DEV__) {
        ...
        await patch(`
          ...
          export default function Parent() {
            return Devjs.createElement(Child);
          }
    ...
    it('does not crash when changing Hook order inside a memo+forwardRef-ed HOC with direct call', async () => {
      if (__DEV__) {
        await render(`
          const {useEffect, memo, forwardRef} = Devjs;
      ...
      if (__DEV__) {
        await render(`
          function useFancyState(initialState) {
            return Devjs.useState(initialState);
          }
    ...
    it('does not get confused when component is called early', async () => {
      if (__DEV__) {
        await render(`
          ...
          // to simulate what happens when you call DevjsDOM.render
        ...
        await render(`
          const App = () => {
            const useFancyState = (initialState) => {
              const result = Devjs.useState(initialState);
              return result;
            };
    ...
    it('remounts when switching export from function to class', async () => {
      if (__DEV__) {
        ...
        await patch(`
          export default class App extends Devjs.Component {
            render() {
              return <h1>B1</h1>
            }
          }
    ...
    it('remounts when wrapping export in forwardRef()', async () => {
      if (__DEV__) {
        ...
        await patch(`
          ...
          export default Devjs.forwardRef(App);
        `);
        // Reset (wrapped in HOC).
        expect(container.firstChild).not.toBe(el);
        el = container.firstChild;
    ...
    it('resets useMemoCache cache slots', async () => {
      if (__DEV__) {
        await render(`
          const useMemoCache = require('devjs/compiler-runtime').c;
          let cacheMisses = 0;
          const cacheMiss = (id) => {
            cacheMisses++;
            return id;
