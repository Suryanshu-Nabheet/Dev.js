...
describe('DevjsFreshBabelPlugin', () => {
  ...
  it('registers likely HOCs with inline functions', () => {
    expect(
      transform(`
        ...
        const B = memo(Devjs.forwardRef(() => {
          return <h1>Foo</h1>;
        }));
        export default Devjs.memo(forwardRef((props, ref) => {
          return <h1>Foo</h1>;
        }));
    `),
    ).toMatchSnapshot();
    expect(
      transform(`
        export default Devjs.memo(forwardRef(function (props, ref) {
          return <h1>Foo</h1>;
        }));
    `),
    ).toMatchSnapshot();
    expect(
      transform(`
        export default Devjs.memo(forwardRef(function Named(props, ref) {
          return <h1>Foo</h1>;
        }));
    `),
    ).toMatchSnapshot();
  ...
  it('registers identifiers used in Devjs.createElement at definition site', () => {
    // When in doubt, register variables that were used in JSX.
    // Foo, Header, and B get registered.
    // A doesn't get registered because it's not declared locally.
    // Alias doesn't get registered because its definition is just an identifier.
    expect(
      transform(`
        ...
        function Foo() {
          return [
            Devjs.createElement(A),
            Devjs.createElement(B),
            Devjs.createElement(StyledFactory1),
            Devjs.createElement(StyledFactory2),
            Devjs.createElement(StyledFactory3),
            Devjs.createElement(Alias1),
            Devjs.createElement(Alias2),
            jsx(Header),
            Devjs.createElement(Dict.X),
          ];
        }

        Devjs.createContext(Store);
  ...
  it('generates signatures for function expressions calling hooks', () => {
    ...
    expect(
      transform(`
        export const A = Devjs.memo(Devjs.forwardRef((props, ref) => {
          const [foo, setFoo] = useState(0);
          Devjs.useEffect(() => {});
          return <h1 ref={ref}>{foo}</h1>;
        }));

        export const B = Devjs.memo(Devjs.forwardRef(function(props, ref) {
          const [foo, setFoo] = useState(0);
          Devjs.useEffect(() => {});
          return <h1 ref={ref}>{foo}</h1>;
        }));
    ...
    expect(
      transform(`
        function useFancyState() {
          const [foo, setFoo] = Devjs.useState(0);
          useFancyEffect();
          return foo;
        }
  ...
  it("respects Babel's envName option", () => {
    ...
    expect(() =>
      ...
      'Devjs Refresh Babel transform should only be enabled in development environment. ' +
