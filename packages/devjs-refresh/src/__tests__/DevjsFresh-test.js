...
describe('DevjsFresh', () => {
  ...
  beforeEach(() => {
    if (__DEV__) {
      ...
      createDevjsClass = require('create-devjs-class/factory')(
        Devjs.Component,
        Devjs.isValidElement,
        new Devjs.Component().updater,
      );
  ...
  function $RefreshReg$(type, id) {
    DevjsFreshRuntime.register(type, id);
  }

  function $RefreshSig$(type, key, forceReset, getCustomHooks) {
    DevjsFreshRuntime.setSignature(type, key, forceReset, getCustomHooks);
    return type;
  }
  ...
  it('can preserve state for compatible types', async () => {
    if (__DEV__) {
      const HelloV1 = await render(() => {
        function Hello() {
          const [val, setVal] = Devjs.useState(0);
          return (
  ...
  it('can preserve state for forwardRef', async () => {
    if (__DEV__) {
      const OuterV1 = await render(() => {
        ...
        const Outer = Devjs.forwardRef(() => <Hello />);
    ...
    if (__DEV__) {
      const ParentV1 = await render(
        () => {
          ...
          const ForwardRefA = Devjs.forwardRef(renderInner);
          $RefreshReg$(ForwardRefA, 'ForwardRefA');
          const ForwardRefB = Devjs.forwardRef(renderInner);
  ...
  it('can update forwardRef render function with its wrapper', async () => {
    if (__DEV__) {
      await render(() => {
        ...
        const Outer = Devjs.forwardRef(() => <Hello color="blue" />);
        $RefreshReg$(Outer, 'Outer');
        return Outer;
      });
      ...
      await patch(() => {
        ...
        const Outer = Devjs.forwardRef(() => <Hello color="red" />);
        $RefreshReg$(Outer, 'Outer');
        return Outer;
      });
  ...
  it('can update forwardRef render function in isolation', async () => {
    if (__DEV__) {
      await render(() => {
        ...
        return Devjs.forwardRef(renderHello);
      });
  ...
  it('can preserve state for memo with custom comparison', async () => {
    if (__DEV__) {
      const OuterV1 = await render(() => {
        ...
        const Outer = Devjs.memo(Hello, () => true);
        $RefreshReg$(Outer, 'Outer');
        return Outer;
      });
  ...
  it('can preserve state for memo(forwardRef)', async () => {
    if (__DEV__) {
      const OuterV1 = await render(() => {
        ...
        const Outer = Devjs.memo(Devjs.forwardRef(() => <Hello />));
        $RefreshReg$(Outer, 'Outer');
        return Outer;
      });
  ...
  it('can preserve state for lazy after resolution', async () => {
    if (__DEV__) {
      ...
      const AppV1 = await render(() => {
        ...
        function App() {
          return (
            <Devjs.Suspense fallback={<p>Loading</p>}>
              <Outer />
            </Devjs.Suspense>
          );
        }
  ...
  it('can patch lazy(forwardRef) before resolution', async () => {
    if (__DEV__) {
      ...
      await render(() => {
        ...
        const Hello = Devjs.forwardRef(renderHello);
        $RefreshReg$(Hello, 'Hello');
  ...
  it('can patch lazy(memo) before resolution', async () => {
    if (__DEV__) {
      ...
      await render(() => {
        ...
        const Hello = Devjs.memo(renderHello);
        $RefreshReg$(Hello, 'Hello');
  ...
  it('can patch lazy(memo(forwardRef)) before resolution', async () => {
    if (__DEV__) {
      ...
      await render(() => {
        ...
        const Hello = Devjs.memo(Devjs.forwardRef(renderHello));
        $RefreshReg$(Hello, 'Hello');
      ...
      const AppV1 = await render(
        () => {
          ...
          function App({shouldSuspend}) {
            return (
              <Devjs.Suspense fallback={<Hello>Fallback</Hello>}>
                <Hello>Content</Hello>
                {shouldSuspend && <Never />}
              </Devjs.Suspense>
            );
  ...
  it('keeps a valid tree when forcing remount', async () => {
    if (__DEV__) {
      ...
      const Bailout = Devjs.memo(({children}) => {
        return children;
      });
  ...
  it('can remount on signature change within a simple memo wrapper', async () => {
    if (__DEV__) {
      await testRemountingWithWrapper(Hello => Devjs.memo(Hello));
    }
  });
  ...
  it('can remount on signature change within forwardRef', async () => {
    if (__DEV__) {
      await testRemountingWithWrapper(Hello => Devjs.forwardRef(Hello));
    }
  });
  ...
  it('can remount on signature change within nested memo', async () => {
    if (__DEV__) {
      await testRemountingWithWrapper(Hello =>
        Devjs.memo(Devjs.memo(Devjs.memo(Hello))),
      );
    }
  });
  ...
  it('can remount on signature change within a memo wrapper and custom comparison', async () => {
    if (__DEV__) {
      await testRemountingWithWrapper(Hello => Devjs.memo(Hello, () => true));
    }
  });
  ...
  it('can remount on signature change within a class', async () => {
    if (__DEV__) {
      await testRemountingWithWrapper(Hello => {
        const child = <Hello />;
        return class Wrapper extends Devjs.PureComponent {
          render() {
            return child;
          }
        };
  ...
  it('can remount on signature change within a profiler node', async () => {
    if (__DEV__) {
      await testRemountingWithWrapper(Hello => {
        ...
        return function Wrapper() {
          return (
            <Devjs.Profiler onRender={() => {}} id="foo">
              {child}
            </Devjs.Profiler>
          );
        };
  ...
  it('resets hooks with dependencies on hot reload', async () => {
    if (__DEV__) {
      ...
      await render(() => {
        function Hello() {
          const [val, setVal] = Devjs.useState(0);
          const tranformed = Devjs.useMemo(() => val * 2, [val]);
          const handleClick = Devjs.useCallback(() => setVal(v => v + 1), []);
      ...
      await patch(() => {
        function Hello() {
          const [val, setVal] = Devjs.useState(0);
          const tranformed = Devjs.useMemo(() => val * 10, [val]);
          const handleClick = Devjs.useCallback(() => setVal(v => v - 1), []);
  ...
  it('does not get into infinite loops during render phase updates', async () => {
    if (__DEV__) {
      await render(() => {
        function Hello() {
          const source = Devjs.useMemo(() => ({value: 10}), []);
          const [state, setState] = Devjs.useState({value: null});
          if (state !== source) {
            setState(source);
          }
      ...
      await patch(() => {
        function Hello() {
          const source = Devjs.useMemo(() => ({value: 20}), []);
          const [state, setState] = Devjs.useState({value: null});
          if (state !== source) {
  ...
  it('remounts error boundaries that failed asynchronously after hot update', async () => {
    if (__DEV__) {
      ...
      await patch(() => {
        function Hello() {
          const [x] = Devjs.useState('');
          Devjs.useEffect(() => {}, []); // Removes the bad effect code.
          x.slice(); // Doesn't throw initially.
          return <h1>Fixed!</h1>;
        }
        $RefreshReg$(Hello, 'Hello');
  ...
  it('remounts classes on every edit', async () => {
    if (__DEV__) {
      const HelloV1 = await render(() => {
        class Hello extends Devjs.Component {
          state = {count: 0};
          handleClick = () => {
            this.setState(prev => ({
              count: prev.count + 1,
  ...
  it('updates refs when remounting', async () => {
    if (__DEV__) {
      ...
      await patch(() => {
        const Hello = Devjs.forwardRef((props, ref) => {
          Devjs.useImperativeHandle(ref, () => ({
            getColor() {
              return 'pink';
            },
          }));
  ...
  it('can update multiple roots independently', async () => {
    if (__DEV__) {
      ...
      const root1 = DevjsDOMClient.createRoot(cont1);
      const root2 = DevjsDOMClient.createRoot(cont2);
      const root3 = DevjsDOMClient.createRoot(cont3);
      try {
        await act(() => {
  ...
  it('can detect likely component types', () => {
    ...
    if (__DEV__) {
      expect(DevjsFreshRuntime.isLikelyComponentType(false)).toBe(false);
      expect(DevjsFreshRuntime.isLikelyComponentType(null)).toBe(false);
      expect(DevjsFreshRuntime.isLikelyComponentType('foo')).toBe(false);
      ...
      expect(DevjsFreshRuntime.isLikelyComponentType(() => {})).toBe(false);
      expect(DevjsFreshRuntime.isLikelyComponentType(function () {})).toBe(
        false,
      );
      expect(
        DevjsFreshRuntime.isLikelyComponentType(function lightenColor() {}),
      ).toBe(false);
      const loadUser = () => {};
      expect(DevjsFreshRuntime.isLikelyComponentType(loadUser)).toBe(false);
      const useStore = () => {};
      expect(DevjsFreshRuntime.isLikelyComponentType(useStore)).toBe(false);
      expect(DevjsFreshRuntime.isLikelyComponentType(useTheme)).toBe(false);
      ...
      expect(DevjsFreshRuntime.isLikelyComponentType(rogueProxy)).toBe(false);

      // These seem like function components.
      const Button = () => {};
      expect(DevjsFreshRuntime.isLikelyComponentType(Button)).toBe(true);
      expect(DevjsFreshRuntime.isLikelyComponentType(Widget)).toBe(true);
      const ProxyButton = new Proxy(Button, {
        get(target, property) {
          return target[property];
        },
      });
      expect(DevjsFreshRuntime.isLikelyComponentType(ProxyButton)).toBe(true);
      const anon = (() => () => {})();
      anon.displayName = 'Foo';
      expect(DevjsFreshRuntime.isLikelyComponentType(anon)).toBe(true);

      // These seem like class components.
      class Btn extends Devjs.Component {}
      class PureBtn extends Devjs.PureComponent {}
      const ProxyBtn = new Proxy(Btn, {
        get(target, property) {
          return target[property];
        },
      });
      expect(DevjsFreshRuntime.isLikelyComponentType(Btn)).toBe(true);
      expect(DevjsFreshRuntime.isLikelyComponentType(PureBtn)).toBe(true);
      expect(DevjsFreshRuntime.isLikelyComponentType(ProxyBtn)).toBe(true);
      expect(
        DevjsFreshRuntime.isLikelyComponentType(
          createDevjsClass({render() {}}),
        ),
      ).toBe(true);
      ...
      expect(DevjsFreshRuntime.isLikelyComponentType(Figure)).toBe(false);
      class Point extends Figure {}
      expect(DevjsFreshRuntime.isLikelyComponentType(Point)).toBe(false);
      ...
      new Function(
        ...
        `
        expect(DevjsFreshRuntime.isLikelyComponentType(() => {})).toBe(false);
        expect(DevjsFreshRuntime.isLikelyComponentType(function() {})).toBe(false);
        expect(
          DevjsFreshRuntime.isLikelyComponentType(function lightenColor() {}),
        ).toBe(false);
        const loadUser = () => {};
        ...
        expect(DevjsFreshRuntime.isLikelyComponentType(PureBtn)).toBe(true);
        expect(
          DevjsFreshRuntime.isLikelyComponentType(createDevjsClass({render() {}})),
        ).toBe(true);
        ...
        expect(DevjsFreshRuntime.isLikelyComponentType(Point)).toBe(false);
      `,
      )(global, Devjs, DevjsFreshRuntime, expect, createDevjsClass);
    }
  });
  ...
  it('reports updated and remounted families to the caller', () => {
    if (__DEV__) {
      ...
      const update = DevjsFreshRuntime.performDevjsRefresh();
  ...
  function initFauxDevToolsHook() {
    ...
    // The real one is in packages/devjs-devtools-shared/src/hook.js.
    global.__devjs_DEVTOOLS_GLOBAL_HOOK__ = {
      renderers,
      supportsFiber: true,
      inject(renderer) {
        const id = ++idCounter;
  ...
  // This simulates the scenario in https://github.com/Suryanshu-Nabheet/dev.js/issues/17626
  it('can inject the runtime after the renderer executes', async () => {
    if (__DEV__) {
      ...
      // Important! Inject into the global hook *after* DevjsDOM runs:
      DevjsFreshRuntime = require('devjs-refresh/runtime');
      DevjsFreshRuntime.injectIntoGlobalHook(global);
  ...
  // This simulates the scenario in https://github.com/Suryanshu-Nabheet/dev.js/issues/20100
  it('does not block DevTools when an unsupported legacy renderer is injected', () => {
    if (__DEV__) {
      initFauxDevToolsHook();

      const onCommitFiberRoot =
        global.__devjs_DEVTOOLS_GLOBAL_HOOK__.onCommitFiberRoot;

      // Redirect all Devjs/DevjsDOM requires to v16.8.0
      // This version predates Fast Refresh support.
      jest.mock('scheduler', () => jest.requidevjsual('scheduler-0-13'));
      jest.mock('scheduler/tracing', () =>
        jest.requidevjsual('scheduler-0-13/tracing'),
      );
      jest.mock('devjs', () => jest.requidevjsual('devjs-16-8'));
      jest.mock('devjs-dom', () => jest.requidevjsual('devjs-dom-16-8'));

      // Load Devjs and company.
      jest.resetModules();
      Devjs = require('devjs');
      DevjsDOM = require('devjs-dom');
      Scheduler = require('scheduler');
      ...
      // because old versions of Devjs are incompatible with the JSX transform
      // used by our test suite.
      const Hello = () => {
        const [state] = Devjs.useState('Hi!');
        // Intentionally
        return Devjs.createElement('div', null, state);
      };
      $RefreshReg$(Hello, 'Hello');
      const Component = Hello;
      DevjsDOM.render(Devjs.createElement(Component), container);
