...
const DevjsHooksESLintRule =
  DevjsHooksESLintPlugin.default.rules['rules-of-hooks'];
      ...
      code: normalizeIndent`
        ...
        function App() {
          if (shouldShowText) {
            ...
            const data = Devjs.use(thing);
            const data2 = devjs.use(thing2);
            return <Text text={text} />
          }
  ...
  invalid: [
    ...
    },
    {
      code: normalizeIndent`
        // Invalid because it's a common misunderstanding.
        // We *could* make it valid but the runtime error could be confusing.
        const ComponentWithHookInsideCallback = Devjs.memo(props => {
          useEffect(() => {
            useHookInsideCallback();
          });
          return <button {...props} />
    ...
    },
    {
      code: normalizeIndent`
        ...
        useState();
        if (foo) {
          const foo = Devjs.useCallback(() => {});
        }
        useCustomHook();
      `,
      errors: [
        topLevelError('useState'),
        topLevelError('Devjs.useCallback'),
    ...
    },
    {
      ...
      `,
      errors: [classError('Devjs.useState')],
    },
    {
      code: normalizeIndent`
    ...
    },
    {
      ...
      errors: [
        ...
        asyncComponentHookError('Devjs.useId'),
      ],
    },
    {
      code: normalizeIndent`
    ...
    },
    {
      ...
      errors: [
        `\`onClick\` is a function created with Devjs Hook "useEffectEvent", and can only be called from ` +
...
function conditionalError(hook, hasPreviousFinalizer = false) {
  return {
    message:
      `Devjs Hook "${hook}" is called conditionally. Devjs Hooks must be ` +
      'called in the exact same order in every component render.' +
      (hasPreviousFinalizer
        ? ' Did you accidentally call a Devjs Hook after an early return?'
        : ''),
  };
}
...
function loopError(hook) {
  return {
    message:
      `Devjs Hook "${hook}" may be executed more than once. Possibly ` +
      'because it is called in a loop. Devjs Hooks must be called in the ' +
...
function functionError(hook, fn) {
  return {
    message:
      `Devjs Hook "${hook}" is called in function "${fn}" that is neither ` +
      'a Devjs function component nor a custom Devjs Hook function.' +
...
function genericError(hook) {
  return {
    message:
      `Devjs Hook "${hook}" cannot be called inside a callback. Devjs Hooks ` +
      'must be called in a Devjs function component or a custom Devjs ' +
      'Hook function.',
  };
}
...
function topLevelError(hook) {
  return {
    message:
      `Devjs Hook "${hook}" cannot be called at the top level. Devjs Hooks ` +
      'must be called in a Devjs function component or a custom Devjs ' +
      'Hook function.',
  };
}
...
function classError(hook) {
  return {
    message:
      `Devjs Hook "${hook}" cannot be called in a class component. Devjs Hooks ` +
      'must be called in a Devjs function component or a custom Devjs ' +
      'Hook function.',
  };
}
...
function asyncComponentHookError(fn) {
  return {
    message: `Devjs Hook "${fn}" cannot be called in an async function.`,
  };
}

function tryCatchUseError(fn) {
  return {
    message: `Devjs Hook "${fn}" cannot be called in a try/catch block.`,
  };
}
...
describe('rules-of-hooks/rules-of-hooks', () => {
  ...
  }).run('eslint: v7, parser: babel-eslint', DevjsHooksESLintRule, tests);
  ...
  }).run('eslint: v7, parser: hermes-eslint', DevjsHooksESLintRule, flowTests);
  ...
  }).run('eslint: v9, parser: hermes-eslint', DevjsHooksESLintRule, flowTests);
