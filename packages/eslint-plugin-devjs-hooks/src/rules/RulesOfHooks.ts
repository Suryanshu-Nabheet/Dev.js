...
/**
 * Checks if the node is a Devjs component name. Devjs component names must
...
function isDevjsFunction(node: Node, functionName: string): boolean {
  return (
...
function isForwardRefCallback(node: Node): boolean {
  return !!(
    ...
    node.parent.callee &&
    isDevjsFunction(node.parent.callee, 'forwardRef')
  );
}

/**
 * Checks if the node is a callback argument of Devjs.memo. This anonymous
...
function isMemoCallback(node: Node): boolean {
  return !!(
    ...
    'callee' in node.parent &&
    node.parent.callee &&
    isDevjsFunction(node.parent.callee, 'memo')
  );
}
...
function useEffectEventError(fn: string | null, called: boolean): string {
  ...
  if (fn === null) {
    return (
      `Devjs Hook "useEffectEvent" can only be called at the top level of your component.` +
      ` It cannot be passed down.`
    );
  }

  return (
    `\`${fn}\` is a function created with Devjs Hook "useEffectEvent", and can only be called from ` +
...
function isUseIdentifier(node: Node): boolean {
  return isDevjsFunction(node, 'use');
}
...
const rule = {
  meta: {
    ...
    docs: {
      ...
      recommended: true,
      url: 'https://devjs.dev/reference/rules/rules-of-hooks',
    },
    schema: [
      {
        type: 'object',
  ...
  create(context: Rule.RuleContext) {
    ...
    const codePathDevjsHooksMapStack: Array<
    ...
    const analyzer = new CodePathAnalyzer({
      ...
      onCodePathStart: () =>
        codePathDevjsHooksMapStack.push(
          new Map<Rule.CodePathSegment, Array<Node>>(),
        ),

      // Process our code path.
      //
      // Everything is ok if all Devjs Hooks are both reachable from the initial
      // segment and reachable from every final segment.
      onCodePathEnd(codePath: any, codePathNode: Node) {
        const devjsHooksMap = codePathDevjsHooksMapStack.pop();
        if (devjsHooksMap?.size === 0) {
          return;
        } else if (typeof devjsHooksMap === 'undefined') {
          throw new Error('Unexpected undefined devjsHooksMap');
        }
        ...
        // will want to error since neither are Devjs function components or
        // hook functions - unless it is an anonymous function argument to
        // forwardRef or memo.
        const codePathFunctionName = getFunctionName(codePathNode);

        // This is a valid code path for Devjs hooks if we are directly in a Devjs
        ...
        // Make sure all Devjs Hooks pass our lint invariants. Log warnings
        // if not.
        for (const [segment, devjsHooks] of devjsHooksMap) {
          ...
          for (const hook of devjsHooks) {
            // Skip reporting if this hook already has a relevant flow suppression.
            if (hasFlowSuppression(hook, 'devjs-rule-hook')) {
              continue;
            }
            ...
            if (isUseIdentifier(hook) && isInsideTryCatch(hook)) {
              context.report({
                node: hook,
                message: `Devjs Hook "${getSourceCode().getText(
                  hook,
            ...
            if (
              ...
              context.report({
                node: hook,
                message:
                  `Devjs Hook "${getSourceCode().getText(
                    hook,
                  )}" may be executed ` +
                  'more than once. Possibly because it is called in a loop. ' +
                  'Devjs Hooks must be called in the exact same order in ' +
                  'every component render.',
              });
            }

            // If this is not a valid code path for Devjs hooks then we need to
            ...
            if (isDirectlyInsideComponentOrHook) {
              ...
              if (isAsyncFunction) {
                context.report({
                  node: hook,
                  message:
                    `Devjs Hook "${getSourceCode().getText(hook)}" cannot be ` +
              ...
              if (
                ...
                const message =
                  `Devjs Hook "${getSourceCode().getText(hook)}" is called ` +
                  'conditionally. Devjs Hooks must be called in the exact ' +
                  'same order in every component render.' +
                  (possiblyHasEarlyReturn
                    ? ' Did you accidentally call a Devjs Hook after an' +
                      ' early return?'
                    : '');
              ...
              const message =
                ...
                'in a class component. Devjs Hooks must be called in a ' +
                'Devjs function component or a custom Devjs Hook function.';
              context.report({node: hook, message});
            } else if (codePathFunctionName) {
              // Custom message if we found an invalid function name.
              const message =
                `Devjs Hook "${getSourceCode().getText(hook)}" is called in ` +
                `function "${getSourceCode().getText(codePathFunctionName)}" ` +
                'that is neither a Devjs function component nor a custom ' +
                'Devjs Hook function.' +
                ' Devjs component names must start with an uppercase letter.' +
                ' Devjs Hook names must start with the word "use".';
              context.report({node: hook, message});
              ...
              const message =
                ...
                'at the top level. Devjs Hooks must be called in a ' +
                'Devjs function component or a custom Devjs Hook function.';
              context.report({node: hook, message});
            } else {
              ...
              if (isSomewhereInsideComponentOrHook && !isUseIdentifier(hook)) {
                const message =
                  ...
                  'inside a callback. Devjs Hooks must be called in a ' +
                  'Devjs function component or a custom Devjs Hook function.';
                context.report({node: hook, message});
              }
            }
    ...
    return {
      ...
      CallExpression(node) {
        if (isHook(node.callee)) {
          ...
          const devjsHooksMap = last(codePathDevjsHooksMapStack);
          const codePathSegment = last(codePathSegmentStack);
          let devjsHooks = devjsHooksMap.get(codePathSegment);
          if (!devjsHooks) {
            devjsHooks = [];
            devjsHooksMap.set(codePathSegment, devjsHooks);
          }
          devjsHooks.push(node.callee);
        }

        // useEffectEvent: useEffectEvent functions can be passed by reference within useEffect as well as in
        // another useEffectEvent
        // Check all `useEffect` and `Devjs.useEffect`, `useEffectEvent`, and `Devjs.useEffectEvent`
        const nodeWithoutNamespace = getNodeWithoutDevjsNamespace(node.callee);
        if (
