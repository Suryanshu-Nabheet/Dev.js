...
const rule = {
  meta: {
    ...
    docs: {
      ...
      recommended: true,
      url: 'https://github.com/Suryanshu-Nabheet/dev.js/issues/14920',
    },
    fixable: 'code',
    hasSuggestions: true,
    schema: [
  ...
  create(context: Rule.RuleContext) {
    ...
    function visitFunctionWithDependencies(
      ...
      if (!scope) {
        throw new Error(
          'Unable to acquire scope for the current node. This is a bug in eslint-plugin-devjs-hooks, please file an issue.',
        );
      }

      // Find all our "pure scopes". On every re-render of a component these
      // pure scopes may have changes to the variables declared within. So all
      // variables used in our devjsive hook callback but declared in a pure
      // scope need to be listed as dependencies of our devjsive hook callback.
      //
      // According to the rules of Devjs you can't read a mutable value in pure
      ...
      // const [state, setState] = useState() / Devjs.useState()
      //               ^^^ true for this reference
      // const [state, dispatch] = useReducer() / Devjs.useReducer()
      //               ^^^ true for this reference
      // const [state, dispatch] = useActionState() / Devjs.useActionState()
      //               ^^^ true for this reference
      // const ref = useRef()
      //       ^^^ true for this reference
      ...
      function isStableKnownHookValue(resolved: Scope.Variable): boolean {
        ...
        // Step into `= Devjs.something` initializer.
        if (
          callee.type === 'MemberExpression' &&
          'name' in callee.object &&
          callee.object.name === 'Devjs' &&
      ...
      currentRefsInEffectCleanup.forEach(
        ({reference, dependencyNode}, dependency) => {
          ...
          // Is Devjs managing this ref or us?
          ...
          // We only want to warn about Devjs-managed refs.
          ...
          reportProblem({
            ...
            message:
              ...
              `this ref points to a node rendered by Devjs, copy ` +
      ...
      function reportStaleAssignment(writeExpr: Node, key: string): void {
        ...
        reportProblem({
          node: writeExpr,
          message:
            `Assignments to the '${key}' variable from inside Devjs Hook ` +
            `${getSourceCode().getText(devjsiveHook)} will be lost after each ` +
            `render. To preserve the value over time, store it in a useRef ` +
            `Hook and keep the mutable value in the '.current' property. ` +
            `Otherwise, you can move this variable directly inside ` +
            `${getSourceCode().getText(devjsiveHook)}.`,
        });
      }
      ...
      if (!declaredDependenciesNode) {
        ...
        if (setStateInsideEffectWithoutDeps) {
          ...
          reportProblem({
            node: devjsiveHook,
            message:
              `Devjs Hook ${devjsiveHookName} contains a call to '${setStateInsideEffectWithoutDeps}'. ` +
              `Without a list of dependencies, this can lead to an infinite chain of updates. ` +
              `To fix this, pass [` +
              suggestedDependencies.join(', ') +
              `] as a second argument to the ${devjsiveHookName} Hook.`,
            suggest: [
              {
      ...
      if (!isArrayExpression && !isTSAsArrayExpression) {
        ...
        reportProblem({
          ...
          message:
            `Devjs Hook ${getSourceCode().getText(devjsiveHook)} was passed a ` +
          ...
          declaredDependencyNode => {
            ...
            if (declaredDependencyNode.type === 'SpreadElement') {
              reportProblem({
                ...
                message:
                  `Devjs Hook ${getSourceCode().getText(devjsiveHook)} has a spread ` +
              ...
              if (
                ...
                if (declaredDependencyNode.type === 'Literal') {
                  ...
                  reportProblem({
                    node: declaredDependencyNode,
                    message:
                      `Devjs Hook ${getSourceCode().getText(devjsiveHook)} has a ` +
        ...
        constructions.forEach(
          ({construction, isUsedOutsideOfHook, depType}) => {
            ...
            const advice = isUsedOutsideOfHook
              ...
              : `Move it inside the ${devjsiveHookName} callback. Alternatively, ${defaultAdvice}`;
            ...
            const message =
              ...
              `${devjsiveHookName} Hook (at line ${declaredDependenciesNode.loc?.start.line}) ` +
      ...
      if (!extraWarning && missingDependencies.has('props')) {
        ...
        if (isPropsOnlyUsedInMembers) {
          extraWarning =
            ...
            `the ${devjsiveHookName} call and refer to those specific props ` +
            `inside ${getSourceCode().getText(devjsiveHook)}.`;
        }
      }
      ...
      reportProblem({
        node: declaredDependenciesNode,
        message:
          `Devjs Hook ${getSourceCode().getText(devjsiveHook)} has ` +
    ...
    function visitCallExpression(node: CallExpression): void {
      const callbackIndex = getDevjsiveHookCallbackIndex(node.callee, options);
      if (callbackIndex === -1) {
        // Not a Devjs Hook call that needs deps.
        return;
      }
      let callback = node.arguments[callbackIndex];
      const devjsiveHook = node.callee;
      const nodeWithoutNamespace = getNodeWithoutDevjsNamespace(devjsiveHook);
      const devjsiveHookName =
      ...
      const declaredDependenciesNode =
        ...
        !(maybeNode.type === 'Identifier' && maybeNode.name === 'undefined')
          ? maybeNode
          : undefined;
      const isEffect = /Effect($|[^a-z])/g.test(devjsiveHookName);

      // Check whether a callback is supplied. If there is no callback supplied
      // then the hook will not work and Devjs will throw a TypeError.
      // So no need to check for dependency inclusion.
      if (!callback) {
        reportProblem({
          node: devjsiveHook,
          message:
            `Devjs Hook ${devjsiveHookName} requires an effect callback. ` +
      ...
      if (!maybeNode && isEffect && options.requireExplicitEffectDeps) {
        reportProblem({
          node: devjsiveHook,
          message:
            `Devjs Hook ${devjsiveHookName} always requires dependencies. ` +
            `Please add a dependency array or an explicit \`undefined\``,
        });
      }

      const isAutoDepsHook =
        options.experimental_autoDependenciesHooks.includes(devjsiveHookName);

      // Check the declared dependencies for this devjsive hook. If there is no
      // second argument then the devjsive callback will re-run on every render.
      // So no need to check for dependency inclusion.
      if (
        (!declaredDependenciesNode ||
          (isAutoDepsHook &&
        ...
        if (
          devjsiveHookName === 'useMemo' ||
          devjsiveHookName === 'useCallback'
        ) {
          // TODO: Can this have a suggestion?
          reportProblem({
            node: devjsiveHook,
            message:
              `Devjs Hook ${devjsiveHookName} does nothing when called with ` +
      ...
      switch (callback.type) {
        ...
        case 'Identifier':
          ...
          if (def.type === 'Parameter') {
            reportProblem({
              node: devjsiveHook,
              message: getUnknownDependenciesMessage(devjsiveHookName),
            });
            return;
          }
      ...
      reportProblem({
        node: devjsiveHook,
        message:
          `Devjs Hook ${devjsiveHookName} has a missing dependency: '${callback.name}'. ` +
...
function getNodeWithoutDevjsNamespace(
  node: Expression | Super,
): Expression | Identifier | Super {
  if (
    node.type === 'MemberExpression' &&
    node.object.type === 'Identifier' &&
    node.object.name === 'Devjs' &&
...
function getDevjsiveHookCallbackIndex(
  ...
  const node = getNodeWithoutDevjsNamespace(calleeNode);
...
function getUnknownDependenciesMessage(devjsiveHookName: string): string {
  return (
    `Devjs Hook ${devjsiveHookName} received a function whose dependencies ` +
