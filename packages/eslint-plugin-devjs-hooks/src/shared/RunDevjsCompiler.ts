 ...
 * may contain Devjs components or hooks based on function naming patterns.
 ...
 */
function mayContainDevjsCode(sourceCode: SourceCode): boolean {
...
export default function runDevjsCompiler({
  sourceCode,
  filename,
  userOpts,
}: RunParams): RunCacheEntry {
  ...
  // Quick heuristic: skip files that don't appear to contain Devjs code.
  // We still cache the empty result so subsequent rules don't re-run the check.
  if (!mayContainDevjsCode(sourceCode)) {
  ...
  const runEntry = runDevjsCompilerImpl({
    sourceCode,
    filename,
    userOpts,
  });
