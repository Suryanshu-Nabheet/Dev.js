...
import type {
  ...
  StringDecoder,
  ModuleLoading,
} from './DevjsFlightClientConfig';

import type {
  HintCode,
  HintModel,
} from 'devjs-server/src/DevjsFlightServerConfig';

import type {
  CallServerCallback,
  EncodeFormActionCallback,
} from './DevjsFlightReplyClient';

import type {TemporaryReferenceSet} from './DevjsFlightTemporaryReferences';
...
import {readTemporaryReference} from './DevjsFlightTemporaryReferences';
...
} from './DevjsFlightPerformanceTrack';
...
import {injectInternals} from './DevjsFlightClientDevToolsHook';

import {OMITTED_PROP_ERROR} from 'shared/DevjsFlightPropertyAccess';
...
import type {SharedStateClient} from 'devjs/src/DevjsSharedInternalsClient';
...
// only though.
const DevjsSharedInteralsServer: void | SharedStateServer = (Devjs: any)
  .__SERVER_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;
const DevjsSharedInternals: SharedStateServer | SharedStateClient =
  Devjs.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE ||
  DevjsSharedInteralsServer;
...
type ProfilingResult = {
  track: number,
  endTime: number,
  component: null | DevjsComponentInfo,
};
...
type PendingChunk<T> = {
  ...
  _debugChunk: null | SomeChunk<DevjsDebugInfoEntry>, // DEV-only
  _debugInfo: DevjsDebugInfo, // DEV-only
...
function DevjsPromise(status: any, value: any, reason: any) {
  this.status = status;
  this.value = value;
  this.reason = reason;
...
DevjsPromise.prototype = (Object.create(Promise.prototype): any);
// TODO: This doesn't return a new Promise chain unlike the real .then
DevjsPromise.prototype.then = function <T>(
  this: SomeChunk<T>,
...
type Response = {
  ...
  _debugRootOwner?: null | DevjsComponentInfo, // DEV-only
...
function createPendingChunk<T>(response: Response): PendingChunk<T> {
  ...
  return new DevjsPromise(PENDING, null, null);
}
...
function releasePendingChunk(response: Response, chunk: SomeChunk<any>): void {
  if (__DEV__ && chunk.status === PENDING) {
    if (--response._pendingChunks === 0) {
      ...
      // Wait a short period to see if any more chunks get asked for. E.g. by a Devjs render.
...
function createBlockedChunk<T>(response: Response): BlockedChunk<T> {
  // $FlowFixMe[invalid-constructor] Flow doesn't support functions as constructors
  return new DevjsPromise(BLOCKED, null, null);
}
...
function createErrorChunk<T>(
  ...
  // $FlowFixMe[invalid-constructor] Flow doesn't support functions as constructors
  return new DevjsPromise(ERRORED, null, error);
}

function filterDebugInfo(
  response: Response,
  value: {_debugInfo: DevjsDebugInfo, ...},
) {
...
function moveDebugInfoFromChunkToInnerValue<T>(
  ...
  // value instead. This can be a Devjs element, an array, or an uninitialized
  // Lazy.
  ...
  if (
    ...
    (isArray(resolvedValue) ||
      ...
      resolvedValue.$$typeof === devjs_LAZY_TYPE)
  ) {
...
function createResolvedModelChunk<T>(
  ...
  // $FlowFixMe[invalid-constructor] Flow doesn't support functions as constructors
  return new DevjsPromise(RESOLVED_MODEL, value, response);
}
...
function createResolvedModuleChunk<T>(
  ...
  // $FlowFixMe[invalid-constructor] Flow doesn't support functions as constructors
  return new DevjsPromise(RESOLVED_MODULE, value, null);
}
...
function createInitializedTextChunk(
  ...
  // $FlowFixMe[invalid-constructor] Flow doesn't support functions as constructors
  return new DevjsPromise(INITIALIZED, value, null);
}
...
function createInitializedIteratorResultChunk<T>(
  ...
  // $FlowFixMe[invalid-constructor] Flow doesn't support functions as constructors
  return new DevjsPromise(INITIALIZED, {done: done, value: value}, null);
}
...
>(
  ...
  // $FlowFixMe[invalid-constructor] Flow doesn't support functions as constructors
  return new DevjsPromise(INITIALIZED, value, controller);
}
...
function createResolvedIteratorResultChunk<T>(
  ...
  // $FlowFixMe[invalid-constructor] Flow doesn't support functions as constructors
  return new DevjsPromise(RESOLVED_MODEL, iteratorResultJSON, response);
}
...
function getIOInfoTaskName(ioInfo: DevjsIOInfo): string {
  return ioInfo.name || 'unknown';
}

function getAsyncInfoTaskName(asyncInfo: DevjsAsyncInfo): string {
  return 'await ' + getIOInfoTaskName(asyncInfo.awaited);
}

function getServerComponentTaskName(componentInfo: DevjsComponentInfo): string {
  return '<' + (componentInfo.name || '...') + '>';
}

function getTaskName(type: mixed): string {
  if (type === devjs_FRAGMENT_TYPE) {
    return '<>';
  }
...
function createElement(
  ...
  stack: ?DevjsStackTrace, // DEV-only
  validated: 0 | 1 | 2, // DEV-only
):
  | Devjs$Element<any>
  | LazyComponent<Devjs$Element<any>, SomeChunk<Devjs$Element<any>>> {
  let element: any;
  if (__DEV__) {
  ...
  if (initializingHandler !== null) {
    ...
    if (handler.errored) {
      ...
      const erroredChunk: ErroredChunk<Devjs$Element<any>> = createErrorChunk(
        response,
        handler.reason,
      );
      if (__DEV__) {
        ...
        // serialization.
        const erroredComponent: DevjsComponentInfo = {
    ...
    if (handler.deps > 0) {
      ...
      const blockedChunk: BlockedChunk<Devjs$Element<any>> =
...
function fulfillReference(
  ...
  try {
    ...
    // If the parent object is an unparsed Devjs element tuple, we also need to
    // update the props and owner of the parsed element object (i.e.
    // handler.value).
    if (
      parentObject[0] === devjs_ELEMENT_TYPE &&
      typeof handler.value === 'object' &&
      handler.value !== null &&
      handler.value.$$typeof === devjs_ELEMENT_TYPE
    ) {
...
function rejectReference(
  ...
  if (__DEV__) {
    if (
      ...
      blockedValue !== null &&
      blockedValue.$$typeof === devjs_ELEMENT_TYPE
    ) {
...
function waitForReference<T>(
  ...
  if (
    ...
    if (
      ...
      // If the parent object is an unparsed Devjs element tuple, and this is a reference
    ...
    if (parentChunk !== null) {
      ...
      for (let i = 0; i < referencedDebugInfo.length; ++i) {
        ...
        if (debugInfoEntry.name != null) {
          (debugInfoEntry: DevjsComponentInfo);
  ...
  if (value[0] === '$') {
    ...
    switch (value[1]) {
      ...
      case 'L': {
        ...
        // We create a Devjs.lazy wrapper around any lazy values.
        // When passed into Devjs, we'll know how to suspend on this.
...
function parseModelTuple(
  ...
  if (tuple[0] === devjs_ELEMENT_TYPE) {
    // TODO: Consider having Devjs just directly accept these arrays as elements.
    // Or even change the DevjsElement type to be an array.
...
function ResponseInstance(
  ...
  if (__DEV__) {
    ...
    // getting the owner there as well, but currently the owner of DevjsComponentInfo is typed as only
    // supporting other DevjsComponentInfo as owners (and not Fiber or Fizz's ComponentStackNode).
    // We need to update all the callsites consuming DevjsComponentInfo owners to support those.
    // In the meantime we only check DevjsSharedInteralsServer since we know that in an RSC environment
    // the only owners will be DevjsComponentInfo.
    const rootOwner: null | DevjsComponentInfo =
      DevjsSharedInteralsServer === undefined ||
      DevjsSharedInteralsServer.A === null
        ? null
        : (DevjsSharedInteralsServer.A.getOwner(): any);

    this._debugRootOwner = rootOwner;
    this._debugRootStack =
      rootOwner !== null
        ? // TODO: Consider passing the top frame in so we can avoid internals showing up.
          new Error('devjs-stack-top-frame')
        : null;
...
export type StreamState = {
  ...
  _debugInfo: DevjsIOInfo, // DEV-only
...
function incrementChunkDebugInfo(
  ...
  if (__DEV__ && enableAsyncDebugInfo) {
    const debugInfo: DevjsIOInfo = streamState._debugInfo;
...
function addAsyncInfo(chunk: SomeChunk<any>, asyncInfo: DevjsAsyncInfo): void {
  ...
  if (
    ...
    (isArray(value) ||
      ...
      value.$$typeof === devjs_ELEMENT_TYPE ||
      value.$$typeof === devjs_LAZY_TYPE)
  ) {
...
function resolveChunkDebugInfo(
  ...
  if (__DEV__ && enableAsyncDebugInfo) {
    ...
    if (response._debugIOStarted) {
      ...
      const asyncInfo: DevjsAsyncInfo = {awaited: streamState._debugInfo};
      if (chunk.status === PENDING || chunk.status === BLOCKED) {
...
function resolveErrorProd(response: Response): Error {
  if (__DEV__) {
    ...
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'resolveErrorProd should never be called in development mode. Use resolveErrorDev instead. This is a bug in Devjs.',
    );
  }
...
function resolveErrorDev(
  ...
  if (!__DEV__) {
    ...
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'resolveErrorDev should never be called in production mode. Use resolveErrorProd instead. This is a bug in Devjs.',
    );
  }
...
function createFakeFunction<T>(
  ...
  if (sourceMap) {
    // We use the prefix about://Devjs/ to separate these from other files listed in
...
function initializeFakeTask(
  response: Response,
  debugInfo: DevjsComponentInfo | DevjsAsyncInfo | DevjsIOInfo,
): null | ConsoleTask {
  if (!supportsCreateTask) {
    return null;
  }
  ...
  // instead of the callsite. For DevjsAsyncInfo/DevjsIOInfo, the only thing we're going
  ...
  const taskName =
    ...
    env !== ownerEnv
      ...
      : // Some unfortunate pattern matching to refine the type.
        ...
        ? getServerComponentTaskName(((debugInfo: any): DevjsComponentInfo))
        : debugInfo.name !== undefined
          ? getIOInfoTaskName(((debugInfo: any): DevjsIOInfo))
          : getAsyncInfoTaskName(((debugInfo: any): DevjsAsyncInfo));
...
const createFakeJSXCallStackInDEV: (
  ...
  ? // We use this technique to trick minifiers to preserve the function name.
    (createFakeJSXCallStack.devjs_stack_bottom_frame.bind(
...
function fakeJSXCallSite() {
  ...
  return new Error('devjs-stack-top-frame');
}
...
function initializeDebugInfo(
  ...
  if (!__DEV__) {
    ...
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'initializeDebugInfo should never be called in production mode. This is a bug in Devjs.',
    );
  }
  if (debugInfo.stack !== undefined) {
    const componentInfoOrAsyncInfo: DevjsComponentInfo | DevjsAsyncInfo =
  ...
  if (debugInfo.owner == null && response._debugRootOwner != null) {
    const componentInfoOrAsyncInfo: DevjsComponentInfo | DevjsAsyncInfo =
      // $FlowFixMe: By narrowing `owner` to `null`, we narrowed `debugInfo` to `DevjsComponentInfo`
      debugInfo;
    // $FlowFixMe[cannot-write]
...
function resolveDebugModel(
  ...
  const debugChunk: ResolvedModelChunk<DevjsDebugInfoEntry> =
...
let currentOwnerInDEV: null | DevjsComponentInfo = null;
function getCurrentStackInDEV(): string {
  if (__DEV__) {
    const owner: null | DevjsComponentInfo = currentOwnerInDEV;
    if (owner === null) {
      return '';
    }
...
const replayConsoleWithCallStack = {
  devjs_stack_bottom_frame: function (
    ...
    const prevStack = DevjsSharedInternals.getCurrentStack;
    ...
    try {
      ...
      DevjsSharedInternals.getCurrentStack = prevStack;
    }
  },
};
...
const replayConsoleWithCallStackInDEV: (
  ...
  ? // We use this technique to trick minifiers to preserve the function name.
    (replayConsoleWithCallStack.devjs_stack_bottom_frame.bind(
...
function resolveConsoleEntry(
  ...
  if (!__DEV__) {
    ...
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'resolveConsoleEntry should never be called in production mode. This is a bug in Devjs.',
    );
  }
...
function initializeIOInfo(response: Response, ioInfo: DevjsIOInfo): void {
...
function flushComponentPerformance(
  ...
  if (!enableProfilerTimer || !enableComponentPerformanceTrack) {
    // eslint-disable-next-line devjs-internal/prod-error-codes
    throw new Error(
      'flushComponentPerformance should never be called in production mode. This is a bug in Devjs.',
    );
  }
      ...
      if (endTimeIdx > -1) {
        ...
        for (let j = endTimeIdx - 1; j > i; j--) {
          ...
          if (typeof candidateInfo.name === 'string') {
            ...
            // $FlowFixMe: Refined.
            const componentInfo: DevjsComponentInfo = candidateInfo;
            logComponentInfo(
              response,
              root,
              componentInfo,
            ...
            // $FlowFixMe: Refined.
            const asyncInfo: DevjsAsyncInfo = candidateInfo;
            const env = response._rootEnvironmentName;
...
function processFullStringRow(
  ...
  switch (tag) {
    ...
    case 87 /* "W" */: {
      ...
      throw new Error(
        'Failed to read a RSC payload created by a development version of Devjs ' +
        ...
        if (rowTag !== 84) {
          throw new Error(
            'Binary RSC chunks cannot be encoded as strings. ' +
              'This is a bug in the wiring of the Devjs streams.',
          );
        }
...
function getCurrentOwnerInDEV(): null | DevjsComponentInfo {
...
export function injectIntoDevTools(): boolean {
  const internals: Object = {
    ...
    currentDispatcherRef: DevjsSharedInternals,
    // Enables DevTools to detect reconciler version rather than renderer version
    // which may not match for third party renderers.
    reconcilerVersion: DevjsVersion,
