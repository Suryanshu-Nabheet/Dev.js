...
import {writeTemporaryReference} from './DevjsFlightTemporaryReferences';
...
export type DevjsServerValue =
  ...
  | void
  | bigint
  | $AsyncIterable<DevjsServerValue, DevjsServerValue, void>
  | $AsyncIterator<DevjsServerValue, DevjsServerValue, void>
  | Iterable<DevjsServerValue>
  | Iterator<DevjsServerValue>
  | Array<DevjsServerValue>
  | Map<DevjsServerValue, DevjsServerValue>
  | Set<DevjsServerValue>
  | FormData
  | Date
  | DevjsServerObject
  | Promise<DevjsServerValue>; // Thenable<DevjsServerValue>

type DevjsServerObject = {+[key: string]: DevjsServerValue};
...
export function processReply(
  ...
  let modelRoot: null | DevjsServerValue = root;
  ...
  function serializeBinaryReader(reader: any): string {
    ...
    function progress(entry: {done: boolean, value: DevjsServerValue, ...}) {
      if (entry.done) {
        const blobId = nextPartId++;
  ...
  function serializeAsyncIterable(
    iterable: $AsyncIterable<DevjsServerValue, DevjsServerValue, void>,
    iterator: $AsyncIterator<DevjsServerValue, DevjsServerValue, void>,
  ): string {
    if (formData === null) {
      // Upgrade to use FormData to allow us to stream this value.
      formData = new FormData();
    ...
    function progress(
      entry:
        | {done: false, +value: DevjsServerValue, ...}
        | {done: true, +value: DevjsServerValue, ...},
    ) {
      if (entry.done) {
  ...
  function resolveToJSON(
    this:
      | {+[key: string | number]: DevjsServerValue}
      | $ReadOnlyArray<DevjsServerValue>,
    key: string,
    value: DevjsServerValue,
  ): DevjsJSONValue {
    const parent = this;
    ...
    if (typeof value === 'object') {
      switch ((value: any).$$typeof) {
        case devjs_ELEMENT_TYPE: {
          ...
          throw new Error(
            'Devjs Element cannot be passed to Server Functions from the Client without a ' +
      ...
      if (__DEV__) {
        if ((value: any).$$typeof === devjs_CONTEXT_TYPE) {
          console.error(
            'Devjs Context Providers cannot be passed to Server Functions from the Client.%s',
            describeObjectForErrorMessage(parent, key),
          );
  ...
  function serializeModel(model: DevjsServerValue, id: number): string {
...
function encodeFormData(reference: any): Thenable<FormData> {
  ...
  processReply(
    reference,
    '',
    undefined, // TODO: This means Devjs Elements can't be used as state in progressive enhancement.
...
function isSignatureEqual(
  ...
  // encodeFormData, this means isSignatureEqual can only be called while Devjs
  // is rendering.
...
function parseStackLocation(error: Error): null | DevjsFunctionLocation {
  ...
  if (stack.startsWith('Error: devjs-stack-top-frame\n')) {
...
export function createServerReference<A: Iterable<any>, T>(
  ...
  if (__DEV__) {
    ...
    const location = parseStackLocation(new Error('devjs-stack-top-frame'));
    if (location !== null) {
      const [, filename, line, col] = location;
