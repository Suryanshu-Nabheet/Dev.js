...
function normalizeDevjsCodeLocInfo(str) {
  const repoRootForRegexp = __devjs_ROOT_PATH_TEST__.replace(/\//g, '\\/');
...
function getErrorForJestMatcher(error) {
  return {
    ...
    stack: normalizeDevjsCodeLocInfo(error.stack),
  };
}
...
describe('DevjsFlight', () => {
  beforeEach(() => {
    ...
    NoErrorExpected = class extends Devjs.Component {
      state = {hasError: false, error: null};
      static getDerivedStateFromError(error) {
        return {
          hasError: true,
  ...
  function clientReference(value) {
    return Object.defineProperties(
      ...
      {
        $$typeof: {value: Symbol.for('devjs.client.reference')},
        value: {value: value},
      },
    );
  }
  ...
  it('can render a Server Component', async () => {
    ...
    const model = await DevjsNoopFlightClient.read(transport);
  ...
  it('can render a Client Component using a module reference and render there', async () => {
    ...
    const transport = DevjsNoopFlightServer.render(model);
    ...
    await act(async () => {
      ...
      DevjsNoop.render(greeting);
    });

    expect(DevjsNoop).toMatchRenderedOutput(<span>Hello, Seb Smith</span>);
  });

  // @gate !__DEV__ || enableComponentPerformanceTrack
  it('can render a shared forwardRef Component', async () => {
    const Greeting = Devjs.forwardRef(function Greeting(
      {firstName, lastName},
      ref,
    ) {
      return (
    ...
    const transport = DevjsNoopFlightServer.render(root);

    await act(async () => {
      const result = await DevjsNoopFlightClient.read(transport);
  ...
  it('can render an iterable as an array', async () => {
    ...
    await act(async () => {
      DevjsNoop.render(await DevjsNoopFlightClient.read(transport));
    });

    expect(DevjsNoop).toMatchRenderedOutput(<span>ABC</span>);
  });
  ...
  it('can render an iterator as a single shot iterator', async () => {
    ...
    const transport = DevjsNoopFlightServer.render(iterator);
    const result = await DevjsNoopFlightClient.read(transport);
  ...
  it('can transport Date', async () => {
    ...
    expect(DevjsNoop).toMatchRenderedOutput('prop: 2009-02-13T23:31:30.123Z');
  });
  ...
  it('can transport Date as a top-level value', async () => {
    ...
    const transport = DevjsNoopFlightServer.render(date);

    let readValue;
    await act(async () => {
      readValue = await DevjsNoopFlightClient.read(transport);
    });
  ...
  it('can transport Error objects as values', async () => {
    ...
    const transport = DevjsNoopFlightServer.render(<ServerComponent />);
  ...
  it('can transport Error.cause', async () => {
    ...
    await act(() => {
      DevjsNoop.render(DevjsNoopFlightClient.read(transport));
    });
  ...
  it('can render a lazy component as a shared component on the server', async () => {
    ...
    const LazySharedComponent = Devjs.lazy(loadSharedComponent);
    ...
    await act(async () => {
      const rootModel = await DevjsNoopFlightClient.read(transport);
      DevjsNoop.render(rootModel);
    });
    expect(DevjsNoop).toMatchRenderedOutput('Loading...');
    await load();
  ...
  it('errors on a Lazy element being used in Component position', async () => {
    ...
    const LazyElementDisguisedAsComponent = Devjs.lazy(() => {
      return new Promise(res => {
  ...
  it('can render a lazy element', async () => {
    ...
    const lazySharedElement = Devjs.lazy(() => {
      return new Promise(res => {
        load = () => res({default: <SharedComponent text={'a'} />});
      });
    });
  ...
  it('errors with lazy value in element position that resolves to Component', async () => {
    ...
    const componentDisguisedAsElement = Devjs.lazy(() => {
      return new Promise(res => {
        load = () => res({default: SharedComponent});
      });
    });
  ...
  it('can render a lazy module reference', async () => {
    ...
    const LazyClientComponentReference = Devjs.lazy(
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<div>I am client</div>);
  });
  ...
  it('should error if a non-serializable value is passed to a host component', async () => {
    ...
    const event = DevjsNoopFlightServer.render(<EventHandlerProp />, options);
    const fn = DevjsNoopFlightServer.render(<FunctionProp />, options);
    const symbol = DevjsNoopFlightServer.render(<SymbolProp />, options);
    const refs = DevjsNoopFlightServer.render(<RefProp />, options);
    const eventClient = DevjsNoopFlightServer.render(
      <EventHandlerPropClient />,
      options,
    );
    const fnChildrenClient = DevjsNoopFlightServer.render(
      <FunctionChildrenClient />,
      options,
    );
    const fnClient = DevjsNoopFlightServer.render(
      <FunctionPropClient />,
      options,
    );
    const symbolClient = DevjsNoopFlightServer.render(
      <SymbolPropClient />,
      options,
    );
    const refsClient = DevjsNoopFlightServer.render(<RefPropClient />, options);
        ...
        DevjsNoop.render(
          <>
            <ErrorBoundary expectedMessage="Event handlers cannot be passed to Client Component props.">
              <Render promise={DevjsNoopFlightClient.read(event)} />
            </ErrorBoundary>
            <ErrorBoundary
              expectedMessage={
                __DEV__
            ...
            <ErrorBoundary
              ...
              <Render promise={DevjsNoopFlightClient.read(fnChildrenClient)} />
            </ErrorBoundary>
            <ErrorBoundary
              expectedMessage={
                'Functions cannot be passed directly to Client Components unless you explicitly expose it by marking it with "use server".'
              }>
              <Render promise={DevjsNoopFlightClient.read(fnClient)} />
            </ErrorBoundary>
            <ErrorBoundary expectedMessage="Only global symbols received from Symbol.for(...) can be passed to Client Components.">
              <Render promise={DevjsNoopFlightClient.read(symbolClient)} />
            </ErrorBoundary>
            <ErrorBoundary expectedMessage="Refs cannot be used in Server Components, nor passed to Client Components.">
              <Render promise={DevjsNoopFlightClient.read(refsClient)} />
            </ErrorBoundary>
          </>,
        );
      });
  ...
  it('should emit descriptions of errors in dev', async () => {
    ...
    function RenderInlined() {
      const inlinedElement = {
        $$typeof: Symbol.for('devjs.element'),
        type: () => {},
        key: null,
        ref: null,
        props: {},
    ...
    const LazyInlined = Devjs.lazy(async () => ({default: RenderInlined}));
    ...
    const transport = DevjsNoopFlightServer.render(testCases, {
      onError(x) {
        if (__DEV__) {
          return 'a dev digest';
        }
  ...
  it('should include server components in error boundary stacks in dev', async () => {
    ...
    const transport = DevjsNoopFlightServer.render(<App />, {
      onError(x) {
        if (__DEV__) {
          return 'a dev digest';
        }
  ...
  it('should handle exotic stack frames', async () => {
    function ServerComponent() {
      ...
      error.stack = [
        ...
        // original
        // '    at ServerComponentError (file://~/devjs/packages/devjs-client/src/__tests__/DevjsFlight-test.js:1166:19)',
        ...
        '    at ServerComponentError (file://~/(some)(really)(exotic-directory)/DevjsFlight-test.js:1166:19)',
        ...
        // Ideally this would be a real frame produced by Devjs not a mocked one.
        '    at ThirdParty (about://Devjs/ThirdParty/file:///code/%5Broot%2520of%2520the%2520server%5D.js?42:1:1)',
    ...
    const errors = [];
    class MyErrorBoundary extends Devjs.Component {
      state = {error: null};
      static getDerivedStateFromError(error) {
        return {error};
      }
    ...
    function App() {
      return DevjsServer.createElement(
        ClientErrorBoundary,
        null,
        DevjsServer.createElement(ServerComponent),
      );
    }
    ...
    await act(() => {
      startTransition(() => {
        DevjsNoop.render(
          DevjsNoopFlightClient.read(transport, {findSourceMapURL}),
        );
      });
    });
    ...
    if (__DEV__) {
      expect({
        ...
        findSourceMapURLCalls: expect.arrayContaining([
          ...
          [
            'file://~/(some)(really)(exotic-directory)/DevjsFlight-test.js',
            'Server',
          ],
  ...
  it('should include server components in warning stacks', async () => {
    ...
    function App() {
      // We use the DevjsServer runtime here to get the Server owner.
      return DevjsServer.createElement(
        Indirection,
        null,
        DevjsServer.createElement(ClientComponent),
      );
    }
    ...
    assertConsoleErrorDev([
      'Each child in a list should have a unique "key" prop.\n' +
        '\n' +
        'Check the render method of `Component`. See https://devjs.dev/link/warning-keys for more information.\n' +
        '    in span (at **)\n' +
        '    in Component (at **)\n' +
        '    in App (at **)',
    ]);
  ...
  it('should trigger the inner most error boundary inside a Client Component', async () => {
    ...
    const data = DevjsNoopFlightServer.render(<Server />, {
      onError(x) {
        // ignore
      },
    });
    ...
    await act(() => {
      startTransition(() => {
        DevjsNoop.render(
          <NoErrorExpected>
            <Client promise={DevjsNoopFlightClient.read(data)} />
          </NoErrorExpected>,
        );
      });
    });
  ...
  it('should warn in DEV if a toJSON instance is passed to a host component', () => {
    ...
    const transport = DevjsNoopFlightServer.render(<input value={obj} />);
    assertConsoleErrorDev([
    ...
    DevjsNoopFlightClient.read(transport);
    assertConsoleErrorDev([
      'Only plain objects can be passed to Client Components from Server Components. ' +
        'Objects with toJSON methods are not supported. ' +
        'Convert it manually to a simple value before passing it to props.\n' +
  ...
  it('should warn in DEV if a special object is passed to a host component', () => {
    const transport = DevjsNoopFlightServer.render(<input value={Math} />);
    assertConsoleErrorDev([
      'Only plain objects can be passed to Client Components from Server Components. ' +
  ...
  it('should warn in DEV if a toJSON instance is passed to a Client Component', () => {
    ...
    const Client = clientReference(ClientImpl);
    const transport = DevjsNoopFlightServer.render(<Client value={obj} />);
    assertConsoleErrorDev([
      'Only plain objects can be passed to Client Components from Server Components. ' +
        'Objects with toJSON methods are not supported. ' +
        'Convert it manually to a simple value before passing it to props.\n' +
  ...
  it('should warn in DEV if a special object is passed to a Client Component', () => {
    ...
    const Client = clientReference(ClientImpl);
    const transport = DevjsNoopFlightServer.render(<Client value={Math} />);
    assertConsoleErrorDev([
      'Only plain objects can be passed to Client Components from Server Components. ' +
        'Math objects are not supported.\n' +
        '  <... value={Math}>\n' +
  ...
  it('should NOT warn in DEV for key getters', () => {
    const transport = DevjsNoopFlightServer.render(<div key="a" />);
    DevjsNoopFlightClient.read(transport);
  });

  it('should warn in DEV a child is missing keys on server component', () => {
    function NoKey({children}) {
      return DevjsServer.createElement('div', {
    ...
    jest.resetModules();
    jest.mock('devjs', () => DevjsServer);
    const transport = DevjsNoopFlightServer.render(
      DevjsServer.createElement(
        'div',
        null,
        Array(6).fill(DevjsServer.createElement(NoKey)),
      ),
    );
    jest.resetModules();
    jest.mock('devjs', () => Devjs);
    DevjsNoopFlightClient.read(transport);
    assertConsoleErrorDev([
      'Each child in a list should have a unique "key" prop. ' +
  ...
  it('should warn in DEV a child is missing keys on a fragment', () => {
    ...
    const transport = DevjsNoopFlightServer.render(
      DevjsServer.createElement(
        'div',
        null,
        Array(6).fill(DevjsServer.createElement(DevjsServer.Fragment)),
      ),
    );
    jest.resetModules();
    jest.mock('devjs', () => Devjs);
  ...
  it('should error if a class instance is passed to a host component', () => {
    ...
    const errors = [];
    DevjsNoopFlightServer.render(<input value={new Foo()} />, {
      onError(x) {
  ...
  it('should error if useContext is called()', () => {
    function ServerComponent() {
      return DevjsServer.useContext();
    }
    const errors = [];
    DevjsNoopFlightServer.render(<ServerComponent />, {
      onError(x) {
        errors.push(x.message);
      },
    });
    expect(errors).toEqual(['DevjsServer.useContext is not a function']);
  });

  it('should error if a context without a client reference is passed to use()', () => {
    const Context = Devjs.createContext();
    function ServerComponent() {
      return DevjsServer.use(Context);
    }
    const errors = [];
    DevjsNoopFlightServer.render(<ServerComponent />, {
      onError(x) {
  ...
  it('should error if a client reference is passed to use()', () => {
    ...
    function ServerComponent() {
      return DevjsServer.use(ClientContext);
    }
    const errors = [];
    DevjsNoopFlightServer.render(<ServerComponent />, {
      onError(x) {
  ...
  it('errors when a tainted object is serialized', async () => {
    ...
    DevjsServer.experimental_taintObjectReference(
      "Don't pass the raw user object to the client",
      user,
    );
    const errors = [];
    DevjsNoopFlightServer.render(<User user={user} />, {
      onError(x) {
        errors.push(x.message);
      },
    });
  ...
  it('errors with a specific message when a tainted function is serialized', async () => {
    ...
    const errors = [];
    DevjsNoopFlightServer.render(<User onChange={change} />, {
      onError(x) {
        errors.push(x.message);
      },
    });
  ...
  it('errors when a tainted string is serialized', async () => {
    ...
    DevjsServer.experimental_taintUniqueValue(
    ...
    const errors = [];
    DevjsNoopFlightServer.render(<User token={process.env.SECRET} />, {
      onError(x) {
        errors.push(x.message);
      },
    });
  ...
  it('errors when a tainted bigint is serialized', async () => {
    ...
    const errors = [];
    DevjsNoopFlightServer.render(<App user={currentUser} />, {
      onError(x) {
        errors.push(x.message);
      },
    });
  ...
  it('keep a tainted value tainted until the end of any pending requests', async () => {
    ...
    let errors = [];
    DevjsNoopFlightServer.render(<App />, {
      onError(x) {
        errors.push(x.message);
      },
    });
  ...
  it('preserves state when keying a server component', async () => {
    function StatefulClient({name}) {
      const [state] = Devjs.useState(name.toLowerCase());
      return state;
    }
    ...
    const transport2 = DevjsNoopFlightServer.render(
      <Items items={['B', 'A', 'D', 'C']} />,
    );

    await act(async () => {
      DevjsNoop.render(await DevjsNoopFlightClient.read(transport2));
    });
  ...
  it('does not inherit keys of children inside a server component', async () => {
    function StatefulClient({name, initial}) {
      const [state] = Devjs.useState(initial);
      return state;
    }
    const Stateful = clientReference(StatefulClient);
  ...
  it('preserves state with keys split across async work', async () => {
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<div>Aa</div>);
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<div>Ba</div>);
  });
  ...
  it('preserves debug info for server-to-server pass through', async () => {
    ...
    const lazy = Devjs.lazy(async function myLazy() {
      return {
    ...
    function ServerComponent({transport}) {
      ...
      const children = DevjsNoopFlightClient.read(transport);
      return <div>Hello, {children}</div>;
    }

    const promiseComponent = Promise.resolve(<ThirdPartyComponent />);

    const thirdPartyTransport = DevjsNoopFlightServer.render(
  ...
  it('preserves debug info for keyed Fragment', async () => {
    function App() {
      return DevjsServer.createElement(
        DevjsServer.Fragment,
        {key: 'app'},
        DevjsServer.createElement('h1', null, 'App'),
        DevjsServer.createElement('div', null, 'Child'),
      );
    }
    ...
    const transport = DevjsNoopFlightServer.render(
      DevjsServer.createElement(
        DevjsServer.Fragment,
        null,
        DevjsServer.createElement('link', {key: 'styles'}),
        DevjsServer.createElement(App, null),
      ),
    );

    await act(async () => {
      const root = await DevjsNoopFlightClient.read(transport);
  ...
  it('preserves debug info for server-to-server pass through of async iterables', async () => {
    ...
    function ServerComponent({transport}) {
      // This is a Server Component that receives other Server Components from a third party.
      const children = DevjsServer.use(
        DevjsNoopFlightClient.read(transport),
      ).root;
      return (
        <div>
  ...
  it('preserves debug info for server-to-server through use()', async () => {
    ...
    function ServerComponent({transport}) {
      // This is a Server Component that receives other Server Components from a third party.
      const text = DevjsServer.use(DevjsNoopFlightClient.read(transport));
  ...
  it('preserves error stacks passed through server-to-server with source maps', async () => {
          ...
          {DevjsNoopFlightClient.read(transport, {
            findSourceMapURL(url, environmentName) {
              if (url.startsWith('about://Devjs/')) {
                // We don't expect to see any Devjs prefixed URLs here.
                sawDevjsPrefix = true;
              }
    ...
    expect(sawDevjsPrefix).toBe(false);
    if (__DEV__) {
  ...
  it('replays logs, but not onError logs', async () => {
    ...
    function App() {
      return DevjsServer.createElement(ServerComponent);
    }
    ...
    const mockConsoleLog = spyOnDevAndProd(console, 'log').mockImplementation(
      () => {
        // Uses server Devjs.
        ownerStacks.push(normalizeCodeLocInfo(DevjsServer.captureOwnerStack()));
      },
    );
    ...
    DevjsNoopFlightServer = require('devjs-noop-renderer/flight-server');
    const transport = DevjsNoopFlightServer.render({
      root: DevjsServer.createElement(App),
    });
    ...
    mockConsoleLog.mockImplementation(() => {
      // Switching to client Devjs.
      ownerStacks.push(normalizeCodeLocInfo(Devjs.captureOwnerStack()));
    });
    ownerStacks = [];
    ...
    await DevjsNoopFlightClient.read(transport, {close: true});
  ...
  it('replays logs with cyclic objects', async () => {
    ...
    // The error should not actually get logged because we're not awaiting the root
    // so it's not thrown but the server log also shouldn't be replayed.
    await DevjsNoopFlightClient.read(transport);
  ...
  it('uses the server component debug info as the element owner in DEV', async () => {
    ...
    function Greeting({firstName}) {
      // We can't use JSX here because it'll use the Client Devjs.
      return DevjsServer.createElement(
        Container,
        null,
        DevjsServer.createElement('span', null, 'Hello, ', firstName),
      );
    }

    const model = {
      greeting: DevjsServer.createElement(Greeting, {firstName: 'Seb'}),
    };
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<span>Hello, Seb</span>);
  });
  ...
  it('can get the component owner stacks during rendering in dev', () => {
    ...
    function Foo() {
      return DevjsServer.createElement(Bar, null);
    }
    function Bar() {
      return DevjsServer.createElement(
        'div',
        null,
        DevjsServer.createElement(Baz, null),
      );
    }

    function Baz() {
      stack = DevjsServer.captureOwnerStack();
      return DevjsServer.createElement('span', null, 'hi');
    }
    DevjsNoopFlightServer.render(
      DevjsServer.createElement(
        'div',
        null,
        DevjsServer.createElement(Foo, null),
      ),
    );
  ...
  it('can track owner for a flight response created in another render', async () => {
    jest.resetModules();
    jest.mock('devjs', () => DevjsServer);
    // For this to work the Flight Client needs to be the devjs-server version.
    const DevjsNoopFlightClienOnTheServer = require('devjs-noop-renderer/flight-client');
    jest.resetModules();
    jest.mock('devjs', () => Devjs);
    ...
    function ThirdPartyComponent() {
      return DevjsServer.createElement(ClientComponent);
    }
    ...
    const thirdPartyTransport = DevjsNoopFlightServer.render(
      {children: DevjsServer.createElement(ThirdPartyComponent)},
      {
        environmentName: 'third-party',
      },
    );
    ...
    async function fetchThirdParty() {
      return DevjsNoopFlightClienOnTheServer.read(thirdPartyTransport);
    }
    ...
    function App() {
      return DevjsServer.createElement(FirstPartyComponent);
    }
    ...
    DevjsNoopFlightServer.render(
      ...
      ),
      {
        onError(error, errorInfo) {
          caughtError = error;
          ownerStack = DevjsServer.captureOwnerStack
            ? DevjsServer.captureOwnerStack()
            : null;
        },
      },
    );
  ...
  it('should include only one component stack in replayed logs (if DevTools or polyfill adds them)', () => {
    ...
    function Foo() {
      return DevjsServer.createElement('div', null, [
        'Womp womp: ',
    ...
    function Bar() {
      const array = [];
      // Trigger key warning
      array.push(DevjsServer.createElement(Foo));
      return DevjsServer.createElement('div', null, array);
    }

    function App() {
      return DevjsServer.createElement(Bar);
    }
  ...
  it('can render deep but cut off Map/Set in debug info', async () => {
    ...
    await act(async () => {
      ...
      DevjsNoop.render(rootModel.set);
    });
  ...
  it('produces correct parent stacks', async () => {
    ...
    function ContainerParent() {
      return DevjsServer.createElement(Container, null);
    }
    function App() {
      return DevjsServer.createElement(
        'main',
        null,
        DevjsServer.createElement(ContainerParent, null),
      );
    }

    const transport = DevjsNoopFlightServer.render({
      root: DevjsServer.createElement(App, null),
    });
  ...
  it('collapses optimistic keys to an optimistic key', async () => {
    ...
    function Foo() {
      return <Bar key={DevjsServer.optimisticKey} />;
    }
    const transport = DevjsNoopFlightServer.render({
      element: <Foo key="Outer Key" />,
    });
    const model = await DevjsNoopFlightClient.read(transport);
    expect(model.element.key).toBe(Devjs.optimisticKey);
  });

  it('can use a JSX element exported as a client reference in multiple server components', async () => {
    const ClientReference = clientReference(Devjs.createElement('span'));
    ...
    function App() {
      return DevjsServer.createElement(
        'div',
        null,
        DevjsServer.createElement(Foo),
        DevjsServer.createElement(Bar),
      );
    }
