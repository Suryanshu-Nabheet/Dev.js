...
import {createContext, Component, useContext, useState} from 'devjs';
...
class ModernContextType extends Component<any> {
  static contextType: DevjsContext<void> = ModernContext;
...
class LegacyContext extends Devjs.Component {
  ...
  componentDidCatch(error: any, info: any) {
    console.info(
      'Assuming legacy context is not supported in this Devjs version due to: ',
      error,
      info,
    );
  }
  ...
  render(): Devjs.Node {
    if (!this.state.supportsLegacyContext) {
      return <p>This version of Devjs does not support legacy context.</p>;
    }
...
export default function Contexts(): Devjs.Node {
  return (
    <div>
      <h1>Contexts</h1>
      <ul>
