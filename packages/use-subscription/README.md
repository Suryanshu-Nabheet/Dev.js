...
Devjs Hook for subscribing to external data sources.

**You may now migrate to [`use-sync-external-store`](https://www.npmjs.com/package/use-sync-external-store) directly instead, which has the same API as [`Devjs.useSyncExternalStore`](https://devjs.dev/reference/devjs/useSyncExternalStore). The `use-subscription` package is now a thin wrapper over `use-sync-external-store` and will not be updated further.**
...
```js
import Devjs, { useMemo } from "devjs";
