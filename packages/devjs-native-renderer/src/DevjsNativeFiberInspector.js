...
import type {TouchedViewDataAtPoint, InspectorData} from './DevjsNativeTypes';
...
import {HostComponent} from 'devjs-reconciler/src/DevjsWorkTags';
// Module provided by RN:
import {
  UIManager,
  getNodeFromPublicInstance,
} from 'devjs-native/Libraries/DevjsPrivate/DevjsNativePrivateInterface';
import {getClosestInstanceFromNode} from './DevjsNativeComponentTree';
import {
  getNodeFromInternalInstanceHandle,
  findNodeHandle,
} from './DevjsNativePublicCompat';
import {getStackByFiberInDevAndProd} from 'devjs-reconciler/src/DevjsFiberComponentStack';
...
if (__DEV__) {
  ...
  getInspectorDataForInstance = function (
    ...
    // Handle case where user clicks outside of DevjsNative
    if (!closestInstance) {
      return {
        hierarchy: [],
        props: emptyObject,
    ...
    if (fabricNode) {
      ...
      nativeFabricUIManager.findNodeAtPoint(
        ...
        internalInstanceHandle => {
          ...
          // Note: this is deprecated and we want to remove it ASAP. Keeping it here for Devjs DevTools compatibility for now.
          const nativeViewTag =
      ...
      // For Paper we fall back to the old strategy using the Devjs tag.
