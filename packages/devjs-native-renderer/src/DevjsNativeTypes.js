...
type InspectorDataGetter = (
  <TElementType: Devjs.ElementType>(
    componentOrHandle: Devjs.ElementRef<TElementType> | number,
  ) => ?number,
) => $ReadOnly<{
...
export type RenderRootOptions = {
  ...
  onCaughtError?: (
    ...
    errorInfo: {
      ...
      // $FlowFixMe[value-as-type] Component in devjs repo is any-typed, but it will be well typed externally.
      +errorBoundary?: ?Devjs.Component<any, any>,
    },
  ) => void,
  onRecoverableError?: (
    error: mixed,
...
/**
 * Flat DevjsNative renderer bundles are too big for Flow to parse efficiently.
 * Provide minimal Flow typing for the high-level RN API and call it a day.
 */
export type DevjsNativeType = {
  findHostInstance_DEPRECATED<TElementType: Devjs.ElementType>(
    componentOrHandle: ?(Devjs.ElementRef<TElementType> | number),
  ): ?PublicInstance,
  findNodeHandle<TElementType: Devjs.ElementType>(
    componentOrHandle: ?(Devjs.ElementRef<TElementType> | number),
  ): ?number,
  ...
  render(
    element: Devjs.MixedElement,
    containerTag: number,
    callback: ?() => void,
    options: ?RenderRootOptions,
  ): ?Devjs.ElementRef<Devjs.ElementType>,
...
export type DevjsFabricEventTouch = {
  identifier: number,
  locationX: number,
  locationY: number,
  pageX: number,
...
export type DevjsFabricEvent = {
  touches: Array<DevjsFabricEventTouch>,
  changedTouches: Array<DevjsFabricEventTouch>,
  targetTouches: Array<DevjsFabricEventTouch>,
  target: number,
  ...
};
