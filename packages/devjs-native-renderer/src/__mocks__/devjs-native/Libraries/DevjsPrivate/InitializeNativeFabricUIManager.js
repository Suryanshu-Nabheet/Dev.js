...
const RCTFabricUIManager = {
  ...
  createNode: jest.fn(
    function createNode(devjsTag, viewName, rootTag, props, eventTarget) {
      if (allocatedTags.has(devjsTag)) {
        throw new Error(`Created two native views with tag ${devjsTag}`);
      }

      allocatedTags.add(devjsTag);
      return {
        devjsTag: devjsTag,
        viewName: viewName,
        props: props,
...
// DOMRect isn't provided by jsdom, but it's used by `DevjsFabricHostComponent`.
