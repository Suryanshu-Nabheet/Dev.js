...
const RCTUIManager = {
  ...
  createView: jest.fn(function createView(devjsTag, viewName, rootTag, props) {
    if (views.has(devjsTag)) {
      throw new Error(`Created two native views with tag ${devjsTag}`);
    }
  ...
  setChildren: jest.fn(function setChildren(parentTag, devjsTags) {
  ...
  manageChildren: jest.fn(function manageChildren(
    ...
    if (addChildDevjsTags.length !== addAtIndices.length) {
      throw new Error(
        `Mismatched add indices ${addChildDevjsTags} and ${addAtIndices}`,
      );
    }
    ...
    addChildDevjsTags.forEach((tag, i) => {
      indicesToInsert.push([addAtIndices[i], addChildDevjsTags[i]]);
    });
