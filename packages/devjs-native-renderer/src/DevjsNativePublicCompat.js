...
import type {Node} from './DevjsNativeTypes';
import type {ElementRef, ElementType} from 'devjs';
import type {PublicInstance} from 'devjs-native/Libraries/DevjsPrivate/DevjsNativePrivateInterface';
...
import {doesFiberContain} from 'devjs-reconciler/src/DevjsFiberTreeReflection';
...
export function isChildPublicInstance(
  ...
  if (__DEV__) {
    ...
    if (
      ...
      // $FlowExpectedError[prop-missing] Don't check via `instanceof DevjsNativeFiberHostComponent`, so it won't be leaked to Fabric.
    ...
    const parentInternalInstanceHandle =
      // $FlowExpectedError[incompatible-call] Type for parentInstance should have been PublicInstance from DevjsFiberConfigFabric.
      getInternalInstanceHandleFromPublicInstance(parentInstance);
    const childInternalInstanceHandle =
      // $FlowExpectedError[incompatible-call] Type for childInstance should have been PublicInstance from DevjsFiberConfigFabric.
