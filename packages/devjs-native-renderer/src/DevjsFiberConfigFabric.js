...
import type {
  ...
  ViewConfig,
} from './DevjsNativeTypes';
import {dispatchEvent} from './DevjsFabricEventEmitter';
import {
  NoEventPriority,
...
import {HostText} from 'devjs-reconciler/src/DevjsWorkTags';
import {
...
import {
  DevjsNativeViewConfigRegistry,
  deepFreezeAndThrowOnMutationInDev,
  createPublicInstance,
  createPublicTextInstance,
  createAttributePayload,
  diffAttributePayloads,
  type PublicInstance as DevjsNativePublicInstance,
...
import {getClosestInstanceFromNode} from './DevjsFabricComponentTree';
...
} from './DevjsNativeFiberInspector';

import {passChildrenWhenCloningPersistedNodes} from 'shared/DevjsFeatureFlags';
import {devjs_CONTEXT_TYPE} from 'shared/DevjsSymbols';
import type {DevjsContext} from 'shared/DevjsTypes';

export {default as rendererVersion} from 'shared/DevjsVersion'; // TODO: Consider exporting the devjs-native version.
export const rendererPackageName = 'devjs-native-renderer';
...
const {get: getViewConfigForType} = DevjsNativeViewConfigRegistry;
...
export type Instance = {
  ...
  canonical: {
    ...
    currentProps: Props,
    // Reference to the Devjs handle (the fiber)
...
export type PublicInstance = DevjsNativePublicInstance;
...
export * from 'devjs-reconciler/src/DevjsFiberConfigWithNoMutation';
export * from 'devjs-reconciler/src/DevjsFiberConfigWithNoHydration';
export * from 'devjs-reconciler/src/DevjsFiberConfigWithNoScopes';
export * from 'devjs-reconciler/src/DevjsFiberConfigWithNoTestSelectors';
export * from 'devjs-reconciler/src/DevjsFiberConfigWithNoResources';
export * from 'devjs-reconciler/src/DevjsFiberConfigWithNoSingletons';
...
export function getPublicInstanceFromInternalInstanceHandle(
  ...
  // Devjs resets all the fields in the fiber when the component is unmounted
...
function getPublicInstanceFromHostFiber(fiber: Fiber): PublicInstance {
  ...
  if (publicInstance === null) {
    throw new Error('Expected to find a host node. This is a bug in Devjs.');
  }
...
export function shouldSetTextContent(type: string, props: Props): boolean {
  ...
  // More context @ github.com/Suryanshu-Nabheet/dev.js/pull/8560#discussion_r92111303
  return false;
}
...
// The Fabric renderer is secondary to the existing Devjs Native renderer.
...
export function befodevjsiveInstanceBlur(
  internalInstanceHandle: InternalInstanceHandle,
) {
  // noop
}
...
export function deleteChildFromFragmentInstance(
  ...
  if (enableFragmentRefsInstanceHandles) {
    if (publicInstance.unstable_devjsFragments != null) {
      publicInstance.unstable_devjsFragments.delete(fragmentInstance);
    }
  }
}
...
export const supportsMicrotasks: boolean =
  typeof RN$enableMicrotasksInDevjs !== 'undefined' &&
  !!RN$enableMicrotasksInDevjs;
