...
// And registers view manager event types with DevjsNativeViewConfigRegistry.
const fakeRequireNativeComponent = (uiViewClassName, validAttributes) => {
  ...
  return createDevjsNativeComponentClass(uiViewClassName, getViewConfig);
};
...
beforeEach(() => {
  ...
  RCTEventEmitter =
    require('devjs-native/Libraries/DevjsPrivate/DevjsNativePrivateInterface').RCTEventEmitter;
  Devjs = require('devjs');
...
test('fails to register the same event name with different types', async () => {
  const InvalidEvents = createDevjsNativeComponentClass('InvalidEvents', () => {
    if (!__DEV__) {
  ...
  await expect(
    async () => await act(() => DevjsNative.render(<InvalidEvents />, 1)),
...
test('fails if unknown/unsupported event types are dispatched', () => {
  ...
  DevjsNative.render(<View onUnspecifiedEvent={() => {}} />, 1);
...
test('handles events on text nodes', () => {
  ...
  class ContextHack extends Devjs.Component {
  ...
  assertConsoleErrorDev([
    'ContextHack uses the legacy childContextTypes API which will soon be removed. ' +
      'Use Devjs.createContext() instead. ' +
