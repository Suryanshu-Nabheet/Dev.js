...
describe('DevjsFabric', () => {
  ...
  it('should be able to create and render a native component', async () => {
    ...
    await act(() => {
      DevjsFabric.render(<View foo="test" />, 1, null, true);
    });
  ...
  it('should be able to create and update a native component', async () => {
    ...
    await act(() => {
      DevjsFabric.render(<View foo="foo" />, 11, null, true);
    });

    expect(nativeFabricUIManager.createNode).toHaveBeenCalledTimes(1);

    await act(() => {
      DevjsFabric.render(<View foo="bar" />, 11, null, true);
    });
  ...
  it('should not call FabricUIManager.cloneNode after render for properties that have not changed', async () => {
    ...
    await act(() => {
      DevjsFabric.render(<Text foo="a">1</Text>, 11, null, true);
    });
    ...
    await act(() => {
      DevjsFabric.render(<Text foo="b">1</Text>, 11, null, true);
    });
    expect(nativeFabricUIManager.cloneNode).not.toBeCalled();
    expect(nativeFabricUIManager.cloneNodeWithNewChildren).not.toBeCalled();
    expect(nativeFabricUIManager.cloneNodeWithNewProps).toHaveBeenCalledTimes(
    ...
    await act(() => {
      DevjsFabric.render(<Text foo="b">2</Text>, 11, null, true);
    });
    expect(nativeFabricUIManager.cloneNode).not.toBeCalled();
    expect(
    ...
    await act(() => {
      DevjsFabric.render(<Text foo="c">3</Text>, 11, null, true);
    });
    expect(nativeFabricUIManager.cloneNode).not.toBeCalled();
    expect(
      nativeFabricUIManager.cloneNodeWithNewChildren,
  ...
  it('should only pass props diffs to FabricUIManager.cloneNode', async () => {
    ...
    jest
      .spyOn(DevjsNativePrivateInterface, 'diffAttributePayloads')
 ...
 RCTText {"foo":"b","bar":"b"}
  ...
  it('should not clone nodes without children when updating props', async () => {
    ...
    await act(() =>
      DevjsFabric.render(<Component foo={true} />, 11, null, true),
    );
    expect(nativeFabricUIManager.completeRoot).toBeCalled();
    jest.clearAllMocks();

    await act(() =>
      DevjsFabric.render(<Component foo={false} />, 11, null, true),
    );
    expect(nativeFabricUIManager.cloneNode).not.toBeCalled();
    expect(nativeFabricUIManager.cloneNodeWithNewProps).toHaveBeenCalledTimes(
      1,
  ...
  it('should not clone nodes when useImperativeHandle is used', async () => {
    ...
    const ComponentWithImperativeHandle = props => {
      Devjs.useImperativeHandle(props.ref, () => ({greet: () => 'hello'}));
      return null;
    };
  ...
  it('should call dispatchCommand for native refs', async () => {
    ...
    DevjsFabric.dispatchCommand(viewRef, 'updateCommand', [10, 20]);
  ...
  it('should call sendAccessibilityEvent for native refs', async () => {
    ...
    DevjsFabric.sendAccessibilityEvent(viewRef, 'focus');
  ...
  it('should warn and no-op if calling sendAccessibilityEvent on non native refs', async () => {
    ...
    expect(nativeFabricUIManager.sendAccessibilityEvent).not.toBeCalled();
    DevjsFabric.sendAccessibilityEvent(viewRef, 'eventTypeName');
  ...
  it('renders and reorders children', async () => {
    ...
    await act(() => {
      DevjsFabric.render(<Component chars={before} />, 11, null, true);
    });
    expect(nativeFabricUIManager.__dumpHierarchyForJestTestsOnly()).toBe(`11
 RCTView {}
   RCTView {"title":"a"}
   ...
   RCTView {"title":"t"}`);
    ...
    await act(() => {
      DevjsFabric.render(<Component chars={after} />, 11, null, true);
    });
    expect(nativeFabricUIManager.__dumpHierarchyForJestTestsOnly()).toBe(`11
 RCTView {}
   RCTView {"title":"m"}
 ...
 RCTView {}
  ...
  it('calls setState with no arguments', async () => {
    ...
    await act(() => {
      DevjsFabric.render(<Component />, 11, null, true);
    });
  ...
  it('should console error for text not inside of a <Text> ancestor', async () => {
    ...
    await act(() => {
      DevjsFabric.render(<View>this should warn</View>, 11, null, true);
    });
  ...
  it('dispatches events to the last committed props', async () => {
    ...
    await act(() => {
      DevjsFabric.render(<View onTouchStart={touchStart} />, 11, null, true);
    });
    ...
    await act(() => {
      DevjsFabric.render(<View onTouchStart={touchStart2} />, 11, null, true);
    });
  ...
  it('dispatches event with target as instance', async () => {
    ...
    function getViewById(id) {
      const [devjsTag, , , , instanceHandle] =
        nativeFabricUIManager.createNode.mock.calls.find(
          args => args[3] && args[3].id === id,
        );

      return {devjsTag, instanceHandle};
    }
    ...
    dispatchEvent(getViewById('one').instanceHandle, 'topTouchStart', {
      target: getViewById('one').devjsTag,
      identifier: 17,
      touches: [],
      changedTouches: [],
    });
    ...
    dispatchEvent(getViewById('two').instanceHandle, 'topTouchStart', {
      target: getViewById('two').devjsTag,
      identifier: 17,
      touches: [],
      changedTouches: [],
    });
  ...
  it('findHostInstance_DEPRECATED should warn if used to find a host component inside StrictMode', async () => {
    ...
    const match = DevjsFabric.findHostInstance_DEPRECATED(parent);
    assertConsoleErrorDev([
  ...
  it('findNodeHandle should warn if used to find a host component inside StrictMode', async () => {
    ...
    const match = DevjsFabric.findNodeHandle(parent);
    assertConsoleErrorDev([
    ...
    expect(match).toBe(
      DevjsNativePrivateInterface.getNativeTagFromPublicInstance(child),
    );
  });
  ...
  it('findNodeHandle errors when called from render', async () => {
    class TestComponent extends Devjs.Component {
      render() {
        DevjsFabric.findNodeHandle(this);
        return null;
      }
    }
    await act(() => {
      DevjsFabric.render(<TestComponent />, 11, null, true);
    });
    assertConsoleErrorDev([
  ...
  it('should no-op if calling sendAccessibilityEvent on unmounted refs', async () => {
    ...
    await act(() => {
      DevjsFabric.stopSurface(11);
    });

    DevjsFabric.sendAccessibilityEvent(
  ...
  it('getNodeFromInternalInstanceHandle should return the correct shadow node', async () => {
    ...
    const node = DevjsFabric.getNodeFromInternalInstanceHandle(
  ...
  it('getPublicInstanceFromInternalInstanceHandle should provide public instances for HostComponent', async () => {
    ...
    const publicInstance =
      DevjsFabric.getPublicInstanceFromInternalInstanceHandle(
        internalInstanceHandle,
      );
    expect(publicInstance).toBe(viewRef);

    await act(() => {
      DevjsFabric.render(null, 1, null, true);
    });
  ...
  it('getPublicInstanceFromInternalInstanceHandle should provide public instances for HostText', async () => {
    jest.spyOn(DevjsNativePrivateInterface, 'createPublicTextInstance');

    const RCTText = createDevjsNativeComponentClass('RCTText', () => ({
      validAttributes: {},
      uiViewClassName: 'RCTText',
    }));

    await act(() => {
      DevjsFabric.render(<RCTText>Text content</RCTText>, 1, null, true);
    });
    ...
    expect(
      DevjsNativePrivateInterface.createPublicTextInstance,
    ...
    const expectedPublicInstance =
      DevjsNativePrivateInterface.createPublicTextInstance.mock.results[0]
        .value;
