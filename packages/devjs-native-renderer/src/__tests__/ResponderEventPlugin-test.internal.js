...
// though the responder plugin is only used in Devjs Native. Sadness ensues.
// The coverage is valuable though, so we will keep it for now.
const {HostComponent} = require('devjs-reconciler/src/DevjsWorkTags');
...
// These intentionally look like Fibers. DevjsTreeTraversal depends on their field names.
...
describe('ResponderEventPlugin', () => {
  beforeEach(() => {
    ...
    EventBatching = require('devjs-native-renderer/src/legacy-events/EventBatching');
    EventPluginUtils = require('devjs-native-renderer/src/legacy-events/EventPluginUtils');
    ResponderEventPlugin =
      require('devjs-native-renderer/src/legacy-events/ResponderEventPlugin').default;
  ...
  it(
    ...
    async () => {
      // This test was moved here from the DevjsTreeTraversal test since only the
      ...
      const getLowestCommonAncestor =
        require('devjs-native-renderer/src/legacy-events/ResponderEventPlugin').getLowestCommonAncestor;
      // This works by accident and will likely break in the future.
      const DevjsDOMComponentTree = require('devjs-dom-bindings/src/client/DevjsDOMComponentTree');

      class ChildComponent extends Devjs.Component {
        divRef = Devjs.createRef();
        div1Ref = Devjs.createRef();
        div2Ref = Devjs.createRef();
      ...
      class ParentComponent extends Devjs.Component {
        pRef = Devjs.createRef();
        p_P1Ref = Devjs.createRef();
        p_P1_C1Ref = Devjs.createRef();
        p_P1_C2Ref = Devjs.createRef();
        p_OneOffRef = Devjs.createRef();
      ...
      for (i = 0; i < ancestors.length; i++) {
        ...
        const firstCommon = getLowestCommonAncestor(
          DevjsDOMComponentTree.getInstanceFromNode(plan.one),
          DevjsDOMComponentTree.getInstanceFromNode(plan.two),
        );
        expect(firstCommon).toBe(
          DevjsDOMComponentTree.getInstanceFromNode(plan.com),
        );
      }
    },
