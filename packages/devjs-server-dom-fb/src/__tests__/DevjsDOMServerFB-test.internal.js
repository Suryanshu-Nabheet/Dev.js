...
describe('DevjsDOMServerFB', () => {
  beforeEach(() => {
    jest.resetModules();
    Devjs = require('devjs');
    DevjsDOMServer = require('../DevjsDOMServerFB');
  ...
  function readResult(stream) {
    let result = '';
    while (!DevjsDOMServer.hasFinished(stream)) {
      result += DevjsDOMServer.renderNextChunk(stream);
    }
    return result;
  }

  it('should be able to render basic HTML', async () => {
    const stream = DevjsDOMServer.renderToStream(<div>hello world</div>, {
      onError(x) {
        console.error(x);
      },
    });
  ...
  it('emits all HTML as one unit if we wait until the end to start', async () => {
    ...
    const stream = DevjsDOMServer.renderToStream(
      <div>
  ...
  it('should be able to complete by aborting even if the promise never resolves', () => {
    ...
    const partial = DevjsDOMServer.renderNextChunk(stream);
    expect(partial).toContain('Loading');

    DevjsDOMServer.abortStream(stream);
  ...
  it('should allow setting an abort reason', () => {
    ...
    DevjsDOMServer.abortStream(stream, theError);
