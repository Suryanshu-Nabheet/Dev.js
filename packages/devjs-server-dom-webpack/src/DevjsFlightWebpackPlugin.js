...
import type {ImportManifestEntry} from './shared/DevjsFlightImportMetadata';
...
// you might want them.
const clientImportName = 'devjs-server-dom-webpack/client';
...
const PLUGIN_NAME = 'Devjs Server Plugin';

export default class DevjsFlightWebpackPlugin {
  ...
  constructor(options: Options) {
    ...
    this.clientManifestFilename =
      options.clientManifestFilename || 'devjs-client-manifest.json';
    this.serverConsumerManifestFilename =
      options.serverConsumerManifestFilename || 'devjs-ssr-manifest.json';
  }
        ...
        function () {
          if (clientFileNameFound === false) {
            compilation.warnings.push(
              new WebpackError(
                `Client runtime at ${clientImportName} was not found. Devjs Server Components module map file ${_this.clientManifestFilename} was not created.`,
              ),
            );
            return;
          }
