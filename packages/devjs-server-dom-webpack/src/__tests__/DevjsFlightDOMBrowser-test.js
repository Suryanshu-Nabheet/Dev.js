...
describe('DevjsFlightDOMBrowser', () => {
  beforeEach(() => {
    jest.resetModules();
    ...
    DevjsServerDOM = require('devjs-dom');
    ...
    jest.mock('devjs-server-dom-webpack/static', () =>
      require('devjs-server-dom-webpack/static.browser'),
    );
    ...
    DevjsDOMFizzServer = require('devjs-dom/server.browser');
      ...
      if (!resolvedModuleData) {
        throw new Error(
          'Could not find the module "' +
            ref +
            '" in the Devjs Client Manifest. ' +
            'This is probably a bug in the Devjs Server Components bundler.',
        );
      }
    }
  ...
  async function callServer(actionId, body) {
    ...
    const args = await DevjsServerDOMServer.decodeReply(body, webpackServerMap);
  ...
  it('should resolve client components (with async chunks) when referenced in props', async () => {
    ...
    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(<Server />, webpackMap),
    );
  ...
  it('should handle references to deduped owner objects', async () => {
    // This is replicating Devjs components as generated by @svgr/webpack:
    let path1a: Devjs.DevjsNode;
    let path1b: Devjs.DevjsNode;
    let path2: Devjs.DevjsNode;

    function Svg1() {
      return DevjsServer.createElement(
        'svg',
        {id: '1'},
        path1a || (path1a = DevjsServer.createElement('path', {})),
        path1b || (path1b = DevjsServer.createElement('path', {})),
      );
    }
    ...
    function Svg2() {
      return DevjsServer.createElement(
        'svg',
        {id: '2'},
        path2 || (path2 = DevjsServer.createElement('path', {})),
      );
    }
    ...
    function Server() {
      return DevjsServer.createElement(
        ...
        {},
        DevjsServer.createElement(Svg1),
        DevjsServer.createElement(Svg2),
      );
    }
    ...
    let response = DevjsServerDOMClient.createFromReadableStream(stream);
    const container = document.createElement('div');
    const root = DevjsDOMClient.createRoot(container);
  ...
  it('basic use(promise)', async () => {
    function Server() {
      return (
        DevjsServer.use(Promise.resolve('A')) +
        DevjsServer.use(Promise.resolve('B')) +
        DevjsServer.use(Promise.resolve('C'))
      );
    }

    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(<Server />),
    );
    const response = DevjsServerDOMClient.createFromReadableStream(stream);
  ...
  it('use(promise) in multiple components', async () => {
    function Child({prefix}) {
      return (
        prefix +
        DevjsServer.use(Promise.resolve('C')) +
        DevjsServer.use(Promise.resolve('D'))
      );
    }
    ...
    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(<Parent />),
    );
    const response = DevjsServerDOMClient.createFromReadableStream(stream);
  ...
  it('using a rejected promise will throw', async () => {
    ...
    function Server() {
      return (
        DevjsServer.use(promiseA) +
        DevjsServer.use(promiseB) +
        DevjsServer.use(promiseC)
      );
    }
  ...
  it("use a promise that's already been instrumented and resolved", async () => {
    ...
    function Server() {
      return DevjsServer.use(thenable);
    }
  ...
  it('can pass a server function by importing from client back to server', async () => {
    ...
    const ServerModuleBImportedOnClient = {
      upper: DevjsServerDOMClient.createServerReference(
  ...
  it('propagates server reference errors to the client', async () => {
    ...
    const response = DevjsServerDOMClient.createFromReadableStream(stream, {
      async callServer(actionId, args) {
        ...
        result.catch(() => {});
        return DevjsServerDOMClient.createFromReadableStream(
          DevjsServerDOMServer.renderToReadableStream(result, null, {
  ...
  it('can pass an async server exports that resolves later to an outline object like a Map', async () => {
    ...
    // prettier-ignore
    const response =
      await DevjsServerDOMClient.createFromReadableStream(stream);
    ...
    const body = await DevjsServerDOMClient.encodeReply(map);
    const resultPromise = DevjsServerDOMServer.decodeReply(
      body,
      webpackServerMap,
    );
  ...
  it('supports Float hints before the first await in server components in Fiber', async () => {
    ...
    async function ServerComponent() {
      DevjsServerDOM.preload('before', {as: 'style'});
      await 1;
      DevjsServerDOM.preload('after', {as: 'style'});
    ...
    expect(document.head.innerHTML).toBe(
      gate(f => f.www)
        ? // The www entrypoints for DevjsDOM and DevjsDOMClient are unified so even
  ...
  it('Does not support Float hints in server components anywhere in Fizz', async () => {
    ...
    async function ServerComponent() {
      DevjsDOM.preload('before', {as: 'style'});
      await 1;
      DevjsDOM.preload('after', {as: 'style'});
      return <ClientComponent />;
    }
    ...
    let fizzPromise;
    await act(async () => {
      fizzPromise = DevjsDOMFizzServer.renderToReadableStream(<App />);
    });
  ...
  it('can dedupe references inside promises', async () => {
    ...
    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(object, webpackMap),
    );
  ...
  it('should resolve a cycle between debug info and the value it produces', async () => {
    ...
    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(element, webpackMap),
    );
  ...
  it('does not close the response early when using a fast debug channel', async () => {
    ...
    const rscStream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(<Component />, webpackMap, {
        debugChannel: {
  ...
  it('should fully resolve debug info when transported through a (slow) debug channel', async () => {
    function Paragraph({children}) {
      return DevjsServer.createElement('p', null, children);
    }
    ...
    const app = DevjsServer.createElement(
      DevjsServer.Fragment,
      null,
      DevjsServer.createElement(Paragraph, null, 'foo'),
      DevjsServer.createElement(Paragraph, null, 'bar'),
    );
    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(
        {
    ...
    const container = document.createElement('div');
    const clientRoot = DevjsDOMClient.createRoot(container);
        ...
        [
          ...
          },
          {
            ...
            "stack": [
              [
                "Object.<anonymous>",
                "/packages/devjs-server-dom-webpack/src/__tests__/DevjsFlightDOMBrowser-test.js",
                2989,
                19,
                2973,
                89,
