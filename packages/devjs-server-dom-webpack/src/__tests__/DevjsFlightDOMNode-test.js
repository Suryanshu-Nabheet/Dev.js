...
describe('DevjsFlightDOMNode', () => {
  beforeEach(() => {
    ...
    jest.mock('devjs-server-dom-webpack/server', () =>
      jest.requidevjsual('devjs-server-dom-webpack/server.node'),
    );
    ...
    jest.mock('devjs-server-dom-webpack/static', () =>
      jest.requidevjsual('devjs-server-dom-webpack/static.node'),
    );
    ...
    jest.mock('devjs-server-dom-webpack/client', () =>
      jest.requidevjsual('devjs-server-dom-webpack/client.node'),
    );

    Devjs = require('devjs');
    DevjsDOMServer = require('devjs-dom/server.node');
  ...
  it('should allow an alternative module mapping to be used for SSR', async () => {
    ...
    function ClientRoot() {
      ...
      response = DevjsServerDOMClient.createFromNodeStream(
        readable,
        serverConsumerManifest,
      );
      return use(response);
    ...
    const ssrStream = await serverAct(() =>
      DevjsDOMServer.renderToPipeableStream(<ClientRoot />),
    );
  ...
  it('should encode long string in a compact format', async () => {
    ...
    const parsedResult = DevjsServerDOMClient.createFromNodeStream(readable, {
      moduleMap: {},
      moduleLoading: webpackModuleLoading,
    });
  ...
  it('should be able to serialize any kind of typed array', async () => {
    ...
    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToPipeableStream(buffers),
    );
    const readable = new Stream.PassThrough(streamOptions);
    const promise = DevjsServerDOMClient.createFromNodeStream(readable, {
      moduleMap: {},
      moduleLoading: webpackModuleLoading,
    });
    stream.pipe(readable);
  ...
  it('should cancel the underlying and transported ReadableStreams when we are cancelled', async () => {
    ...
    const result = await DevjsServerDOMClient.createFromNodeStream(readable, {
      moduleMap: {},
      moduleLoading: webpackModuleLoading,
    });
  ...
  it('can prerender', async () => {
    ...
    const response = DevjsServerDOMClient.createFromNodeStream(prelude, {
      serverConsumerManifest: {
        moduleMap: null,
        moduleLoading: null,
      },
    ...
    const ssrStream = await serverAct(() =>
      DevjsDOMServer.renderToPipeableStream(
  ...
  it('includes source locations in component and owner stacks for halted Client components', async () => {
    ...
    function ServerComponent() {
      ...
      return DevjsServer.createElement(ClientComponentOnTheClient, {
        p1: p1,
        p2: p2,
        p3: p3,
      });
      ...
      return DevjsServer.createElement(
        ...
        DevjsServer.createElement(
          ...
          DevjsServer.createElement(
            DevjsServer.Suspense,
            {fallback: 'Loading...'},
            DevjsServer.createElement(ServerComponent, null),
          ),
        ),
      );
    }
    ...
    const response = DevjsServerDOMClient.createFromNodeStream(
      readable,
      serverConsumerManifest,
    );
    ...
    if (__DEV__) {
      expect(ignoreListStack(ownerStack)).toBe(
        ...
        '' +
          ...
          // Just make sure they still point at Devjs.use(p2)
          (gate(flags => flags.enableAsyncDebugInfo)
            ? '\n    at SharedComponent (./DevjsFlightDOMNode-test.js:813:7)'
            : '') +
          '\n    at ServerComponent (file://./DevjsFlightDOMNode-test.js:835:26)' +
          '\n    at App (file://./DevjsFlightDOMNode-test.js:852:25)',
      );
    } else {
      expect(ownerStack).toBeNull();
    }
  ...
  it('includes deeper location for aborted stacks', async () => {
    async function getData() {
      const signal = DevjsServer.cacheSignal();
  ...
  it('should not corrupt the Node.js Buffer pool by detaching ArrayBuffers when using Web Streams', async () => {
    ...
    const readable = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(fileChunk, webpackMap),
    );
  ...
  describe('with real timers', () => {
    ...
    // to be used to accurately represent how this interacts with Devjs's
    // internal scheduling.
    ...
    it('should use late-arriving I/O debug info to enhance component and owner stacks when aborting a prerender', async () => {
      ...
      async function Dynamic() {
        ...
        return DevjsServer.createElement('p', null, data1, ' ', data2);
      }
      ...
      function App() {
        return DevjsServer.createElement(
          ...
          DevjsServer.createElement(
            'body',
            null,
            DevjsServer.createElement(Dynamic),
          ),
        );
      }
      ...
      await new Promise(resolve => {
        setTimeout(async () => {
          const stream = DevjsServerDOMServer.renderToPipeableStream(
      ...
      const {prelude} = await new Promise(resolve => {
        ...
        setTimeout(() => {
          result = DevjsDOMFizzStatic.prerenderToNodeStream(
            Devjs.createElement(ClientRoot),
            {
      ...
      if (__DEV__) {
        expect(
          ...
          '\n' +
            '    in Dynamic' +
            (gate(flags => flags.enableAsyncDebugInfo)
              ? ' (file://DevjsFlightDOMNode-test.js:1416:27)\n'
              : '\n') +
            '    in body\n' +
            '    in html\n' +
            '    in App (file://DevjsFlightDOMNode-test.js:1429:25)\n' +
            '    in ClientRoot (DevjsFlightDOMNode-test.js:1504:16)',
        );
      } else {
        expect(
          normalizeCodeLocInfo(componentStack, {preserveLocation: true}),
      ...
      if (__DEV__) {
        if (gate(flags => flags.enableAsyncDebugInfo)) {
          expect(
            ...
            '\n' +
              '    in Dynamic (file://DevjsFlightDOMNode-test.js:1416:27)\n' +
              '    in App (file://DevjsFlightDOMNode-test.js:1429:25)',
          );
        } else {
          expect(
    ...
    async function reencodeFlightStream(
      ...
      await new Promise(async resolve => {
        ...
        const decoded = await DevjsServerDOMClient.createFromNodeStream(
          serverStream,
          serverConsumerManifest,
          {
    ...
    it('can preserve old IO info when decoding and re-encoding a stream with options.startTime', async () => {
      ...
      async function Dynamic() {
        ...
        return DevjsServer.createElement('p', null, data);
      }
        ...
        return DevjsServer.createElement(
          ...
          DevjsServer.createElement(
            ...
            DevjsServer.createElement(
              ...
              // DevjsServer.createElement(DevjsServer.createElement(Dynamic)),
              DevjsServer.createElement(
                'section',
                null,
                DevjsServer.createElement(Dynamic),
      ...
      await new Promise(async (resolve, reject) => {
        ...
        const decodedPromise = DevjsServerDOMClient.createFromNodeStream(
          serverStream,
          serverConsumerManifest,
          {
        ...
        setTimeout(() => {
          ssrStream = DevjsDOMServer.renderToPipeableStream(
            Devjs.createElement(ClientRoot, {
  ...
  it('warns with a tailored message if eval is not available in dev', async () => {
    ...
    try {
      const readable = await serverAct(() =>
        DevjsServerDOMServer.renderToReadableStream({}, webpackMap),
      );

      assertConsoleErrorDev([]);

      await DevjsServerDOMClient.createFromReadableStream(readable, {
        serverConsumerManifest: {
          moduleMap: null,
          moduleLoading: null,
        },
