...
import {insertNodesAndExecuteScripts} from 'devjs-dom/src/test-utils/FizzTestUtils';
...
describe('DevjsFlightDOMForm', () => {
  beforeEach(() => {
    ...
    DevjsServerDOMServer = require('devjs-server-dom-webpack/server.edge');
    ...
    jest.resetModules();
    Devjs = require('devjs');
    DevjsServerDOMClient = require('devjs-server-dom-webpack/client.edge');
    ...
    // useActionState = require('devjs-dom').useFormState;
    // }
    useActionState = require('devjs').useActionState;
  ...
  async function POST(formData) {
    const boundAction = await DevjsServerDOMServer.decodeAction(
      formData,
      webpackServerMap,
    );
    const returnValue = boundAction();
    const formState = await DevjsServerDOMServer.decodeFormState(
      await returnValue,
      formData,
      webpackServerMap,
    );
  ...
  it('can submit a passed server action without hydrating it', async () => {
    ...
    const rscStream = DevjsServerDOMServer.renderToReadableStream(<App />);
    ...
    const ssrStream = await DevjsDOMServer.renderToReadableStream(response);
  ...
  it('can submit an imported server action without hydrating it', async () => {
    ...
    const serverAction = DevjsServerDOMClient.createServerReference(
      ServerModule.$$id,
    );
    function App() {
      return (
    ...
    const ssrStream = await DevjsDOMServer.renderToReadableStream(<App />);
    await readIntoContainer(ssrStream);
  ...
  it('can bind an imported server action on the client without hydrating it', async () => {
    ...
    const ssrStream = await DevjsDOMServer.renderToReadableStream(<Client />);
    await readIntoContainer(ssrStream);
  ...
  it('can bind a server action on the client without hydrating it', async () => {
    ...
    const rscStream = DevjsServerDOMServer.renderToReadableStream(
  ...
  it('useActionState can reuse state during MPA form submission', async () => {
    ...
    const postbackRscStream = DevjsServerDOMServer.renderToReadableStream(
      <ClientRef action={serverAction} />,
      webpackMap,
    );
    const postbackResponse = DevjsServerDOMClient.createFromReadableStream(
      postbackRscStream,
      {
        serverConsumerManifest: {
          moduleMap: null,
    ...
    const postbackSsrStream = await DevjsDOMServer.renderToReadableStream(
      postbackResponse,
      {formState: formState},
    );
    ...
    if (__DEV__) {
      ...
      await act(() => {
        DevjsDOMClient.hydrateRoot(container, postbackResponse, {
          formState: formState,
        });
      });
      expect(container.textContent).toBe('121');
  ...
  it(
    ...
    async () => {
      ...
      const postbackRscStream2 = DevjsServerDOMServer.renderToReadableStream(
        // Note: `.bind` is the same as an inline closure with 'use server'
        <ClientRef action={serverAction.bind(null, 5)} />,
        webpackMap,
      );
      const postbackResponse2 = DevjsServerDOMClient.createFromReadableStream(
        postbackRscStream2,
        {
          serverConsumerManifest: {
            moduleMap: null,
      ...
      const postbackSsrStream2 = await DevjsDOMServer.renderToReadableStream(
        postbackResponse2,
  ...
  it('useActionState can return JSX state during MPA form submission', async () => {
    ...
    assertConsoleErrorDev([
      'Failed to serialize an action for progressive enhancement:\n' +
        'Error: Devjs Element cannot be passed to Server Functions from the Client without a temporary reference set. Pass a TemporaryReferenceSet to the options.\n' +
        '  [<div/>]\n' +
        '   ^^^^^^' +
        '\n    in <stack>',
    ]);
  ...
  it('useActionState can return binary state during MPA form submission', async () => {
    ...
    async function submitTheForm() {
      ...
      const postbackResponse =
        await DevjsServerDOMClient.createFromReadableStream(postbackRscStream, {
          serverConsumerManifest: {
            moduleMap: null,
            moduleLoading: null,
          },
