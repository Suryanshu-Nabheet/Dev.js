...
describe('DevjsFlightDOM', () => {
  beforeEach(() => {
    // For this first reset we are going to load the dom-node version of devjs-server-dom-webpack/server
    ...
    FlightDevjs = require('devjs');
    FlightDevjsDOM = require('devjs-dom');

    jest.mock('devjs-server-dom-webpack/server', () =>
      require('devjs-server-dom-unbundled/server.node'),
    );
    jest.mock('devjs-server-dom-webpack/static', () =>
      require('devjs-server-dom-unbundled/static.node'),
    );
    ...
    DevjsDOMFizzServer = require('devjs-dom/server.node');
  ...
  it('should resolve HTML using Node streams', async () => {
    ...
    const {pipe} = await serverAct(() =>
      DevjsServerDOMServer.renderToPipeableStream(<App />, webpackMap),
    );
    pipe(writable);
  ...
  it('should resolve the root', async () => {
    ...
    const {writable, readable} = getTestStream();
    const {pipe} = await serverAct(() =>
      DevjsServerDOMServer.renderToPipeableStream(<RootModel />, webpackMap),
    );
    pipe(writable);
    const response = DevjsServerDOMClient.createFromReadableStream(readable);
  ...
  it('should unwrap async module references using use', async () => {
    ...
    function ServerComponent() {
      const text = FlightDevjs.use(AsyncModuleRef);
      return <p>{text}</p>;
    }
  ...
  it('should error when a bundler uses async ESM modules with createClientModuleProxy', async () => {
    ...
    const errorMessage = `The module "${Object.keys(webpackMap).at(0)}" is marked as an async ESM module but was loaded as a CJS proxy. This is probably a bug in the Devjs Server Components bundler.`;
  ...
  it('should be able to import a name called "then"', async () => {
    ...
    const {writable, readable} = getTestStream();
    const {pipe} = await serverAct(() =>
      DevjsServerDOMServer.renderToPipeableStream(<ThenRef />, webpackMap),
    );
    pipe(writable);
    const response = DevjsServerDOMClient.createFromReadableStream(readable);
  ...
  it('does not throw when Devjs inspects any deep props', () => {
    const ClientModule = clientExports({
      Component: function () {},
    });
  ...
  it('can render a client Context.Provider from a server component', async () => {
    ...
    const TestContext = Devjs.createContext('default');
    const ClientModule = clientExports({
      TestContext,
    });
    ...
    function ClientConsumer() {
      const value = Devjs.useContext(TestContext);
    ...
    const {writable, readable} = getTestStream();
    const {pipe} = await serverAct(() =>
      DevjsServerDOMServer.renderToPipeableStream(<ServerApp />, webpackMap),
    );
    pipe(writable);
    const response = DevjsServerDOMClient.createFromReadableStream(readable);
  ...
  it('should progressively reveal server components', async () => {
    ...
    const {writable, readable} = getTestStream();
    const {pipe} = await serverAct(() =>
      DevjsServerDOMServer.renderToPipeableStream(model, webpackMap, {
        onError(x) {
  ...
  it('should preserve state of client components on refetch', async () => {
    ...
    const response1 = DevjsServerDOMClient.createFromReadableStream(
      stream1.readable,
    );
    await act(() => {
      root.render(
    ...
    const response2 = DevjsServerDOMClient.createFromReadableStream(
      stream2.readable,
    );
    await act(() => {
      root.render(
  ...
  it('should support float methods when rendering in Fiber', async () => {
    ...
    async function ServerComponent() {
      FlightDevjsDOM.prefetchDNS('d before');
      FlightDevjsDOM.preconnect('c before');
      FlightDevjsDOM.preconnect('c2 before', {crossOrigin: 'anonymous'});
      FlightDevjsDOM.preload('l before', {as: 'style'});
      FlightDevjsDOM.preloadModule('lm before');
      FlightDevjsDOM.preloadModule('lm2 before', {crossOrigin: 'anonymous'});
      FlightDevjsDOM.preinit('i before', {as: 'script'});
      FlightDevjsDOM.preinitModule('m before');
      FlightDevjsDOM.preinitModule('m2 before', {crossOrigin: 'anonymous'});
      await 1;
      FlightDevjsDOM.prefetchDNS('d after');
      FlightDevjsDOM.preconnect('c after');
      FlightDevjsDOM.preconnect('c2 after', {crossOrigin: 'anonymous'});
      FlightDevjsDOM.preload('l after', {as: 'style'});
      FlightDevjsDOM.preloadModule('lm after');
      FlightDevjsDOM.preloadModule('lm2 after', {crossOrigin: 'anonymous'});
      FlightDevjsDOM.preinit('i after', {as: 'script'});
      FlightDevjsDOM.preinitModule('m after');
      FlightDevjsDOM.preinitModule('m2 after', {crossOrigin: 'anonymous'});
    ...
    function getResponse() {
      if (response === null) {
        response = DevjsServerDOMClient.createFromReadableStream(readable);
      }
      return response;
    }
  ...
  it('should support float methods when rendering in Fizz', async () => {
    ...
    function getResponse() {
      if (response === null) {
        response =
          DevjsServerDOMClient.createFromReadableStream(flightReadable);
      }
      return response;
    }
    ...
    await serverAct(async () => {
      DevjsDOMFizzServer.renderToPipeableStream(<App />).pipe(fizzWritable);
    });
  ...
  it('supports Float hints from concurrent Flight -> Fizz renders', async () => {
    ...
    async function ServerComponent1() {
      FlightDevjsDOM.preload('before1', {as: 'style'});
      await 1;
      FlightDevjsDOM.preload('after1', {as: 'style'});
      return <ClientComponent />;
    }

    async function ServerComponent2() {
      FlightDevjsDOM.preload('before2', {as: 'style'});
      await 1;
      FlightDevjsDOM.preload('after2', {as: 'style'});
      return <ClientComponent />;
    }
    ...
    function getResponse(stream) {
      ...
      if (!response) {
        response = DevjsServerDOMClient.createFromReadableStream(stream);
    ...
    await serverAct(async () => {
      DevjsDOMFizzServer.renderToPipeableStream(
  ...
  it('supports deduping hints by Float key', async () => {
    ...
    async function ServerComponent() {
      FlightDevjsDOM.prefetchDNS('dns');
      FlightDevjsDOM.preconnect('preconnect');
      FlightDevjsDOM.preload('load', {as: 'style'});
      FlightDevjsDOM.preinit('init', {as: 'script'});
      // again but vary preconnect to demonstrate crossOrigin participates in the key
      FlightDevjsDOM.prefetchDNS('dns');
      FlightDevjsDOM.preconnect('preconnect', {crossOrigin: 'anonymous'});
      FlightDevjsDOM.preload('load', {as: 'style'});
      FlightDevjsDOM.preinit('init', {as: 'script'});
      await 1;
      // after an async point
      FlightDevjsDOM.prefetchDNS('dns');
      FlightDevjsDOM.preconnect('preconnect', {crossOrigin: 'use-credentials'});
      FlightDevjsDOM.preload('load', {as: 'style'});
      FlightDevjsDOM.preinit('init', {as: 'script'});
      return <ClientComponent />;
    }
  ...
  it('preloads resources without needing to render them', async () => {
    ...
    const {writable, readable} = getTestStream();
    const {pipe} = await serverAct(() =>
      DevjsServerDOMServer.renderToPipeableStream(<Component />, webpackMap),
    );
    pipe(writable);
  ...
  it('can abort synchronously during render', async () => {
    ...
    await serverAct(() => {
      const {pipe, abort} = DevjsServerDOMServer.renderToPipeableStream(
        <App />,
        webpackMap,
      );
  ...
  it('can prerender', async () => {
    ...
    const {pendingResult} = await serverAct(async () => {
      ...
      return {
        pendingResult: DevjsServerDOMStaticServer.prerenderToNodeStream(
          <App />,
          webpackMap,
        ),
      };
  ...
  it('does not propagate abort reasons errors when aborting a prerender', async () => {
    ...
    const response = DevjsServerDOMClient.createFromReadableStream(preludeWeb);
    ...
    let abortFizz;
    await serverAct(async () => {
      const {pipe, abort} = DevjsDOMFizzServer.renderToPipeableStream(
        Devjs.createElement(ClientApp),
        {
          onError(error) {
            errors.push(error);
