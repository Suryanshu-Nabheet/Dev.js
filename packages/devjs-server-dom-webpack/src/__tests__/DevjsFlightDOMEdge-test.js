...
let DevjsServerDOMStaticServer;
...
function normalizeSerializedContent(str) {
  return str.replaceAll(__devjs_ROOT_PATH_TEST__, '**');
}

describe('DevjsFlightDOMEdge', () => {
  beforeEach(() => {
    ...
    jest.mock('devjs-server-dom-webpack/server', () =>
      require('devjs-server-dom-webpack/server.edge'),
    );
    ...
    DevjsServerDOMServer = require('devjs-server-dom-webpack/server');
    jest.mock('devjs-server-dom-webpack/static', () =>
      require('devjs-server-dom-webpack/static.edge'),
    );
    DevjsServerDOMStaticServer = require('devjs-server-dom-webpack/static');

    jest.resetModules();
    __unmockDevjs();
    jest.unmock('devjs-server-dom-webpack/server');
    jest.mock('devjs-server-dom-webpack/client', () =>
      require('devjs-server-dom-webpack/client.edge'),
    );
    Devjs = require('devjs');
    DevjsDOMServer = require('devjs-dom/server.edge');
    DevjsDOMFizzStatic = require('devjs-dom/static.edge');
    DevjsServerDOMClient = require('devjs-server-dom-webpack/client');
    use = Devjs.use;
  });
  ...
  it('should allow an alternative module mapping to be used for SSR', async () => {
    ...
    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(<App />, webpackMap),
    );
    ...
    const ssrStream = await serverAct(() =>
      DevjsDOMServer.renderToReadableStream(<ClientRoot />),
    );
  ...
  it('should resolve cyclic references in client component props after two rounds of serialization and deserialization', async () => {
    ...
    const stream1 = DevjsServerDOMServer.renderToReadableStream(
      <Devjs.Fragment key="this-key-is-important-to-repro-a-prior-cycle-serialization-bug">
        <ClientComponent data={cyclic} />
      </Devjs.Fragment>,
      webpackMap,
    );

    const promise = DevjsServerDOMClient.createFromReadableStream(stream1, {
      serverConsumerManifest: {
        moduleMap: {
    ...
    const stream2 = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(promise, webpackMap, {
        onError(error) {
          errors.push(error);
        },
      }),
    ...
    const element = await serverAct(() =>
      DevjsServerDOMClient.createFromReadableStream(stream2, {
        serverConsumerManifest: {
          moduleMap: null,
          moduleLoading: null,
        },
    ...
    const ssrStream = await serverAct(() =>
      DevjsDOMServer.renderToReadableStream(element),
    );
    const result = await readResult(ssrStream);
  ...
  it('should load a server reference on a consuming server and pass it back', async () => {
    ...
    DevjsServerDOMClient.registerServerReference(
      ServerModule.greet,
    ...
    const body = await DevjsServerDOMClient.encodeReply({
      method: result.method,
      boundMethod: result.boundMethod,
    });
    const replyResult = await DevjsServerDOMServer.decodeReply(
      body,
      webpackServerMap,
    );
  ...
  it('should encode long string in a compact format', async () => {
    ...
    const result = await DevjsServerDOMClient.createFromReadableStream(
      stream2,
      {
        serverConsumerManifest: {
          moduleMap: null,
  ...
  it('should encode repeated objects in a compact format by deduping', async () => {
    ...
    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(props),
    );
    const [stream1, stream2] = passThrough(stream).tee();
  ...
  it('should execute repeated server components only once', async () => {
    ...
    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(children),
    );
    const [stream1, stream2] = passThrough(stream).tee();
    ...
    const model = await DevjsServerDOMClient.createFromReadableStream(stream2, {
      serverConsumerManifest: {
        moduleMap: null,
        moduleLoading: null,
      },
    ...
    const ssrStream = await serverAct(() =>
      DevjsDOMServer.renderToReadableStream(model),
    );
    // Should still match the result when parsed
    const result = await readResult(ssrStream);
  ...
  it('should break up large sync components by outlining into streamable elements', async () => {
    ...
    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(paragraphs),
    );
    ...
    // Use the SSR render to resolve any lazy elements
    const ssrStream = await serverAct(() =>
      DevjsDOMServer.renderToReadableStream(result),
    );
    const html = await readResult(ssrStream);

    const ssrStream2 = await serverAct(() =>
      DevjsDOMServer.renderToReadableStream(paragraphs),
    );
  ...
  it('regression: should not leak serialized size', async () => {
    ...
    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(model),
    );

    const result = await DevjsServerDOMClient.createFromReadableStream(stream, {
      serverConsumerManifest: {
        moduleMap: null,
        moduleLoading: null,
      },
    ...
    const result2 = await DevjsServerDOMClient.createFromReadableStream(
      stream2,
      {
        serverConsumerManifest: {
          moduleMap: null,
  ...
  it('should be able to serialize any kind of typed array', async () => {
    ...
    const stream = await serverAct(() =>
      passThrough(DevjsServerDOMServer.renderToReadableStream(buffers)),
    );
    const result = await DevjsServerDOMClient.createFromReadableStream(stream, {
      serverConsumerManifest: {
        moduleMap: null,
  ...
  it('should be able to serialize a blob', async () => {
    ...
    const stream = await serverAct(() =>
      passThrough(DevjsServerDOMServer.renderToReadableStream(blob)),
    );
    const result = await DevjsServerDOMClient.createFromReadableStream(stream, {
      serverConsumerManifest: {
        moduleMap: null,
  ...
  it('can transport FormData (blobs)', async () => {
    ...
    const stream = await serverAct(() =>
      passThrough(DevjsServerDOMServer.renderToReadableStream(formData)),
    );
    const result = await DevjsServerDOMClient.createFromReadableStream(stream, {
      serverConsumerManifest: {
        moduleMap: null,
  ...
  it('can pass an async import that resolves later to an outline object like a Map', async () => {
    ...
    const stream = await serverAct(() =>
      passThrough(DevjsServerDOMServer.renderToReadableStream(map, webpackMap)),
    );

    // Parsing the root blocks because the module hasn't loaded yet
    const resultPromise = DevjsServerDOMClient.createFromReadableStream(
      stream,
      {
        serverConsumerManifest: {
          moduleMap: null,
  ...
  it('can pass an async import to a ReadableStream while enqueuing in order', async () => {
    ...
    const stream = await serverAct(() =>
      passThrough(DevjsServerDOMServer.renderToReadableStream(s, webpackMap)),
    );

    const result = await serverAct(() =>
      DevjsServerDOMClient.createFromReadableStream(stream, {
        serverConsumerManifest: {
          moduleMap: null,
          moduleLoading: null,
        },
  ...
  it('warns if passing a this argument to bind() of a server reference', async () => {
    ...
    const ServerModuleImportedOnClient = {
      greet: DevjsServerDOMClient.createServerReference(
        ServerModule.greet.$$id,
  ...
  it('should supports ReadableStreams with typed arrays', async () => {
    ...
    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(s, {}),
    );
  ...
  it('should support binary ReadableStreams', async () => {
    ...
    const rscStream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(stream, {}),
    );
  ...
  it('supports async server component debug info as the element owner in DEV', async () => {
    ...
    async function Greeting({firstName}) {
      ...
      const child = DevjsServer.createElement(
        'span',
        null,
        'Hello, ' + firstName,
      );
      ...
      return DevjsServer.createElement(Container, null, child);
    }
    ...
    const stream = await serverAct(() =>
      DevjsServerDOMServer.renderToReadableStream(model, webpackMap),
    );
    ...
    const ssrStream = await serverAct(() =>
      DevjsDOMServer.renderToReadableStream(rootModel.greeting),
    );
    const result = await readResult(ssrStream);
    expect(result).toEqual('<span>Hello, Seb</span>');

    // Resolve the Devjs Lazy wrapper which must have resolved by now.
  ...
  it('supports server components in ssr component stacks', async () => {
    ...
    const model = {
      root: DevjsServer.createElement(Erroring),
    };
    ...
    const result = serverAct(() =>
      DevjsDOMServer.renderToReadableStream(<div>{rootModel.root}</div>, {
  ...
  it('can prerender', async () => {
    ...
    const {pendingResult} = await serverAct(async () => {
      ...
      return {
        pendingResult: DevjsServerDOMStaticServer.prerender(
          <App />,
          webpackMap,
        ),
      };
    ...
    const response = DevjsServerDOMClient.createFromReadableStream(prelude, {
      serverConsumerManifest: {
        moduleMap: null,
        moduleLoading: null,
      },
    ...
    // Use the SSR render to resolve any lazy elements
    const ssrStream = await serverAct(() =>
      DevjsDOMServer.renderToReadableStream(
        Devjs.createElement(ClientRoot, {response}),
      ),
    );
    // Should still match the result when parsed
    const result = await readResult(ssrStream);
  ...
  it('does not propagate abort reasons errors when aborting a prerender', async () => {
    ...
    function App() {
      return (
        <div>
          <DevjsServer.Suspense fallback="loading...">
            <Greeting />
  ...
  it('should be able to handle a rejected promise in prerender', async () => {
    ...
    const {prelude} = await DevjsServerDOMStaticServer.prerender(
  ...
  it('does not include source locations in component stacks for halted components', async () => {
      ...
      return DevjsServer.createElement(
        ...
        DevjsServer.createElement(
          ...
          DevjsServer.createElement(
            DevjsServer.Suspense,
            {fallback: 'Loading...'},
            DevjsServer.createElement(Component, null),
          ),
        ),
      );
    }
    ...
    const errors = [];
    const prerenderResult = DevjsServerDOMStaticServer.prerender(
    ...
    const prerenderResponse = DevjsServerDOMClient.createFromReadableStream(
    ...
    const fizzPrerenderStreamResult = DevjsDOMFizzStatic.prerender(
      Devjs.createElement(ClientRoot, {response: prerenderResponse}),
      {
  ...
  it('can transport debug info through a separate debug channel', async () => {
    ...
    function App() {
      return DevjsServer.createElement(
        DevjsServer.Suspense,
        null,
        DevjsServer.createElement(ClientComponentOnTheClient, null),
      );
    }
  ...
  it('should properly resolve with deduped objects', async () => {
    ...
    const stream = DevjsServerDOMServer.renderToReadableStream(root);
  ...
  it('does not leak the server reference code', async () => {
    ...
    expect(
      DevjsServerDOMServer.registerServerReference(foo, 'foo-id').toString(),
    ).toBe('function () { [omitted code] }');

    expect(
      DevjsServerDOMServer.registerServerReference(bar, 'bar-id').toString(),
    ).toBe('function () { [omitted code] }');

    expect(
      DevjsServerDOMServer.registerServerReference(
        anonymous,
        'anonymous-id',
      ).toString(),
    ).toBe('function () { [omitted code] }');
