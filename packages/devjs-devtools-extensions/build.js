 ...
 * or running a local build via one of the `devjs-build-fordevtools*` scripts.
 */
...
const build = async (tempPath, manifestPath, envExtension = {}) => {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    manifest.key = 'devjsdevtoolslocalbuilduniquekey';
  }
  ...
  const zipStream = createWriteStream(join(tempPath, 'DevjsDevTools.zip'));
...
const postProcess = async (tempPath, destinationPath, webpackStatsFilePath) => {
  ...
  const packedSourcePath = join(tempPath, 'DevjsDevTools.zip');
  const packedDestPath = join(destinationPath, 'DevjsDevTools.zip');
