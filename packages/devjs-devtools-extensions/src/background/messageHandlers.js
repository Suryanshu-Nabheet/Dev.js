...
export function handleDevjsDevToolsHookMessage(message, sender) {
  const {payload} = message;

  switch (payload?.type) {
    case 'devjs-renderer-attached': {
      setExtensionIconAndPopup(payload.devjsBuildType, sender.tab.id);
      ...
      payload.versions.forEach(version => {
        if (EXTENSION_CONTAINED_VERSIONS.includes(version)) {
          executeScriptInMainWorld({
            ...
            files: [`/build/devjs_devtools_backend_${version}.js`],
          });
        }
      });
    ...
    case 'fetch-file-with-cache': {
      ...
      if (!tabId || !url) {
        ...
        chrome.runtime.sendMessage({
          source: 'devjs-devtools-background',
          payload: {
