...
import {COMPACT_VERSION_NAME} from 'devjs-devtools-extensions/src/utils';
import {getIsReloadAndProfileSupported} from 'devjs-devtools-shared/src/utils';
import {
...
function welcome(event: $FlowFixMe) {
  if (
    event.source !== window ||
    event.data.source !== 'devjs-devtools-content-script'
  ) {
    return;
  }
  ...
  //
  // See https://github.com/Suryanshu-Nabheet/dev.js/issues/24162
  if (welcomeHasInitialized) {
    console.warn(
      'Devjs DevTools detected duplicate welcome "message" events from the content script.',
    );
    return;
  }
  ...
  setup(window.__devjs_DEVTOOLS_GLOBAL_HOOK__);
}
...
function registerRenderer(renderer: DevjsRenderer, hook: DevToolsHook) {
...
function activateBackend(version: string, hook: DevToolsHook) {
  ...
  agent.addListener('shutdown', () => {
    ...
    hook.emit('shutdown');
    delete window.__devjs_DEVTOOLS_BACKEND_MANAGER_INJECTED__;
  });

  initBackend(hook, agent, window, getIsReloadAndProfileSupported());

  // Setup Devjs Native style editor if a renderer like devjs-native-web has injected it.
...
function updateRequiredBackends() {
  ...
  window.postMessage(
    {
      source: 'devjs-devtools-backend-manager',
      payload: {
 ...
 * We can't use `devjsDevToolsAgent` field on a global Hook object, because it only cleaned up after both Frontend and Backend initialized
 */
if (!window.__devjs_DEVTOOLS_BACKEND_MANAGER_INJECTED__) {
  window.__devjs_DEVTOOLS_BACKEND_MANAGER_INJECTED__ = true;
