...
function injectProxy({target}: {target: any}) {
  ...
  if (!window.__devjs_DEVTOOLS_PROXY_INJECTED__) {
    window.__devjs_DEVTOOLS_PROXY_INJECTED__ = true;
...
window.addEventListener('pagehide', function ({target}) {
  ...
  delete window.__devjs_DEVTOOLS_PROXY_INJECTED__;
});
...
function sayHelloToBackendManager() {
  window.postMessage(
    {
      source: 'devjs-devtools-content-script',
      hello: true,
    },
    '*',
  );
...
function handleMessageFromPage(event: any) {
  ...
  switch (event.data.source) {
    ...
    case 'devjs-devtools-backend-manager': {
...
// Creates port from application page to the Devjs DevTools' service worker
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  switch (msg?.source) {
    case 'devtools-page-eval': {
      ...
      window.postMessage(
        {
          source: 'devjs-devtools-content-script-eval',
          payload: {
            requestId,
            scriptId,
            args,
...
window.addEventListener('message', event => {
  if (event.data?.source === 'devjs-devtools-content-script-eval-response') {
    ...
    try {
      ...
      console.warn(
        'Devjs DevTools Content Script eval response error occurred:',
        e,
      );
    } finally {
