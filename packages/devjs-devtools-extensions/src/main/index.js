...
import type {RootType} from 'devjs-dom/src/client/DevjsDOMRoot';
import type {FrontendBridge, Message} from 'devjs-devtools-shared/src/bridge';
import type {
  TabID,
  ViewElementSource,
} from 'devjs-devtools-shared/src/devtools/views/DevTools';
import type {SourceSelection} from 'devjs-devtools-shared/src/devtools/views/Editor/EditorPane';
import type {Element} from 'devjs-devtools-shared/src/frontend/types';

import {createElement} from 'devjs';
import {flushSync} from 'devjs-dom';
...
import {
  localStorageGetItem,
  localStorageSetItem,
} from 'devjs-devtools-shared/src/storage';
...
import {checkConditions} from 'devjs-devtools-shared/src/devtools/views/Editor/utils';
import * as parseHookNames from 'devjs-devtools-shared/src/hooks/parseHookNames';
...
import {startDevjsPolling} from './devjsPolling';
...
function createBridge() {
  ...
  const onBrowserElementSelectionChanged = () =>
    setDevjsSelectionFromBrowser(bridge);
...
function createElementsInspectPanel() {
  ...
  elementsPanel.createSidebarPane('Devjs Element ⚛', createdPane => {
...
function performInTabNavigationCleanup() {
  // Potentially, if devjs hasn't loaded yet and user performs in-tab navigation
  clearDevjsPollingInstance();
  ...
  // If panels were already created, and we have already mounted Devjs root to display
  ...
  // Devjs DevTools tab, when user does in-tab navigation
...
function performFullCleanup() {
  // Potentially, if devjs hasn't loaded yet and user closed the browser DevTools
  clearDevjsPollingInstance();
...
function mountDevjsDevTools() {
  devjsPollingInstance = null;
...
let devjsPollingInstance = null;
function clearDevjsPollingInstance() {
  devjsPollingInstance?.abort();
  devjsPollingInstance = null;
}

function showNoDevjsDisclaimer() {
  if (componentsPortalContainer) {
    componentsPortalContainer.innerHTML =
      '<h1 class="no-devjs-disclaimer">Looks like this page doesn\'t have Devjs, or it hasn\'t been loaded yet.</h1>';
...
function mountDevjsDevToolsWhenDevjsHasLoaded() {
  devjsPollingInstance = startDevjsPolling(
    mountDevjsDevTools,
    5, // ~5 seconds
    showNoDevjsDisclaimer,
  );
}
...
// This debounced callback primarily used to avoid mounting Devjs DevTools multiple times, which results
...
const debouncedMountDevjsDevToolsCallback = debounce(
  mountDevjsDevToolsWhenDevjsHasLoaded,
  500,
);

// Clean up everything, but start mounting Devjs DevTools panels if user stays at this page
function onNavigatedToOtherPage() {
  performInTabNavigationCleanup();
  debouncedMountDevjsDevToolsCallback();
}
