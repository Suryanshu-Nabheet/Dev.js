...
import type {
  ImportMetadata,
  ImportManifestEntry,
} from '../shared/DevjsFlightImportMetadata';
...
import {prepareDestinationWithChunks} from 'devjs-client/src/DevjsFlightClientConfig';
      ...
      if (!resolvedModuleData) {
        throw new Error(
          'Could not find the module "' +
            metadata[ID] +
            '" in the Devjs Server Consumer Manifest. ' +
...
export function getModuleDebugInfo<T>(
  ...
  const chunks = metadata[CHUNKS];
  const debugInfo: DevjsDebugInfo = [];
  let i = 0;
