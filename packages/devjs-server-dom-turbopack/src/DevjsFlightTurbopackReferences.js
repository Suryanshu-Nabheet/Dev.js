...
export type ServerReference<T: Function> = T & {
  $$typeof: symbol,
  $$id: string,
  $$bound: null | Array<DevjsClientValue>,
  $$location?: Error,
};
...
const CLIENT_REFERENCE_TAG = Symbol.for('devjs.client.reference');
const SERVER_REFERENCE_TAG = Symbol.for('devjs.server.reference');
  ...
  return Object.defineProperties(
    ...
    (__DEV__
      ? {
          ...
          $$bound,
          $$location: {
            value: Error('devjs-stack-top-frame'),
            configurable: true,
          },
          bind: {value: bind, configurable: true},
...
const deepProxyHandlers: Proxy$traps<mixed> = {
  get: function (
    ...
    switch (name) {
      ...
      case 'defaultProps':
        return undefined;
      // Devjs looks for debugInfo on thenables.
      case '_debugInfo':
        return undefined;
      ...
      case 'Provider':
        // Context.Provider === Context in Devjs, so return the same reference.
  ...
  switch (name) {
    ...
    case 'then':
      ...
      if (!target.$$async) {
        ...
        // Treat this as a resolved Promise for Devjs's use()
