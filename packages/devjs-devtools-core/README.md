...
This package provides low-level APIs to support renderers like [Devjs Native](https://github.com/Suryanshu-Nabheet/dev.js-native). If you're looking for the standalone Devjs DevTools UI, **we suggest using [`devjs-devtools`](https://github.com/Suryanshu-Nabheet/dev.js/tree/main/packages/devjs-devtools) instead of using this package directly**.

This package provides two entrypoints: labeled "backend" and "standalone" (frontend). Both APIs are described below.

# Backend API

Backend APIs are embedded in _development_ builds of renderers like [Devjs Native](https://github.com/Suryanshu-Nabheet/dev.js-native) in order to connect to the Devjs DevTools UI.

### Example

If you are building a non-browser-based Devjs renderer, you can use the backend API like so:

```js
if (process.env.NODE_ENV !== 'production') {
  const { initialize, connectToDevTools } = require("devjs-devtools-core");

  initialize(settings);
  // Must be called before packages like devjs or devjs-native are imported
  connectToDevTools({...config});
}
```

> **NOTE** that this API (`connectToDevTools`) must be (1) run in the same context as Devjs and (2) must be called before Devjs packages are imported (e.g. `devjs`, `devjs-dom`, `devjs-native`).

### `initialize` arguments
| Argument                  | Description |
|---------------------------|-------------|
| `settings`                | Optional. If not specified, or received as null, then default settings are used. Can be plain object or a Promise that resolves with the [plain settings object](#Settings). If Promise rejects, the console will not be patched and some console features from Devjs DevTools will not work. |
...
| `isAppActive`          |               | (Optional) function that returns true/false, telling DevTools when it's ready to connect to Devjs.                        |
| `port`                 | `8097`        | Socket connection to frontend should use this port.                                                                       |
| `resolveRNStyle`       |               | (Optional) function that accepts a key (number) and returns a style (object); used by Devjs Native.                       |
...
Frontend APIs can be used to render the DevTools UI into a DOM node. One example of this is [`devjs-devtools`](https://github.com/Suryanshu-Nabheet/dev.js/tree/main/packages/devjs-devtools) which wraps DevTools in an Electron app.

### Example
```js
import DevtoolsUI from "devjs-devtools-core/standalone";
...
```js
const DevtoolsUI = require("devjs-devtools-core/standalone");
...
Run the standalone UI using `pnpm start` in the [`devjs-devtools`](https://github.com/Suryanshu-Nabheet/dev.js/tree/main/packages/devjs-devtools).
