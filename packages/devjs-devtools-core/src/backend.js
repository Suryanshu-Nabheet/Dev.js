...
import Agent from 'devjs-devtools-shared/src/backend/agent';
import Bridge from 'devjs-devtools-shared/src/bridge';
import {installHook} from 'devjs-devtools-shared/src/hook';
import {initBackend} from 'devjs-devtools-shared/src/backend';
import {__DEBUG__} from 'devjs-devtools-shared/src/constants';
import setupNativeStyleEditor from 'devjs-devtools-shared/src/backend/NativeStyleEditor/setupNativeStyleEditor';
import {
...
import type {ResolveNativeStyle} from 'devjs-devtools-shared/src/backend/NativeStyleEditor/setupNativeStyleEditor';
...
export function connectToDevTools(options: ?ConnectOptions) {
  const hook: ?DevToolsHook = window.__devjs_DEVTOOLS_GLOBAL_HOOK__;
  if (hook == null) {
  ...
  ws.onopen = function () {
    ...
    // Setup Devjs Native style editor if the environment supports it.
    if (resolveRNStyle != null || hook.resolveRNStyle != null) {
      ...
      // Instead it relies on the Devjs Native Inspector to lazily inject them.
  ...
  function handleMessage(event: MessageEvent<>) {
    ...
    try {
      ...
      console.error(
        '[Devjs DevTools] Failed to parse JSON: ' + (event.data: any),
      );
      return;
    }
    messageListeners.forEach(fn => {
      try {
        ...
        console.log('[Devjs DevTools] Error calling listener', data);
