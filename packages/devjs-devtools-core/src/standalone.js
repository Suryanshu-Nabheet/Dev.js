...
import {localStorageSetItem} from 'devjs-devtools-shared/src/storage';
...
function hookNamesModuleLoaderFunction() {
  return import(
    /* webpackChunkName: 'parseHookNames' */ 'devjs-devtools-shared/src/hooks/parseHookNames'
  );
}
...
const log = (...args: Array<mixed>) => console.log('[Devjs DevTools]', ...args);
log.warn = (...args: Array<mixed>) => console.warn('[Devjs DevTools]', ...args);
log.error = (...args: Array<mixed>) =>
  console.error('[Devjs DevTools]', ...args);
...
function canViewElementSourceFunction(
  _source: DevjsFunctionLocation | DevjsCallSite,
  ...
  httpServer.on('request', (request: $FlowFixMe, response: $FlowFixMe) => {
    ...
    response.end(
      backendFile.toString() +
        '\n;' +
        `DevjsDevToolsBackend.initialize(undefined, undefined, undefined, ${componentFiltersString});` +
        '\n' +
        `DevjsDevToolsBackend.connectToDevTools({port: ${port}, host: '${host}', useHttps: ${
          useHttps ? 'true' : 'false'
        }});
        `,
    );
