...
import * as DevjsART from 'devjs-art';
...
// Since they must be on top, we need to import this before DevjsDOM.
import Circle from 'devjs-art/Circle';
import Rectangle from 'devjs-art/Rectangle';
import Wedge from 'devjs-art/Wedge';
...
describe('DevjsART', () => {
  ...
  beforeEach(() => {
    ...
    Group = DevjsART.Group;
    Shape = DevjsART.Shape;
    Surface = DevjsART.Surface;

    groupRef = Devjs.createRef();
    TestComponent = class extends Devjs.Component {
      group = groupRef;
      ...
      render() {
        const a = (
          <Shape
            ...
            fill={new DevjsART.LinearGradient(['black', 'white'])}
            key="a"
            width={50}
            height={50}
            x={50}
  ...
  it('renders composite with lifecycle inside group', async () => {
    ...
    class CustomShape extends Devjs.Component {
      render() {
        return <Shape />;
      }
...
describe('DevjsARTComponents', () => {
  ...
  it('should generate a <Shape> with props for drawing the Circle', async () => {
    let circle;
    await act(() => {
      circle = DevjsTestRenderer.create(
  ...
  it('should generate a <Shape> with props for drawing the Rectangle', async () => {
    let rectangle;
    await act(() => {
      rectangle = DevjsTestRenderer.create(
  ...
  it('should generate a <Shape> with props for drawing the Wedge', async () => {
    let wedge;
    await act(() => {
      wedge = DevjsTestRenderer.create(
