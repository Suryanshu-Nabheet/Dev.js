...
import {TYPES, EVENT_TYPES, childrenAsString} from './DevjsARTInternals';
...
export const rendererPackageName = 'devjs-art';
...
export function createInstance(type, props, internalInstanceHandle) {
  ...
  if (!instance) {
    throw new Error(`DevjsART does not support the type "${type}"`);
  }
...
// The ART renderer is secondary to the Devjs DOM renderer.
...
export function insertBefore(parentInstance, child, beforeChild) {
  if (child === beforeChild) {
    throw new Error('DevjsART: Can not insert node before itself');
  }
...
export function getCurrentGestureOffset(provider: GestureTimeline): number {
  throw new Error('startGestureTransition is not yet supported in devjs-art.');
}
