...
import DevjsSharedInternalsServer from 'devjs-server/src/DevjsSharedInternalsServer';
import DevjsSharedInternalsClient from 'shared/DevjsSharedInternals';
...
type DevjsMarkupNodeList =
  // This is the intersection of DevjsNodeList and DevjsClientValue minus
  // Client/ServerReferences.
  | component(...props: any)
  | LazyComponent<DevjsMarkupNodeList, any>
  ...
  | void
  | bigint
  | $AsyncIterable<DevjsMarkupNodeList, DevjsMarkupNodeList, void>
  | $AsyncIterator<DevjsMarkupNodeList, DevjsMarkupNodeList, void>
  | Iterable<DevjsMarkupNodeList>
  | Iterator<DevjsMarkupNodeList>
  | Array<DevjsMarkupNodeList>
  | Promise<DevjsMarkupNodeList>; // Thenable<DevjsMarkupNodeList>
...
export function experimental_renderToHTML(
  ...
  return new Promise((resolve, reject) => {
      ...
      if (onError) {
        if (__DEV__) {
          const prevGetCurrentStackImpl =
            DevjsSharedInternalsServer.getCurrentStack;
          ...
          DevjsSharedInternalsServer.getCurrentStack =
            DevjsSharedInternalsClient.getCurrentStack;
          try {
    ...
    const root = getFlightRoot<DevjsNodeList>(flightResponse);
