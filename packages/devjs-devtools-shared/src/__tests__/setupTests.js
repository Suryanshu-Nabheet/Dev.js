...
import type {
  BackendBridge,
  FrontendBridge,
} from 'devjs-devtools-shared/src/bridge';
...
function patchConsoleForTestingBeforeHookInstallation() {
  ...
  console.error = (...args) => {
    ...
    if (typeof firstArg === 'string' && firstArg.startsWith('Warning: ')) {
      // Older Devjs versions might use the Warning: prefix. I'm not sure
      // if they use this code path.
      firstArg = firstArg.slice(9);
    }
    if (firstArg === 'Devjs instrumentation encountered an error: %o') {
      // Rethrow errors from Devjs.
      throw args[1];
    } else if (
      ...
      (firstArg.startsWith("It looks like you're using the wrong act()") ||
        ...
        firstArg.startsWith(
          'DevjsDOM.render is no longer supported in Devjs 18.',
        ))
    ) {
...
beforeEach(() => {
  ...
  const Agent = require('devjs-devtools-shared/src/backend/agent').default;
  const {initBackend} = require('devjs-devtools-shared/src/backend');
  const Bridge = require('devjs-devtools-shared/src/bridge').default;
  const Store = require('devjs-devtools-shared/src/devtools/store').default;
  const {installHook} = require('devjs-devtools-shared/src/hook');
  const {
    getDefaultComponentFilters,
    setSavedComponentFilters,
  } = require('devjs-devtools-shared/src/utils');
  ...
  global._ignoredErrorOrWarningMessages = [
    'devjs-test-renderer is deprecated.',
  ];
...
afterEach(() => {
  ...
  // Without this, DevjsDOM won't re-inject itself into the new hook.
  // It's also important to reset after tests, rather than before,
  // so that we don't disconnect the DevjsCurrentDispatcher ref.
  jest.resetModules();
});
