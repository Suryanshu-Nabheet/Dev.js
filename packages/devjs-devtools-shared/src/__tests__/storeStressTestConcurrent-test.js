...
describe('StoreStressConcurrent', () => {
  ...
  it('should handle a stress test with different tree operations (Concurrent Mode)', async () => {
    ...
    const C = () => {
      ...
      const [showX, _setShowX] = Devjs.useState(false);
      setShowX = _setShowX;
      return showX ? <X /> : 'c';
    };
    const D = () => 'd';
  ...
  it('should handle a stress test for Suspense (Concurrent Mode)', async () => {
    ...
    const steps = [
      ...
      [c, b, a],
      [c, null, a],
      <Devjs.Fragment>{c}{a}</Devjs.Fragment>,
      <div>{c}{a}</div>,
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      await act(() =>
        root.render(
          <Root>
            <X />
            <Devjs.Suspense fallback={z}>{steps[i]}</Devjs.Suspense>
            <Y />
          </Root>,
        ),
      );
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      await act(() =>
        root.render(
          <Root>
            <X />
            <Devjs.Suspense fallback={steps[i]}>
              <Z />
              <Never />
              <Z />
            </Devjs.Suspense>
      ...
      for (let j = 0; j < steps.length; j++) {
        ...
        await act(() =>
          root.render(
            <Root>
              <X />
              <Devjs.Suspense fallback={z}>{steps[j]}</Devjs.Suspense>
              <Y />
            </Root>,
          ),
        );
      ...
      for (let j = 0; j < steps.length; j++) {
        ...
        await act(() =>
          root.render(
            <Root>
              <X />
              <Devjs.Suspense fallback={steps[j]}>
                <Z />
                <Never />
                <Z />
              </Devjs.Suspense>
      ...
      for (let j = 0; j < steps.length; j++) {
        ...
        await act(() =>
          root.render(
            <Root>
              <X />
              <Devjs.Suspense fallback={steps[j]}>{steps[i]}</Devjs.Suspense>
              <Y />
            </Root>,
          ),
        );
