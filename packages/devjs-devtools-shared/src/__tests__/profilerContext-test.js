...
import type {Context} from 'devjs-devtools-shared/src/devtools/views/Profiler/ProfilerContext';
import type {DispatcherContext} from 'devjs-devtools-shared/src/devtools/views/Components/TreeContext';
...
describe('ProfilerContext', () => {
  ...
  beforeEach(() => {
    ...
    ProfilerContext =
      require('devjs-devtools-shared/src/devtools/views/Profiler/ProfilerContext').ProfilerContext;
    ProfilerContextController =
      require('devjs-devtools-shared/src/devtools/views/Profiler/ProfilerContext').ProfilerContextController;
    StoreContext =
  ...
  it('updates updates profiling support based on the attached roots (legacy render)', async () => {
    ...
    function ContextReader() {
      context = Devjs.useContext(ProfilerContext);
      return null;
    }
    ...
    await utils.actAsync(() => DevjsDOM.unmountComponentAtNode(containerA));
    expect(context.supportsProfiling).toBe(true);

    await utils.actAsync(() => DevjsDOM.unmountComponentAtNode(containerB));
    expect(context.supportsProfiling).toBe(false);
  });
  ...
  it('should auto-select the root ID matching the Components tab selection if it has profiling data (createRoot)', async () => {
    ...
    const rootOne = DevjsDOMClient.createRoot(containerOne);
    const rootTwo = DevjsDOMClient.createRoot(containerTwo);
  ...
  it('should maintain root selection between profiling sessions so long as there is data for that root (legacy render)', async () => {
    ...
    function ContextReader() {
      ...
      inspectedElementID =
        Devjs.useContext(TreeStateContext).inspectedElementID;
      return null;
    }
  ...
  it('should toggle profiling when the keyboard shortcut is pressed', async () => {
    // Context providers
    const Profiler =
      require('devjs-devtools-shared/src/devtools/views/Profiler/Profiler').default;
    const {
      TimelineContextController,
    } = require('devjs-devtools-timeline/src/TimelineContext');
    const {
      SettingsContextController,
    } = require('devjs-devtools-shared/src/devtools/views/Settings/SettingsContext');
    const {
      ModalDialogContextController,
    } = require('devjs-devtools-shared/src/devtools/views/ModalDialog');
    ...
    const profilerRoot = DevjsDOMClient.createRoot(profilerContainer);
