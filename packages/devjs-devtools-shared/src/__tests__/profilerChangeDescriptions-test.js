...
describe('Profiler change descriptions', () => {
  ...
  it('should identify useContext as the cause for a re-render', () => {
    ...
    function Child() {
      const context = Devjs.useContext(Context);
      return context;
    }
    ...
    const MemoizedChild = Devjs.memo(Child, areEqual);
    const ForwardRefChild = Devjs.forwardRef(
    ...
    const App = function App() {
      const [val, dispatch] = Devjs.useReducer(x => x + 1, 0);
