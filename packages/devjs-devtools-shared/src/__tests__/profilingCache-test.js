...
describe('ProfilingCache', () => {
  ...
  it('should collect data for each root (including ones added or mounted after profiling started) (legacy render)', () => {
    ...
    const MemoizedChild = Devjs.memo(Child);
    ...
    utils.act(() => DevjsDOM.unmountComponentAtNode(containerB));
    utils.act(() =>
      legacyRender(
        <RootA>
          <Parent count={0} />
    ...
    utils.act(() => DevjsDOM.unmountComponentAtNode(containerA));
  ...
  it('should collect data for each root (including ones added or mounted after profiling started) (createRoot)', () => {
    ...
    const rootC = DevjsDOMClient.createRoot(containerC);
  ...
  it('should properly detect changed hooks', () => {
    ...
    const Component = ({count, string}) => {
      ...
      setState = Devjs.useState('abc')[1];
      dispatch = Devjs.useReducer(reducer, {value: true})[1];
      Devjs.useSyncExternalStore(
        subscribe,
        getClientSnapshot,
        getServerSnapshot,
      );
      ...
      Devjs.useContext(Context);

      // These hooks never change in a way that schedules an update.
      Devjs.useCallback(() => () => {}, [string]);
      Devjs.useMemo(() => string, [string]);
      Devjs.useCallback(() => () => {}, [count]);
      Devjs.useMemo(() => count, [count]);
      Devjs.useCallback(() => () => {});
      Devjs.useMemo(() => string);

      // These hooks never change in a way that schedules an update.
      Devjs.useEffect(() => {}, [string]);
      Devjs.useLayoutEffect(() => {}, [string]);
      Devjs.useEffect(() => {}, [count]);
      Devjs.useLayoutEffect(() => {}, [count]);
      Devjs.useEffect(() => {});
  ...
  it('should detect what hooks changed in a render with custom and composite hooks', () => {
    ...
    function useCustomHook() {
      const [value, setValue] = Devjs.useState('custom');
      Devjs.useEffect(() => {}, [value]);
    ...
    const Component = () => {
      // Hook 0: useState
      const [state, _setState] = Devjs.useState('initial');
      setState = _setState;

      // Hook 1: useSyncExternalStore (composite hook - internally uses multiple hooks)
      const storeValue = Devjs.useSyncExternalStore(
        subscribe,
        getSnapshot,
        getSnapshot,
      );
      ...
      const [actionState, _actionStateDispatch] = Devjs.useActionState(
        (_prev, action) => action,
        'action-initial',
      );
      ...
      const [finalState, _setFinalState] = Devjs.useState('final');
  ...
  it('should detect context changes or lack of changes with conditional use()', () => {
    const ContextA = Devjs.createContext(0);
    const ContextB = Devjs.createContext(1);
    let setState = null;

    const Component = () => {
      // These hooks may change and initiate re-renders.
      let state;
      [state, setState] = Devjs.useState('abc');

      let result = state;

      if (state.includes('a')) {
        result += Devjs.use(ContextA);
      }

      result += Devjs.use(ContextB);
  ...
  it('should calculate durations correctly for suspended views', async () => {
    ...
    if (Devjs.version.startsWith('17')) {
      // Devjs 17 will mount all children until it suspends in a LegacyHidden
  ...
  // @devjsVersion >= 18.0.0
  // @devjsVersion <= 18.2.0
  it('should handle unexpectedly shallow suspense trees for devjs v[18.0.0 - 18.2.0] (legacy render)', () => {
    utils.act(() => store.profilerStore.startProfiling());
    utils.act(() =>
      legacyRender(<Devjs.Suspense />, document.createElement('div')),
    );
    utils.act(() => store.profilerStore.stopProfiling());
  ...
  // @devjsVersion >= 18.0.0
  // @devjsVersion <= 18.2.0
  it('should handle unexpectedly shallow suspense trees for devjs v[18.0.0 - 18.2.0] (createRoot)', () => {
    utils.act(() => store.profilerStore.startProfiling());
    utils.act(() => render(<Devjs.Suspense />));
    utils.act(() => store.profilerStore.stopProfiling());
  ...
  // See https://github.com/Suryanshu-Nabheet/dev.js/issues/18831
  // @devjsVersion >= 16.9
  it('should not crash during route transitions with Suspense', () => {
    const RouterContext = Devjs.createContext();
    ...
    // Mimics https://github.com/DevjsTraining/devjs-router/blob/master/packages/devjs-router/modules/Router.js
    function Router({children}) {
      const [path, setPath] = Devjs.useState('/');
      return (
    ...
    // Mimics https://github.com/DevjsTraining/devjs-router/blob/master/packages/devjs-router/modules/Switch.js
    function Switch({children}) {
      return (
        <RouterContext.Consumer>
          {context => {
            let element = null;
            Devjs.Children.forEach(children, child => {
              if (context.path === child.props.path) {
                element = child.props.children;
              }
            });
            return element ? Devjs.cloneElement(element) : null;
          }}
        </RouterContext.Consumer>
      );
    }

    // Mimics https://github.com/DevjsTraining/devjs-router/blob/master/packages/devjs-router/modules/Route.js
    function Route({children, path}) {
      return null;
    }

    const linkRef = Devjs.createRef();

    // Mimics https://github.com/DevjsTraining/devjs-router/blob/master/packages/devjs-router-dom/modules/Link.js
  ...
  it('components that were deleted and added to updaters during the layout phase should not crash', () => {
    ...
    function Child() {
      const [, setState] = Devjs.useState(false);
    ...
    function App() {
      const [childUnmounted, _setChildUnmounted] = Devjs.useState(false);
