...
} from 'devjs-devtools-shared/src/__tests__/utils';
...
describe('InspectedElement', () => {
  ...
  beforeEach(() => {
    ...
    InspectedElementContext =
      require('devjs-devtools-shared/src/devtools/views/Components/InspectedElementContext').InspectedElementContext;
    InspectedElementContextController =
      require('devjs-devtools-shared/src/devtools/views/Components/InspectedElementContext').InspectedElementContextController;
    SettingsContextController =
      require('devjs-devtools-shared/src/devtools/views/Settings/SettingsContext').SettingsContextController;
    StoreContext =
    ...
    TreeStateContext =
      require('devjs-devtools-shared/src/devtools/views/Components/TreeContext').TreeStateContext;
    TreeDispatcherContext =
      require('devjs-devtools-shared/src/devtools/views/Components/TreeContext').TreeDispatcherContext;
  ...
  function useInspectedElement() {
    const {inspectedElement} = Devjs.useContext(InspectedElementContext);
    return inspectedElement;
  }

  function useInspectElementPath() {
    const {inspectPaths} = Devjs.useContext(InspectedElementContext);
    return inspectPaths;
  }
        ...
        "hooks": [
          {
            ...
            "hookSource": {
              ...
              "fileName": "devjs-devtools-shared/src/__tests__/inspectedElement-test.js",
  ...
  it('should have hasLegacyContext flag set to either "true" or "false" depending on which context API is used.', async () => {
    ...
    // Legacy Context API.
    class LegacyContextProvider extends Devjs.Component<any> {
    ...
    class LegacyContextConsumer extends Devjs.Component<any> {
      static contextTypes = {
        bool: PropTypes.bool,
      };
      render() {
    ...
    // Modern Context API
    const BoolContext = Devjs.createContext(contextData.bool);
    BoolContext.displayName = 'BoolContext';

    class ModernContextType extends Devjs.Component<any> {
    ...
    const ModernContext = Devjs.createContext();
  ...
  it('should poll for updates for the currently selected element', async () => {
    ...
    // 1. Running only check-for-updates schedules a transition that Devjs never renders.
  ...
  it('should not re-render a function with hooks if it did not update since it was last inspected', async () => {
    ...
    // Target should have been rendered once (by DevjsDOM) and once by DevTools for inspection.
    inspectedElement = await inspectElementAtIndex(1);
    expect(targetRenderCount).toBe(2);
    expect(inspectedElement.props).toMatchInlineSnapshot(`
      {
  ...
  // See github.com/Suryanshu-Nabheet/dev.js/issues/22241#issuecomment-931299972
  it('should properly recover from a cache miss on the frontend', async () => {
    ...
    const {
      clearCacheForTests,
    } = require('devjs-devtools-shared/src/inspectedElementMutableSource');
    clearCacheForTests();
    ...
    await utils.actAsync(() =>
      render(
        <Example
          ...
          proxy={proxyInstance}
          devjs_element={<span />}
          devjs_lazy={Devjs.lazy(async () => ({default: 'foo'}))}
          regexp={/abc/giu}
          set={setShallow}
  ...
  it('should support Thenables in Devjs 19', async () => {
    const Example = () => null;
  ...
  it('should support Promises in Devjs 18', async () => {
    const Example = () => null;
  ...
  it.skip('should inspect hooks for components that only use context (legacy render)', async () => {
    const Context = Devjs.createContext(true);
    const Example = () => {
      const value = Devjs.useContext(Context);
      return value;
    };
  ...
  it('should enable inspected values to be stored as global variables', async () => {
    ...
    jest.runOnlyPendingTimers();
    expect(console.log).toHaveBeenCalledWith('$devjsTemp0');
    expect(global.$devjsTemp0).toBe(nestedObject);
    ...
    jest.runOnlyPendingTimers();
    expect(console.log).toHaveBeenCalledWith('$devjsTemp1');
    expect(global.$devjsTemp1).toBe(nestedObject.a.b);
  });
  ...
  // See github.com/Suryanshu-Nabheet/dev.js/issues/21654
  ...
  // TODO(hoxyq): Enable this test for versions ~18, currently broken
  // Regression test for github.com/Suryanshu-Nabheet/dev.js/issues/22099
  // @devjsVersion <= 18.2
  // eslint-disable-next-line jest/no-disabled-tests
  ...
  // @devjsVersion <= 18.2
  // eslint-disable-next-line jest/no-disabled-tests
  it.skip('should display the root type for DevjsDOM.hydrate', async () => {
    const Example = () => <div />;
    ...
    await utils.actAsync(() => {
      ...
      withErrorsOrWarningsIgnored(
        ['DevjsDOM.hydrate has not been supported since Devjs 18'],
        () => {
          DevjsDOM.hydrate(<Example />, container);
        },
      );
    }, false);
  ...
  // @devjsVersion <= 18.2
  // eslint-disable-next-line jest/no-disabled-tests
  it.skip('should display the root type for DevjsDOM.render', async () => {
    const Example = () => <div />;
  ...
  it('should display the root type for DevjsDOMClient.hydrateRoot', async () => {
    const Example = () => <div />;
  ...
  it('should display the root type for DevjsDOMClient.createRoot', async () => {
    const Example = () => <div />;

    await utils.actAsync(() => {
      const container = document.createElement('div');
      DevjsDOMClient.createRoot(container).render(<Example />);
    }, false);
  ...
  describe('$r', () => {
    ...
    it('should support memoized function components', async () => {
      const Example = Devjs.memo(function Example(props) {
        const [count] = Devjs.useState(1);
        return count;
      });
    ...
    it('should support forward refs', async () => {
      const Example = Devjs.forwardRef(function Example(props, ref) {
        const [count] = Devjs.useState(1);
        return count;
      });
  ...
  describe('inline errors and warnings', () => {
    ...
    it('from devjs get recorded without a component stack', async () => {
      const Example = () => {
        return [<div />];
      };
  ...
  describe('error boundary', () => {
    it('can toggle error', async () => {
      class LocalErrorBoundary extends Devjs.Component<any> {
  ...
  it('should properly handle when components filters are updated', async () => {
    ...
    let dispatch;
    const Capture = () => {
      dispatch = Devjs.useContext(TreeDispatcherContext);
      state = Devjs.useContext(TreeStateContext);
      return null;
    };
