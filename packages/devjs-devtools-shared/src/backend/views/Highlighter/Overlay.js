...
import type Agent from 'devjs-devtools-shared/src/backend/agent';
...
export default class Overlay {
  ...
  constructor(agent: Agent) {
    ...
    const currentWindow = window.__devjs_DEVTOOLS_TARGET_WINDOW__ || window;
    this.window = currentWindow;

    // When opened in shells/dev, the tooltip should be bound by the app iframe, not by the topmost window.
    const tipBoundsWindow = window.__devjs_DEVTOOLS_TARGET_WINDOW__ || window;
  ...
  inspect(nodes: $ReadOnlyArray<HTMLElement | Text>, name?: ?string) {
    // We can't get the size of text nodes or comment nodes. Devjs as of v15
