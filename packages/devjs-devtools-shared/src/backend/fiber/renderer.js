...
import type {HooksNode, HooksTree} from 'devjs-debug-tools/src/DevjsDebugHooks';
...
import {
  ...
  gte,
  serializeToString,
} from 'devjs-devtools-shared/src/backend/utils';
import {
  extractLocationFromComponentStack,
  extractLocationFromOwnerStack,
  parseStackTrace,
} from 'devjs-devtools-shared/src/backend/utils/parseStackTrace';
import {
  cleanForBridge,
  copyWithDelete,
  copyWithRename,
...
import {inspectHooksOfFiber} from 'devjs-debug-tools';
import {
...
import {getIODescription} from 'shared/DevjsIODescription';
...
import type {Fiber, FiberRoot} from 'devjs-reconciler/src/DevjsInternalTypes';
import type {
  ChangeDescription,
  CommitDataBackend,
  DevToolsHook,
...
type FiberInstance = {
  ...
  source: null | string | Error | DevjsFunctionLocation, // source location of this component function, or owned child stack
  ...
  suspendedBy: null | Array<DevjsAsyncInfo>, // things that suspended in the children position of this component
...
type FilteredFiberInstance = {
  ...
  firstChild: null | DevToolsInstance,
  nextSibling: null | DevToolsInstance,
  source: null | string | Error | DevjsFunctionLocation, // always null here.
  logCount: number, // total number of errors/warnings last seen
  treeBaseDuration: number, // the profiled time of the last render of this subtree
  suspendedBy: null | Array<DevjsAsyncInfo>, // only used at the root
  suspenseNode: null | SuspenseNode,
  data: Fiber, // one of a Fiber pair
};
...
// It's basically a virtual Fiber. This is not a semantic concept in Devjs.
// It only exists as a virtual concept to let the same Element in the DevTools
// persist. To be selectable separately from all DevjsComponentInfo and overtime.
type VirtualInstance = {
  ...
  firstChild: null | DevToolsInstance,
  nextSibling: null | DevToolsInstance,
  source: null | string | Error | DevjsFunctionLocation, // source location of this server component, or owned child stack
  logCount: number, // total number of errors/warnings last seen
  treeBaseDuration: number, // the profiled time of the last render of this subtree
  suspendedBy: null | Array<DevjsAsyncInfo>, // things that blocked the server component's child from rendering
  suspenseNode: null,
  ...
  data: DevjsComponentInfo,
};

function createVirtualInstance(
  debugEntry: DevjsComponentInfo,
): VirtualInstance {
  return {
...
// A Generic Rect super type which can include DOMRect and other objects with similar shape like in Devjs Native.
...
type SuspenseNode = {
  ...
  suspendedBy: Map<DevjsIOInfo, Set<DevToolsInstance>>, // Tracks which data we're suspended by and the children that suspend it.
  ...
  // a promise, an older version of Devjs or because we're inspecting prod.
...
// Some environments (e.g. Devjs Native / Hermes) don't support the performance API yet.
...
export function getInternalDevjsConstants(version: string): {
  getDisplayNameForFiber: getDisplayNameForFiberType,
  getTypeSymbol: getTypeSymbolType,
  DevjsPriorityLevels: DevjsPriorityLevelsType,
  DevjsTypeOfWork: WorkTagMap,
  StrictModeBits: number,
  SuspenseyImagesMode: number,
} {
  // **********************************************************
  // The section below is copied from files in Devjs repo.
  // Keep it in sync, and add version guards if it changes.
  //
  // Technically these priority levels are invalid for versions before 16.9,
  // but 16.9 is the first version to report priority level to DevTools,
  // so we can avoid checking for earlier versions and support pre-16.9 canary releases in the process.
  let DevjsPriorityLevels: DevjsPriorityLevelsType = {
  ...
  let DevjsTypeOfWork: WorkTagMap = ((null: any): WorkTagMap);
  ...
  function getDisplayNameForFiber(
    ...
    if (
      ...
      (fiber.updateQueue?.memoCache != null ||
        (Array.isArray(fiber.memoizedState?.memoizedState) &&
          fiber.memoizedState.memoizedState[0]?.[devjs_MEMO_CACHE_SENTINEL]) ||
        fiber.memoizedState?.memoizedState?.[devjs_MEMO_CACHE_SENTINEL])
    ) {
    ...
    switch (tag) {
      ...
      case LazyComponent:
        ...
        // Once a Lazy component loads its inner component, Devjs replaces the tag and type.
        // This display name will only show up in console logs when DevTools DEBUG mode is on.
        return 'Lazy';
      case MemoComponent:
      case SimpleMemoComponent:
        // Display name in Devjs does not use `Memo` as a wrapper but fallback name.
        return getWrappedDisplayName(
          elementType,
          resolvedType,
          'Memo',
...
function getPublicInstance(instance: HostInstance): HostInstance {
  ...
  if (typeof instance === 'object' && instance !== null) {
    ...
    // Devjs Native. Legacy. Paper.
    if (typeof instance._nativeTag === 'number') {
      return instance._nativeTag;
    }
  }

  // Devjs Web. Usually a DOM element.
  return instance;
}
...
export function attach(
  ...
  const {
    ...
    StrictModeBits,
    SuspenseyImagesMode,
  } = getInternalDevjsConstants(version);
  const {
    ActivityComponent,
    ClassComponent,
    ContextConsumer,
  ...
  if (typeof injectProfilingHooks === 'function') {
    const response = createProfilingHooks({
      ...
      workTagMap: DevjsTypeOfWork,
      devjsVersion: version,
    });
  ...
  function shouldFilterFiber(fiber: Fiber): boolean {
    ...
    switch (tag) {
      case DehydratedSuspenseComponent:
        ...
        // https://github.com/bvaughn/devjs-devtools-experimental/issues/197
        return true;
      case HostPortal:
      case HostText:
      case LegacyHiddenComponent:
    ...
    /* DISABLED: https://github.com/Suryanshu-Nabheet/dev.js/pull/28417
  ...
  function untrackFiber(nearestInstance: DevToolsInstance, fiber: Fiber) {
    ...
    // is Devjs Native Paper which tracks tags but that support is eventually going away
  ...
  function getContextChanged(prevFiber: Fiber, nextFiber: Fiber): boolean {
    ...
    while (prevContext && nextContext) {
      // Note this only works for versions of Devjs that support this key (e.v. 18+)
      // For older versions, there's no good way to read the current context value after render has completed.
      // This is because Devjs maintains a stack of context values during render,
  ...
  function didFiberRender(prevFiber: Fiber, nextFiber: Fiber): boolean {
    switch (nextFiber.tag) {
      ...
      case ForwardRef:
        ...
        // releasing DevTools in lockstep with Devjs, we should import a
        // function from the reconciler instead.
        const PerformedWork = 0b000000000000000000000000001;
        return (getFiberFlags(nextFiber) & PerformedWork) === PerformedWork;
      // Note: ContextConsumer only gets PerformedWork effect in 16.3.3+
      // so it won't get highlighted with Devjs 16.3.0 to 16.3.2.
      default:
  ...
  function flushPendingEvents(root: FiberInstance | null): void {
    ...
    if (numUnmountSuspenseIDs > 0) {
      ...
      // They were inserted parents-first by Devjs, but we want children-first.
  ...
  function measureHostInstance(instance: HostInstance): null | Array<Rect> {
    ...
    // TODO: Consider making this capability injected by the DevjsRenderer.
    if (typeof instance !== 'object' || instance === null) {
      return null;
    }
    if (
  ...
  function recordReconnect(
    ...
    if (isRoot) {
      ...
      // This check is a guard to handle a Devjs element that has been modified
  ...
  function insertSuspendedBy(asyncInfo: DevjsAsyncInfo): void {
    ...
    if (parentInstance.kind === FIBER_INSTANCE) {
      ...
      if (
        ...
        if (parent === null) {
          ...
          throw new Error(
            'Did not find a suitable instance for this async info. This is a bug in Devjs.',
          );
        }
  ...
  function getAwaitInSuspendedByFromIO(
    suspensedBy: Array<DevjsAsyncInfo>,
    ioInfo: DevjsIOInfo,
  ): null | DevjsAsyncInfo {
  ...
  function getVirtualEndTime(ioInfo: DevjsIOInfo): number {
    if (ioInfo.env != null) {
  ...
  function removePreviousSuspendedBy(
    instance: DevToolsInstance,
    previousSuspendedBy: null | Array<DevjsAsyncInfo>,
  ...
  function consumeSuspenseNodesOfExistingInstance(
    ...
    while (suspenseNode !== null) {
      ...
      if (
        ...
        previouslyReconciledSiblingSuspenseNode = suspenseNode;
        // While Devjs didn't rerender this node, it's possible that it was affected by
  ...
  function getSecondaryEnvironmentName(
    ...
    if (debugInfo != null) {
      const componentInfo: DevjsComponentInfo = (debugInfo[index]: any);
  ...
  function trackDebugInfoFromLazyType(fiber: Fiber): void {
    ...
    if (typeSymbol === LAZY_SYMBOL_STRING) {
      const debugInfo: ?DevjsDebugInfo = type._debugInfo;
      if (debugInfo) {
        for (let i = 0; i < debugInfo.length; i++) {
          const debugEntry = debugInfo[i];
          if (debugEntry.awaited) {
            const asyncInfo: DevjsAsyncInfo = (debugEntry: any);
  ...
  const hostAsyncInfoCache: WeakMap<{...}, DevjsAsyncInfo> = new WeakMap();
  ...
  function trackDebugInfoFromHostComponent(
    ...
    if ((fiber.mode & SuspenseyImagesMode) === 0) {
      ...
      // all images and we'd need some other check for if the version of Devjs has that enabled.
      return;
    }
    ...
    while (fiber !== null && fiber !== lastChild) {
      ...
      if (fiber._debugInfo) {
        for (let i = 0; i < fiber._debugInfo.length; i++) {
          ...
          const componentInfo: DevjsComponentInfo = (debugEntry: any);
    ...
    if (isProfiling) {
      ...
      if (
        ...
        if (actualDuration != null) {
          // The actual duration reported by Devjs includes time spent working on children.
  ...
  function updateVirtualInstanceRecursively(
    ...
    try {
      ...
      // DevjsComponentInfo instance, such as when refreshing Server Components, then
          ...
          if (level === virtualLevel) {
            if (
              ...
              if (
                ...
                componentInfo.key !== devjs_OPTIMISTIC_KEY
              ) {
  ...
  function updateFiberRecursively(
    ...
    try {
      ...
      // Devjs re-parents this content within a hidden Fragment while the fallback is showing.
      ...
      // inside DevjsFiberBeginWork in the Devjs source code.
      if (prevDidTimeout && nextDidTimeOut) {
        ...
        // Note: don't emulate fallback unmount because Devjs actually did it.
        // 2. Mount primary set
  ...
  function handlePostCommitFiberRoot(root: FiberRoot) {
    ...
    if (needsToFlushComponentLogs) {
      ...
      if (rootInstance === undefined) {
        throw new Error(
          'Should have a root instance for a committed root. This is a bug in Devjs DevTools.',
        );
      }
  ...
  function handleCommitFiberRoot(
    ...
    if (isProfiling && isProfilingSupported) {
      ...
      currentCommitProfilingMetadata = {
        ...
        updaters: null,
        // Initialize to null; if new enough Devjs version is running,
  ...
  function instanceToSerializedElement(
    ...
    if (instance.kind === FIBER_INSTANCE) {
      ...
      return {
        ...
        id: instance.id,
        key:
          fiber.key === devjs_OPTIMISTIC_KEY
            ? 'Devjs.optimisticKey'
            : fiber.key,
        env: null,
        stack:
  ...
  function getUnfilteredOwner(
    owner: DevjsComponentInfo | Fiber | null | void,
  ): DevjsComponentInfo | Fiber | null {
    if (owner == null) {
      return null;
    }
    if (typeof owner.tag === 'number') {
      ...
      const ownerInfo: DevjsComponentInfo = (owner: any); // Refined
  ...
  function findNearestOwnerInstance(
    parentInstance: null | DevToolsInstance,
    owner: void | null | DevjsComponentInfo | Fiber,
  ...
  function getSuspendedByOfSuspenseNode(
    ...
    // Collect all DevjsAsyncInfo that was suspending this SuspenseNode but
  ...
  function getSuspendedByOfInstance(
    ...
    const foundIOEntries: Set<DevjsIOInfo> = new Set();
    const streamEntries: Map<Promise<mixed>, DevjsAsyncInfo> = new Map();
    const result: Array<SerializedAsyncInfo> = [];
    ...
    for (let i = 0; i < hooks.length; i++) {
      ...
      if (debugInfo != null && debugInfo.indexOf(asyncInfo) !== -1) {
        ...
        if (
          ...
          // Unfortunately this is in a slightly different format. TODO: Unify HookNode with DevjsCallSite.
          const callSite: DevjsCallSite = [
            source.functionName,
            source.fileName,
            source.lineNumber,
            source.columnNumber,
  ...
  function serializeAsyncInfo(
    ...
    let awaitStack =
      asyncInfo.debugStack == null
        ? null
        : // While we have a DevjsStackTrace on ioInfo.stack, that will point to the location on
  ...
  // Fast path props lookup for Devjs Native style editor.
  ...
  function inspectFiberInstanceRaw(
    ...
    if (isErrorBoundary(fiber)) {
      ...
      //
      // TODO: This flag is a leaked implementation detail. Once we start
      // releasing DevTools in lockstep with Devjs, we should import a function
  ...
  function inspectElement(
    ...
    try {
      ...
      // the error name is synced with DevjsDebugHooks
      if (error.name === 'DevjsDebugToolsRenderError') {
        ...
        if (error.cause != null) {
          ...
          console.error(
            'Devjs DevTools encountered an error while trying to inspect hooks. ' +
      ...
      // the error name is synced with DevjsDebugHooks
      if (error.name === 'DevjsDebugToolsUnsupportedHookError') {
        return {
          type: 'error',
          ...
          responseID: requestID,
          message:
            'Unsupported hook in the devjs-debug-tools package: ' +
            error.message,
        };
      }
    ...
    if (typeof getTimelineData === 'function') {
      ...
      if (currentTimelineData) {
        ...
        timelineData = {
          ...
          laneToDevjsMeasureKeyValueArray: Array.from(
            laneToDevjsMeasureMap.entries(),
          ),
        };
      }
    }
  ...
  // Devjs will switch between these implementations depending on whether
  ...
  function shouldErrorFiberAccordingToMap(fiber: any): boolean {
    if (typeof setErrorHandler !== 'function') {
      throw new Error(
        'Expected overrideError() to not get called for earlier Devjs versions.',
      );
    }
    ...
    if (status === false) {
      ...
      // since DevTools needs to tell Devjs to act differently than it normally would
      // (don't just re-render the failed boundary, but reset its errored state too).
      // So we can only clear it after telling Devjs to reset the state.
      // Technically this is premature and we should schedule it for later,
      // since the render could always fail without committing the updated error boundary,
      // but since this is a DEV-only feature, the simplicity is worth the trade off.
      forceErrorForFibers.delete(fiber);
      if (forceErrorForFibers.size === 0) {
        // Last override is gone. Switch Devjs back to fast path.
        setErrorHandler(shouldErrorFiberAlwaysNull);
      }
      return false;
    }
  ...
  function overrideError(id: number, forceError: boolean) {
    ...
    if (forceErrorForFibers.size === 1) {
      // First override is added. Switch Devjs to slower path.
      setErrorHandler(shouldErrorFiberAccordingToMap);
    }
  ...
  function overrideSuspense(id: number, forceFallback: boolean) {
    if (
      ...
      throw new Error(
        'Expected overrideSuspense() to not get called for earlier Devjs versions.',
      );
    }
    const devtoolsInstance = idToDevToolsInstanceMap.get(id);
    if (devtoolsInstance === undefined) {
  ...
  function overrideSuspenseMilestone(suspendedSet: Array<FiberInstance['id']>) {
    if (
      ...
      throw new Error(
        'Expected overrideSuspenseMilestone() to not get called for earlier Devjs versions.',
      );
    }
    ...
    for (let i = 0; i < suspendedSet.length; ++i) {
      ...
      if (instance.kind === FIBER_INSTANCE) {
        ...
        if (
          ...
          // For now we rely on Devjs's batching of updates.
          scheduleUpdate(fiber);
          resuspended = true;
        }
      } else {
  ...
  function getPathFrame(fiber: Fiber): PathFrame {
    ...
    return {
      displayName,
      key: key === devjs_OPTIMISTIC_KEY ? null : key,
      index,
    };
  }
  ...
  function getVirtualPathFrame(virtualInstance: VirtualInstance): PathFrame {
    return {
      ...
      key:
        ...
        virtualInstance.data.key === devjs_OPTIMISTIC_KEY
          ? null
