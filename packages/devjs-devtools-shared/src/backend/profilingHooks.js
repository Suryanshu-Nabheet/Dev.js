...
import type {Wakeable} from 'shared/DevjsTypes';
import type {
  BatchUID,
  InternalModuleSourceToRanges,
  LaneToLabelMap,
  DevjsComponentMeasure,
  DevjsLane,
  DevjsMeasure,
  DevjsMeasureType,
  ...
  SuspenseEvent,
  TimelineData,
} from 'devjs-devtools-timeline/src/types';

import isArray from 'shared/isArray';
import {
  devjs_TOTAL_NUM_LANES,
  SCHEDULING_PROFILER_VERSION,
} from 'devjs-devtools-timeline/src/constants';
...
export function createProfilingHooks({
  ...
  let currentDevjsComponentMeasure: DevjsComponentMeasure | null = null;
  let currentDevjsMeasuresStack: Array<DevjsMeasure> = [];
  ...
  function getInternalModuleRanges() {
    ...
    if (
      typeof __devjs_DEVTOOLS_GLOBAL_HOOK__ !== 'undefined' &&
      typeof __devjs_DEVTOOLS_GLOBAL_HOOK__.getInternalModuleRanges ===
        'function'
    ) {
      ...
      const ranges = __devjs_DEVTOOLS_GLOBAL_HOOK__.getInternalModuleRanges();

      // This check would not be required,
      // except that it's possible for things to override __devjs_DEVTOOLS_GLOBAL_HOOK__.
      if (isArray(ranges)) {
        return ranges;
      }
    }
  ...
  function laneToLanesArray(lanes: Lane) {
    ...
    let lane = 1;
    for (let index = 0; index < devjs_TOTAL_NUM_LANES; index++) {
      if (lane & lanes) {
  ...
  function markMetadata() {
    markAndClear(`--devjs-version-${devjsVersion}`);
    ...
    if (ranges) {
      for (let i = 0; i < ranges.length; i++) {
        ...
        if (isArray(range) && range.length === 2) {
          ...
          markAndClear(`--devjs-internal-module-start-${startStackFrame}`);
          markAndClear(`--devjs-internal-module-stop-${stopStackFrame}`);
        }
      }
    }
    ...
    if (laneToLabelMap != null) {
      ...
      markAndClear(`--devjs-lane-labels-${labels}`);
    }
  }
  ...
  function recordDevjsMeasureStarted(
    ...
    let depth = 0;
    if (currentDevjsMeasuresStack.length > 0) {
      const top =
        currentDevjsMeasuresStack[currentDevjsMeasuresStack.length - 1];
      depth = top.type === 'render-idle' ? top.depth : top.depth + 1;
    }

    const lanesArray = laneToLanesArray(lanes);

    const devjsMeasure: DevjsMeasure = {
      type,
    ...
    currentDevjsMeasuresStack.push(devjsMeasure);

    if (currentTimelineData) {
      const {batchUIDToMeasuresMap, laneToDevjsMeasureMap} =
        currentTimelineData;

      let devjsMeasures = batchUIDToMeasuresMap.get(currentBatchUID);
      if (devjsMeasures != null) {
        devjsMeasures.push(devjsMeasure);
      } else {
        batchUIDToMeasuresMap.set(currentBatchUID, [devjsMeasure]);
      }

      lanesArray.forEach(lane => {
        devjsMeasures = laneToDevjsMeasureMap.get(lane);
        if (devjsMeasures) {
          devjsMeasures.push(devjsMeasure);
        }
      });
  ...
  function recordDevjsMeasureCompleted(type: DevjsMeasureType): void {
    const currentTime = getRelativeTime();

    if (currentDevjsMeasuresStack.length === 0) {
      console.error(
        'Unexpected type "%s" completed at %sms while currentDevjsMeasuresStack is empty.',
        type,
        currentTime,
      );
    ...
    const top = currentDevjsMeasuresStack.pop();
  ...
  function markCommitStarted(lanes: Lanes): void {
    ...
    recordDevjsMeasureStarted('commit', lanes);
  ...
  function markCommitStopped(): void {
    ...
    recordDevjsMeasureCompleted('commit');
    recordDevjsMeasureCompleted('render-idle');
    if (supportsUserTimingV3) {
  ...
  function markComponentRenderStopped(): void {
    ...
    if (currentDevjsComponentMeasure) {
      if (currentTimelineData) {
      ...
      currentDevjsComponentMeasure.duration =
        // $FlowFixMe[incompatible-use] found when upgrading Flow
        getRelativeTime() - currentDevjsComponentMeasure.timestamp;
      currentDevjsComponentMeasure = null;
    }
  ...
  function markLayoutEffectsStarted(lanes: Lanes): void {
    ...
    recordDevjsMeasureStarted('layout-effects', lanes);
    if (supportsUserTimingV3) {
  ...
  function markLayoutEffectsStopped(): void {
    ...
    recordDevjsMeasureCompleted('layout-effects');
    if (supportsUserTimingV3) {
  ...
  function markPassiveEffectsStarted(lanes: Lanes): void {
    ...
    recordDevjsMeasureStarted('passive-effects', lanes);
    if (supportsUserTimingV3) {
  ...
  function markPassiveEffectsStopped(): void {
    ...
    recordDevjsMeasureCompleted('passive-effects');
    if (supportsUserTimingV3) {
  ...
  function markRenderStarted(lanes: Lanes): void {
    ...
    if (
      currentDevjsMeasuresStack.length === 0 ||
      currentDevjsMeasuresStack[currentDevjsMeasuresStack.length - 1].type !==
        'render-idle'
    ) {
      recordDevjsMeasureStarted('render-idle', lanes);
    }

    recordDevjsMeasureStarted('render', lanes);
    if (supportsUserTimingV3) {
  ...
  function markRenderYielded(): void {
    ...
    recordDevjsMeasureCompleted('render');
    if (supportsUserTimingV3) {
  ...
  function markStateUpdateScheduled(fiber: Fiber, lane: Lane): void {
    ...
    // TODO (timeline) Record and cache component stack
    if (currentTimelineData) {
      const event: DevjsScheduleStateUpdateEvent = {
        componentName,
  ...
  function toggleProfilingStatus(
    ...
    if (isProfiling !== value) {
      ...
      if (isProfiling) {
          ...
          if (ranges) {
            for (let i = 0; i < ranges.length; i++) {
              ...
              if (isArray(range) && range.length === 2) {
                ...
                markAndClear(
                  `--devjs-internal-module-start-${startStackFrame}`,
                );
                markAndClear(`--devjs-internal-module-stop-${stopStackFrame}`);
              }
            }
        ...
        const laneToDevjsMeasureMap = new Map<DevjsLane, DevjsMeasure[]>();
        let lane = 1;
        for (let index = 0; index < devjs_TOTAL_NUM_LANES; index++) {
          laneToDevjsMeasureMap.set(lane, []);
          lane *= 2;
        }

        currentBatchUID = 0;
        currentDevjsComponentMeasure = null;
        currentDevjsMeasuresStack = [];
        ...
        if (recordTimeline) {
          currentTimelineData = {
            ...
            // Data logged by Devjs during profiling session.
            componentMeasures: [],
            schedulingEvents: [],
            suspenseEvents: [],
            thrownErrors: [],

            // Data inferred based on what Devjs logs.
        ...
        // If we succeed with moving Devjs DevTools Timeline Profiler to Performance panel, then Timeline Profiler would probably be removed.
