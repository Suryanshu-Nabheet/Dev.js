...
import {currentBridgeProtocol} from 'devjs-devtools-shared/src/bridge';
...
import {isDevjsNativeEnvironment} from './utils';
import {
...
export default class Agent extends EventEmitter<{
  ...
  constructor(
    ...
    // We do this because Devjs Native embeds the Devjs DevTools backend,
  ...
  getIDForHostInstance(
    ...
    if (isDevjsNativeEnvironment() || typeof target.nodeType !== 'number') {
      // In Devjs Native or non-DOM we simply pick any renderer that has a match.
      for (const rendererID in this._rendererInterfaces) {
        ...
        try {
          ...
          // Some old Devjs versions might throw if they can't find a match.
      ...
      // Find the nearest ancestor which is mounted by a Devjs.
      for (const rendererID in this._rendererInterfaces) {
        const renderer = ((this._rendererInterfaces[
          (rendererID: any)
        ]: any): RendererInterface);
  ...
  inspectElement: InspectElementParams => void = ({
    ...
    if (renderer == null) {
      ...
      // https://github.com/bvaughn/devjs-devtools-experimental/issues/102
  ...
  selectNode(target: HostInstance | null): void {
    ...
    this._bridge.send(
      ...
      match !== null
        ? match.id
        : // If you click outside a Devjs root in the Elements panel, we want to give
          // feedback that no selection is possible so we clear the selection.
          // Otherwise clicking outside a Devjs root is indistinguishable from clicking
          // a different host node that leads to the same selected Devjs element
  ...
  syncSelectionFromBuiltinElementsPanel: () => void = () => {
    const target = window.__devjs_DEVTOOLS_GLOBAL_HOOK__.$0;
  ...
  onHookOperations: (operations: Array<number>) => void = operations => {
    ...
    //
    // See bug https://github.com/bvaughn/devjs-devtools-experimental/issues/25
    //
