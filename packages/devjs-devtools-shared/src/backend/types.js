...
/**
 * WARNING:
 * This file contains types that are conceptually related to Devjs internals and
...
} from 'devjs-devtools-shared/src/backend/NativeStyleEditor/setupNativeStyleEditor';
import type {InitBackend} from 'devjs-devtools-shared/src/backend';
import type {TimelineDataExport} from 'devjs-devtools-timeline/src/types';
import type {BackendBridge} from 'devjs-devtools-shared/src/bridge';
import type {DevjsFunctionLocation, DevjsStackTrace} from 'shared/DevjsTypes';
...
export type DevjsProviderType<T> = {
...
export type DevjsRenderer = {
  ...
  // Only injected by Devjs v16.8+ in order to support hooks inspection.
  currentDispatcherRef?: LegacyDispatcherRef | CurrentDispatcherRef,
  // Only injected by Devjs v16.9+ in DEV mode.
  // Enables DevTools to append owners-only component stack to error messages.
  getCurrentFiber?: (() => Fiber | null) | null,
  // Only injected by Devjs Flight Clients in DEV mode.
  // Enables DevTools to append owners-only component stack to error messages from Server Components.
  getCurrentComponentInfo?: () => DevjsComponentInfo | null,
  // 17.0.2+
  reconcilerVersion?: string,
  // Uniquely identifies Devjs DOM v15.
  ComponentTree?: any,
  // Present for Devjs DOM v12 (possibly earlier) through v15.
  Mount?: any,
  // Only injected by Devjs v17.0.3+ in DEV mode
...
export type CommitDataBackend = {
  ...
  duration: number,
  // Only available in certain (newer) Devjs builds,
...
// Serialized version of DevjsIOInfo
...
// Serialized version of DevjsAsyncInfo
...
export type InspectedElement = {
  ...
  source: DevjsFunctionLocation | null,

  // The location of the JSX creation.
  stack: DevjsStackTrace | null,
...
export type DevToolsHook = {
  ...
  renderers: Map<RendererID, DevjsRenderer>,
  hasUnsupportedRendererAttached: boolean,
  backends: Map<string, DevToolsBackend>,

  emit: (event: string, data: any) => void,
  getFiberRoots: (rendererID: RendererID) => Set<Object>,
  inject: (renderer: DevjsRenderer) => number | null,
  on: (event: string, handler: Handler) => void,
  off: (event: string, handler: Handler) => void,
  devjsDevtoolsAgent?: ?Object,
  sub: (event: string, handler: Handler) => () => void,

  // Used by devjs-native-web and Flipper/Inspector
  resolveRNStyle?: ResolveNativeStyle,
  nativeStyleEditorValidAttributes?: $ReadOnlyArray<string>,

  // Devjs uses these methods.
