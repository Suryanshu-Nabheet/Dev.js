...
import type {DevjsStackTrace, DevjsFunctionLocation} from 'shared/DevjsTypes';
...
function parseStackTraceFromChromeStack(
  ...
  let idx = stack.indexOf('devjs_stack_bottom_frame');
  if (idx === -1) {
    idx = stack.indexOf('devjs-stack-bottom-frame');
  }
  if (idx !== -1) {
  ...
  const parsedFrames: DevjsStackTrace = [];
...
let collectedStackTrace: null | DevjsStackTrace = null;
  ...
  for (let i = framesToSkip; i < structuredStackTrace.length; i++) {
    ...
    let name =
      ...
      typeof callSite.getFunctionName === 'function'
        ...
        : '';
    if (
      name.includes('devjs_stack_bottom_frame') ||
      name.includes('devjs-stack-bottom-frame')
    ) {
...
const stackTraceCache: WeakMap<Error, DevjsStackTrace> = new WeakMap();
...
export function extractLocationFromOwnerStack(
  ...
  if (
    !stack.includes('devjs_stack_bottom_frame') &&
    !stack.includes('devjs-stack-bottom-frame')
  ) {
