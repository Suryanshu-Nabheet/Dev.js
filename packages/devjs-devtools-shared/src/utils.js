...
const LEGACY_devjs_PROVIDER_TYPE: symbol = Symbol.for('devjs.provider');
...
// Mirror https://github.com/Suryanshu-Nabheet/dev.js/blob/7c21bf72ace77094fd1910cc350a548287ef8350/packages/shared/getComponentName.js#L27-L37
...
export function getDisplayName(
  ...
  // github.com/Suryanshu-Nabheet/dev.js-devtools/issues/803
...
export function persistableComponentFilters(
  ...
  return componentFilters.filter(f => {
    return (
      ...
      // To find the original location, Devjs DevTools will perform symbolication, source maps are required for that.
...
// Pulled from devjs-compat
// https://github.com/developit/pdevjs-compat/blob/7c5de00e7c85e2ffd011bf3af02899b63f699d3a/src/index.js#L349
...
// Fork of packages/devjs-is/src/DevjsIs.js:30, but with legacy element type
// Which has been changed in https://github.com/Suryanshu-Nabheet/dev.js/pull/28813
...
export function getDisplayNameForDevjsElement(
  ...
  switch (elementType) {
    ...
    case LEGACY_devjs_PROVIDER_TYPE:
  ...
  switch (type) {
    ...
    case 'devjs_element':
      return `<${truncateForDisplay(
        getDisplayNameForDevjsElement(data) || 'Unknown',
      )} />`;
    case 'devjs_lazy':
      ...
      // In practice, this is really just coming from Devjs.lazy helper or Flight.
      const payload = data._payload;
      if (payload !== null && typeof payload === 'object') {
        if (payload._status === 0) {
          // Devjs.lazy constructor pending
          return `pending lazy()`;
        }
        if (payload._status === 1 && payload._result != null) {
          // Devjs.lazy constructor fulfilled
        ...
        if (payload._status === 2) {
          // Devjs.lazy constructor rejected
          if (showFormattedValue) {
        ...
        if (payload.status === 'pending' || payload.status === 'blocked') {
          // Devjs Flight pending
          return `pending lazy()`;
        }
        if (payload.status === 'fulfilled') {
          // Devjs Flight fulfilled
          if (showFormattedValue) {
...
// Expected to be used only by browser extension and devjs-devtools-inline
