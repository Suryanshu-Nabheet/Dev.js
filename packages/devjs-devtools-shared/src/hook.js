 ...
 * Note: this global hook __devjs_DEVTOOLS_GLOBAL_HOOK__ is a de facto public API.
...
} from 'devjs-devtools-shared/src/constants';
import attachRenderer from './attachRenderer';
import formatConsoleArguments from 'devjs-devtools-shared/src/backend/utils/formatConsoleArguments';
import formatWithStyles from 'devjs-devtools-shared/src/backend/utils/formatWithStyles';

// Devjs's custom built component stack strings match "\s{4}in"
...
// We add a suffix to some frames that older versions of Devjs didn't do.
...
export function installHook(
  ...
  if (target.hasOwnProperty('__devjs_DEVTOOLS_GLOBAL_HOOK__')) {
    return null;
  }

  function detectDevjsBuildType(renderer: DevjsRenderer) {
    try {
      if (typeof renderer.version === 'string') {
        ...
        // Devjs 16 uses flat bundles. If we report the bundle as production
      ...
      if (renderer.Mount && renderer.Mount._renderNewRootComponent) {
        ...
        // Check for Devjs DOM Stack < 15.1.0 in development.
        // If it contains "storedMeasure" call, it's wrapped in DevjsPerf (DEV only).
        ...
        if (renderRootCode.indexOf('should be a pure function') !== -1) {
          ...
          // (such as the official Devjs DEV UMD).
  ...
  function checkDCE(fn: Function) {
    // This runs for production versions of Devjs.
    ...
    try {
      ...
      if (code.indexOf('^_^') > -1) {
        ...
        setTimeout(function () {
          throw new Error(
            'Devjs is running in production mode, but dead code ' +
              'elimination has not been applied. Read how to correctly ' +
              'configure Devjs for production: ' +
              'https://devjs.dev/link/perf-use-production-build',
          );
        });
      }
    } catch (err) {}
  ...
  let uidCounter = 0;
  function inject(renderer: DevjsRenderer): number {
    const id = ++uidCounter;
    renderers.set(id, renderer);

    const devjsBuildType = hasDetectedBadDCE
      ? 'deadcode'
      : detectDevjsBuildType(renderer);
  ...
  function patchConsoleForStrictMode() {
    ...
    // by Devjs DevTools hook to append component stacks and other possible features.
      ...
      const overrideMethod: (...args: Array<any>) => void = (...args) => {
        ...
        for (const rendererInterface of hook.rendererInterfaces.values()) {
          ...
          try {
            ...
            // Don't let a DevTools or Devjs internal error interfere with logging.
            setTimeout(() => {
              throw error;
            }, 0);
          }
          ...
          try {
            if (settings.appendComponentStack && getComponentStack != null) {
              ...
              const topFrame = Error('devjs-stack-top-frame');
              ...
              if (match !== null) {
                ...
                if (componentStack !== '') {
                  ...
                  if (alreadyHasComponentStack) {
                    ...
                    // Otherwise we assume it was a non-Devjs stack.
                    if (
        ...
        if (settings.breakOnConsoleErrors) {
          // --- Welcome to debugging with Devjs DevTools ---
  ...
  const renderers = new Map<RendererID, DevjsRenderer>();
  ...
  const hook: DevToolsHook = {
    ...
    // Devjs v16 checks the hook for this to ensure DevTools is new enough.
    supportsFiber: true,

    // Devjs Flight Client checks the hook for this to ensure DevTools is new enough.
    supportsFlight: true,

    // Devjs calls these methods.
    checkDCE,
    ...
    // These internal Devjs modules to report their own boundaries
  ...
  if (maybeSettingsOrSettingsPromise == null) {
    ...
    Promise.resolve(maybeSettingsOrSettingsPromise)
      ...
      .catch(() => {
        targetConsole.error(
          "Devjs DevTools failed to get Console Patching settings. Console won't be patched and some console features will not work.",
        );
      });
  }
  ...
  Object.defineProperty(
    target,
    '__devjs_DEVTOOLS_GLOBAL_HOOK__',
    ({
