...
import {withSyncPerfMeasurements} from 'devjs-devtools-shared/src/PerformanceLoggingUtils';
import traverse from '@babel/traverse';

import type {HooksNode} from 'devjs-debug-tools/src/DevjsDebugHooks';
...
function checkNodeLocation(
  ...
  if (column !== null) {
    ...
    //
    // For more info see https://github.com/Suryanshu-Nabheet/dev.js/pull/21833#discussion_r666831276
    column -= 1;
    if (
...
// Returns all AST Nodes associated with 'potentialDevjsHookASTNode'
function getFilteredHookASTNodes(
  potentialDevjsHookASTNode: NodePath,
  potentialHooksFound: Array<NodePath>,
  source: string,
): Array<NodePath> {
  let nodesAssociatedWithDevjsHookASTNode: NodePath[] = [];
  if (nodeContainsHookVariableName(potentialDevjsHookASTNode)) {
    ...
    nodesAssociatedWithDevjsHookASTNode.unshift(potentialDevjsHookASTNode);
  } else {
    // Case 3.
    ...
    nodesAssociatedWithDevjsHookASTNode = potentialHooksFound.filter(hookNode =>
      filterMemberNodesOfTargetHook(potentialDevjsHookASTNode, hookNode),
    );
  }
  return nodesAssociatedWithDevjsHookASTNode;
}
...
export function getHookName(
  ...
  let potentialDevjsHookASTNode = null;
  if (originalSourceColumnNumber === 0) {
    ...
    if (matchingNodes.length === 1) {
      potentialDevjsHookASTNode = matchingNodes[0];
    }
  } else {
    potentialDevjsHookASTNode = hooksFromAST.find(node => {
      const nodeLocationCheck = checkNodeLocation(
        node,
        originalSourceLineNumber,
  ...
  if (!potentialDevjsHookASTNode) {
    return null;
  }

  // nodesAssociatedWithDevjsHookASTNode could directly be used to obtain the hook variable name
  // depending on the type of potentialDevjsHookASTNode
  try {
    const nodesAssociatedWithDevjsHookASTNode = withSyncPerfMeasurements(
      'getFilteredHookASTNodes()',
      () =>
        getFilteredHookASTNodes(
          potentialDevjsHookASTNode,
          hooksFromAST,
          originalSourceCode,
        ),
    );
    ...
    const name = withSyncPerfMeasurements('getHookNameFromNode()', () =>
      getHookNameFromNode(
        hook,
        nodesAssociatedWithDevjsHookASTNode,
        potentialDevjsHookASTNode,
      ),
    );
...
function getHookNameFromNode(
  ...
  nodesAssociatedWithDevjsHookASTNode: NodePath[],
  potentialDevjsHookASTNode: NodePath,
): string | null {
  let hookVariableName: string | null;
  const isCustomHook = originalHook.id === null;

  switch (nodesAssociatedWithDevjsHookASTNode.length) {
    case 1:
      // CASE 1A (nodesAssociatedWithDevjsHookASTNode[0] !== potentialDevjsHookASTNode):
      // const flagState = useState(true); -> later referenced as
      // const [flag, setFlag] = flagState;
      //
      // CASE 1B (nodesAssociatedWithDevjsHookASTNode[0] === potentialDevjsHookASTNode):
      // const [flag, setFlag] = useState(true); -> we have access to the hook variable straight away
      //
      // CASE 1C (isCustomHook && nodesAssociatedWithDevjsHookASTNode[0] === potentialDevjsHookASTNode):
      ...
      if (
        isCustomHook &&
        nodesAssociatedWithDevjsHookASTNode[0] === potentialDevjsHookASTNode
      ) {
      ...
      hookVariableName = getHookVariableName(
        nodesAssociatedWithDevjsHookASTNode[0],
      );
      break;
    ...
    case 2:
      ...
      nodesAssociatedWithDevjsHookASTNode =
        nodesAssociatedWithDevjsHookASTNode.filter(hookPath =>
          filterMemberWithHookVariableName(hookPath),
        );

      if (nodesAssociatedWithDevjsHookASTNode.length !== 1) {
    ...
    default:
      ...
      //
      // Case > 2 (fallback):
      // const someState = Devjs.useState(() => 0)
      //
      ...
      hookVariableName = getHookVariableName(potentialDevjsHookASTNode);
      break;
  }
...
// Copied from packages/eslint-plugin-devjs-hooks/src/RulesOfHooks
function isHookName(name: string): boolean {
  return /^use[A-Z0-9].*$/.test(name);
}

// Check if the AST Node COULD be a Devjs Hook
...
/// Check whether 'node' is hook declaration of form useState(0); OR Devjs.useState(0);
...
function isBuiltInHookThatReturnsTuple(path: NodePath): boolean {
  ...
  return (
    isDevjsFunction(callee, 'useState') ||
    isDevjsFunction(callee, 'useReducer') ||
    isDevjsFunction(callee, 'useTransition')
  );
}
