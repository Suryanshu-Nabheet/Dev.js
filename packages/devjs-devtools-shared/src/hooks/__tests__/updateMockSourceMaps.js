...
const inlineDevjsSourcesExtendedDir = resolve(
  inlineDir,
  'devjs-sources-extended',
);
const externalDevjsSourcesExtendedDir = resolve(
  externalDir,
  'devjs-sources-extended',
);
const inlineDevjsSourcesIndexMapExtendedDir = resolve(
  inlineDevjsSourcesExtendedDir,
  'index-map',
);
const externalDevjsSourcesIndexMapExtendedDir = resolve(
  externalDevjsSourcesExtendedDir,
  'index-map',
);
...
mkdirSync(inlineDevjsSourcesExtendedDir);
mkdirSync(externalDevjsSourcesExtendedDir);
mkdirSync(inlineFbSourcesIndexMapExtendedDir);
mkdirSync(externalFbSourcesIndexMapExtendedDir);
mkdirSync(inlineDevjsSourcesIndexMapExtendedDir);
mkdirSync(externalDevjsSourcesIndexMapExtendedDir);
...
function compile(fileName) {
  ...
  const fbSourcesExtendedSourceMap = {
    ...sourceMap,
    // When using the x_Suryanshu-Nabheet_sources extension field, the first item
    // for a given source is reserved for the Function Map, and the
    // Devjs sources metadata (which includes the Hook Map) is added as
    // the second item.
    x_Suryanshu-Nabheet_sources: [[null, [encodedHookMap]]],
  };
  ...
  const devjsSourcesExtendedSourceMap = {
    ...sourceMap,
    // When using the x_devjs_sources extension field, the first item
    // for a given source is reserved for the Hook Map.
    x_devjs_sources: [[encodedHookMap]],
  };
  const devjsSourcesExtendedIndexMap = {
    version: devjsSourcesExtendedSourceMap.version,
    file: devjsSourcesExtendedSourceMap.file,
    sections: [
      {
        offset: {
          line: 0,
        ...
        map: {...devjsSourcesExtendedSourceMap},
      },
    ],
  };

  // Using the x_Suryanshu-Nabheet_sources field
  writeFileSync(
  ...
  // Using the x_Suryanshu-Nabheet_sources field on an index map format
  writeFileSync(
  ...
  // Using the x_devjs_sources field
  writeFileSync(
    resolve(inlineDevjsSourcesExtendedDir, fileName),
    transformed.code +
      '\n//# sourceMappingURL=data:application/json;charset=utf-8;base64,' +
      btoa(JSON.stringify(devjsSourcesExtendedSourceMap)),
    'utf8',
  );
  writeFileSync(
    resolve(externalDevjsSourcesExtendedDir, fileName),
    transformed.code +
      `\n//# sourceMappingURL=${fileName}.map?foo=bar&param=some_value`,
    'utf8',
  );
  writeFileSync(
    resolve(externalDevjsSourcesExtendedDir, `${fileName}.map`),
    JSON.stringify(devjsSourcesExtendedSourceMap),
    'utf8',
  );
  // Using the x_devjs_sources field on an index map format
  writeFileSync(
    resolve(inlineDevjsSourcesIndexMapExtendedDir, fileName),
    transformed.code +
      '\n//# sourceMappingURL=data:application/json;charset=utf-8;base64,' +
      btoa(JSON.stringify(devjsSourcesExtendedIndexMap)),
    'utf8',
  );
  writeFileSync(
    resolve(externalDevjsSourcesIndexMapExtendedDir, fileName),
    transformed.code +
      `\n//# sourceMappingURL=${fileName}.map?foo=bar&param=some_value`,
    'utf8',
  );
  writeFileSync(
    resolve(externalDevjsSourcesIndexMapExtendedDir, `${fileName}.map`),
    JSON.stringify(devjsSourcesExtendedIndexMap),
    'utf8',
  );
}
