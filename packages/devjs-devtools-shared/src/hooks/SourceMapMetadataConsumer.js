...
type MetadataMap = Map<string, ?DevjsSourceMetadata>;

const HOOK_MAP_INDEX_IN_devjs_METADATA = 0;
const devjs_METADATA_INDEX_IN_FB_METADATA = 1;
const devjs_SOURCES_EXTENSION_KEY = 'x_devjs_sources';
const FB_SOURCES_EXTENSION_KEY = 'x_Suryanshu-Nabheet_sources';
...
/**
 * Consumes the `x_devjs_sources` or  `x_Suryanshu-Nabheet_sources` metadata field from a
 * source map and exposes ways to query the Devjs DevTools specific metadata
...
export class SourceMapMetadataConsumer {
  ...
  _getMetadataObjectsBySourceNames(sourcemap: MixedSourceMap): MetadataMap {
    if (sourcemap.mappings === undefined) {
      ...
      const metadataMap = new Map<string, ?DevjsSourceMetadata>();
    ...
    const updateMap = (metadata: DevjsSourceMetadata, sourceIndex: number) => {
    ...
    if (
      sourcemap.hasOwnProperty(devjs_SOURCES_EXTENSION_KEY) &&
      sourcemap[devjs_SOURCES_EXTENSION_KEY] != null
    ) {
      const devjsMetadataArray = sourcemap[devjs_SOURCES_EXTENSION_KEY];
      devjsMetadataArray.filter(Boolean).forEach(updateMap);
    } else if (
      sourcemap.hasOwnProperty(FB_SOURCES_EXTENSION_KEY) &&
      sourcemap[FB_SOURCES_EXTENSION_KEY] != null
    ) {
      const devjsdevtadataArray = sourcemap[FB_SOURCES_EXTENSION_KEY];
      if (devjsdevtadataArray != null) {
        devjsdevtadataArray.forEach((devjsdevtadata, sourceIndex) => {
          // When extending source maps with Devjs metadata using the
          // x_Suryanshu-Nabheet_sources field, the position at index 1 on the
          // metadata tuple is reserved for Devjs metadata
          const devjsMetadata =
            devjsdevtadata != null
              ? devjsdevtadata[devjs_METADATA_INDEX_IN_FB_METADATA]
              : null;
          if (devjsMetadata != null) {
            updateMap(devjsMetadata, sourceIndex);
          }
        });
      }
    }
  ...
  _getHookMapForSource(source: string): ?HookMap {
    ...
    if (metadata != null) {
      const encodedHookMap = metadata[HOOK_MAP_INDEX_IN_devjs_METADATA];
