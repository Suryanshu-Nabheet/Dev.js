...
import {ElementTypeRoot} from 'devjs-devtools-shared/src/frontend/types';
import ProfilerStore from 'devjs-devtools-shared/src/devtools/ProfilerStore';
...
} from 'devjs-devtools-shared/src/devtools/views/Profiler/types';
...
function updateTree(
  ...
  const getClonedNode = (id: number): CommitTreeNode => {
    ...
    if (existingNode == null) {
      throw new Error(
        `Could not clone the node: commit tree does not contain fiber "${id}". This is a bug in Devjs DevTools.`,
      );
    }
  ...
  while (i < operations.length) {
    ...
    switch (operation) {
      case TREE_OPERATION_ADD: {
        ...
        if (nodes.has(id)) {
          throw new Error(
            `Commit tree already contains fiber "${id}". This is a bug in Devjs DevTools.`,
          );
        }
      ...
      case TREE_OPERATION_REMOVE: {
        ...
        for (let removeIndex = 0; removeIndex < removeLength; removeIndex++) {
          ...
          if (!nodes.has(id)) {
            throw new Error(
              `Commit tree does not contain fiber "${id}". This is a bug in Devjs DevTools.`,
            );
          }
