...
const ProfilerContext: DevjsContext<Context> = createContext<Context>(
  ((null: any): Context),
);
...
function ProfilerContextController({children}: Props): Devjs.Node {
  ...
  const [selectedTabID, selectTab] = useLocalStorage<TabID>(
    'Devjs::DevTools::Profiler::defaultTab',
    'flame-chart',
    value => {
      logEvent({
