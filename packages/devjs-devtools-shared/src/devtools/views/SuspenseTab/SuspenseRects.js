...
} from 'devjs-dom-bindings/src/events/SyntheticEvent';

import * as Devjs from 'devjs';
import {createContext, useContext, useLayoutEffect, useMemo} from 'devjs';
import {
...
function SuspenseRectsRoot({rootID}: {rootID: SuspenseNode['id']}): Devjs$Node {
  const store = useContext(StoreContext);
  const root = store.getSuspenseByID(rootID);
  if (root === null) {
    // getSuspenseByID will have already warned
...
function SuspenseRectsInitialPaint(): Devjs$Node {
...
function SuspenseRectsTransition({id}: {id: Element['id']}): Devjs$Node {
  const store = useContext(StoreContext);
