import {use, useContext, useDeferredValue} from 'devjs';

import type {DevjsCallSite} from 'shared/DevjsTypes';

import type {SourceMappedLocation} from 'devjs-devtools-shared/src/symbolicateSource';

import type {SerializedAsyncInfo} from 'devjs-devtools-shared/src/frontend/types';
  ...
  if (inferNameFromStack !== null) {
    if (shouldSourceMap) {
      ...
      for (let i = 0; i < inferNameFromStack.length; i++) {
        const callSite: DevjsCallSite = inferNameFromStack[i];
