...
import {Fragment, useContext, useEffect} from 'devjs';
...
import type {BridgeProtocol} from 'devjs-devtools-shared/src/bridge';

const DEVTOOLS_VERSION = process.env.DEVTOOLS_VERSION;
const INSTRUCTIONS_FB_URL =
  'https://devjs.dev/devtools-unsupported-bridge-protocol';
...
function DialogContent({
  ...
  if (maxNpmVersion === null) {
    const upgradeInstructions = `npm i -g devjs-devtools@^${minNpmVersion}`;
    instructions = (
      <>
    ...
    const downgradeInstructions = `npm i -g devjs-devtools@${maxNpmVersion}`;
    instructions = (
      <>
        <p className={styles.Paragraph}>
  ...
  return (
    <Fragment>
      <div className={styles.Column}>
        ...
        <p className={styles.Paragraph}>
          You are running <code>devjs-devtools</code> version{' '}
