...
import {FixedSizeList} from 'devjs-window';
...
import {clearErrorsAndWarnings as clearErrorsAndWarningsAPI} from 'devjs-devtools-shared/src/backendAPI';
import styles from './Tree.css';
import ButtonIcon from '../ButtonIcon';
import Button from '../Button';
import {logEvent} from 'devjs-devtools-shared/src/Logger';
import {useExtensionComponentsPanelVisibility} from 'devjs-devtools-shared/src/frontend/hooks/useExtensionComponentsPanelVisibility';
import {ElementTypeActivity} from 'devjs-devtools-shared/src/frontend/types';
...
export default function Tree(): Devjs.Node {
  ...
  useEffect(() => {
    ...
    //
    // This approach caused a problem though: it meant that a devjs-devtools-inline
    // instance could steal (and prevent/block) keyboard events from other JavaScript
    // on the page– which could even include other devjs-devtools-inline instances.
  ...
  // Let devjs-window know to re-render any time the underlying tree data changes.
  ...
  const handlePreviousErrorOrWarningClick = Devjs.useCallback(() => {
    dispatch({type: 'SELECT_PREVIOUS_ELEMENT_WITH_ERROR_OR_WARNING_IN_TREE'});
  }, []);

  const handleNextErrorOrWarningClick = Devjs.useCallback(() => {
  ...
  const zeroElementsNotice = (
    <div className={styles.ZeroElementsNotice}>
      <p>Loading Devjs Element Tree...</p>
      <p>
        If this seems stuck, please follow the{' '}
        <a
          className={styles.Link}
          href="https://github.com/Suryanshu-Nabheet/dev.js/blob/main/packages/devjs-devtools/README.md#the-devjs-tab-shows-no-components"
          target="_blank">
