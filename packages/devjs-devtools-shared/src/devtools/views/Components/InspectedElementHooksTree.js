...
import {useCallback, useContext, useState} from 'devjs';
...
import {getHookSourceLocationKey} from 'devjs-devtools-shared/src/hookSourceLocation';
...
import isArray from 'devjs-devtools-shared/src/isArray';
...
import type {HookNames} from 'devjs-devtools-shared/src/frontend/types';
...
export function InnerHooksTreeView({
  ...
  inspectedElement,
  path,
}: InnerHooksTreeViewProps): Devjs.Node {
...
function HookView({
  ...
  // Certain hooks are not editable at all (as identified by devjs-debug-tools).
...
export default (Devjs.memo(InspectedElementHooksTree): component(
  ...props: HookViewProps
));
