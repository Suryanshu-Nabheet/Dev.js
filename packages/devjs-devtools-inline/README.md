...
This package can be used to embed Devjs DevTools into browser-based tools like [CodeSandbox](https://codesandbox.io/), [StackBlitz](https://stackblitz.com/), and [Replay](https://replay.io).

If you're looking for the standalone Devjs DevTools UI, **we suggest using [`devjs-devtools`](https://github.com/Suryanshu-Nabheet/dev.js/tree/main/packages/devjs-devtools) instead of using this package directly**.

---

> **Note** that this package (and the DevTools UI) relies on several _experimental_ APIs that are **only available in the [experimental release channel](https://devjsjs.org/docs/release-channels.html#experimental-channel)**. This means that you will need to install `devjs@experimental` and `devjs-dom@experimental`.
...
Installs the global hook on the window/global object. This hook is how Devjs and DevTools communicate.

> **This method must be called before Devjs is loaded.** (This includes `import`/`require` statements and `<script>` tags that include Devjs.)
...
### Example
```js
import { activate, initialize } from 'devjs-devtools-inline/backend';

// This should be the iframe the Devjs application is running in.
const iframe = document.getElementById(frameID);
const contentWindow = iframe.contentWindow;

// Call this before importing Devjs (or any other packages that might import Devjs).
...
### `initialize(windowOrGlobal)`
Configures the DevTools interface to listen to the `window` (or `global` object) the backend was injected into. This method returns a Devjs component that can be rendered directly.

> Because the DevTools interface makes use of several new Devjs concurrent features (like Suspense) **it should be rendered using `DevjsDOMClient.createRoot` instead of `DevjsDOM.render`.**

### Example
```js
import { initialize } from 'devjs-devtools-inline/frontend';

// This should be the iframe the backend hook has been installed in.
const iframe = document.getElementById(frameID);
const contentWindow = iframe.contentWindow;

// This returns a Devjs component that can be rendered into your app.
...
const hookNamesModuleLoaderFunction = () => import('devjs-devtools-inline/hookNames');
...
import { initialize as initializeFrontend } from 'devjs-devtools-inline/frontend';

// The Devjs app you want to inspect with DevTools is running within this iframe:
const iframe = document.getElementById('target');
const { contentWindow } = iframe;

// Installs the global hook into the iframe.
// This must be called before Devjs is loaded into that frame.
initializeBackend(contentWindow);

// Initialize DevTools UI to listen to the hook we just installed.
// This returns a Devjs component we can render anywhere in the parent window.
// This also must be called before Devjs is loaded into the iframe
const DevTools = initializeFrontend(contentWindow);

// Devjs application can be injected into <iframe> at any time now...
...
// Be sure to use DevjsDOMClient.createRoot() to render this component.
...
// The DevTools hook needs to be installed before Devjs is even required!
...
// This returns a Devjs component we can render anywhere in the main window.
// Be sure to use DevjsDOMClient.createRoot() to render this component.
const DevTools = initialize(contentWindow);
...
} from 'devjs-devtools-inline/frontend';
...
// Initialize the DevTools backend before importing Devjs (or any other packages that might import Devjs).
initializeBackend(contentWindow);
...
Below is an example of an advanced integration that could be used to connect Devjs running in a Node process to Devjs DevTools running in a browser.
...
const {
  ...
  createBridge,
  initialize,
} = require('devjs-devtools-inline/backend');
...
##### Sample Web frontend
```js
import { createElement } from 'devjs';
...
To test package changes, refer to the [`devjs-devtools-shell` README](https://github.com/Suryanshu-Nabheet/dev.js/blob/main/packages/devjs-devtools-shell/README.md).
