...
const {runOnlyForDevjsRange} = require('./utils');
...
test.describe('Components', () => {
  ...
  test('Should display initial Devjs components', async () => {
    const appRowCount = await page.evaluate(() => {
      const {createTestNameSelector, findAllNodes} = window.devjs_DOM_APP;
  ...
  test('Should display newly added Devjs components', async () => {
  ...
  test('Should allow elements to be inspected', async () => {
    ...
    // Prop names/values may not be editable based on the Devjs version.
    // If they're not editable, make sure they degrade gracefully
    const isEditableName = semver.gte(config.use.devjs_version, '17.0.0');
    const isEditableValue = semver.gte(config.use.devjs_version, '16.8.0');
  ...
  test('Should allow inspecting source of the element', async () => {
    ...
    const sourceText = await page.evaluate(() => {
      const {createTestNameSelector, findAllNodes} = window.devjs_DOM_DEVTOOLS;
      const container = document.getElementById('devtools');
    ...
    // If Devjs version is specified, the e2e-regression.html page will be used
  ...
  test('should allow props to be edited', async () => {
    ...
    await page.evaluate(() => {
      const {createTestNameSelector, focusWithin} = window.devjs_DOM_DEVTOOLS;
      const container = document.getElementById('devtools');
