...
import {devjs_ELEMENT_TYPE} from './DevjsSymbols';
      ...
      if (addedProperties >= OBJECT_WIDTH_LIMIT) {
        properties.push([
          prefix +
            ...
            'Only ' +
            OBJECT_WIDTH_LIMIT +
            ' properties are shown. Devjs will not log more properties of this object.',
          '',
        ]);
        break;
      }
...
function readDevjsElementTypeof(value: Object): mixed {
  ...
  switch (typeof value) {
    case 'object':
      if (value === null) {
        ...
        if (readDevjsElementTypeof(value) === devjs_ELEMENT_TYPE) {
          // JSX
    ...
    if (prevPropertiesChecked > OBJECT_WIDTH_LIMIT) {
      properties.push([
        'Previous object has more than ' +
          OBJECT_WIDTH_LIMIT +
          ' properties. Devjs will not attempt to diff objects with too many properties.',
        '',
      ]);
      isDeeplyEqual = false;
      break;
    ...
    if (key in prev) {
      ...
      if (prevValue !== nextValue) {
        ...
        if (indent >= 3) {
          ...
          prevValue !== null &&
          nextValue !== null &&
          readDevjsElementTypeof(prevValue) ===
            readDevjsElementTypeof(nextValue)
        ) {
          if (readDevjsElementTypeof(nextValue) === devjs_ELEMENT_TYPE) {
            if (
