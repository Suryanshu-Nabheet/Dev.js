...
import {devjs_ELEMENT_TYPE, devjs_FRAGMENT_TYPE} from 'shared/DevjsSymbols';
  ...
  if (actualYields.length !== 0) {
    const error = Error(
      'Log of yielded values is not empty. ' +
        'Call expect(DevjsTestRenderer).unstable_toHaveYielded(...) first.',
    );
  ...
  if (actualJSON === null || typeof actualJSON === 'string') {
    ...
    if (actualJSON.length === 0) {
      ...
      if (actualJSXChildren === null || typeof actualJSXChildren === 'string') {
        ...
        actualJSX = createJSXElementForTestComparison(devjs_FRAGMENT_TYPE, {
