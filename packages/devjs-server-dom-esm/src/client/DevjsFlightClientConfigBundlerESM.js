...
import type {ModuleLoading} from 'devjs-client/src/DevjsFlightClientConfig';
...
import {prepareDestinationForModuleImpl} from 'devjs-client/src/DevjsFlightClientConfig';
...
// We cache DevjsIOInfo across requests so that inner refreshes can dedupe with outer.
const moduleIOInfoCache: Map<string, DevjsIOInfo> = __DEV__
  ? new Map()
  : (null: any);
...
export function getModuleDebugInfo<T>(
  ...
  if (ioInfo === undefined) {
    ...
    const fakeStack = new Error('devjs-stack-top-frame');
    if (fakeStack.stack.startsWith('Error: devjs-stack-top-frame')) {
      // Looks like V8
      fakeStack.stack =
        'Error: devjs-stack-top-frame\n' +
