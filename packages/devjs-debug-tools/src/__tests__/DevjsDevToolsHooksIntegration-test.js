...
describe('Devjs hooks DevTools integration', () => {
  let Devjs;
  let DevjsDebugTools;
  ...
  it('should support editing useState hooks', async () => {
    ...
    let renderer;
    await act(() => {
      renderer = DevjsTestRenderer.create(<MyComponent />, {
    ...
    const tree = DevjsDebugTools.inspectHooksOfFiber(fiber);
  ...
  it('should support editable useReducer hooks', async () => {
    ...
    let dispatchFn;
    function MyComponent() {
      const [state, dispatch] = Devjs.useReducer(reducer, initialData);
      dispatchFn = dispatch;
      return (
        <div>
  ...
  it('should handle interleaved stateful hooks (e.g. useState) and non-stateful hooks (e.g. useContext)', async () => {
    ...
    function useCustomHook() {
      ...
      const [state, setState] = Devjs.useState({count: context});
      Devjs.useDebugValue(state.count);
      setStateFn = setState;
      return state.count;
    }
