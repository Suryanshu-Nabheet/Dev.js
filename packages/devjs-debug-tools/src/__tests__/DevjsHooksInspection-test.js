...
describe('DevjsHooksInspection', () => {
  ...
  it('should inspect a simple useState hook', () => {
    function Foo(props) {
      const [state] = Devjs.useState('hello world');
      return <div>{state}</div>;
    }
    const tree = DevjsDebugTools.inspectHooks(Foo, {});
  ...
  it('should inspect a simple custom hook', () => {
    function useCustom(value) {
      const [state] = Devjs.useState(value);
      Devjs.useDebugValue('custom hook label');
      return state;
    }
    function Foo(props) {
  ...
  it('should inspect a tree of multiple levels of hooks', () => {
    function effect() {}
    function useCustom(value) {
      const [state] = Devjs.useReducer((s, a) => s, value);
      Devjs.useEffect(effect);
      return state;
    }
  ...
  it('should not confuse built-in hooks with custom hooks that have the same name', () => {
    function useState(value) {
      Devjs.useState(value);
      Devjs.useDebugValue('custom useState');
    }
    function useFormStatus() {
      Devjs.useState('custom useState');
      Devjs.useDebugValue('custom useFormStatus');
    }
    function Foo(props) {
      useFormStatus();
      useState('Hello, Dave!');
  ...
  it('should inspect use() calls for Promise and Context', async () => {
    const MyContext = Devjs.createContext('hi');
    ...
    function useCustom() {
      const value = Devjs.use(promise);
      const [state] = Devjs.useState(value);
      return state;
    }
    function Foo(props) {
      const value1 = Devjs.use(MyContext);
