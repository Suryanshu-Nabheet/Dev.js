...
import type {TransitionStatus} from 'devjs-reconciler/src/DevjsFiberConfig';
...
type CurrentDispatcherRef = typeof DevjsSharedInternals;
...
function readContext<T>(context: DevjsContext<T>): T {
  if (currentFiber === null) {
    // Hook inspection without access to the Fiber tree
    // e.g. when warming up the primitive stack cache or during `DevjsDebugTools.inspectHooks()`.
    return context._currentValue;
  } else {
    if (currentContextDependency === null) {
      throw new Error(
        'Context reads do not line up with context dependencies. This is a bug in Devjs Debug Tools.',
      );
    }
    ...
    if (hasOwnProperty.call(currentContextDependency, 'memoizedValue')) {
      ...
      // Before Devjs 18, we did not have `memoizedValue` so we rely on `setupContexts` in those versions.
...
function useContext<T>(context: DevjsContext<T>): T {
  const value = readContext(context);
  hookLog.push({
...
// in case future versions of Devjs adds more hooks
const DispatcherProxyHandler: Proxy$traps<DispatcherType> = {
  get(target: DispatcherType, prop: string) {
    ...
    // Note: This error name needs to stay in sync with devjs-devtools-shared
    // TODO: refactor this if we ever combine the devtools and debug tools packages
    error.name = 'DevjsDebugToolsUnsupportedHookError';
    throw error;
  },
};
...
function isDevjsWrapper(functionName: void | string, wrapperName: string) {
...
function findPrimitiveIndex(hookStack: ParsedStackFrame[], hook: HookLogEntry) {
  ...
  for (let i = 0; i < primitiveStack.length && i < hookStack.length; i++) {
    ...
    // For Devjs Native (uses Hermes), these source fields will be identical and skipped
    if (primitiveStack[i].source !== hookStack[i].source) {
      // If the next two frames are functions called `useX` then we assume that they're part of the
      // wrappers that the Devjs package or other packages adds around the dispatcher.
      if (
        i < hookStack.length - 1 &&
        isDevjsWrapper(hookStack[i].functionName, hook.dispatcherHookName)
      ) {
        i++;
      }
      if (
  ...
  for (let i = 0; i < readHookLog.length; i++) {
    ...
    if (displayName === null && primitiveFrame !== null) {
      displayName =
        ...
        // Older versions of Devjs do not have sourcemaps.
...
function handleRenderFunctionError(error: any): void {
  ...
  // Note: This error name needs to stay in sync with devjs-devtools-shared
  // TODO: refactor this if we ever combine the devtools and debug tools packages
  wrapperError.name = 'DevjsDebugToolsRenderError';
...
export function inspectHooks<Props>(
  renderFunction: Props => Devjs$Node,
  props: Props,
  ...
  if (currentDispatcher == null) {
    currentDispatcher = DevjsSharedInternals;
  }
...
function setupContexts(contextMap: Map<DevjsContext<any>, any>, fiber: Fiber) {
  let current: null | Fiber = fiber;
  while (current) {
    if (current.tag === ContextProvider) {
      let context: DevjsContext<any> = current.type;
...
function restoreContexts(contextMap: Map<DevjsContext<any>, any>) {
  contextMap.forEach((value, context) => (context._currentValue = value));
}

function inspectHooksOfForwardRef<Props, Ref>(
  renderFunction: (Props, Ref) => Devjs$Node,
  props: Props,
  ref: Ref,
  currentDispatcher: CurrentDispatcherRef,
): HooksTree {
...
function resolveDefaultProps(Component: any, baseProps: any) {
  if (Component && Component.defaultProps) {
    // Resolve default props. Taken from DevjsElement
...
export function inspectHooksOfFiber(
  ...
  if (hasOwnProperty.call(currentFiber, 'dependencies')) {
    ...
    throw new Error(
      'Unsupported Devjs version. This is a bug in Devjs Debug Tools.',
    );
  }
  ...
  // Only used for versions of Devjs without memoized context value in context dependencies.
  const contextMap = new Map<DevjsContext<any>, any>();
  try {
    if (
