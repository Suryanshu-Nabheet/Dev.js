...
contextBridge.exposeInMainWorld('api', {
  ...
  getDevTools() {
    let devtools;
    try {
      devtools = require('devjs-devtools-core/standalone').default;
    } catch (err) {
      alert(
        err.toString() +
          '\n\nDid you run `yarn` and `pnpm run build` in packages/devjs-devtools-core?',
      );
    }
    return devtools;
  },
  readEnv() {
    ...
    const port = +process.env.devjs_DEVTOOLS_PORT || +process.env.PORT || 8097;
