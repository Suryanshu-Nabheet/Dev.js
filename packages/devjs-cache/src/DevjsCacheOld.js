...
import type {DevjsContext, Thenable} from 'shared/DevjsTypes';
...
function readContext(Context: DevjsContext<mixed>) {
...
const CacheContext = Devjs.createContext<mixed>(null);
...
export function unstable_createResource<I, K: string | number, V>(
  ...
  const resource = {
    read(input: I): V {
      // devjs-cache currently doesn't rely on context, but it may in the
