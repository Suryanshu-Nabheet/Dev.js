...
import {ownerStackLimit, enableOptimisticKey} from 'shared/DevjsFeatureFlags';
...
function getOwner() {
  if (__DEV__) {
    const dispatcher = DevjsSharedInternals.A;
    if (dispatcher === null) {
      return null;
    }
...
// The lower the limit, the more likely we'll not reach devjs_stack_bottom_frame
...
function UnknownOwner() {
  /** @noinline */
  return (() => Error('devjs-stack-top-frame'))();
}
const createFakeCallStack = {
  devjs_stack_bottom_frame: function (callStackForError) {
...
if (__DEV__) {
  ...
  unknownOwnerDebugStack = createFakeCallStack.devjs_stack_bottom_frame.bind(
    createFakeCallStack,
    UnknownOwner,
  )();
 ...
 * will not work. Instead test $$typeof field against Symbol.for('devjs.transitional.element') to check
 * if something is a Devjs Element.
 *
 * @internal
 */
function DevjsElement(type, key, props, owner, debugStack, debugTask) {
  ...
  if (__DEV__) {
    ...
    // To make comparing DevjsElements easier for testing purposes, we make
...
export function jsxProd(type, config, maybeKey) {
  ...
  if (maybeKey !== undefined) {
    if (enableOptimisticKey && maybeKey === devjs_OPTIMISTIC_KEY) {
  ...
  return DevjsElement(type, key, props, getOwner(), undefined, undefined);
}
...
export function jsxProdSignatureRunningInDevWithDynamicChildren(
  ...
  if (__DEV__) {
    ...
    const trackActualOwner =
      __DEV__ &&
      DevjsSharedInternals.recentlyCreatedOwnerStacks++ < ownerStackLimit;
    let debugStackDEV = false;
    if (__DEV__) {
      if (trackActualOwner) {
        const previousStackTraceLimit = Error.stackTraceLimit;
        Error.stackTraceLimit = ownerStackTraceLimit;
        debugStackDEV = Error('devjs-stack-top-frame');
...
function jsxDEVImpl(
  ...
  if (__DEV__) {
    ...
    if (children !== undefined) {
      if (isStaticChildren) {
        if (isArray(children)) {
          ...
          console.error(
            'Devjs.jsx: Static children should always be an array. ' +
              'You are likely explicitly calling Devjs.jsxs or Devjs.jsxDEV. ' +
      ...
      if (!didWarnAboutKeySpread[componentName + beforeExample]) {
        ...
        console.error(
          'A props object containing a "key" prop is being spread into JSX:\n' +
            '  let props = %s;\n' +
            '  <%s {...props} />\n' +
            'Devjs keys must be passed directly to JSX without using spread:\n' +
            '  let props = %s;\n' +
            '  <%s key={someKey} {...props} />',
          beforeExample,
          componentName,
    ...
    if (hasValidKey(config)) {
      if (enableOptimisticKey && config.key === devjs_OPTIMISTIC_KEY) {
        key = devjs_OPTIMISTIC_KEY;
      } else {
        if (__DEV__) {
          checkKeyStringCoercion(config.key);
    ...
    return DevjsElement(type, key, props, getOwner(), debugStack, debugTask);
  }
}

/**
 * Create and return a new DevjsElement of the given type.
 * See https://devjsjs.org/docs/devjs-api.html#createelement
 */
...
export function cloneAndReplaceKey(oldElement, newKey) {
  const clonedElement = DevjsElement(
    oldElement.type,
    newKey,
    oldElement.props,
...
/**
 * Clone and return a new DevjsElement using element as the starting point.
 * See https://devjsjs.org/docs/devjs-api.html#cloneelement
 */
export function cloneElement(element, config, children) {
  if (element === null || element === undefined) {
    throw new Error(
      `The argument must be a Devjs element, but you passed ${element}.`,
    );
  }
