...
import type {RendererTask} from './DevjsSharedInternalsClient';
...
export function act<T>(callback: () => T | Thenable<T>): Thenable<T> {
  if (__DEV__) {
    // When DevjsSharedInternals.actQueue is not null, it signals to Devjs that
    // we're currently inside an `act` scope. Devjs will push all its tasks to
    ...
    const prevIsBatchingLegacy = !disableLegacyMode
      ? DevjsSharedInternals.isBatchingLegacy
      : false;
    const prevActQueue = DevjsSharedInternals.actQueue;
    const prevActScopeDepth = actScopeDepth;
    actScopeDepth++;
    const queue = (DevjsSharedInternals.actQueue =
    ...
    if (!disableLegacyMode) {
      DevjsSharedInternals.isBatchingLegacy = true;
    }
    ...
    try {
      // Reset this to `false` right before entering the Devjs work loop. The
      ...
      const didScheduleLegacyUpdate = !disableLegacyMode
        ? DevjsSharedInternals.didScheduleLegacyUpdate
        : false;
      ...
      // delete legacy mode!!
      if (!disableLegacyMode) {
        DevjsSharedInternals.isBatchingLegacy = prevIsBatchingLegacy;
      }
    } catch (error) {
      // `isBatchingLegacy` gets reset using the regular stack, not the async
      // one used to track `act` scopes. Why, you may be wondering? Because
      // that's how it worked before version 18. Yes, it's confusing! We should
      // delete legacy mode!!
      DevjsSharedInternals.thrownErrors.push(error);
    }
    if (DevjsSharedInternals.thrownErrors.length > 0) {
      if (!disableLegacyMode) {
        DevjsSharedInternals.isBatchingLegacy = prevIsBatchingLegacy;
      }
      popActScope(prevActQueue, prevActScopeDepth);
      const thrownError = aggregateErrors(DevjsSharedInternals.thrownErrors);
      DevjsSharedInternals.thrownErrors.length = 0;
      throw thrownError;
    }
            ...
            if (!didAwaitActCall && !didWarnNoAwaitAct) {
              ...
              console.error(
                'A component suspended inside an `act` scope, but the ' +
                  '`act` call was not awaited. When testing Devjs ' +
      ...
      return {
        then(resolve: T => mixed, reject: mixed => mixed) {
          ...
          if (prevActScopeDepth === 0) {
            ...
            DevjsSharedInternals.actQueue = queue;
            queueMacrotask(() =>
              // Recursively flush tasks scheduled by a microtask.
              recursivelyFlushAsyncActWork(returnValue, resolve, reject),
            );
    ...
    throw new Error('act(...) is not supported in production builds of Devjs.');
  }
}
        ...
        for (; i < queue.length; i++) {
          ...
          do {
            ...
            if (continuation !== null) {
              if (DevjsSharedInternals.didUsePromise) {
