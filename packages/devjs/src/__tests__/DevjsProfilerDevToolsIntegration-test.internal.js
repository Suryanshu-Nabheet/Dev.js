...
describe('DevjsProfiler DevTools integration', () => {
  let Devjs;
  ...
  it('should auto-Profile all fibers if the DevTools hook is detected', async () => {
    const App = ({multiplier}) => {
      ...
      return (
        <Devjs.Profiler id="Profiler" onRender={onRender}>
    ...
    let rendered;
    await act(() => {
      rendered = DevjsTestRenderer.create(<App multiplier={1} />, {
  ...
  it('should reset the fiber stack correctly after an error when profiling host roots', async () => {
    ...
    let rendered;
    await act(() => {
      rendered = DevjsTestRenderer.create(
        <div>
