...
describe('DevjsChildren', () => {
  ...
  it('should support identity for simple', () => {
    ...
    Devjs.Children.forEach(instance.props.children, callback, context);
    expect(callback).toHaveBeenCalledWith(simpleKid, 0);
    callback.mockClear();
    const mappedChildren = Devjs.Children.map(
  ...
  it('should support Portal components', () => {
    ...
    const devjsPortal = DevjsDOM.createPortal(simpleChild, portalContainer);

    const parentInstance = <div>{devjsPortal}</div>;
    Devjs.Children.forEach(parentInstance.props.children, callback, context);
    expect(callback).toHaveBeenCalledWith(devjsPortal, 0);
    callback.mockClear();
    const mappedChildren = Devjs.Children.map(
    ...
    expect(callback).toHaveBeenCalledWith(devjsPortal, 0);
    expect(mappedChildren[0]).toEqual(devjsPortal);
  });
  ...
  it('should retain key across two mappings', () => {
    ...
    Devjs.Children.forEach(forcedKeys.props.children, callback, context);
    assertCalls();
  ...
  it('should pass key to returned component', () => {
    ...
    const instance = <div>{simpleKid}</div>;
    const mappedChildren = Devjs.Children.map(instance.props.children, mapFn);

    expect(Devjs.Children.count(mappedChildren)).toBe(1);
  ...
  it('should invoke callback with the right context', () => {
    ...
    const simpleKid = <span key="simple" />;
    const instance = <div>{simpleKid}</div>;
    Devjs.Children.forEach(instance.props.children, callback, scopeTester);
  ...
  it('should be called for each child in array', () => {
    ...
    Devjs.Children.forEach(instance.props.children, callback);
    expect(callback).toHaveBeenCalledWith(zero, 0);
    expect(callback).toHaveBeenCalledWith(one, 1);
    expect(callback).toHaveBeenCalledWith(two, 2);
    expect(callback).toHaveBeenCalledWith(three, 3);
    ...
    expect(Devjs.Children.count(mappedChildren)).toBe(4);
  ...
  it('should retain key across two mappings with conditions', () => {
    ...
    const mappedChildrenForcedKeys = Devjs.Children.map(
      forcedKeys.props.children,
      mapFn,
    );
    ...
    const remappedChildrenForcedKeys = Devjs.Children.map(
  ...
  it('should not throw if key provided is a dupe with array key', () => {
    ...
    expect(function () {
      Devjs.Children.map(instance.props.children, mapFn);
    }).not.toThrow();
  });
  ...
  it('should use the same key for a cloned element', () => {
    ...
    const mapped = Devjs.Children.map(
      instance.props.children,
      element => element,
    );

    const mappedWithClone = Devjs.Children.map(
      instance.props.children,
      element => Devjs.cloneElement(element),
    );
  ...
  it('should use the same key for a cloned element with key', () => {
    ...
    const mappedWithClone = Devjs.Children.map(
      instance.props.children,
      element => Devjs.cloneElement(element, {key: 'unique'}),
    );
  ...
  it('should return 0 for null children', () => {
    const numberOfChildren = Devjs.Children.count(null);
    expect(numberOfChildren).toBe(0);
  });

  it('should return 0 for undefined children', () => {
    const numberOfChildren = Devjs.Children.count(undefined);
    expect(numberOfChildren).toBe(0);
  });
  ...
  it('should return 1 for single child', () => {
    const simpleKid = <span key="simple" />;
    const instance = <div>{simpleKid}</div>;
    const numberOfChildren = Devjs.Children.count(instance.props.children);
  ...
  it('should flatten children to an array', () => {
    expect(Devjs.Children.toArray(undefined)).toEqual([]);
    expect(Devjs.Children.toArray(null)).toEqual([]);

    expect(Devjs.Children.toArray(<div />).length).toBe(1);
    expect(Devjs.Children.toArray([<div />]).length).toBe(1);
    expect(Devjs.Children.toArray(<div />)[0].key).toBe(
      Devjs.Children.toArray([<div />])[0].key,
    );

    const flattened = Devjs.Children.toArray([
    ...
    const reversed = Devjs.Children.toArray([
    ...
    expect(Devjs.Children.toArray([1, 'two', null, undefined, true])).toEqual([
      1,
      'two',
    ]);
  });
  ...
  it('warns for mapped list children without keys', async () => {
    ...
    function ComponentRenderingMappedChildren({children}) {
      return (
        <div>
          {Devjs.Children.map(children, child => (
            <div />
          ))}
        </div>
      );
    ...
    if (__DEV__) {
      ...
      expect(calls[0][0]).toEqual(
        'Each child in a list should have a unique "key" prop.%s%s See https://devjs.dev/link/warning-keys for more information.',
      );
    }
  });
  ...
  it('warns for cloned list children without keys', async () => {
    function ComponentRenderingClonedChildren({children}) {
      return (
        <div>
          {Devjs.Children.map(children, child => Devjs.cloneElement(child))}
        </div>
      );
    }
  ...
  it('warns for flattened list children without keys', async () => {
    function ComponentRenderingFlattenedChildren({children}) {
      return <div>{Devjs.Children.toArray(children)}</div>;
    }
  ...
  it('should combine keys when map returns an array', () => {
    ...
    const mappedChildren = Devjs.Children.map(
      ...
      kid => [
        ...
        <span key="y" />,
        kid,
        kid && Devjs.cloneElement(kid, {key: 'z'}),
        <hr />,
      ],
    );
  ...
  it('should throw on object', () => {
    expect(function () {
      Devjs.Children.forEach({a: 1, b: 2}, function () {}, null);
    }).toThrowError(
      'Objects are not valid as a Devjs child (found: object with keys ' +
        '{a, b}).' +
        (__DEV__
  ...
  it('should render Devjs.lazy after suspending', async () => {
    const lazyElement = Devjs.lazy(async () => ({default: <div key="hi" />}));
    function Component() {
      return Devjs.Children.map([lazyElement], c =>
        Devjs.cloneElement(c, {children: 'hi'}),
      );
    }
    const container = document.createElement('div');
    const root = DevjsDOMClient.createRoot(container);
  ...
  it('should render cached Promises after suspending', async () => {
    ...
    function Component() {
      return Devjs.Children.map([promise], c =>
        Devjs.cloneElement(c, {children: 'hi'}),
      );
    }
    const container = document.createElement('div');
  ...
  it('should throw on regex', () => {
    ...
    expect(function () {
      Devjs.Children.forEach(/abc/, function () {}, null);
    }).toThrowError(
      'Objects are not valid as a Devjs child (found: /abc/).' +
        (__DEV__
          ? ' If you meant to render a collection of children, use an ' +
            'array instead.'
          : ''),
  ...
  describe('with fragments enabled', () => {
    it('warns for keys for arrays of elements in a fragment', async () => {
      class ComponentReturningArray extends Devjs.Component {
        render() {
          return [<div />, <div />];
        }
      }
