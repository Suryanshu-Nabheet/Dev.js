...
describe('DevjsClassEquivalence', () => {
  it('tests the same thing for es6 classes and CoffeeScript', () => {
    const result1 = runJest('DevjsCoffeeScriptClass-test.coffee');
    const result2 = runJest('DevjsES6Class-test.js');
    compareResults(result1, result2);
  });

  it('tests the same thing for es6 classes and TypeScript', () => {
    const result1 = runJest('DevjsTypeScriptClass-test.ts');
    const result2 = runJest('DevjsES6Class-test.js');
    compareResults(result1, result2);
  });
});
  ...
  const result = spawnSync(
    ...
    [command, testFile, defaultReporter, equivalenceReporter],
    {
      ...
      env: Object.assign({}, process.env, {
        devjs_CLASS_EQUIVALENCE_TEST: 'true',
      }),
    },
  );
