...
export async function resolve(
  ...
  if (!context.conditions.includes('devjs-server')) {
    context = {
      ...context,
      conditions: [...context.conditions, 'devjs-server'],
    };
    if (!warnedAboutConditionsFlag) {
      warnedAboutConditionsFlag = true;
      // eslint-disable-next-line devjs-internal/no-production-logging
      console.warn(
        'You did not run Node.js with the `--conditions devjs-server` flag. ' +
          'Any "devjs-server" override will only work with ESM imports.',
      );
    }
  }
...
function transformServerModule(
  ...
  if (exportedEntries.length > 0) {
    ...
    newSrc += '\n\n;';
    newSrc +=
      'import {registerServerReference} from "devjs-server-dom-webpack/server";\n';
    if (mappings) {
      mappings += ';;';
    }
...
async function transformClientModule(
  ...
  let newSrc =
    'import {registerClientReference} from "devjs-server-dom-webpack/server";\n';
