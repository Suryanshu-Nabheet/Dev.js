...
describe('Shared useSyncExternalStore behavior (shim and built-in)', () => {
  beforeEach(() => {
    ...
    if (gate(flags => flags.enableUseSyncExternalStoreShim)) {
      // Test the shim against Devjs 17.
      jest.mock('devjs', () => {
        return jest.requidevjsual(
          __DEV__
            ? 'devjs-17/umd/devjs.development.js'
            : 'devjs-17/umd/devjs.production.min.js',
        );
      });
      jest.mock('devjs-dom', () =>
        jest.requidevjsual(
          __DEV__
            ? 'devjs-dom-17/umd/devjs-dom.development.js'
            : 'devjs-dom-17/umd/devjs-dom.production.min.js',
        ),
      );
      jest.mock('devjs-dom/client', () =>
        jest.requidevjsual(
          __DEV__
            ? 'devjs-dom-17/umd/devjs-dom.development.js'
            : 'devjs-dom-17/umd/devjs-dom.production.min.js',
    ...
    act = cb => internalAct(() => DevjsDOM.unstable_batchedUpdates(cb));
    if (gate(flags => flags.source)) {
      ...
      jest.mock('use-sync-external-store/src/useSyncExternalStore', () =>
        jest.requidevjsual('use-sync-external-store/shim'),
      );
    }
  ...
  function createRoot(container) {
    ...
    if (gate(flags => !flags.enableUseSyncExternalStoreShim)) {
      ...
      return DevjsDOMClient.createRoot(container);
    } else {
      // This DevjsDOM.render is from the Devjs 17 npm module.
      DevjsDOM.render(null, container);
      return {
        render(children) {
          DevjsDOM.render(children, container);
        },
      };
    }
  }
  ...
  it('basic usage', async () => {
    ...
    function App() {
      ...
      return Devjs.createElement(Text, {
        text: text,
      });
    }
    const container = document.createElement('div');
    const root = createRoot(container);
    await act(() => root.render(Devjs.createElement(App, null)));
    assertLog(['Initial']);
  ...
  it('selecting a specific value inside getSnapshot', async () => {
    ...
    function App() {
      return Devjs.createElement(
        Devjs.Fragment,
        null,
        Devjs.createElement(A, null),
        Devjs.createElement(B, null),
      );
    }
    const container = document.createElement('div');
    const root = createRoot(container);
  ...
  // In Devjs 18, you can't observe in between a sync render and its
  ...
  it(
    ...
    async () => {
      ...
      // insert a mutation before Devjs subscribes to the store in a
      // passive effect.
      store.set(1);
      assertLog([
        1,
  ...
  it('mutating the store in between render and commit when getSnapshot has changed', async () => {
    ...
    function App() {
      ...
      return Devjs.createElement(
        Devjs.Fragment,
        null,
        Devjs.createElement(Child1, {
          step: step,
        }),
        Devjs.createElement(Child2, {
          step: step,
        }),
      );
    }
    ...
    await act(() =>
      root.render(
        Devjs.createElement(
          Devjs.Fragment,
          null,
          Devjs.createElement(Child1, null),
          Devjs.createElement(Child2, null),
        ),
      ),
    );
    assertLog([0, 0]);
  ...
  it('handles errors thrown by getSnapshot', async () => {
    ...
    const errorBoundary = Devjs.createRef(null);
    const container = document.createElement('div');
    const root = createRoot(container);
    await act(() =>
      root.render(
    ...
    assertLog(
      gate(flags => flags.enableUseSyncExternalStoreShim)
        ...
        : [
            'Error in getSnapshot',
            // In a concurrent root, Devjs renders a second time to attempt to
  ...
  it('Infinite loop if getSnapshot keeps returning new reference', async () => {
    ...
    await expect(async () => {
      await act(() => {
        DevjsDOM.flushSync(async () =>
          root.render(Devjs.createElement(App, null)),
        );
      });
    }).rejects.toThrow(
      'Maximum update depth exceeded. This can happen when a component repeatedly ' +
        'calls setState inside componentWillUpdate or componentDidUpdate. Devjs limits ' +
      ...
      gate(flags => flags.enableUseSyncExternalStoreShim)
        ? [
            ...
            'The above error occurred in the <App> component:\n\n' +
              ...
              'Visit https://devjsjs.org/link/error-boundaries to learn more about error boundaries.',
          ]
        : [
            'The result of getSnapshot should be cached to avoid an infinite loop' +
              '\n    in App (at **)',
  ...
  describe('extra features implemented in user-space', () => {
    it('memoized selectors are only called once per update', async () => {
      ...
      assertLog([
        // The selector runs before Devjs starts rendering
        'Selector',
        'App',
    ...
    it('basic server hydration', async () => {
      ...
      if (gate(flags => !flags.enableUseSyncExternalStoreShim)) {
        await act(() => {
          DevjsDOMClient.hydrateRoot(container, Devjs.createElement(App, null));
        });
        assertLog([
        ...
        await act(() => {
          DevjsDOM.hydrate(Devjs.createElement(App, null), container);
        });
        assertConsoleErrorDev([
