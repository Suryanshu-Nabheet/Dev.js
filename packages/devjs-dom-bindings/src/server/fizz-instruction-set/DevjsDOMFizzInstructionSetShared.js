...
export function revealCompletedBoundaries(batch) {
  ...
  for (let i = 0; i < batch.length; i += 2) {
    ...
    // This is similar to clearSuspenseBoundary in DevjsFiberConfigDOM.
    ...
    if (suspenseNode['_devjsRetry']) {
      requestAnimationFrame(suspenseNode['_devjsRetry']);
    }
  }
  batch.length = 0;
}
...
export function revealCompletedBoundariesWithViewTransitions(
  ...
  try {
    const existingTransition = document['__devjsViewTransition'];
    ...
    if (shouldStartViewTransition) {
      const transition = (document['__devjsViewTransition'] = document[
        'startViewTransition'
      ]({
        update: () => {
      ...
      transition.finished.finally(() => {
        if (document['__devjsViewTransition'] === transition) {
          document['__devjsViewTransition'] = null;
        }
      });
...
export function clientRenderBoundary(
  ...
  // Tell Devjs to retry it if the parent already hydrated.
  if (suspenseNode['_devjsRetry']) {
    suspenseNode['_devjsRetry']();
  }
}
...
export function listenToFormSubmissionsForReplaying() {
  ...
  // This event listener should be above the Devjs one. That way when
  // we preventDefault in Devjs's handling we also prevent this event
  // from queing it. Since Devjs listens to the root and the top most
  ...
  addEventListener('submit', event => {
    ...
    // This will also prevent other Devjs's on the same page from listening.
    ...
    // Devjss on the same page since each one will preventDefault for the next one.
    // This means that this protocol is shared with any Devjs version that shares the same
    ...
    (root['$$devjsFormReplay'] = root['$$devjsFormReplay'] || []).push(
      form,
      submitter,
      formData,
    );
