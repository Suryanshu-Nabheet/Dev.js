...
import type {
  ...
  ImportMap,
} from 'devjs-dom/src/shared/DevjsDOMTypes';
...
import type {
  ...
  Chunk,
  PrecomputedChunk,
} from 'devjs-server/src/DevjsServerStreamConfig';

import type {FormStatus} from '../shared/DevjsDOMFormActions';
...
import {checkControlledValueProps} from '../shared/DevjsControlledValuePropTypes';
import {validateProperties as validateARIAProperties} from '../shared/DevjsDOMInvalidARIAHook';
import {validateProperties as validateInputProperties} from '../shared/DevjsDOMNullInputValuePropHook';
import {validateProperties as validateUnknownProperties} from '../shared/DevjsDOMUnknownPropertyHook';
...
import {
  ...
  markShellTime,
} from './fizz-instruction-set/DevjsDOMFizzInstructionSetInlineCodeStrings';

import {getValueDescriptorExpectingObjectForWarning} from '../shared/DevjsDOMResourceValidation';

import {NotPending} from '../shared/DevjsDOMFormActions';
  ...
  if (__DEV__) {
    if (onHeaders && typeof maxHeadersLength === 'number') {
      if (maxHeadersLength <= 0) {
        console.error(
          'Devjs expected a positive non-zero `maxHeadersLength` option but found %s instead. When using the `onHeaders` option you may supply an optional `maxHeadersLength` option as well however, when setting this value to zero or less no headers will be captured.',
...
const actionJavaScriptURL = stringToPrecomputedChunk(
  escapeTextForBrowser(
    ...
    "javascript:throw new Error('Devjs form unexpectedly submitted.')",
  ),
);
...
function pushFormActionAttribute(
  ...
  if (typeof formAction === 'function') {
      ...
      if (name !== null && !didWarnFormActionName) {
        ...
        console.error(
          'Cannot specify a "name" prop for a button that specifies a function as a formAction. ' +
            'Devjs needs it to encode which action should be invoked. It will get overridden.',
        );
      }
      if (
        ...
        console.error(
          'Cannot specify a formEncType or formMethod for a button that specifies a ' +
            'function as a formAction. Devjs provides those automatically. They will get overridden.',
        );
      }
    ...
    if (customFields !== null) {
      ...
      // manually submitted or if someone calls stopPropagation before Devjs gets the event.
...
function pushAttribute(
  ...
  switch (name) {
    ...
    case 'ref':
      // Ignored. These are built-in to Devjs on the client.
      return;
    case 'autoFocus':
    case 'multiple':
    case 'muted': {
    ...
    case 'preserveAlpha': {
      ...
      // In Devjs, we let users pass `true` and `false` even though technically
        ...
        if (value === '' && !didWarnForNewBooleanPropsWithEmptyValue[name]) {
          ...
          console.error(
            'Received an empty string for a boolean attribute `%s`. ' +
              ...
              'pass `true` if you used an empty string in earlier versions of Devjs to indicate this attribute is true.',
            name,
          );
        }
      }
    ...
    if (typeof innerHTML !== 'object' || !('__html' in innerHTML)) {
      throw new Error(
        '`props.dangerouslySetInnerHTML` must be in the form `{__html: ...}`. ' +
          'Please visit https://devjs.dev/link/dangerously-set-inner-html ' +
    ...
    if (
      ...
      console.error(
        'Select elements must be either controlled or uncontrolled ' +
          ...
          'https://devjs.dev/link/controlled-components',
      );
      ...
      if (__DEV__) {
        if (innerHTML !== null) {
          if (!didWarnInvalidOptionInnerHTML) {
            ...
            console.error(
              'Pass a `value` prop if you set dangerouslyInnerHTML so Devjs knows ' +
      ...
      if (
        ...
        console.error(
          'Cannot specify a encType or method for a form that specifies a ' +
            'function as the action. Devjs provides those automatically. ' +
    ...
    if (__DEV__) {
      if (rel === 'stylesheet' && typeof props.precedence === 'string') {
        if (typeof href !== 'string' || !href) {
          console.error(
            'Devjs encountered a `<link rel="stylesheet" .../>` with a `precedence` prop and expected the `href` prop to be a non-empty string but ecountered %s instead. If your intent was to have Devjs hoist and deduplciate this stylesheet using the `precedence` prop ensure there is a non-empty string `href` prop as well, otherwise remove the `precedence` prop.',
      ...
      if (__DEV__) {
        if (typeof precedence === 'string') {
          if (props.disabled != null) {
            console.error(
              'Devjs encountered a `<link rel="stylesheet" .../>` with a `precedence` prop and a `disabled` prop. The presence of the `disabled` prop indicates an intent to manage the stylesheet active state from your from your Component code and Devjs will not hoist or deduplicate this stylesheet. If your intent was to have Devjs hoist and deduplciate this stylesheet using the `precedence` prop remove the `disabled` prop, otherwise remove the `precedence` prop.',
            );
            ...
            const propDescription =
              props.onLoad && props.onError
                ...
                : props.onLoad
                  ...
                  : '`onError` prop';
            console.error(
              'Devjs encountered a `<link rel="stylesheet" .../>` with a `precedence` prop and %s. The presence of loading and error handlers indicates an intent to manage the stylesheet loading state from your from your Component code and Devjs will not hoist or deduplicate this stylesheet. If your intent was to have Devjs hoist and deduplciate this stylesheet using the `precedence` prop remove the %s, otherwise remove the `precedence` prop.',
              propDescription,
              propDescription,
            );
          }
...
function pushStyle(
        ...
        const childType =
          typeof child === 'function'
            ...
            : typeof child === 'symbol'
              ...
              : 'an Array';
        console.error(
          'Devjs expect children of <style> tags to be a string, number, or object with a `toString` method but found %s instead. ' +
  ...
  if (__DEV__) {
    if (href.includes(' ')) {
      console.error(
        'Devjs expected the `href` prop for a <style> tag opting into hoisting semantics using the `precedence` prop to not have any spaces but ecountered spaces instead. using spaces in this prop will cause hydration of this style to fail on the client. The href for the <style> where this ocurred is "%s".',
        href,
      );
    }
  }
  ...
  if (resourceState !== EXISTS) {
    ...
    if (__DEV__) {
      if (resourceState) {
        console.error(
          'Devjs encountered a hoistable style tag for the same href as a preload: "%s". When using a style tag to inline styles you should not also preload it as a stylsheet.',
          href,
        );
      }
    }
    ...
    if (!nonceStyle || nonceStyle === nonce) {
      if (__DEV__) {
        if (!nonceStyle && nonce) {
          console.error(
            'Devjs encountered a style tag with `precedence` "%s" and `nonce` "%s". When Devjs manages style rules using `precedence` it will only include a nonce attributes if you also provide the same style nonce value as a render option.',
            precedence,
            nonce,
          );
        }
      ...
      console.error(
        'Devjs encountered a style tag with `precedence` "%s" and `nonce` "%s". When Devjs manages style rules using `precedence` it will only include rules if the nonce matches the style nonce "%s" that was included with this render.',
        precedence,
        nonce,
        nonceStyle,
      );
  ...
  if (__DEV__) {
    if (hasOwnProperty.call(props, 'children')) {
      ...
      if (Array.isArray(children) && children.length > 1) {
        console.error(
          'Devjs expects the `children` prop of <title> tags to be a string, number, bigint, or object with a novel `toString` method but found an Array with length %s instead.' +
            ' Browsers treat all child Nodes of <title> tags as Text content and Devjs expects to be able to convert `children` of <title> tags to a single string value' +
        ...
        console.error(
          'Devjs expect children of <title> tags to be a string, number, bigint, or object with a novel `toString` method but found %s instead.' +
            ' Browsers treat all child Nodes of <title> tags as Text content and Devjs expects to be able to convert children of <title>' +
        ...
        if (child.$$typeof != null) {
          console.error(
            'Devjs expects the `children` prop of <title> tags to be a string, number, bigint, or object with a novel `toString` method but found an object that appears to be' +
              ' a Devjs element which never implements a suitable `toString` method. Browsers treat all child Nodes of <title> tags as Text content and Devjs expects to' +
              ' be able to convert children of <title> tags to a single string value which is why rendering Devjs elements is not supported. If the `children` of <title> is' +
              ' a Devjs Component try moving the <title> tag into that component. If the `children` of <title> is some HTML markup change it to be Text only to be valid HTML.',
          );
        } else {
          console.error(
            'Devjs expects the `children` prop of <title> tags to be a string, number, bigint, or object with a novel `toString` method but found an object that does not implement' +
              ' a suitable `toString` method. Browsers treat all child Nodes of <title> tags as Text content and Devjs expects to be able to convert children of <title> tags' +
              ...
              ' is an object in error and change it to a string or number value if so. Otherwise implement a `toString` method that Devjs can use to produce a valid <title>.',
          );
        }
      }
    }
...
import {doctypeChunk as DOCTYPE} from 'devjs-server/src/DevjsFizzConfig';
    ...
    if (
      ...
      console.error(
        'A component is `contentEditable` and contains `children` managed by ' +
          'Devjs. It is now your responsibility to guarantee that none of ' +
...
export function writeStartSegment(
  ...
  switch (formatContext.insertionMode) {
    ...
    default: {
      throw new Error('Unknown insertion mode. This is a bug in Devjs.');
    }
  }
}
  ...
  return escaped.replace(regexForJSStringsInInstructionScripts, match => {
    switch (match) {
      ...
      default: {
        ...
        throw new Error(
          'escapeJSStringsForInstructionScripts encountered a match it does not know how to replace. this means the match regex and the replacement characters are no longer in sync. This is a bug in Devjs',
        );
      }
    }
  });
  ...
  return escaped.replace(regexForJSStringsInScripts, match => {
    switch (match) {
      ...
      default: {
        // eslint-disable-next-line devjs-internal/prod-error-codes
        throw new Error(
          'escapeJSObjectForInstructionScripts encountered a match it does not know how to replace. this means the match regex and the replacement characters are no longer in sync. This is a bug in Devjs',
        );
      }
    }
  });
...
function flushStyleTagsLateForBoundary(
  ...
  if (__DEV__) {
    if (rules.length > 0 && hrefs.length === 0) {
      console.error(
        'Devjs expected to have at least one href for an a hoistable style but found none. This is a bug in Devjs.',
      );
    }
  }
  let i = 0;
...
export function writeHoistables(
  ...
  // tags when using Devjs but if you do they are going to be treated like regular inline
...
// value and read with JSON.parse in DevjsDOMServerExternalRuntime.js
// E.g.
...
function preloadBootstrapScriptOrModule(
  ...
  if (__DEV__) {
    if (
      ...
      // This is coded as a Devjs error because it should be impossible for a userspace preload to preempt this call
      ...
      console.error(
        'Internal Devjs Error: Devjs expected bootstrap script or module with src "%s" to not have been preloaded already. please file an issue',
        href,
      );
    }
  }
...
function escapeHrefForLinkHeaderURLContextReplacer(match: string): string {
  switch (match) {
    ...
    default: {
      // eslint-disable-next-line devjs-internal/prod-error-codes
      throw new Error(
        'escapeLinkHrefForHeaderContextReplacer encountered a match it does not know how to replace. this means the match regex and the replacement characters are no longer in sync. This is a bug in Devjs',
      );
    }
  }
}
...
function escapeStringForLinkHeaderQuotedParamValueContextReplacer(
  ...
  switch (match) {
    ...
    default: {
      // eslint-disable-next-line devjs-internal/prod-error-codes
      throw new Error(
        'escapeStringForLinkHeaderQuotedParamValueContextReplacer encountered a match it does not know how to replace. this means the match regex and the replacement characters are no longer in sync. This is a bug in Devjs',
      );
    }
  }
}
  ...
  if (onHeaders) {
    ...
    if (headers) {
      ...
      if (linkHeader) {
        ...
        // it Devjs will not provide any headers
        onHeaders({});
      }
      return;
    }
