...
export function validateLinkPropsForStyleResource(props: any): boolean {
  if (__DEV__) {
    ...
    if (includedProps.length) {
      console.error(
        'Devjs encountered a <link rel="stylesheet" href="%s" ... /> with a `precedence` prop that' +
          ' also included %s. The presence of loading and error handlers indicates an intent to manage' +
          ' the stylesheet loading state from your from your Component code and Devjs will not hoist or' +
          ' deduplicate this stylesheet. If your intent was to have Devjs hoist and deduplciate this stylesheet' +
