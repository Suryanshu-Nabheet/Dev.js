...
import {getParentHydrationBoundary} from './DevjsFiberConfigDOM';

import {enableScopeAPI} from 'shared/DevjsFeatureFlags';

import {enableInternalInstanceMap} from 'shared/DevjsFeatureFlags';

const randomKey = Math.random().toString(36).slice(2);
const internalInstanceKey = '__devjsFiber$' + randomKey;
const internalPropsKey = '__devjsProps$' + randomKey;
const internalContainerInstanceKey = '__devjsContainer$' + randomKey;
const internalEventHandlersKey = '__devjsEvents$' + randomKey;
const internalEventHandlerListenersKey = '__devjsListeners$' + randomKey;
const internalEventHandlesSetKey = '__devjsHandles$' + randomKey;
const internalRootNodeResourcesKey = '__devjsResources$' + randomKey;
const internalHoistableMarker = '__devjsMarker$' + randomKey;
const internalScrollTimer = '__devjsScroll$' + randomKey;
...
export function getClosestInstanceFromNode(targetNode: Node): null | Fiber {
  ...
  // If the direct event target isn't a Devjs owned DOM node, we need to look
  // to see if one of its parents is a Devjs owned DOM node.
  let parentNode = targetNode.parentNode;
  while (parentNode) {
    // We'll check if this is a container root that could include
    // Devjs nodes in the future. We need to check this first because
    ...
    if (targetInst) {
      ...
      // also have been non-Devjs nodes. We need to answer which one.

      // If we the instance doesn't have any children, then there can't be
      // a nested suspense boundary within it. So we can use this as a fast
      // bailout. Most of the time, when people add non-Devjs children to
...
/**
 * Given a DOM node, return the DevjsDOMComponent or DevjsDOMTextComponent
 * instance, or null if the node was not rendered by this Devjs.
 */
...
/**
 * Given a DevjsDOMComponent or DevjsDOMTextComponent, return the corresponding
 * DOM node.
 */
...
export function setEventHandlerListeners(
  scope: EventTarget | DevjsScopeInstance,
  listeners: Set<DevjsDOMEventHandleListener>,
): void {
...
export function getEventHandlerListeners(
  scope: EventTarget | DevjsScopeInstance,
): null | Set<DevjsDOMEventHandleListener> {
...
export function addEventHandleToTarget(
  ...
  eventHandle: DevjsDOMEventHandle,
): void {
