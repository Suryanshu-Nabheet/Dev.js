...
import {getCurrentFiberOwnerNameInDevOrNull} from 'devjs-reconciler/src/DevjsCurrentFiber';

import {getFiberCurrentPropsFromNode} from './DevjsDOMComponentTree';
...
import {queueChangeEvent} from '../events/DevjsDOMEventReplaying';
...
export function updateInput(
  ...
  if (disableInputAttributeSyncing) {
    // When not syncing the value attribute, Devjs only assigns a new value
    // whenever the defaultValue Devjs prop has changed. When not present,
    // Devjs does nothing
    ...
    // properties:
    //  1. The value Devjs property
    //  2. The defaultValue Devjs property
  ...
  if (disableInputAttributeSyncing) {
    ...
    // controllable from the defaultValue Devjs property. It needs to be
...
export function initInput(
  ...
  if (value != null || defaultValue != null) {
    ...
    if (!isHydrating || enableHydrationChangeEvent) {
      if (disableInputAttributeSyncing) {
        ...
        // directly to the Devjs prop. Only assign it if it exists.
        if (value != null) {
        ...
        //
        //   1. The value Devjs property when present
        //   2. The defaultValue Devjs property when present
        //   3. An empty string
    ...
    if (disableInputAttributeSyncing) {
      ...
      // directly from the defaultValue Devjs property (when present)
      if (defaultValue != null) {
  ...
  if (disableInputAttributeSyncing) {
    ...
    //
    //   1. The checked Devjs property when present
    //   2. The defaultChecked Devjs property when present
    //   3. Otherwise, false
    node.defaultChecked = !node.defaultChecked;
...
export function restoreControlledInputState(element: Element, props: Object) {
  ...
  if (props.type === 'radio' && name != null) {
    ...
    for (let i = 0; i < group.length; i++) {
      ...
      // This will throw if radio buttons rendered by different copies of Devjs
      // and the same name are rendered into the same form (same as #1939).
      // That's probably okay; we don't support it just as we don't support
      // mixing Devjs radio buttons with non-Devjs ones.
      const otherProps: any = getFiberCurrentPropsFromNode(otherNode);

      if (!otherProps) {
        throw new Error(
          'DevjsDOMInput: Mixing Devjs and non-Devjs radio inputs with the ' +
...
//
// https://github.com/Suryanshu-Nabheet/dev.js/issues/7253
