...
} from 'devjs-reconciler/src/DevjsTestSelectors';
import type {DevjsContext, DevjsScopeInstance} from 'shared/DevjsTypes';
import type {AncestorInfoDev} from './validateDOMNesting';
import type {FormStatus} from 'devjs-dom-bindings/src/shared/DevjsDOMFormActions';
import type {
  CrossOriginEnum,
  PreloadImplOptions,
...
import {setSrcObject} from './DevjsDOMSrcObject';

import {getCurrentRootHostContainer} from 'devjs-reconciler/src/DevjsFiberHostContext';
import {runWithFiberInDEV} from 'devjs-reconciler/src/DevjsCurrentFiber';

import hasOwnProperty from 'shared/hasOwnProperty';
import {checkAttributeStringCoercion} from 'shared/CheckStringCoercion';
import {devjs_CONTEXT_TYPE} from 'shared/DevjsSymbols';

export {
  setCurrentUpdatePriority,
  getCurrentUpdatePriority,
  resolveUpdatePriority,
} from './DevjsDOMUpdatePriority';
import {
  precacheFiberNode,
  updateFiberProps,
...
} from 'devjs-reconciler/src/DevjsFiberTreeReflection';
import {compareDocumentPositionForEmptyFragment} from 'shared/DevjsDOMFragmentRefShared';
...
} from './DevjsDOMComponent';
import {hydrateInput} from './DevjsDOMInput';
import {hydrateTextarea} from './DevjsDOMTextarea';
import {hydrateSelect} from './DevjsDOMSelect';
import {getSelectionInformation, restoreSelection} from './DevjsInputSelection';
...
import {
  isEnabled as DevjsBrowserEventEmitterIsEnabled,
  setEnabled as DevjsBrowserEventEmitterSetEnabled,
} from '../events/DevjsDOMEventListener';
...
import {
  flushEventReplaying,
  retryIfBlockedOn,
} from '../events/DevjsDOMEventReplaying';
...
import {
  ...
  HostText,
  HostSingleton,
} from 'devjs-reconciler/src/DevjsWorkTags';
import {listenToAllSupportedEvents} from '../events/DOMPluginEventSystem';
import {validateLinkPropsForStyleResource} from '../shared/DevjsDOMResourceValidation';
import escapeSelectorAttributeValueInsideDoubleQuotes from './escapeSelectorAttributeValueInsideDoubleQuotes';
import {flushSyncWork as flushSyncWorkOnAllRoots} from 'devjs-reconciler/src/DevjsFiberWorkLoop';
import {requestFormReset as requestFormResetOnFiber} from 'devjs-reconciler/src/DevjsFiberHooks';
...
export const rendererPackageName = 'devjs-dom';
...
export type Container =
  | interface extends Element {_devjsRootContainer?: FiberRoot}
  | interface extends Document {_devjsRootContainer?: FiberRoot}
  | interface extends DocumentFragment {_devjsRootContainer?: FiberRoot};
export type Instance = Element;
export type TextInstance = Text;

type InstanceWithFragmentHandles = Instance & {
  unstable_devjsFragments?: Set<FragmentInstanceType>,
};

declare class ActivityInterface extends Comment {}
declare class SuspenseInterface extends Comment {
  _devjsRetry: void | (() => void);
}
...
export * from 'devjs-reconciler/src/DevjsFiberConfigWithNoPersistence';
...
export function prepareForCommit(containerInfo: Container): Object | null {
  eventsEnabled = DevjsBrowserEventEmitterIsEnabled();
  ...
  DevjsBrowserEventEmitterSetEnabled(false);
  return activeInstance;
}

export function befodevjsiveInstanceBlur(internalInstanceHandle: Object): void {
  if (enableCreateEventHandleAPI) {
    DevjsBrowserEventEmitterSetEnabled(true);
...
export function resetAfterCommit(containerInfo: Container): void {
  ...
  DevjsBrowserEventEmitterSetEnabled(eventsEnabled);
  eventsEnabled = null;
      ...
      switch (type) {
            ...
            if (enableTrustedTypesIntegration && !didWarnScriptTags) {
              console.error(
                'Encountered a script tag while rendering Devjs component. ' +
                  'Scripts inside Devjs components are never executed when rendering ' +
        ...
        case 'select': {
          if (typeof props.is === 'string') {
            ...
            // See discussion in https://github.com/Suryanshu-Nabheet/dev.js/pull/6896
              ...
              if (
                ...
                console.error(
                  'The tag <%s> is unrecognized in this browser. ' +
                    'If you meant to render a Devjs component, start its name with ' +
                    'an uppercase letter.',
                  type,
                );
              }
...
// if a component just imports DevjsDOM (e.g. for findDOMNode).
...
export function prepareScopeUpdate(
  scopeInstance: DevjsScopeInstance,
  internalInstanceHandle: Object,
): void {
  if (enableScopeAPI) {
...
function warnForDevjsChildrenConflict(container: Container): void {
  if (__DEV__) {
    if ((container: any).__devjsWarnedAboutChildrenConflict) {
      return;
    }
    ...
    if (props !== null) {
      ...
      if (fiber !== null) {
        if (
          ...
          (container: any).__devjsWarnedAboutChildrenConflict = true;
          ...
          runWithFiberInDEV(fiber, () => {
            console.error(
              'Cannot use a ref on a Devjs element as a container to `createRoot` or `createPortal` ' +
                'if that element also sets "children" text content using Devjs. It should be a leaf with no children. ' +
          ...
          runWithFiberInDEV(fiber, () => {
            console.error(
              'Cannot use a ref on a Devjs element as a container to `createRoot` or `createPortal` ' +
                'if that element also sets "dangerouslySetInnerHTML" using Devjs. It should be a leaf with no children. ' +
                "Otherwise it's ambiguous which children should be used.",
            );
          });
        }
...
export function appendChildToContainer(
  ...
  if (__DEV__) {
    warnForDevjsChildrenConflict(container);
  }
  ...
  // through the Devjs tree. However, on Mobile Safari the click would
  // never bubble through the *DOM* tree unless an ancestor with onclick
  // event exists. So we wouldn't see it and dispatch it.
  // This is why we ensure that non Devjs root containers have inline onclick
  // defined.
  // https://github.com/Suryanshu-Nabheet/dev.js/issues/11918
  const devjsRootContainer = container._devjsRootContainer;
  if (
    (devjsRootContainer === null || devjsRootContainer === undefined) &&
...
function dispatchBeforeDetachedBlur(
  ...
  if (enableCreateEventHandleAPI) {
    ...
    // can propagate through the Devjs internal tree.
...
export function restoreRootViewTransitionName(rootContainer: Container): void {
  ...
  if (rootContainer.nodeType === DOCUMENT_NODE) {
    ...
    // clone the whole document outside of the Devjs too.
        ...
        if (__DEV__) {
          ...
          return new Error(
            'A ViewTransition timed out because a Navigation stalled. ' +
              'This can happen if a Navigation is blocked on Devjs itself. ' +
        ...
        if (__DEV__) {
          ...
          return new Error(
            'A ViewTransition was aborted early. This might be because you have ' +
              ...
              "a time. To avoid this, use only Devjs's built-in <ViewTransition> " +
              'to coordinate.',
            {cause: error},
          );
        }
...
export function startViewTransition(
  ...
  try {
    ...
    // $FlowFixMe[prop-missing]
    ownerDocument.__devjsViewTransition = transition;
    ...
    const handleError = (error: mixed) => {
      // $FlowFixMe[prop-missing]
      if (ownerDocument.__devjsViewTransition === transition) {
        // $FlowFixMe[prop-missing]
        ownerDocument.__devjsViewTransition = null;
      }
      try {
...
FragmentInstance.prototype.compareDocumentPosition = function (
  ...
  // the state of the Devjs tree. If it doesn't, we have a case of portaled or
...
function addFragmentHandleToInstance(
  ...
  if (enableFragmentRefsInstanceHandles) {
    if (instance.unstable_devjsFragments == null) {
      instance.unstable_devjsFragments = new Set();
    }
    instance.unstable_devjsFragments.add(fragmentInstance);
  }
}
...
export function deleteChildFromFragmentInstance(
  ...
  if (enableFragmentRefsInstanceHandles) {
    if (instance.unstable_devjsFragments != null) {
      instance.unstable_devjsFragments.delete(fragmentInstance);
    }
  }
}
  ...
  while (nextNode) {
    ...
    switch (node.nodeName) {
      ...
      case 'BODY': {
        ...
        // If these singleton instances had previously been rendered with Devjs they
      ...
      // yet until the closing tag is parsed. If something causes Devjs to call clearContainer while
    ...
    if (element.nodeName.toLowerCase() !== type.toLowerCase()) {
      ...
      switch (type) {
        ...
        case 'script': {
          ...
          // in theory lead to a hydration error later if a 3rd party injected an inline script before the Devjs rendered nodes.
...
export function describeHydratableInstanceForDevWarnings(
  ...
  // Reverse engineer a pseudo devjs-element from hydratable instance
...
export function resolveSingletonInstance(
  ...
  switch (type) {
    case 'html': {
      ...
      if (!documentElement) {
        throw new Error(
          'Devjs expected an <html> element (document.documentElement) to exist in the Document but one was' +
            ' not found. Devjs never removes the documentElement for any Document it renders into so' +
    ...
    case 'head': {
      ...
      if (!head) {
        throw new Error(
          'Devjs expected a <head> element (document.head) to exist in the Document but one was' +
            ' not found. Devjs never removes the head for any Document it renders into so' +
            ' the cause is likely in some other script running on this page.',
        );
      }
      return head;
    ...
    case 'body': {
      ...
      if (!body) {
        throw new Error(
          'Devjs expected a <body> element (document.body) to exist in the Document but one was' +
            ' not found. Devjs never removes the body for any Document it renders into so' +
            ' the cause is likely in some other script running on this page.',
        );
      }
      return body;
    ...
    default: {
      throw new Error(
        'resolveSingletonInstance was called with an element type that is not supported. This is a bug in Devjs.',
      );
    }
  }
}
...
export function acquireSingletonInstance(
  ...
  if (__DEV__) {
    ...
    switch (type) {
      ...
      default: {
        console.error(
          'acquireSingletonInstance was called with an element type that is not supported. This is a bug in Devjs.',
        );
      }
    }
  }
...
function flushSyncWork() {
  if (disableLegacyMode) {
    ...
    throw new Error(
      'flushSyncWork should not be called from builds that support legacy mode. This is a bug in Devjs.',
    );
  }
}
...
function requestFormReset(form: HTMLFormElement) {
  ...
  if (
    ...
    // This form was either not rendered by this Devjs renderer (or it's an
...
// how we resolve the HoistableRoot for DevjsDOM.pre*() methods. Because we support calling
...
// This is notable because nowhere else in DevjsDOM do we actually reference the global document or window
...
export function getResource(
  ...
  if (!resourceRoot) {
    throw new Error(
      '"resourceRoot" was expected to exist. This is a bug in Devjs.',
    );
  }
  switch (type) {
    case 'meta':
  ...
  + ${describeLinkForResourceErrorDEV(pendingProps)}`;
    ...
    default: {
      throw new Error(
        `getResource encountered a type it did not expect: "${type}". this is a bug in Devjs.`,
      );
    }
  }
}
...
export function acquireResource(
  ...
  if (resource.instance === null) {
    switch (resource.type) {
      ...
      default: {
        throw new Error(
          `acquireResource encountered a resource type it did not expect: "${resource.type}". this is a bug in Devjs.`,
        );
      }
    }
  } else {
...
export function hydrateHoistable(
  ...
  getInstance: switch (type) {
    ...
    default:
      throw new Error(
        `getNodesForType encountered a type it did not expect: "${type}". This is a bug in Devjs.`,
      );
  }
      ...
      if (
        ...
        console.error(
          'Cannot render a <%s> outside the main document if it has an `itemProp` prop. `itemProp` suggests the tag belongs to an' +
            ' `itemScope` which can appear anywhere in the DOM. If you were intending for Devjs to hoist this <%s> remove the `itemProp` prop.' +
          ...
          if (outsideHostContainerContext) {
            console.error(
              'Cannot render a <style> outside the main document without knowing its precedence and a unique href key.' +
                ' Devjs can hoist and deduplicate <style> tags if you provide a `precedence` prop along with an `href` prop that' +
...
export function suspendOnActiveViewTransition(
  ...
  const ownerDocument =
    rootContainer.nodeType === DOCUMENT_NODE
      ...
      : rootContainer.ownerDocument;
  // $FlowFixMe[prop-missing]
  const activeViewTransition = ownerDocument.__devjsViewTransition;
...
export const HostTransitionContext: DevjsContext<TransitionStatus> = {
