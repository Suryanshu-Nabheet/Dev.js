...
import type {KnownDevjsSyntheticEvent} from '../DevjsSyntheticEventType';
...
import {getNearestMountedFiber} from 'devjs-reconciler/src/DevjsFiberTreeReflection';
...
function extractEvents(
  ...
  if (isOverEvent && !isReplayingEvent(nativeEvent)) {
    ...
    if (related) {
      // If the related node is managed by Devjs, we can assume that we have
  ...
  const leave: KnownDevjsSyntheticEvent = new SyntheticEventCtor(
    leaveEventType,
  ...
  let enter: KnownDevjsSyntheticEvent | null = null;
  ...
  if (nativeTargetInst === targetInst) {
    const enterEvent: KnownDevjsSyntheticEvent = new SyntheticEventCtor(
      enterEventType,
