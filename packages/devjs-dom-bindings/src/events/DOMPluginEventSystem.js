...
import {getLowestCommonAncestor} from 'devjs-reconciler/src/DevjsFiberTreeReflection';
...
} from '../client/DevjsDOMComponentTree';
import {COMMENT_NODE, DOCUMENT_NODE} from '../client/HTMLNodeType';
import {batchedUpdates} from './DevjsDOMUpdateBatching';
...
import {createEventListenerWrapperWithPriority} from './DevjsDOMEventListener';
import {
  removeEventListener,
...
function extractEvents(
  ...
  // us to ship builds of Devjs without the polyfilled plugins below.
...
const listeningMarker = '_devjsListening' + Math.random().toString(36).slice(2);
...
function addTrappedEventListener(
  ...
  if (passiveBrowserEventsSupported) {
    ...
    // passive by default on document. Devjs doesn't bind them
    // to document anymore, but changing this now would undo
    // the performance wins from the change. So we emulate
    // the existing behavior manually on the roots now.
    // https://github.com/Suryanshu-Nabheet/dev.js/issues/19651
    if (
...
export function dispatchEventForPluginEventSystem(
  ...
  if (
    ...
    if (
      ...
      // match Devjs < 16 behavior of delegated clicks to the doc.
      domEventName === 'click' &&
    ...
    if (targetInst !== null) {
      ...
      // attached to each Devjs Root and Devjs Portal Root. Together,
...
export function accumulateSinglePhaseListeners(
  ...
  devjsName: string | null,
  nativeEventType: string,
  inCapturePhase: boolean,
  accumulateTargetOnly: boolean,
  nativeEvent: AnyNativeEvent,
): Array<DispatchListener> {
  const captureName = devjsName !== null ? devjsName + 'Capture' : null;
  const devjsEventName = inCapturePhase ? captureName : devjsName;
  ...
  while (instance !== null) {
    ...
    if (
      ...
      // Standard Devjs on* listeners, i.e. onClick or onClickCapture
      if (devjsEventName !== null) {
        const listener = getListener(instance, devjsEventName);
        if (listener != null) {
          listeners.push(
      ...
      // Scopes
      const devjsScopeInstance = stateNode;
      const eventHandlerListeners =
        getEventHandlerListeners(devjsScopeInstance);
      if (eventHandlerListeners !== null) {
        eventHandlerListeners.forEach(entry => {
          if (
            entry.type === nativeEventType &&
    ...
    // continue to propagate through the Devjs fiber tree to find other
    // listeners.
    if (accumulateTargetOnly) {
      break;
    }
    // If we are processing the onBeforeBlur event, then we need to take
    // into consideration that part of the Devjs tree might have been hidden
...
export function accumulateTwoPhaseListeners(
  ...
  const captureName = devjsName + 'Capture';
  ...
  while (instance !== null) {
    ...
    if (
      ...
      const bubbleListener = getListener(instance, devjsName);
...
function getParent(inst: Fiber | null): Fiber | null {
  ...
  do {
    ...
    // host node but that wouldn't work for Devjs Native and doesn't let us
...
function accumulateEnterLeaveListenersForEvent(
  dispatchQueue: DispatchQueue,
  event: KnownDevjsSyntheticEvent,
  target: Fiber,
  common: Fiber | null,
  inCapturePhase: boolean,
): void {
  const registrationName = event._devjsName;
  const listeners: Array<DispatchListener> = [];
...
export function accumulateEnterLeaveTwoPhaseListeners(
  dispatchQueue: DispatchQueue,
  leaveEvent: KnownDevjsSyntheticEvent,
  enterEvent: null | KnownDevjsSyntheticEvent,
  from: Fiber | null,
  to: Fiber | null,
): void {
  const common =
...
export function accumulateEventHandleNonManagedNodeListeners(
  ...
  if (eventListeners !== null) {
    eventListeners.forEach(entry => {
      if (entry.type === devjsEventType && entry.capture === inCapturePhase) {
        listeners.push(
