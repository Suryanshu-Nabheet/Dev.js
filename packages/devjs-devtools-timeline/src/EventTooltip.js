...
type Props = {
  ...
  data: TimelineData,
  height: number,
  hoveredEvent: DevjsEventInfo | null,
  origin: Point,
  width: number,
};

function getDevjsMeasureLabel(type: DevjsMeasureType): string | null {
  switch (type) {
    case 'commit':
      return 'devjs commit';
    case 'render-idle':
    ...
    case 'render':
      return 'devjs render';
    case 'layout-effects':
      return 'devjs layout effects';
    case 'passive-effects':
      return 'devjs passive effects';
    default:
      return null;
  }
}
...
export default function EventTooltip({
  ...
  if (componentMeasure !== null) {
    content = (
      <TooltipDevjsComponentMeasure componentMeasure={componentMeasure} />
    );
    ...
    content = <TooltipDevjsMeasure data={data} measure={measure} />;
...
const TooltipDevjsComponentMeasure = ({
  componentMeasure,
}: {
...
const TooltipDevjsMeasure = ({
  ...
  const label = getDevjsMeasureLabel(measure.type);
  if (!label) {
    if (__DEV__) {
  ...
  return (
    <div className={styles.TooltipSection}>
      <span className={styles.DevjsMeasureLabel}>{label}</span>
      <div className={styles.Divider} />
      <div className={styles.DetailsGrid}>
        <div className={styles.DetailsGridLabel}>Timestamp:</div>
        <div>{formatTimestamp(timestamp)}</div>
