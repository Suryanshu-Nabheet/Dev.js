...
type ProcessorState = {
  ...
  batchUID: BatchUID,
  currentDevjsComponentMeasure: DevjsComponentMeasure | null,
...
const WARNING_STRINGS = {
  ...
  NESTED_UPDATE:
    ...
    'Nested updates require Devjs to re-render synchronously before the browser can paint. ' +
...
export function getLanesFromTransportDecimalBitmask(
  ...
  const lanes = [];
  let powersOfTwo = 0;
  while (powersOfTwo <= devjs_TOTAL_NUM_LANES) {
...
function markWorkStarted(
  ...
  lanes.forEach(lane => {
    ((currentProfilerData.laneToDevjsMeasureMap.get(
      lane,
    ): any): DevjsMeasure[]).push(measure);
  });
}
...
function processEventDispatch(
  ...
  if (type.startsWith('devjs-')) {
    const stackTrace = data.stackTrace;
    if (stackTrace) {
      const topFrame = stackTrace[stackTrace.length - 1];
      if (topFrame.url.includes('/devjs-dom.')) {
        // Filter out fake Devjs events dispatched by invokeGuardedCallbackDev.
        return;
      }
    }
  }
...
function processTimelineEvent(
  ...
  switch (cat) {
    ...
    case 'blink.user_timing':
      if (name.startsWith('--devjs-version-')) {
        const [devjsVersion] = name.slice(16).split('-');
        currentProfilerData.devjsVersion = devjsVersion;
      ...
      } else if (name.startsWith('--devjs-lane-labels-')) {
        ...
        for (let i = 0; i < state.nativeEventStack.length; i++) {
          ...
          // If Devjs work was scheduled during an event handler, and the event had a long duration,
          // it might be because the Devjs render was long and stretched the event.
          // It might also be that the Devjs work was short and that something else stretched the event.
          // Make a note of this event for now and we'll examine the batch of Devjs render work later.
          // (We can't know until we're done processing the Devjs update anyway.)
      ...
      } else if (name.startsWith('--devjs-internal-module-start-')) {
      ...
      } else if (name.startsWith('--devjs-internal-module-stop-')) {
...
function assertNoOverlappingComponentMeasure(state: ProcessorState) {
  if (state.currentDevjsComponentMeasure !== null) {
    console.error(
      'Component measure started while another measure in progress:',
      state.currentDevjsComponentMeasure,
    );
  }
}
...
function assertCurrentComponentMeasureType(
  ...
  } else if (state.currentDevjsComponentMeasure.type !== type) {
    console.error(
      `Component measure type "${type}" stopped while type ${state.currentDevjsComponentMeasure.type} in progress`,
    );
  }
}

function processDevjsComponentMeasure(
  name: string,
  startTime: Milliseconds,
  currentProfilerData: TimelineData,
  state: ProcessorState,
  ...
  if (name.startsWith('--component-render-start-')) {
    ...
    if (state.currentDevjsComponentMeasure !== null) {
      const componentMeasure = state.currentDevjsComponentMeasure;
      componentMeasure.duration = startTime - componentMeasure.timestamp;

      state.currentDevjsComponentMeasure = null;
...
function preprocessFlamechart(rawData: TimelineEvent[]): Flamechart {
  let parsedData;
  try {
    parsedData = importFromChromeTimeline(rawData, 'devjs-devtools');
  } catch (error) {
...
export default async function preprocessData(
  ...
  const laneToDevjsMeasureMap: Map<DevjsLane, Array<DevjsMeasure>> = new Map();
  for (let lane: DevjsLane = 0; lane < devjs_TOTAL_NUM_LANES; lane++) {
  ...
  // can deduce that there are no flame chart events. As we expect Devjs
  ...
  // don't bother finding Devjs events.
  ...
  if (profilerVersion === null) {
    if (
      ...
      // No profiler version could indicate data was logged using an older build of Devjs,
      // before an explicitly profiler version was included in the logging data.
      // But it could also indicate that the website was either not using Devjs, or using a production build.
      // The easiest way to check for this case is to see if the data contains any scheduled updates or render work.
      throw new InvalidProfileError(
        'No Devjs marks were found in the provided profile.' +
          ' Please provide profiling data from an Devjs application running in development or profiling mode.',
      );
    }
  ...
  state.potentialLongEvents.forEach(([nativeEvent, batchUID]) => {
    // See how long the subsequent batch of Devjs work was.
