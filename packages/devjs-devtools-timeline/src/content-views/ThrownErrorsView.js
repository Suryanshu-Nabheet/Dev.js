...
import {
  COLORS,
  TOP_ROW_PADDING,
  devjs_EVENT_DIAMETER,
  BORDER_SIZE,
} from './constants';

const EVENT_ROW_HEIGHT_FIXED =
  TOP_ROW_PADDING + devjs_EVENT_DIAMETER + TOP_ROW_PADDING;
...
export class ThrownErrorsView extends View {
  ...
  _drawSingleThrownError(
    ...
    const radius = devjs_EVENT_DIAMETER / 2;
    const eventRect: Rect = {
      origin: {
        x: x - radius,
        y: baseY,
      },
      size: {width: devjs_EVENT_DIAMETER, height: devjs_EVENT_DIAMETER},
    };
    ...
    const fillStyle = showHoverHighlight
      ? COLORS.devjs_THROWN_ERROR_HOVER
      : COLORS.devjs_THROWN_ERROR;
  ...
  draw(context: CanvasRenderingContext2D) {
    ...
    if (rectIntersectsRect(borderFrame, visibleArea)) {
      ...
      context.fillStyle = COLORS.devjs_WORK_BORDER;
      context.fillRect(
