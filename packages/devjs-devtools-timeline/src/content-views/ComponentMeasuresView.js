...
import type {DevjsComponentMeasure, TimelineData, ViewState} from '../types';
import type {
  Interaction,
  IntrinsicSize,
...
export class ComponentMeasuresView extends View {
  ...
  _hoveredComponentMeasure: DevjsComponentMeasure | null = null;
  _intrinsicSize: IntrinsicSize;
  _profilerData: TimelineData;
  _viewState: ViewState;

  onHover: ((event: DevjsComponentMeasure | null) => void) | null = null;
  ...
  setHoveredEvent(hoveredEvent: DevjsComponentMeasure | null) {
  ...
  /**
   * Draw a single `DevjsComponentMeasure` as a box/span with text inside of it.
   */
  _drawSingleDevjsComponentMeasure(
    context: CanvasRenderingContext2D,
    rect: Rect,
    componentMeasure: DevjsComponentMeasure,
    scaleFactor: number,
    ...
    if (warning !== null) {
      ...
      switch (type) {
        ...
        case 'layout-effect-mount':
          context.fillStyle = showHoverHighlight
            ? COLORS.devjs_LAYOUT_EFFECTS_HOVER
            : COLORS.devjs_LAYOUT_EFFECTS;
        ...
        case 'passive-effect-mount':
          context.fillStyle = showHoverHighlight
            ? COLORS.devjs_PASSIVE_EFFECTS_HOVER
            : COLORS.devjs_PASSIVE_EFFECTS;
  ...
  draw(context: CanvasRenderingContext2D) {
    ...
    componentMeasures.forEach(componentMeasure => {
      didDrawMeasure =
        this._drawSingleDevjsComponentMeasure(
          context,
          visibleArea,
          componentMeasure,
          scaleFactor,
    ...
    if (!didDrawMeasure) {
      drawText(
        '(zoom or pan to see Devjs components)',
        context,
        visibleArea,
        visibleArea,
