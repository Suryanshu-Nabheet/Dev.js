...
import type {DevjsContext, RefObject} from 'shared/DevjsTypes';
...
export type Context = {
  ...
  viewState: ViewState,
  selectEvent: DevjsEventInfo => void,
  selectedEvent: DevjsEventInfo,
};

const TimelineContext: DevjsContext<Context> = createContext<Context>(
  ((null: any): Context),
);
...
function TimelineContextController({children}: Props): Devjs.Node {
  ...
  const [selectedEvent, selectEvent] = useState<DevjsEventInfo | null>(null);
