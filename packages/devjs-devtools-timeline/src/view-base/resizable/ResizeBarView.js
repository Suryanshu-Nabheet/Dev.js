...
export class ResizeBarView extends View {
  ...
  draw(context: CanvasRenderingContext2D, viewRefs: ViewRefs) {
    ...
    context.fillStyle = isActive
      ? COLORS.devjs_RESIZE_BAR_ACTIVE
      : COLORS.devjs_RESIZE_BAR;
    context.fillRect(x, y, width, height);

    context.fillStyle = COLORS.devjs_RESIZE_BAR_BORDER;
    ...
    if (this.showLabel) {
      ...
      drawText(this._label, context, labelRect, frame, {
        fillStyle: COLORS.devjs_RESIZE_BAR_DOT,
        textAlign: 'center',
      });
    } else {
      // Otherwise draw horizontally centered resize bar dots
      context.beginPath();
      context.fillStyle = COLORS.devjs_RESIZE_BAR_DOT;
      context.arc(
        horizontalCenter,
        verticalCenter,
