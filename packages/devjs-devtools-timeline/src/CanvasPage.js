...
import type {DevjsEventInfo, TimelineData, ViewState} from './types';
...
import AutoSizer from 'devjs-virtualized-auto-sizer';
...
import type {ContextMenuRef} from 'devjs-devtools-shared/src/devtools/ContextMenu/types';
...
function CanvasPage({profilerData, viewState}: Props): Devjs.Node {
  return (
    <div
...
const EMPTY_CONTEXT_INFO: DevjsEventInfo = {
...
function AutoSizedCanvas({
  ...
  const [hoveredEvent, setHoveredEvent] = useState<DevjsEventInfo | null>(null);
  const [lastHoveredEvent, setLastHoveredEvent] =
    useState<DevjsEventInfo | null>(null);
  ...
  const devjsMeasuresViewRef = useRef<null | DevjsMeasuresView>(null);
  ...
  useLayoutEffect(() => {
    ...
    const devjsMeasuresView = new DevjsMeasuresView(
      surface,
      defaultFrame,
      data,
    );
    devjsMeasuresViewRef.current = devjsMeasuresView;
    const devjsMeasuresViewWrapper = createViewHelper(
      devjsMeasuresView,
      'devjs scheduling',
      true,
      true,
    ...
    if (devjsMeasuresViewWrapper !== null) {
      rootView.addSubview(devjsMeasuresViewWrapper);
    }
  ...
  const interactor = useCallback((interaction: Interaction) => {
    ...
    // For more info see: https://github.com/Suryanshu-Nabheet/dev.js/pull/22005#issuecomment-896953399
    surface.displayIfNeeded();

    canvas.style.cursor = surface.getCurrentCursor() || 'default';

    // Defer drawing to canvas until Devjs's commit phase, to avoid drawing
  ...
  useEffect(() => {
    ...
    const {current: devjsMeasuresView} = devjsMeasuresViewRef;
    if (devjsMeasuresView) {
      devjsMeasuresView.onHover = measure => {
  ...
  useLayoutEffect(() => {
    ...
    const devjsMeasuresView = devjsMeasuresViewRef.current;
    if (devjsMeasuresView) {
      devjsMeasuresView.setHoveredMeasure(
  ...
  // Draw to canvas in Devjs's commit phase
  useLayoutEffect(() => {
    surfaceRef.current.displayIfNeeded();
  });
