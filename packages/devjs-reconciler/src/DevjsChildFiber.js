...
import type {Fiber} from './DevjsInternalTypes';
...
import {NoMode, ConcurrentMode} from './DevjsTypeOfMode';
import {
  getIteratorFn,
  ASYNC_ITERATOR,
  devjs_ELEMENT_TYPE,
...
import {isCompatibleFamilyForHotReloading} from './DevjsFiberHotReloading';
import {getIsHydrating} from './DevjsFiberHydrationContext';
import {pushTreeFork} from './DevjsFiberTreeContext';
import {
  SuspenseException,
...
import {readContextDuringReconciliation} from './DevjsFiberNewContext';

import {runWithFiberInDEV} from './DevjsCurrentFiber';
...
let currentDebugInfo: null | DevjsDebugInfo = null;

function pushDebugInfo(
  debugInfo: null | DevjsDebugInfo,
): null | DevjsDebugInfo {
  if (!__DEV__) {
    return null;
  }
    ...
    if (debugInfo != null) {
      for (let i = debugInfo.length - 1; i >= 0; i--) {
        if (debugInfo[i].name != null) {
          const componentInfo: DevjsComponentInfo = debugInfo[i];
        ...
        runWithFiberInDEV(
          ...
          erroredKey => {
            if (enableFragmentRefs) {
              console.error(
                'Invalid prop `%s` supplied to `Devjs.Fragment`. ' +
                  'Devjs.Fragment can only have `key`, `ref`, and `children` props.',
                erroredKey,
              );
            } else {
              console.error(
                'Invalid prop `%s` supplied to `Devjs.Fragment`. ' +
                  'Devjs.Fragment can only have `key` and `children` props.',
                erroredKey,
              );
            }
          },
...
function coerceRef(workInProgress: Fiber, element: DevjsElement): void {
...
function throwOnInvalidObjectTypeImpl(returnFiber: Fiber, newChild: Object) {
  if (newChild.$$typeof === devjs_LEGACY_ELEMENT_TYPE) {
    throw new Error(
...
function createChildReconciler(
  ...
  function mapRemainingChildren(
  ...
  ): Map<string | number | DevjsOptimisticKey, Fiber> {
    ...
    while (existingChild !== null) {
      if (existingChild.key === null) {
        ...
        enableOptimisticKey &&
        existingChild.key === devjs_OPTIMISTIC_KEY
      ) {
  ...
  function updateElement(
    ...
    if (elementType === devjs_FRAGMENT_TYPE) {
    ...
    if (current !== null) {
      if (
        ...
        (typeof elementType === 'object' &&
          elementType !== null &&
          elementType.$$typeof === devjs_LAZY_TYPE &&
  ...
  function createChild(
    ...
    if (typeof newChild === 'object' && newChild !== null) {
      ...
      if (newChild.$$typeof === devjs_CONTEXT_TYPE) {
        const context: DevjsContext<mixed> = (newChild: any);
        return createChild(
          returnFiber,
  ...
  function updateFromMap(
    existingChildren: Map<string | number | DevjsOptimisticKey, Fiber>,
    returnFiber: Fiber,
    newIdx: number,
    newChild: any,
    lanes: Lanes,
      ...
      if (newFiber !== null) {
        ...
        if (shouldTrackSideEffects) {
          ...
          if (currentFiber !== null) {
            ...
            if (
              enableOptimisticKey &&
              currentFiber.key === devjs_OPTIMISTIC_KEY
            ) {
  ...
  function reconcileSingleElement(
    ...
    while (child !== null) {
      ...
      if (
        child.key === key ||
        (enableOptimisticKey && child.key === devjs_OPTIMISTIC_KEY)
      ) {
        const elementType = element.type;
        if (elementType === devjs_FRAGMENT_TYPE) {
    ...
    if (element.type === devjs_FRAGMENT_TYPE) {
      const created = createFiberFromFragment(
        element.props.children,
        returnFiber.mode,
        lanes,
  ...
  function reconcileChildFibersImpl(
    ...
    const isUnkeyedUnrefedTopLevelFragment =
      ...
      newChild !== null &&
      newChild.type === devjs_FRAGMENT_TYPE &&
      newChild.key === null &&
    ...
    if (
      ...
      if (isArray(children)) {
        ...
        if (typeof iteratorFn === 'function') {
          ...
          enableAsyncIterableChildren &&
          children.$$typeof === devjs_ELEMENT_TYPE &&
