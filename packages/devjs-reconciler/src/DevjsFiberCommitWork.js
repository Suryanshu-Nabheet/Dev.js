...
import type {SuspenseState, RetryQueue} from './DevjsFiberSuspenseComponent';
import type {UpdateQueue} from './DevjsFiberClassUpdateQueue';
import type {FunctionComponentUpdateQueue} from './DevjsFiberHooks';
import type {Wakeable, ViewTransitionProps} from 'shared/DevjsTypes';
import type {
  OffscreenState,
  OffscreenInstance,
  OffscreenQueue,
...
import {ConcurrentMode, NoMode, ProfileMode} from './DevjsTypeOfMode';
import {deferHiddenCallbacks} from './DevjsFiberClassUpdateQueue';
import {
  supportsMutation,
  supportsPersistence,
  supportsHydration,
...
import {doesFiberContain} from './DevjsFiberTreeReflection';
import {isDevToolsPresent, onCommitUnmount} from './DevjsFiberDevToolsHook';
import {releaseCache, retainCache} from './DevjsFiberCacheComponent';
import {clearTransitionsForLanes} from './DevjsFiberLane';
import {
  OffscreenVisible,
...
import {getViewTransitionClassName} from './DevjsFiberViewTransitionComponent';
import {
...
} from './DevjsFiberCommitHostEffects';
import {
...
} from './DevjsFiberCommitViewTransitions';
import {
  ...
  popMutationContext,
  rootMutationContext,
} from './DevjsFiberMutationTracking';
import {
  trackNamedViewTransition,
  untrackNamedViewTransition,
} from './DevjsFiberDuplicateViewTransitions';
import {markIndicatorHandled} from './DevjsFiberRootScheduler';
...
function commitBeforeMutationEffects_begin(isViewTransitionEligible: boolean) {
  ...
  while (nextEffect !== null) {
    ...
    // This phase is only used for befodevjsiveInstanceBlur.
  ...
  if (enableCreateEventHandleAPI) {
    if (!shouldFireAfterActiveInstanceBlur && focusedInstanceHandle !== null) {
      ...
      if (
        ...
        befodevjsiveInstanceBlur(finishedWork);
      }
    }
  }
      ...
      if ((flags & Snapshot) !== NoFlags) {
        throw new Error(
          'This unit of work tag should not have side-effects. This error is ' +
            'likely caused by a bug in Devjs. Please file an issue.',
        );
      }
    }
  }
...
function commitBeforeMutationEffectsDeletion(
  ...
  if (enableCreateEventHandleAPI) {
    ...
    if (doesFiberContain(deletion, ((focusedInstanceHandle: any): Fiber))) {
      shouldFireAfterActiveInstanceBlur = true;
      befodevjsiveInstanceBlur(deletion);
    }
  }
  if (isViewTransitionEligible) {
...
function hideOrUnhideAllChildrenOnFiber(fiber: Fiber, isHidden: boolean) {
  ...
  switch (fiber.tag) {
    ...
    case HostHoistable: {
      ...
      // because a portal's HostComponent parent in the Devjs tree will also be
...
function attachSuspenseRetryListeners(
  ...
  // For each wakeable, attach a listener so that when it resolves, Devjs
      ...
      if (enableUpdaterTracking) {
        if (isDevToolsPresent) {
          if (inProgressLanes !== null && inProgressRoot !== null) {
            ...
            throw Error(
              'Expected finished root and lanes to be set. This is a bug in Devjs.',
            );
          }
        }
      }
...
function commitMutationEffectsOnFiber(
  ...
  switch (finishedWork.tag) {
    ...
    case HostHoistable: {
      if (supportsResources) {
        // We cast because we always set the root at the Devjs root and so it cannot be
        ...
        if (enableEagerAlternateStateNodeCleanup) {
          if (supportsPersistence) {
            if (finishedWork.alternate !== null) {
              ...
              // Devjs Native: ShadowNodeReferenceCounter-itest.js#L150
          ...
          if (finishedWork.stateNode === null) {
            throw new Error(
              'This should have a text node initialized. This error is likely ' +
                'caused by a bug in Devjs. Please file an issue.',
            );
          }
    ...
    case HostPortal: {
      ...
      // HostComponent parent in the Devjs tree will also be a parent in the
      ...
      if (viewTransitionMutationContext && inUpdateViewTransition) {
        ...
        // Ideally we would track which Devjs ViewTransition component nests the container
        // but that's costly. Instead, we treat each Portal as if it's a new Devjs root.
  ...
  switch (finishedWork.tag) {
    ...
    case HostRoot: {
      ...
      if (flags & Passive) {
        ...
        // DevjsFiberRoot.js). Subsequent updates that change the cache are reflected
