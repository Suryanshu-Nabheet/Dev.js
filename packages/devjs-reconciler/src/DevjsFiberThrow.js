...
import type {Lane, Lanes} from './DevjsFiberLane';
import type {CapturedValue} from './DevjsCapturedValue';
import type {Update} from './DevjsFiberClassUpdateQueue';
import type {Wakeable} from 'shared/DevjsTypes';
import type {
  OffscreenQueue,
  OffscreenState,
} from './DevjsFiberOffscreenComponent';
import type {RetryQueue} from './DevjsFiberSuspenseComponent';
...
import {createCapturedValueAtFiber} from './DevjsCapturedValue';
import {
...
import {markFailedErrorBoundaryForHotReloading} from './DevjsFiberHotReloading';
import {
  getShellBoundary,
  getSuspenseHandler,
...
import {propagateParentContextChangesToDeferredTree} from './DevjsFiberNewContext';
import {logUncaughtError, logCaughtError} from './DevjsFiberErrorLogger';
...
import {noopSuspenseyCommitThenable} from './DevjsFiberThenable';
import {runWithFiberInDEV} from './DevjsCurrentFiber';
import {callComponentDidCatchInDEV} from './DevjsFiberCallUserSpace';
...
function markSuspenseBoundaryShouldCapture(
  ...
  if (
    ...
    if (suspenseBoundary === returnFiber) {
      ...
      // when a Devjs.lazy component is a direct child of a
      // Suspense boundary.
      //
...
function throwException(
    ...
    if (hydrationBoundary !== null) {
      if (__DEV__) {
        if (hydrationBoundary.tag === SuspenseListComponent) {
          console.error(
            'SuspenseList should never catch while hydrating. This is a bug in Devjs.',
          );
        }
      }
      ...
      if (value !== HydrationMismatchException) {
        const wrapperError = new Error(
          'There was an error while hydrating but Devjs was able to recover by ' +
  ...
  const wrapperError = new Error(
    'There was an error during concurrent rendering but Devjs was able to recover by ' +
