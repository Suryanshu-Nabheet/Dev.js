...
import {get as getInstance, set as setInstance} from 'shared/DevjsInstanceMap';
import shallowEqual from 'shared/shallowEqual';
import getComponentNameFromFiber from 'devjs-reconciler/src/getComponentNameFromFiber';
import getComponentNameFromType from 'shared/getComponentNameFromType';
import assign from 'shared/assign';
import isArray from 'shared/isArray';
import {devjs_CONTEXT_TYPE, devjs_CONSUMER_TYPE} from 'shared/DevjsSymbols';

import {NoMode, StrictLegacyMode, StrictEffectsMode} from './DevjsTypeOfMode';
...
import {NoLanes} from './DevjsFiberLane';
import {
  cacheContext,
  getMaskedContext,
  getUnmaskedContext,
...
import {requestUpdateLane, scheduleUpdateOnFiber} from './DevjsFiberWorkLoop';
import {
  markForceUpdateScheduled,
  markStateUpdateScheduled,
  setIsStrictModeForDevtools,
} from './DevjsFiberDevToolsHook';
import {startUpdateTimerByLane} from './DevjsProfilerTimer';
...
function checkShouldComponentUpdate(
  ...
  if (ctor.prototype && ctor.prototype.isPureDevjsComponent) {
    return (
...
function checkClassInstance(workInProgress: Fiber, ctor: any, newProps: any) {
  ...
  if (__DEV__) {
    ...
    if (
      ...
      !instance.getInitialState.isDevjsClassApproved &&
      !instance.state
    ) {
      console.error(
    ...
    if (
      ...
      !instance.getDefaultProps.isDevjsClassApproved
    ) {
      console.error(
    ...
    if (disableLegacyContext) {
      if (ctor.childContextTypes && !didWarnAboutChildContextTypes.has(ctor)) {
        ...
        console.error(
          '%s uses the legacy childContextTypes API which was removed in Devjs 19. ' +
      ...
      if (ctor.contextTypes && !didWarnAboutContextTypes.has(ctor)) {
        ...
        console.error(
          '%s uses the legacy contextTypes API which was removed in Devjs 19. ' +
            'Use Devjs.createContext() with static contextType instead. ' +
    ...
    if (
      ctor.prototype &&
      ctor.prototype.isPureDevjsComponent &&
    ...
    if (typeof instance.componentDidReceiveProps === 'function') {
      console.error(
        '%s has a method called ' +
          ...
          'run side-effects or mutations after Devjs has updated the UI, use componentDidUpdate().',
        name,
      );
    }
...
function constructClassInstance(
    ...
    if ('contextType' in ctor) {
      const isValid =
        ...
        (contextType !== undefined &&
          contextType.$$typeof === devjs_CONTEXT_TYPE);
      ...
      if (!isValid && !didWarnAboutInvalidateContextType.has(ctor)) {
        ...
        } else if (contextType.$$typeof === devjs_CONSUMER_TYPE) {
        ...
        console.error(
          '%s defines an invalid contextType. ' +
            'contextType should point to the Context object returned by Devjs.createContext().%s',
          getComponentNameFromType(ctor) || 'Component',
          addendum,
        );
      }
  ...
  if (__DEV__) {
    instance._devjsInternalInstance = fakeInternalInstance;
  }
  ...
  if (__DEV__) {
    ...
    // Don't warn about devjs-lifecycles-compat polyfilled methods though.
    if (
  ...
  // DevjsFiberLegacyContext usually updates this cache but can't for newly-created instances.
...
function mountClassInstance(
  ...
  if (__DEV__) {
    ...
    if (workInProgress.mode & StrictLegacyMode) {
      DevjsStrictModeWarnings.recordLegacyContextWarning(
        workInProgress,
        instance,
      );
    }

    DevjsStrictModeWarnings.recordUnsafeLifecycleWarnings(
      workInProgress,
      instance,
    );
  }
  ...
  // In order to support devjs-lifecycles-compat polyfilled components,
