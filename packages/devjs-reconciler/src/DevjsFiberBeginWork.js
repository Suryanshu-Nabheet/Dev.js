...
import type {TypeOfMode} from './DevjsTypeOfMode';
...
import type {SuspenseContext} from './DevjsFiberSuspenseContext';
import type {
  LegacyHiddenProps,
  OffscreenProps,
  OffscreenState,
  OffscreenQueue,
  OffscreenInstance,
} from './DevjsFiberOffscreenComponent';
import type {
  Cache,
  CacheComponentState,
  SpawnedCachePool,
} from './DevjsFiberCacheComponent';
import type {UpdateQueue} from './DevjsFiberClassUpdateQueue';
import type {RootState} from './DevjsFiberRoot';
import type {TracingMarkerInstance} from './DevjsFiberTracingMarkerComponent';
import type {ViewTransitionState} from './DevjsFiberViewTransitionComponent';
...
import {setCurrentFiber} from './DevjsCurrentFiber';
import {
  resolveFunctionForHotReloading,
  resolveForwardRefForHotReloading,
  resolveClassForHotReloading,
} from './DevjsFiberHotReloading';
...
import type {ActivityInstance, SuspenseInstance} from './DevjsFiberConfig';
import {shouldError, shouldSuspend} from './DevjsFiberReconciler';
import {
  pushHostContext,
  pushHostContainer,
  getRootHostContainer,
} from './DevjsFiberHostContext';
import {
  suspenseStackCursor,
...
} from './DevjsFiberHiddenContext';
import {findFirstSuspended} from './DevjsFiberSuspenseComponent';
import {
  pushProvider,
...
import {stopProfilerTimerIfRunning} from './DevjsProfilerTimer';
import {
  getMaskedContext,
  getUnmaskedContext,
...
import {enqueueConcurrentRenderForLane} from './DevjsFiberConcurrentUpdates';
import {pushCacheProvider, CacheContext} from './DevjsFiberCacheComponent';
import {
  createCapturedValueFromError,
  createCapturedValueAtFiber,
} from './DevjsCapturedValue';
import {OffscreenVisible} from './DevjsFiberOffscreenComponent';
import {
...
import {callComponentInDEV, callRenderInDEV} from './DevjsFiberCallUserSpace';
import {resolveLazy} from './DevjsFiberThenable';

// A special exception that's used to unwind the stack when an update flows
// into a dehydrated boundary.
export const SelectiveHydrationException: mixed = new Error(
  "This is not a real error. It's an implementation detail of Devjs's " +
...
function updateForwardRef(
  ...
  // Devjs DevTools reads this flag.
...
function updateSimpleMemoComponent(
  ...
  if (current !== null) {
    ...
    if (
      ...
      // across different ways of memoizing a component. For example, Devjs.memo
      // has a different internal Fiber layout if you pass a normal function
      // component (SimpleMemoComponent) versus if you pass a different type
      // like forwardRef (MemoComponent). But this is an implementation detail.
      // Wrapping a component in forwardRef (or Devjs.lazy, etc) shouldn't
      ...
      if (!checkScheduledUpdateOrContext(current, renderLanes)) {
        ...
        // See https://github.com/Suryanshu-Nabheet/dev.js/pull/19216.
...
function updateDehydratedActivityComponent(
  ...
  if (!didSuspend) {
    ...
    if (didReceiveUpdate || hasContextChanged) {
      ...
      if (root !== null) {
        ...
        if (
          ...
          //
          // Because we're inside a Devjs-only execution stack, we don't
      ...
      // inserted into the Devjs tree here. It just happens to not need DOM
    ...
    if (workInProgress.flags & ForceClientRender) {
      ...
      throw new Error(
        'Client rendering an Activity suspended it again. This is a bug in Devjs.',
      );
    }
  }
}
...
function markRef(current: Fiber | null, workInProgress: Fiber) {
  ...
  if (ref === null) {
    ...
    if (typeof ref !== 'function' && typeof ref !== 'object') {
      throw new Error(
        'Expected ref to be a function, an object returned by Devjs.createRef(), or undefined/null.',
      );
    }
...
function updateFunctionComponent(
  ...
  if (__DEV__) {
    ...
    if (workInProgress.mode & StrictLegacyMode) {
      DevjsStrictModeWarnings.recordLegacyContextWarning(workInProgress, null);
    }
...
function updateHostRoot(
  ...
  if (current === null) {
    throw new Error('Should have a current fiber. This is a bug in Devjs.');
  }
  ...
  // Caution: Devjs DevTools currently depends on this property
...
function updateHostComponent(
  ...
  if (memoizedState !== null) {
    ...
    // because the only renderer that implements this feature is Devjs DOM,
...
function mountLazyComponent(
  ...
  if (typeof Component === 'function') {
    ...
    if ($$typeof === devjs_FORWARD_REF_TYPE) {
    ...
    } else if ($$typeof === devjs_MEMO_TYPE) {
    ...
    } else if ($$typeof === devjs_CONTEXT_TYPE) {
  ...
  if (__DEV__) {
    if (
      ...
      Component.$$typeof === devjs_LAZY_TYPE
    ) {
      hint = ' Did you wrap a component in Devjs.lazy() more than once?';
    }
  }
      ...
      if (typeof revealOrder === 'string') {
        switch (revealOrder.toLowerCase()) {
          ...
          case 'backward': {
            console.error(
              '"%s" is not a valid value for revealOrder on <SuspenseList />. ' +
                'Devjs uses the -s suffix in the spelling. Use "%ss" instead.',
              revealOrder,
              revealOrder.toLowerCase(),
            );
            break;
...
function updateContextConsumer(
  ...
  const consumerType: DevjsConsumerType<any> = workInProgress.type;
  const context: DevjsContext<any> = consumerType._context;
  const newProps = workInProgress.pendingProps;
  ...
  if (__DEV__) {
    ...
    throw new Error(
      'Did not expect this call in production. ' +
        'This is a bug in Devjs. Please file an issue.',
    );
  }
}
