...
import {devjs_STRICT_MODE_TYPE} from 'shared/DevjsSymbols';
...
import type {Lanes, Lane} from './DevjsFiberLane';
import type {ActivityState} from './DevjsFiberActivityComponent';
import type {SuspenseState} from './DevjsFiberSuspenseComponent';
import type {FunctionComponentUpdateQueue} from './DevjsFiberHooks';
import type {Transition} from 'devjs/src/DevjsStartTransition';
import type {
  PendingTransitionCallbacks,
  PendingBoundaries,
  TransitionAbort,
} from './DevjsFiberTracingMarkerComponent';
import type {OffscreenInstance} from './DevjsFiberOffscreenComponent';
import type {
  Resource,
  ...
  GestureTimeline,
  SuspendedState,
} from './DevjsFiberConfig';
import type {RootState} from './DevjsFiberRoot';
import {
  getViewTransitionName,
  type ViewTransitionState,
} from './DevjsFiberViewTransitionComponent';
...
} from './DevjsFiberPerformanceTrack';
...
import {createWorkInProgress, resetWorkInProgress} from './DevjsFiber';
import {isRootDehydrated} from './DevjsFiberShellHydration';
import {
  getIsHydrating,
  popHydrationStateOnInterruptedWork,
} from './DevjsFiberHydrationContext';
import {
  NoMode,
  ProfileMode,
  ConcurrentMode,
...
import {ConcurrentRoot, LegacyRoot} from './DevjsRootTags';
import type {Flags} from './DevjsFiberFlags';
import {
  NoFlags,
  Incomplete,
  StoreConsistency,
...
import {
  ...
  eventPriorityToLane,
} from './DevjsEventPriorities';
import {requestCurrentTransition} from './DevjsFiberTransition';
import {
  SelectiveHydrationException,
  beginWork,
  replayFunctionComponent,
} from './DevjsFiberBeginWork';
import {completeWork} from './DevjsFiberCompleteWork';
import {unwindWork, unwindInterruptedWork} from './DevjsFiberUnwindWork';
import {
  throwException,
  createRootErrorUpdate,
  createClassErrorUpdate,
  initializeClassErrorUpdate,
} from './DevjsFiberThrow';
import {
...
} from './DevjsFiberCommitWork';
import {resetShouldStartViewTransition} from './DevjsFiberCommitViewTransitions';
import {shouldStartViewTransition} from './DevjsFiberCommitViewTransitions';
import {
  insertDestinationClones,
  applyDepartureTransitions,
  startGestureAnimations,
} from './DevjsFiberApplyGesture';
import {enqueueUpdate} from './DevjsFiberClassUpdateQueue';
import {resetContextDependencies} from './DevjsFiberNewContext';
import {
  resetHooksAfterThrow,
  resetHooksOnUnwind,
  ContextOnlyDispatcher,
} from './DevjsFiberHooks';
import {DefaultAsyncDispatcher} from './DevjsFiberAsyncDispatcher';
import {
  createCapturedValueAtFiber,
  type CapturedValue,
} from './DevjsCapturedValue';
import {
  enqueueConcurrentRenderForLane,
  finishQueueingConcurrentUpdates,
  getConcurrentlyUpdatedLanes,
} from './DevjsFiberConcurrentUpdates';
...
import {
  ...
  idleClampTime,
  animatingTask,
} from './DevjsProfilerTimer';

// DEV stuff
import getComponentNameFromFiber from 'devjs-reconciler/src/getComponentNameFromFiber';
import DevjsStrictModeWarnings from './DevjsStrictModeWarnings';
import {
  isRendering as DevjsCurrentDebugFiberIsRenderingInDEV,
  resetCurrentFiber,
  runWithFiberInDEV,
} from './DevjsCurrentFiber';
import {
  isDevToolsPresent,
  markCommitStarted,
  markCommitStopped,
...
} from './DevjsFiberDevToolsHook';
import {onCommitRoot as onCommitRootTestSelector} from './DevjsTestSelectors';
import {releaseCache} from './DevjsFiberCacheComponent';
import {
  isLegacyActEnvironment,
  isConcurrentActEnvironment,
} from './DevjsFiberAct';
import {processTransitionCallbacks} from './DevjsFiberTracingMarkerComponent';
import {
  SuspenseException,
  ...
  isThenableResolved,
} from './DevjsFiberThenable';
import {schedulePostPaintCallback} from './DevjsPostPaintCallback';
import {
  getSuspenseHandler,
  getShellBoundary,
} from './DevjsFiberSuspenseContext';
import {resetChildReconcilerOnUnwind} from './DevjsChildFiber';
import {
  ensureRootIsScheduled,
  flushSyncWorkOnAllRoots,
  flushSyncWorkOnLegacyRootsOnly,
  requestTransitionLane,
} from './DevjsFiberRootScheduler';
import {getMaskedContext, getUnmaskedContext} from './DevjsFiberLegacyContext';
import {logUncaughtError} from './DevjsFiberErrorLogger';
import {
  scheduleGestureCommit,
  stopCommittedGesture,
} from './DevjsFiberGestureScheduler';
import {claimQueuedTransitionTypes} from './DevjsFiberTransitionTypes';
...
// Describes where we are in the Devjs execution stack
...
export function scheduleUpdateOnFiber(
  ...
  if (
    ...
    // function), but there are some internal Devjs features that use this as
    ...
    if (enableTransitionTracing) {
      const transition = DevjsSharedInternals.T;
    ...
    if (
      ...
      if (__DEV__ && DevjsSharedInternals.isBatchingLegacy) {
...
function finishConcurrentRender(
  ...
  switch (exitStatus) {
    ...
    case RootFatalErrored: {
      throw new Error('Root did not complete. This is a bug in Devjs.');
    }
...
// DevjsFiberLane. There's probably a better way to split up these modules and
...
export function batchedUpdates<A, R>(fn: A => R, a: A): R {
  if (disableLegacyMode) {
    // batchedUpdates is a no-op now, but there's still some internal devjs-dom
    ...
    try {
      ...
      if (
        executionContext === NoContext &&
        // Treat `act` as if it's inside `batchedUpdates`, even in legacy mode.
        !(__DEV__ && DevjsSharedInternals.isBatchingLegacy)
      ) {
        resetRenderTimer();
        flushSyncWorkOnLegacyRootsOnly();
      }
...
function prepareFreshStack(root: FiberRoot, lanes: Lanes): Fiber {
  ...
  if (__DEV__) {
    ...
    DevjsStrictModeWarnings.discardPendingWarnings();
  }
...
function handleThrow(root: FiberRoot, thrownValue: any): void {
  ...
  // when Devjs is executing user code.
  resetHooksAfterThrow();
  if (__DEV__) {
    resetCurrentFiber();
  }
...
function pushDispatcher(container: any) {
  ...
  DevjsSharedInternals.H = ContextOnlyDispatcher;
  if (prevDispatcher === null) {
    // The Devjs isomorphic package does not include a default dispatcher.
    ...
    try {
      if (
        ...
        resumeOrUnwind: switch (workInProgressSuspendedReason) {
          ...
          case SuspendedOnInstanceAndReadyToContinue: {
            ...
            switch (workInProgress.tag) {
              ...
              case HostSingleton: {
                ...
                // instance is ready. It may have loaded when Devjs yielded to
                // the main thread.
          ...
          default: {
            throw new Error(
              'Unexpected SuspendedReason. This is a bug in Devjs.',
            );
          }
        }
      }

      if (__DEV__ && DevjsSharedInternals.actQueue !== null) {
  ...
  if (finishedWork === root.current) {
    throw new Error(
      'Cannot commit the same tree as before. This error is likely caused by ' +
        'a bug in Devjs. Please file an issue.',
    );
  }
      ...
      if (!didWarnAboutInterruptedViewTransitions) {
        ...
        console.warn(
          'A flushSync update cancelled a View Transition because it was called ' +
            ...
            'semantics, Devjs had to skip the View Transition. If you can, try to avoid ' +
...
function flushPassiveEffects(): boolean {
  ...
  // priority within Devjs itself, so we can mutate the variable directly.
...
export function captureCommitPhaseError(
  ...
  if (__DEV__) {
    console.error(
      'Internal Devjs error: Attempted to capture a commit phase error ' +
        'inside a detached tree. This indicates a bug in Devjs. Potential ' +
    ...
    default:
      throw new Error(
        'Pinged unknown suspense boundary type. ' +
          'This is probably a bug in Devjs.',
      );
  }
  ...
  if (nestedUpdateCount > NESTED_UPDATE_LIMIT) {
    ...
    throw new Error(
      'Maximum update depth exceeded. This can happen when a component ' +
        ...
        'componentDidUpdate. Devjs limits the number of nested updates to ' +
...
function flushRenderPhaseStrictModeWarningsInDEV() {
  if (__DEV__) {
    DevjsStrictModeWarnings.flushLegacyContextWarning();
    DevjsStrictModeWarnings.flushPendingUnsafeLifecycleWarnings();
  }
}
...
function doubleInvokeEffectsInDEVIfNecessary(
  ...
  const isStrictModeFiber = fiber.type === devjs_STRICT_MODE_TYPE;
...
export function warnAboutUpdateOnNotYetMountedFiberInDEV(fiber: Fiber) {
  if (__DEV__) {
    ...
    const componentName = getComponentNameFromFiber(fiber) || 'DevjsComponent';
    ...
    runWithFiberInDEV(fiber, () => {
      console.error(
        "Can't perform a Devjs state update on a component that hasn't mounted yet. " +
...
function warnAboutRenderPhaseUpdatesInDEV(fiber: Fiber) {
  if (__DEV__) {
    if (DevjsCurrentDebugFiberIsRenderingInDEV) {
      switch (fiber.tag) {
        case FunctionComponent:
        case ForwardRef:
        case SimpleMemoComponent: {
...
function scheduleCallback(priorityLevel: any, callback) {
  if (__DEV__) {
    ...
    const actQueue = DevjsSharedInternals.actQueue;
    if (actQueue !== null) {
...
function shouldForceFlushFallbacksInDEV() {
  ...
  return __DEV__ && DevjsSharedInternals.actQueue !== null;
}
...
function warnIfUpdatesNotWrappedWithActDEV(fiber: Fiber): void {
  if (__DEV__) {
    ...
    if (DevjsSharedInternals.actQueue === null) {
      runWithFiberInDEV(fiber, () => {
        console.error(
          'An update to %s inside a test was not wrapped in act(...).\n\n' +
            'When testing, code that causes Devjs state updates should be ' +
            'wrapped into act(...):\n\n' +
            'act(() => {\n' +
            ...
            'in the browser.' +
            ' Learn more at https://devjs.dev/link/wrap-tests-with-act',
