...
import type {StackCursor} from './DevjsFiberStack';
import type {Lanes} from './DevjsFiberLane';
import type {TransitionStatus} from './DevjsFiberConfig';
import type {Hook} from './DevjsFiberHooks';

import {isPrimaryRenderer, HostTransitionContext} from './DevjsFiberConfig';
import {createCursor, push, pop} from './DevjsFiberStack';
import {
  ContextProvider,
  DehydratedFragment,
  SuspenseComponent,
} from './DevjsWorkTags';
import {NoLanes, isSubsetOfLanes, mergeLanes} from './DevjsFiberLane';
import {
  NoFlags,
  DidPropagateContext,
  NeedsPropagation,
...
import {getHostTransitionProvider} from './DevjsFiberHostContext';
...
export function resetContextDependencies(): void {
  // This is called right before Devjs yields execution, to ensure `readContext`
    ...
    if (list !== null) {
      ...
      findChangedDep: while (dep !== null) {
        ...
        findContext: for (let i = 0; i < contexts.length; i++) {
          const context: DevjsContext<T> = contexts[i];
      ...
      if (parentSuspense === null) {
        throw new Error(
          'We just came from a parent so we must have had a parent. This is a bug in Devjs.',
        );
      }
...
function propagateParentContextChanges(
  ...
  while (parent !== null) {
    ...
    if (parent.tag === ContextProvider) {
      ...
      if (oldProps !== null) {
        const context: DevjsContext<any> = parent.type;
      ...
      // provider. E.g. in Devjs DOM, this would be a <form />.
      const currentParent = parent.alternate;
      if (currentParent === null) {
        throw new Error('Should have a current fiber. This is a bug in Devjs.');
      }
...
export function readContext<T>(context: DevjsContext<T>): T {
  if (__DEV__) {
...
function readContextForConsumer<T>(
  ...
  const contextItem = {
    context: ((context: any): DevjsContext<mixed>),
    memoizedValue: value,
    next: null,
  };
