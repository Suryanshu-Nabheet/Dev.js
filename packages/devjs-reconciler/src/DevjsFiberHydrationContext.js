...
import type {HydrationDiffNode} from './DevjsFiberHydrationDiffs';
...
import {createFiberFromDehydratedFragment} from './DevjsFiber';
import {
...
import {OffscreenLane} from './DevjsFiberLane';
import {
  getSuspendedTreeContext,
  restoreSuspendedTreeContext,
} from './DevjsFiberTreeContext';
import {queueRecoverableErrors} from './DevjsFiberWorkLoop';
import {getRootHostContainer, getHostContext} from './DevjsFiberHostContext';
import {describeDiff} from './DevjsFiberHydrationDiffs';
...
function buildHydrationDiffNode(
  ...
  if (fiber.return === null) {
    ...
    if (hydrationDiffRootDEV === null) {
      ...
      throw new Error(
        'Saw multiple hydration diff roots in a pass. This is a bug in Devjs.',
      );
...
function warnIfHydrating() {
  if (__DEV__) {
    if (isHydrating) {
      console.error(
        'We should not be hydrating here. This is a bug in Devjs. Please file a bug.',
      );
    }
  }
}
...
export const HydrationMismatchException: mixed = new Error(
  'Hydration Mismatch Exception: This is not a real error, and should not leak into ' +
    "userspace. If you're seeing this, it's likely a bug in Devjs.",
);
