...
import type {FragmentInstanceType} from './DevjsFiberConfig';
import type {ViewTransitionState} from './DevjsFiberViewTransitionComponent';

import {getViewTransitionName} from './DevjsFiberViewTransitionComponent';
...
import {NoFlags} from './DevjsFiberFlags';
import getComponentNameFromFiber from 'devjs-reconciler/src/getComponentNameFromFiber';
import {resolveClassComponentProps} from './DevjsFiberClassComponent';
import {
...
import {didWarnAboutReassigningProps} from './DevjsFiberBeginWork';
import {
        ...
        if (instance.props !== finishedWork.memoizedProps) {
          console.error(
            'Expected %s props to match memoized props before ' +
              'componentDidMount. ' +
              'This might either be because of a bug in Devjs, or because ' +
    ...
    if (shouldProfile(finishedWork)) {
      ...
      if (__DEV__) {
        runWithFiberInDEV(
          ...
          prevProps,
          prevState,
          instance.__devjsInternalSnapshotBeforeUpdate,
        );
      } else {
        try {
...
export function commitClassSnapshot(finishedWork: Fiber, current: Fiber) {
  ...
  try {
    ...
    instance.__devjsInternalSnapshotBeforeUpdate = snapshot;
  } catch (error) {
    captureCommitPhaseError(finishedWork, finishedWork.return, error);
  }
}
