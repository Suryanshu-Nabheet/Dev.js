...
describe('DevjsSuspenseWithNoopRenderer', () => {
  beforeEach(() => {
    jest.resetModules();

    Devjs = require('devjs');
    Fragment = Devjs.Fragment;
    ...
    getCacheForType = Devjs.unstable_getCacheForType;
  ...
  it('suspends rendering and continues later', async () => {
    ...
    Devjs.startTransition(() => {
      DevjsNoop.render(<Foo renderBar={true} />);
    });
    await waitForAll([
      'Foo',
      'Bar',
  ...
  it('when something suspends, unwinds immediately without rendering siblings', async () => {
    ...
    DevjsNoop.render(<Suspense fallback={<Text text="Loading..." />} />);
    await waitForAll([]);
    Devjs.startTransition(() => {
      DevjsNoop.render(
        <Suspense fallback={<Text text="Loading..." />}>
  ...
  it('retries on error', async () => {
    ...
    const errorBoundary = Devjs.createRef();
    ...
    Devjs.startTransition(() => {
      DevjsNoop.render(<App renderContent={true} />);
    });
  ...
  it('retries on error after falling back to a placeholder', async () => {
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="Loading..." />);
  ...
  it('can update at a higher priority while in a suspended state', async () => {
    let setHighPri;
    function HighPri() {
      const [text, setText] = Devjs.useState('A');
      setHighPri = setText;
    ...
    let setLowPri;
    function LowPri() {
      const [text, setText] = Devjs.useState('1');
      setLowPri = setText;
  ...
  it('keeps working on lower priority work after being pinged', async () => {
    ...
    DevjsNoop.render(<App showA={false} showB={false} />);
    await waitForAll([]);
    expect(DevjsNoop).toMatchRenderedOutput(null);

    Devjs.startTransition(() => {
      DevjsNoop.render(<App showA={true} showB={false} />);
    });
    await waitForAll(['Suspend! [A]', 'Loading...']);
    expect(DevjsNoop).toMatchRenderedOutput(null);

    Devjs.startTransition(() => {
      DevjsNoop.render(<App showA={true} showB={true} />);
    });
    await waitForAll([
      'Suspend! [A]',
      // pre-warming
  ...
  it('tries rendering a lower priority pending update even if a higher priority one suspends', async () => {
    ...
    // Low pri
    Devjs.startTransition(() => {
      DevjsNoop.render(<App hide={true} />);
    });
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="(empty)" />);
  });
  ...
  it('tries each subsequent level after suspending', async () => {
    ...
    function interrupt() {
      // Devjs has a heuristic to batch all updates that occur within the same
      // event. This is a trick to circumvent that heuristic.
      DevjsNoop.flushSync(() => {
        DevjsNoop.renderToRootWithID(null, 'other-root');
      });
    }
  ...
  it('suspending inside an expired expiration boundary will bubble to the next one', async () => {
    ...
    // The tree commits synchronously
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="Loading (outer)..." />);
  });
  ...
  it('in concurrent mode, does not error when an update suspends without a Suspense boundary during a sync update', () => {
    ...
    expect(() => {
      DevjsNoop.flushSync(() => {
        DevjsNoop.render(<AsyncText text="Async" />);
      });
    }).not.toThrow();
  });
  ...
  it('in legacy mode, errors when an update suspends without a Suspense boundary during a sync update', async () => {
    const root = DevjsNoop.createLegacyRoot();
    await expect(async () => {
  ...
  it('can resume rendering earlier than a timeout', async () => {
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="Async" />);
  });
  ...
  it('starts working on an update even if its priority falls between two suspended levels', async () => {
    ...
    DevjsNoop.render(<App text="S" />);
    await waitForAll(['S']);

    // Schedule an update, and suspend for up to 5 seconds.
    Devjs.startTransition(() => DevjsNoop.render(<App text="A" />));
    // The update should suspend.
    await waitForAll(['Suspend! [A]', 'Loading...']);
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="S" />);

    // Advance time until right before it expires.
    await advanceTimers(4999);
    DevjsNoop.expire(4999);
    await waitForAll([]);
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="S" />);

    // Schedule another low priority update.
    Devjs.startTransition(() => DevjsNoop.render(<App text="B" />));
    ...
    DevjsNoop.render(<App text="C" />);
    await waitForAll(['C']);
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="C" />);
  ...
  describe('legacy mode mode', () => {
    ...
    it('times out immediately', async () => {
      ...
      DevjsNoop.renderLegacySyncRoot(<App />);
      ...
      assertLog(['Result']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Result" />);
    });

    // @gate enableLegacyCache && !disableLegacyMode
    it('times out immediately when Suspense is in legacy mode', async () => {
      class UpdatingText extends Devjs.Component {
        state = {step: 1};
        render() {
      ...
      const text = Devjs.createRef(null);
      function App() {
        return (
    ...
    it('does not re-render siblings in loose mode', async () => {
      class TextWithLifecycle extends Devjs.Component {
        componentDidMount() {
      ...
      class AsyncTextWithLifecycle extends Devjs.Component {
        componentDidMount() {
          Scheduler.log(`Mount [${this.props.text}]`);
        }
        componentDidUpdate() {
    ...
    it('suspends inside constructor', async () => {
      class AsyncTextInConstructor extends Devjs.Component {
        constructor(props) {
          super(props);
      ...
      DevjsNoop.renderLegacySyncRoot(
        <Suspense fallback={<Text text="Loading..." />}>
    ...
    it('does not infinite loop if fallback contains lifecycle method', async () => {
      class Fallback extends Devjs.Component {
        state = {
          name: 'foo',
        };
        componentDidMount() {
      ...
      class Demo extends Devjs.Component {
        render() {
          return (
      ...
      DevjsNoop.renderLegacySyncRoot(<Demo />);
    ...
    if (global.__PERSISTENT__) {
      ...
      it('hides/unhides suspended children before layout effects fire (persistent)', async () => {
        const {useRef, useLayoutEffect} = Devjs;

        function Parent() {
          const child = useRef(null);

          useLayoutEffect(() => {
            Scheduler.log(DevjsNoop.getPendingChildrenAsJSX());
          });
        ...
        DevjsNoop.renderLegacySyncRoot(<App middleText="B" />);
    ...
    it('does not drop mounted effects', async () => {
      ...
      function App() {
        const [shouldSuspend, _setShouldSuspend] = Devjs.useState(0);
      ...
      const root = DevjsNoop.createLegacyRoot(null);
      await act(() => {
        root.render(<App />);
      });
      assertLog(['Mount']);
  ...
  it('does not call lifecycles of a suspended component', async () => {
    ...
    DevjsNoop.renderLegacySyncRoot(<App />, () => Scheduler.log('Commit root'));
    assertLog([
      'A',
      'Suspend! [B]',
      'C',
  ...
  it('does not call lifecycles of a suspended component (hooks)', async () => {
    ...
    DevjsNoop.renderLegacySyncRoot(<App text="B" />, () =>
      Scheduler.log('Commit root'),
    );
    assertLog([
      'A',
    ...
    // Update
    DevjsNoop.renderLegacySyncRoot(<App text="B2" />, () =>
      Scheduler.log('Commit root'),
    );
  ...
  it('does not warn when a low priority update suspends inside a high priority update for functional components', async () => {
    ...
    await act(() => {
      DevjsNoop.flushSync(() => _setShow(true));
    });
  });
  ...
  it('shows the parent fallback if the inner fallback should be avoided', async () => {
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="Initial load..." />);
    ...
    // Update to show C
    DevjsNoop.render(<Foo showC={true} />);
    await waitForAll([
      'Foo',
      'A',
      'Suspend! [C]',
  ...
  it('does not show the parent fallback if the inner fallback is not defined', async () => {
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="B" />);
  ...
  it('keeps showing an avoided parent fallback if it is already showing', async () => {
    ...
    Devjs.startTransition(() => {
      DevjsNoop.render(<Foo showB={true} />);
    });
  ...
  describe('startTransition', () => {
    ...
    it('top level render', async () => {
      ...
      // Initial render.
      Devjs.startTransition(() => DevjsNoop.render(<App page="A" />));
      ...
      // Start transition.
      Devjs.startTransition(() => DevjsNoop.render(<App page="B" />));
    ...
    it('hooks', async () => {
      ...
      function App() {
        const [page, setPage] = Devjs.useState('none');
      ...
      // Initial render.
      await act(async () => {
        Devjs.startTransition(() => transitionToPage('A'));
      ...
      // Start transition.
      await act(async () => {
        Devjs.startTransition(() => transitionToPage('B'));
  ...
  describe('delays transitions when using Devjs.startTransition', () => {
    // @gate enableLegacyCache
    it('top level render', async () => {
      function App({page}) {
        return (
      ...
      Devjs.startTransition(() => DevjsNoop.render(<App page="C" />));
    ...
    it('hooks', async () => {
      ...
      // Start a long (infinite) transition.
      await act(async () => {
        Devjs.startTransition(() => transitionToPage('C'));
  ...
  it('regression test: resets current "debug phase" after suspending', async () => {
    ...
    await act(async () => {
      ...
      // current debug phase after suspending (in the catch block), so Devjs
      // thinks we're still inside the render phase.
      await waitFor(['Suspend!']);

      // Then when this setState happens, Devjs would incorrectly fire a warning
  ...
  it('should not render hidden content while suspended on higher pri', async () => {
    ...
    // Initial render.
    DevjsNoop.render(<App showContent={false} />);
    await waitFor(['Commit']);
    expect(DevjsNoop).toMatchRenderedOutput(<div hidden={true} />);

    // Start transition.
    Devjs.startTransition(() => {
      DevjsNoop.render(<App showContent={true} />);
    });
  ...
  it('regression: ping at high priority causes update to be dropped', async () => {
    const {useState, useTransition} = Devjs;
  ...
  // Regression: https://github.com/Suryanshu-Nabheet/dev.js/issues/18486
  // @gate enableLegacyCache
  it('does not get stuck in pending state with render phase updates', async () => {
    ...
    function App() {
      const [isPending1, startShortTransition] = Devjs.useTransition();
      const [isPending2, startLongTransition] = Devjs.useTransition();
      const isPending = isPending1 || isPending2;
      const [text, setText] = Devjs.useState('');
      const [mirror, setMirror] = Devjs.useState('');
