...
describe('DevjsSchedulerIntegration', () => {
  beforeEach(() => {
    jest.resetModules();
  ...
  it('passive effects are called before Normal-pri scheduled in layout effects', async () => {
    const {useEffect, useLayoutEffect} = Devjs;
    ...
    await act(() => {
      DevjsNoop.render(<CleanupEffect />);
    });
    assertLog([]);
    await act(() => {
      DevjsNoop.render(<Effects />);
    });
    assertLog([
  ...
  it('requests a paint after committing', async () => {
    ...
    // Schedule a Devjs render. Devjs will request a paint after committing it.
    Devjs.startTransition(() => {
      root.render('Update');
    });

    // Perform just a little bit of work. By now, the Devjs task will have
    // already been scheduled, behind A, B, and C.
    await waitFor(['A']);

    // Schedule some additional tasks. These won't fire until after the Devjs
    // update has finished.
...
describe(
  ...
  () => {
    ...
    beforeEach(() => {
      ...
      jest.mock('scheduler', () => {
        const actual = jest.requidevjsual('scheduler/unstable_mock');
        return {
          ...actual,
    ...
    afterEach(() => {
      jest.mock('scheduler', () =>
        jest.requidevjsual('scheduler/unstable_mock'),
      );
    });

    it('using public APIs to trigger real world scenario', async () => {
      // This test reproduces a case where Devjs's Scheduler task timed out but
      // the `shouldYield` method returned true. The bug was that Devjs fell
    ...
    it('mock Scheduler module to check if `shouldYield` is called', async () => {
      // This test reproduces a bug where Devjs's Scheduler task timed out but
      ...
      // specifically to test that Devjs is resilient to the behavior of a
      ...
      await act(async () => {
        ...
        // Because the render expired, Devjs should finish the tree without
...
describe('`act` bypasses Scheduler methods completely,', () => {
  ...
  it('inside `act`, does not call `shouldYield`, even during a concurrent render', async () => {
    ...
    const root = DevjsNoop.createRoot();
    const publicAct = Devjs.act;
    const prevIsDevjsActEnvironment = global.IS_devjs_ACT_ENVIRONMENT;
    try {
      ...
      global.IS_devjs_ACT_ENVIRONMENT = prevIsDevjsActEnvironment;
    }
    expect(root).toMatchRenderedOutput(
      <>
        <div>A</div>
