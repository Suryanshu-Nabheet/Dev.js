...
import {useInsertionEffect} from 'devjs';
...
describe('useEffectEvent', () => {
  ...
  beforeEach(() => {
    ...
    createContext = Devjs.createContext;
    useContext = Devjs.useContext;
    useState = Devjs.useState;
    useEffectEvent = Devjs.useEffectEvent;
  ...
  it('can be defined more than once', async () => {
    ...
    const button = Devjs.createRef(null);
    DevjsNoop.render(<Counter incrementBy={5} />);
    await waitForAll(['Increment', 'Count: 0']);
    expect(DevjsNoop).toMatchRenderedOutput(
      <>
        <span prop="Increment" />
  ...
  it('does not preserve `this` in event functions', async () => {
    class GreetButton extends Devjs.PureComponent {
      greet = () => {
        this.props.onClick();
      };
      render() {
    ...
    const button = Devjs.createRef(null);
    DevjsNoop.render(<Greeter hello={'hej'} />);
  ...
  it('is mutated before all other effects', async () => {
    ...
    DevjsNoop.render(<Counter value={1} />);
    await waitForAll(['Effect value: 1', 'Event value: 1']);

    await act(() => DevjsNoop.render(<Counter value={2} />));
  ...
  it('fires all (interleaved) effects with useEffectEvent in correct order', async () => {
    ...
    await act(async () => {
      DevjsNoop.render(<CounterA count={1} />);
    });
    ...
    await act(async () => {
      DevjsNoop.render(<CounterA count={2} />);
    });
  ...
  it('correctly mutates effect event with Activity', async () => {
    ...
    function CounterA({count, hideChild}) {
      ...
      // this breaks the rules, but ensures the ordering is correct.
      return (
        <Devjs.Activity mode={hideChild ? 'hidden' : 'visible'}>
    ...
    await act(async () => {
      DevjsNoop.render(<CounterA count={1} hideChild={true} />);
      await waitFor([
    ...
    await act(async () => {
      DevjsNoop.render(<CounterA count={2} hideChild={true} />);
    ...
    await act(async () => {
      DevjsNoop.render(<CounterA count={3} hideChild={true} />);
    ...
    await act(async () => {
      DevjsNoop.render(<CounterA count={3} hideChild={false} />);
  ...
  it("doesn't provide a stable identity", async () => {
    ...
    DevjsNoop.render(<Counter shouldRender={true} value={0} />);
    await waitForAll([
      'onClick, shouldRender=true, value=0',
      'onClick, shouldRender=true, value=0',
    ]);

    DevjsNoop.render(<Counter shouldRender={true} value={1} />);
    await waitForAll(['onClick, shouldRender=true, value=1']);

    DevjsNoop.render(<Counter shouldRender={false} value={2} />);
    await waitForAll([
  ...
  it('integration: implements docs chat room example', async () => {
    ...
    await act(() =>
      DevjsNoop.render(<ChatRoom roomId="general" theme="light" />),
    );
    ...
    // change roomId only
    await act(() =>
      DevjsNoop.render(<ChatRoom roomId="music" theme="light" />),
    );
    assertLog([
    ...
    // change theme only
    await act(() => DevjsNoop.render(<ChatRoom roomId="music" theme="dark" />));
    ...
    // change roomId only
    await act(() =>
      DevjsNoop.render(<ChatRoom roomId="travel" theme="dark" />),
    );
    assertLog([
  ...
  it('integration: implements the docs logVisit example', async () => {
    class AddToCartButton extends Devjs.PureComponent {
      addToCart = () => {
        this.props.onClick();
      };
      render() {
  ...
  it('reads the latest context value in memo Components', async () => {
    ...
    let logContextValue;
    const ContextReader = Devjs.memo(function ContextReader() {
  ...
  it('reads the latest context value in forwardRef Components', async () => {
    ...
    let logContextValue;
    const ContextReader = Devjs.forwardRef(function ContextReader(props, ref) {
      const value = useContext(MyContext);
      Scheduler.log('ContextReader: ' + value);
      const fireLogContextValue = useEffectEvent(() => {
        Scheduler.log('ContextReader (Effect event): ' + value);
