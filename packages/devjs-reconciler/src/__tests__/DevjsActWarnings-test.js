...
describe('act warnings', () => {
  ...
  async function withActEnvironment(value, scope) {
    const prevValue = global.IS_devjs_ACT_ENVIRONMENT;
    global.IS_devjs_ACT_ENVIRONMENT = value;
    try {
      return await scope();
    } finally {
      global.IS_devjs_ACT_ENVIRONMENT = prevValue;
    }
  }
  ...
  it('warns about unwrapped updates only if environment flag is enabled', async () => {
    ...
    expect(global.IS_devjs_ACT_ENVIRONMENT).toBe(undefined);
    setState(1);
    await waitForAll([1]);
  ...
  it('warns even if update is synchronous', async () => {
    ...
    await withActEnvironment(true, () => {
      ...
      DevjsNoop.flushSync(() => setState(1));
      assertConsoleErrorDev([
        'An update to App inside a test was not wrapped in act(...).\n' +
          '\n' +
          'When testing, code that causes Devjs state updates should be wrapped into act(...):\n' +
