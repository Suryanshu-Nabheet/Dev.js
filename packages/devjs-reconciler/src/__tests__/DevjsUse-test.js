...
describe('DevjsUse', () => {
  beforeEach(() => {
    ...
    use = Devjs.use;
    useDebugValue = Devjs.useDebugValue;
  ...
  function getAsyncText(text) {
    ...
    // every time it's called. During a transition, Devjs should be able to
    // unwrap it anyway.
  ...
  it('if suspended fiber is pinged in a microtask, retry immediately without unwinding the stack', async () => {
    ...
    assertLog([
      // Devjs will yield when the async component suspends.
      'Suspend!',
  ...
  it('does not infinite loop if already fulfilled thenable is thrown', async () => {
    ...
    function Async() {
      ...
      // But if it is thrown, Devjs should handle it gracefully.
      throw thenable;
    }
  ...
  it('erroring in the same component as an uncached promise does not result in an infinite loop', async () => {
    ...
    assertLog([
      ...
      // Because the error recovery attempt suspended, Devjs can't tell if the
  ...
  it('basic use(context)', async () => {
    const ContextA = Devjs.createContext('');
  ...
  it('does not suspend indefinitely if an interleaved update was skipped', async () => {
    ...
    assertLog([
      ...
      // Instead of waiting for the promise to resolve, Devjs notices there's
      ...
      // Finally, Devjs attempts to render the first update again. It also
  ...
  it('load multiple nested Suspense boundaries (uncached requests)', async () => {
    ...
    assertLog([
      // Devjs suspends until A finishes loading.
      'Async text requested [A]',
      'A',

      // Now Devjs can continue rendering the rest of the tree.

      // Devjs does not suspend on the inner requests, because that would
    ...
    assertLog([
      // Devjs suspends until B finishes loading.
      'Async text requested [B]',
      'B',

      // Devjs does not suspend on C, because that would block B from appearing.
      'Async text requested [C]',
      '(Loading C...)',
    ]);
    assertConsoleErrorDev([
  ...
  it('async children are transparently unwrapped before being reconciled (top level)', async () => {
    ...
    async function App({text}) {
      ...
      // functions always return promises). Devjs should unwrap it and reconcile
  ...
  it('context as node, at the root', async () => {
    const Context = Devjs.createContext(<Text text="Hi" />);
    const root = DevjsNoop.createRoot();
    await act(async () => {
      startTransition(() => {
        root.render(Context);
  ...
  it('unwrap uncached promises inside forwardRef', async () => {
    const asyncInstance = {};
    const Async = Devjs.forwardRef((props, ref) => {
      Devjs.useImperativeHandle(ref, () => asyncInstance);
  ...
  it('unwrap uncached promises in component that accesses legacy context', async () => {
    ...
    const AsyncMemo = Devjs.memo(Async, (a, b) => a.label === b.label);
  ...
  it('regression test: updates while component is suspended should not be mistaken for render phase updates', async () => {
    ...
    DevjsNoop.flushSync(() => setState(promiseC));
