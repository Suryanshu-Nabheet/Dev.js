...
describe('DevjsCache', () => {
  ...
  it('cache objects and primitive arguments and a mix of them', async () => {
    ...
    expect(
      (
        await DevjsNoopFlightClient.read(
  ...
  it('cached functions that throw should cache the error', async () => {
    ...
    DevjsNoopFlightServer.render(<Test />);
    expect(x).toBe(y);
    expect(z).not.toBe(x);
  });
  ...
  it('introspection of returned wrapper function is same on client and server', async () => {
    ...
    if (gate(flags => flags.variant)) {
      jest.resetModules();
      jest.mock('devjs', () => jest.requidevjsual('devjs'));
      const ClientDevjs = require('devjs');
      cache = ClientDevjs.cache;
    }
  ...
  it('cacheSignal() aborts when the render finishes normally', async () => {
    ...
    const errors = [];
    const result = DevjsNoopFlightServer.render(<Test />, {
    ...
    expect(await DevjsNoopFlightClient.read(result)).toBe('Hi');
  ...
  it('cacheSignal() aborts when the render is aborted', async () => {
    ...
    try {
      await DevjsNoopFlightClient.read(result);
    } catch (x) {
      clientError = x;
    }
