...
describe('DevjsIncrementalScheduling', () => {
  beforeEach(() => {
    jest.resetModules();
  ...
  it('searches for work on other roots once the current root completes', async () => {
    DevjsNoop.renderToRootWithID(<span prop="a:1" />, 'a');
    DevjsNoop.renderToRootWithID(<span prop="b:1" />, 'b');
    DevjsNoop.renderToRootWithID(<span prop="c:1" />, 'c');

    await waitForAll([]);

    expect(DevjsNoop.getChildrenAsJSX('a')).toEqual(<span prop="a:1" />);
    expect(DevjsNoop.getChildrenAsJSX('b')).toEqual(<span prop="b:1" />);
    expect(DevjsNoop.getChildrenAsJSX('c')).toEqual(<span prop="c:1" />);
  });
  ...
  it('schedules top-level updates in order of priority', async () => {
    ...
    DevjsNoop.batchedUpdates(() => {
      DevjsNoop.render(<span prop={5} />);
      DevjsNoop.flushSync(() => {
        DevjsNoop.render(<span prop={2} />);
        DevjsNoop.render(<span prop={3} />);
        DevjsNoop.render(<span prop={4} />);
      });
    });
    // The sync updates flush first.
    expect(DevjsNoop).toMatchRenderedOutput(<span prop={4} />);
  ...
  it('schedules top-level updates with same priority in order of insertion', async () => {
    ...
    await waitForAll([]);
    expect(DevjsNoop).toMatchRenderedOutput(<span prop={5} />);
  });
  ...
  it('works on deferred roots in the order they were scheduled', async () => {
    ...
    await act(() => {
      DevjsNoop.renderToRootWithID(<Text text="a:1" />, 'a');
      DevjsNoop.renderToRootWithID(<Text text="b:1" />, 'b');
      DevjsNoop.renderToRootWithID(<Text text="c:1" />, 'c');
    });
    assertLog(['a:1', 'b:1', 'c:1']);

    expect(DevjsNoop.getChildrenAsJSX('a')).toEqual('a:1');
    expect(DevjsNoop.getChildrenAsJSX('b')).toEqual('b:1');
    expect(DevjsNoop.getChildrenAsJSX('c')).toEqual('c:1');

    // Schedule deferred work in the reverse order
    await act(async () => {
      Devjs.startTransition(() => {
        DevjsNoop.renderToRootWithID(<Text text="c:2" />, 'c');
        DevjsNoop.renderToRootWithID(<Text text="b:2" />, 'b');
      });
      ...
      expect(DevjsNoop.getChildrenAsJSX('a')).toEqual('a:1');
      expect(DevjsNoop.getChildrenAsJSX('b')).toEqual('b:1');
      expect(DevjsNoop.getChildrenAsJSX('c')).toEqual('c:2');
      // Schedule last bit of work, it will get processed the last

      Devjs.startTransition(() => {
        DevjsNoop.renderToRootWithID(<Text text="a:2" />, 'a');
      });

      // Keep performing work in the order it was scheduled
      await waitFor(['b:2']);
      expect(DevjsNoop.getChildrenAsJSX('a')).toEqual('a:1');
      expect(DevjsNoop.getChildrenAsJSX('b')).toEqual('b:2');
      expect(DevjsNoop.getChildrenAsJSX('c')).toEqual('c:2');

      await waitFor(['a:2']);
      expect(DevjsNoop.getChildrenAsJSX('a')).toEqual('a:2');
      expect(DevjsNoop.getChildrenAsJSX('b')).toEqual('b:2');
      expect(DevjsNoop.getChildrenAsJSX('c')).toEqual('c:2');
    });
  });
  ...
  it('can opt-in to async scheduling inside componentDidMount/Update', async () => {
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<span prop={2} />);
