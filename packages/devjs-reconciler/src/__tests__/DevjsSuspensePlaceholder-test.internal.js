...
describe('DevjsSuspensePlaceholder', () => {
  beforeEach(() => {
    jest.resetModules();

    DevjsFeatureFlags = require('shared/DevjsFeatureFlags');

    DevjsFeatureFlags.enableProfilerTimer = true;
    Devjs = require('devjs');
  ...
  it('times out children that are already hidden', async () => {
    class HiddenText extends Devjs.PureComponent {
      render() {
    ...
    // Initial mount
    DevjsNoop.render(<App middleText="B" />);
    ...
    // Update
    DevjsNoop.render(<App middleText="B2" />);
    await waitForAll([
      'Suspend! [B2]',
      'Loading...',
      // pre-warming
  ...
  it('times out text nodes', async () => {
    ...
    expect(DevjsNoop).not.toMatchRenderedOutput('ABC');
    ...
    // Render the final update. A should still be hidden, because it was
    // given a `hidden` prop.
    expect(DevjsNoop).toMatchRenderedOutput('AB2C');
  });

  it('preserves host context for text nodes', async () => {
    function App(props) {
      return (
        // uppercase is a special type that causes Devjs Noop to render child
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<uppercase>LOADING...</uppercase>);

    await act(() => jest.advanceTimersByTime(1000));
    assertLog(['Promise resolved [b]', 'a', 'b', 'c']);
    expect(DevjsNoop).toMatchRenderedOutput(<uppercase>ABC</uppercase>);
    ...
    // Render the final update. A should still be hidden, because it was
    // given a `hidden` prop.
    expect(DevjsNoop).toMatchRenderedOutput(<uppercase>AB2C</uppercase>);
  });
  ...
  describe('profiler durations', () => {
    ...
    describe('when suspending during mount', () => {
      ...
      it('properly accounts for base durations when a suspended times out in a legacy tree', async () => {
        DevjsNoop.renderLegacySyncRoot(<App shouldSuspend={true} />);
        assertLog([
          'App',
          'Suspending',
          'Suspend! [Loaded]',
        ...
        assertLog(['Loaded']);
        expect(DevjsNoop).toMatchRenderedOutput('LoadedText');
      ...
      it('properly accounts for base durations when a suspended times out in a concurrent tree', async () => {
        DevjsNoop.render(<App shouldSuspend={true} />);
    ...
    describe('when suspending during update', () => {
      ...
      it('properly accounts for base durations when a suspended times out in a legacy tree', async () => {
        ...
        assertLog(['App', 'Text']);
        expect(DevjsNoop).toMatchRenderedOutput('Text');
        expect(onRender).toHaveBeenCalledTimes(1);
        ...
        DevjsNoop.render(<App shouldSuspend={true} textRenderDuration={5} />);
        assertLog([
          'App',
          'Suspending',
          'Suspend! [Loaded]',
        ...
        assertLog(['Loaded']);
        expect(DevjsNoop).toMatchRenderedOutput('LoadedNew');
      ...
      it('properly accounts for base durations when a suspended times out in a concurrent tree', async () => {
        ...
        // consequence of AsyncText relying on the same timer queue as Devjs's
