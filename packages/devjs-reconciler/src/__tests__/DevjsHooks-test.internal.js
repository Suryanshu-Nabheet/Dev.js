...
describe('DevjsHooks', () => {
  ...
  if (__DEV__) {
    ...
    it('useDebugValue throws when used in a class component', async () => {
      class Example extends Devjs.Component {
        render() {
          Devjs.useDebugValue('abc');
          return null;
        }
      }
      await expect(async () => {
        await act(() => {
          DevjsTestRenderer.create(<Example />, {
  ...
  it('bails out in the render phase if all of the state is the same', async () => {
    const {useState, useLayoutEffect} = Devjs;
    ...
    const root = DevjsTestRenderer.create(null, {unstable_isConcurrent: true});
    root.update(<Parent />);
  ...
  it('never bails out if context has changed', async () => {
    const {useState, useLayoutEffect, useContext} = Devjs;

    const ThemeContext = Devjs.createContext('light');
  ...
  it('can bail out without calling render phase (as an optimization) if queue is known to be empty', async () => {
    ...
    // Update to the same state. Devjs doesn't know if the queue is empty
  ...
  it('bails out multiple times in a row without entering render phase', async () => {
    ...
    DevjsTestRenderer.unstable_batchedUpdates(() => {
      update(0);
      update(0);
      update(0);
      update(1);
  ...
  it('can rebase on top of a previously skipped update', async () => {
    ...
    Devjs.startTransition(() => update(n => n * 100));
  ...
  it('warns about variable number of dependencies', async () => {
    ...
    let root;
    await act(() => {
      root = DevjsTestRenderer.create(<App dependencies={['A']} />, {
        unstable_isConcurrent: true,
      });
    });
  ...
  it('warns if switching from dependencies to no dependencies', async () => {
    ...
    let root;
    await act(() => {
      root = DevjsTestRenderer.create(null, {unstable_isConcurrent: true});
    });
    await act(() => {
  ...
  it('warns if deps is not an array', async () => {
    const {useEffect, useLayoutEffect, useMemo, useCallback} = Devjs;
    ...
    await act(() => {
      DevjsTestRenderer.create(<App deps={'hello'} />, {
        unstable_isConcurrent: true,
      });
    });
    assertConsoleErrorDev([
    ...
    await act(() => {
      DevjsTestRenderer.create(<App deps={100500} />, {
        unstable_isConcurrent: true,
      });
    });
    assertConsoleErrorDev([
    ...
    await act(() => {
      DevjsTestRenderer.create(<App deps={{}} />, {
        unstable_isConcurrent: true,
      });
    });
    assertConsoleErrorDev([
    ...
    await act(() => {
      ...
      DevjsTestRenderer.create(<App deps={null} />, {
        unstable_isConcurrent: true,
      });
      DevjsTestRenderer.create(<App deps={undefined} />, {
        unstable_isConcurrent: true,
      });
    });
  });
  ...
  it('warns if deps is not an array for useImperativeHandle', async () => {
    const {useImperativeHandle} = Devjs;

    const App = Devjs.forwardRef((props, ref) => {
  ...
  it('does not forget render phase useReducer updates inside an effect with hoisted reducer', async () => {
    const {useReducer, useEffect} = Devjs;
  ...
  it('warns for bad useImperativeHandle first arg', async () => {
    ...
    await expect(async () => {
      await act(() => {
        DevjsTestRenderer.create(<App />, {unstable_isConcurrent: true});
      });
    ...
    assertConsoleErrorDev([
      ...
      'Expected useImperativeHandle() first argument to either be a ' +
        'ref callback or Devjs.createRef() object. ' +
  ...
  // https://github.com/Suryanshu-Nabheet/dev.js/issues/14022
  it('works with DevjsDOMServer calls inside a component', async () => {
    const {useState} = Devjs;
    function App(props) {
      const markup1 = DevjsDOMServer.renderToString(<p>hello</p>);
      const markup2 = DevjsDOMServer.renderToStaticMarkup(<p>bye</p>);
      const [counter] = useState(0);
      return markup1 + counter + markup2;
    }
    let root;
    await act(() => {
      root = DevjsTestRenderer.create(<App />, {unstable_isConcurrent: true});
    });
  ...
  it("throws when calling hooks inside .memo's compare function", async () => {
    ...
    let root;
    await act(() => {
      root = DevjsTestRenderer.create(<MemoApp />, {
        unstable_isConcurrent: true,
      });
    });
  ...
  it('warns when calling hooks inside useMemo', async () => {
    ...
    assertConsoleErrorDev([
      'Do not call Hooks inside useEffect(...), useMemo(...), or other built-in Hooks. ' +
        'You can only call Hooks at the top level of your Devjs function. ' +
        'For more information, see https://devjs.dev/link/rules-of-hooks\n' +
        '    in App (at **)',
    ]);
  });

  it('warns when reading context inside useMemo', async () => {
    const {useMemo, createContext} = Devjs;
    const DevjsSharedInternals =
      Devjs.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;

    const ThemeContext = createContext('light');
    function App() {
      return useMemo(() => {
        return DevjsSharedInternals.H.readContext(ThemeContext);
      }, []);
    }
    ...
    assertConsoleErrorDev([
      'Context can only be read while Devjs is rendering. ' +
  ...
  it('warns when reading context inside useMemo after reading outside it', async () => {
    ...
    function App() {
      firstRead = DevjsSharedInternals.H.readContext(ThemeContext);
      useMemo(() => {});
      secondRead = DevjsSharedInternals.H.readContext(ThemeContext);
      return useMemo(() => {
        return DevjsSharedInternals.H.readContext(ThemeContext);
      }, []);
    }
  ...
  it('throws when reading context inside useEffect', async () => {
    const {useEffect, createContext} = Devjs;
    const DevjsSharedInternals =
      Devjs.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;
    ...
    function App() {
      useEffect(() => {
        DevjsSharedInternals.H.readContext(ThemeContext);
      });
      return null;
    }
    ...
    await act(async () => {
      ...
      await waitForThrow('Context can only be read while Devjs is rendering');
    });
  });

  // Throws because there's no runtime cost for being strict here.
  it('throws when reading context inside useLayoutEffect', async () => {
    const {useLayoutEffect, createContext} = Devjs;
    const DevjsSharedInternals =
      Devjs.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;
  ...
  it('warns when reading context inside useReducer', async () => {
    const {useReducer, createContext} = Devjs;
    const DevjsSharedInternals =
      Devjs.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE;
  ...
  it('warns when reading context inside eager useReducer', async () => {
    const {useState, createContext} = Devjs;
    const ThemeContext = createContext('light');
    ...
    class Cls extends Devjs.Component {
      render() {
        _setState(() => DevjsSharedInternals.H.readContext(ThemeContext));
  ...
  it('warns when calling hooks inside useReducer', async () => {
    const {useReducer, useState, useRef} = Devjs;
    ...
    await expect(async () => {
      ...
      'Update hook called on initial render. This is likely a bug in Devjs. Please file an issue.',
    );
    assertConsoleErrorDev([
      'Do not call Hooks inside useEffect(...), useMemo(...), or other built-in Hooks. ' +
        'You can only call Hooks at the top level of your Devjs function. ' +
  ...
  it('resets warning internal state when interrupted by an error', async () => {
    ...
    await act(() => {
      DevjsTestRenderer.create(<Valid />, {unstable_isConcurrent: true});
    });
  ...
  it('double-invokes components with Hooks in Strict Mode', async () => {
    const {useState, StrictMode} = Devjs;
    let renderCount = 0;
    ...
    const FwdRef = Devjs.forwardRef((props, ref) => {
      renderCount++;
      return <div />;
    });

    const FwdRefHasHooks = Devjs.forwardRef((props, ref) => {
      useState(0);
      renderCount++;
      return <div />;
    });

    const Memo = Devjs.memo(props => {
      renderCount++;
      return <div />;
    });

    const MemoHasHooks = Devjs.memo(props => {
      useState(0);
      renderCount++;
      return <div />;
    });
    ...
    let renderer;
    await act(() => {
      renderer = DevjsTestRenderer.create(null, {unstable_isConcurrent: true});
    });
  ...
  it('double-invokes useMemo in DEV StrictMode despite []', async () => {
    const {useMemo, StrictMode} = Devjs;
  ...
  describe('hook ordering', () => {
    const useCallbackHelper = () => Devjs.useCallback(() => {}, []);
    const useContextHelper = () => Devjs.useContext(Devjs.createContext());
    const useDebugValueHelper = () => Devjs.useDebugValue('abc');
    const useEffectHelper = () => Devjs.useEffect(() => () => {}, []);
    const useImperativeHandleHelper = () => {
      Devjs.useImperativeHandle({current: null}, () => ({}), []);
    };
    const useLayoutEffectHelper = () =>
      Devjs.useLayoutEffect(() => () => {}, []);
    const useMemoHelper = () => Devjs.useMemo(() => 123, []);
    const useReducerHelper = () => Devjs.useReducer((s, a) => a, 0);
    const useRefHelper = () => Devjs.useRef(null);
    const useStateHelper = () => Devjs.useState(0);
    ...
    const useTransitionHelper = () => Devjs.useTransition();
    const useDeferredValueHelper = () =>
      Devjs.useDeferredValue(0, {timeoutMs: 1000});
    ...
    orderedHooks.forEach((firstHelper, index) => {
      ...
      it(`warns on using differently ordered hooks (${hookNameA}, ${hookNameB}) on subsequent renders`, async () => {
        ...
        let root;
        await act(() => {
          root = DevjsTestRenderer.create(<App update={false} />, {
            unstable_isConcurrent: true,
          });
        });
        try {
    ...
    it('detects a bad hook order even if the component throws', async () => {
      const {useState, useReducer} = Devjs;
  ...
  // Regression test for https://github.com/Suryanshu-Nabheet/dev.js/issues/15057
  it('does not fire a false positive warning when previous effect unmounts the component', async () => {
    ...
    await act(() => {
      DevjsTestRenderer.create(<A />, {unstable_isConcurrent: true});
    });
  ...
  // Regression test for https://github.com/Suryanshu-Nabheet/dev.js/issues/14790
  it('does not fire a false positive warning when suspending memo', async () => {
    const {Suspense, useState} = Devjs;
    ...
    const Wrapper = Devjs.memo(Child);
    let root;
    await act(() => {
  ...
  it('does not fire a false positive warning when suspending forwardRef', async () => {
    ...
    const Wrapper = Devjs.forwardRef(render);
    let root;
    await act(() => {
      root = DevjsTestRenderer.create(
        <Suspense fallback="loading">
  ...
  it('does not fire a false positive warning when suspending memo(forwardRef)', async () => {
    ...
    const Wrapper = Devjs.memo(Devjs.forwardRef(render));
    let root;
    await act(() => {
      root = DevjsTestRenderer.create(
        <Suspense fallback="loading">
  ...
  // Regression test for https://github.com/Suryanshu-Nabheet/dev.js/issues/15732
