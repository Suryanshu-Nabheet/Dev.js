...
// Devjs DOM versions (16, 17, etc) instead of Devjs Noop.
    ...
    async () => {
      ...
      await act(async () => {
        ...
        await waitForAll([
          ...
          // Before committing the layout effects, Devjs detects that the store
  ...
  it('regression: does not infinite loop for only changing store reference in render', async () => {
    ...
    function StoreText() {
      ...
      useMemo(() => {
        ...
        // This breaks the rules of Devjs, but should be handled gracefully.
