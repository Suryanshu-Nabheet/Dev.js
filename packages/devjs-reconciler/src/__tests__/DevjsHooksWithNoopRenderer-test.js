...
describe('DevjsHooksWithNoopRenderer', () => {
  beforeEach(() => {
    jest.resetModules();
    jest.useFakeTimers();
    ...
    useInsertionEffect = Devjs.useInsertionEffect;
    useLayoutEffect = Devjs.useLayoutEffect;
    useCallback = Devjs.useCallback;
    useMemo = Devjs.useMemo;
    useRef = Devjs.useRef;
    useImperativeHandle = Devjs.useImperativeHandle;
  ...
  function advanceTimers(ms) {
    // Note: This advances Jest's virtual time but not Devjs's. Use
    // DevjsNoop.expire for that.
    if (typeof ms !== 'number') {
  ...
  it('resumes after an interruption', async () => {
    ...
    // Initial mount
    const counter = Devjs.createRef(null);
    DevjsNoop.render(<Counter label="Count" ref={counter} />);
    ...
    await act(async () => {
      ...
      DevjsNoop.flushSync(() => {
        DevjsNoop.render(<Counter label="Total" />);
      });
      assertLog(['Total: 0']);

      // Resume rendering
      await waitForAll(['Total: 11']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Total: 11" />);
    });
  });

  it('throws inside class components', async () => {
    class BadCounter extends Devjs.Component {
      render() {
        const [count] = useState(0);
        return <Text text={this.props.label + ': ' + count} />;
      }
    }
    DevjsNoop.render(<BadCounter />);

    await waitForThrow(
      'Invalid hook call. Hooks can only be called inside of the body of a function component. This could happen for' +
        ' one of the following reasons:\n' +
        '1. You might have mismatching versions of Devjs and the renderer (such as Devjs DOM)\n' +
        '2. You might be breaking the Rules of Hooks\n' +
        '3. You might have more than one copy of Devjs in the same app\n' +
        'See https://devjs.dev/link/invalid-hook-call for tips about how to debug and fix this problem.',
    );
    ...
    DevjsNoop.render(<GoodCounter initialCount={10} />);
    await waitForAll([10]);
  });
  ...
  describe('useState', () => {
    it('simple mount and update', async () => {
      ...
      Counter = forwardRef(Counter);
      const counter = Devjs.createRef(null);
      DevjsNoop.render(<Counter ref={counter} />);
      await waitForAll(['Count: 0']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Count: 0" />);
      ...
      assertLog(['Count: 11']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Count: 11" />);
    });
    ...
    it('lazy state initializer', async () => {
      ...
      Counter = forwardRef(Counter);
      const counter = Devjs.createRef(null);
      DevjsNoop.render(<Counter initialState={42} ref={counter} />);
      await waitForAll(['getInitialState', 'Count: 42']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Count: 42" />);

      await act(() => counter.current.updateCount(7));
      assertLog(['Count: 7']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Count: 7" />);
    });
  ...
  describe('updates during the render phase', () => {
    it('restarts the render function and applies the new updates on top', async () => {
      ...
      DevjsNoop.render(<ScrollView row={10} />);
      await waitForAll(['Scrolling down: true']);
      expect(DevjsNoop).toMatchRenderedOutput(
        <span prop="Scrolling down: true" />,
      );

      DevjsNoop.render(<ScrollView row={2} />);
      await waitForAll(['Scrolling down: false']);
      expect(DevjsNoop).toMatchRenderedOutput(
        <span prop="Scrolling down: false" />,
      );
    ...
    it('warns about render phase update on a different component', async () => {
      ...
      // Bar will update Foo during its render phase. Devjs should warn.
      root.render(
        <>
          <Foo />
      ...
      assertConsoleErrorDev([
        'Cannot update a component (`Foo`) while rendering a different component (`Bar`). ' +
          ...
          'follow the stack trace as described in https://devjs.dev/link/setstate-in-render\n' +
          '    in Bar (at **)',
      ]);
    ...
    it('keeps restarting until there are no more new updates', async () => {
      ...
      expect(DevjsNoop).toMatchRenderedOutput(<span prop={3} />);
    });
    ...
    it('updates multiple times within same render function', async () => {
      ...
      expect(DevjsNoop).toMatchRenderedOutput(<span prop={12} />);
    });
    ...
    it('throws after too many iterations', async () => {
      ...
      DevjsNoop.render(<Counter />);
      await waitForThrow(
        'Too many re-renders. Devjs limits the number of renders to prevent ' +
          'an infinite loop.',
      );
    });
    ...
    it('uses reducer passed at time of render, not time of dispatch', async () => {
      ...
      expect(DevjsNoop).toMatchRenderedOutput(<span prop={21} />);
      ...
      expect(DevjsNoop).toMatchRenderedOutput(<span prop={22} />);
    });
  ...
  describe('useReducer', () => {
    it('simple mount and update', async () => {
      ...
      assertLog(['Count: -2']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Count: -2" />);
    });
    ...
    it('lazy init', async () => {
      ...
      Counter = forwardRef(Counter);
      const counter = Devjs.createRef(null);
      DevjsNoop.render(<Counter initialCount={10} ref={counter} />);
      await waitForAll(['Init', 'Count: 10']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Count: 10" />);
      ...
      assertLog(['Count: 8']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Count: 8" />);
    });

    // Regression test for https://github.com/Suryanshu-Nabheet/dev.js/issues/14360
    it('handles dispatches with mixed priorities', async () => {
      ...
      assertLog(['Count: 4']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Count: 4" />);
    });
  });
  ...
  describe('useEffect', () => {
    it('simple mount and update', async () => {
      ...
      await act(async () => {
        DevjsNoop.render(<Counter count={0} />, () =>
      ...
      await act(async () => {
        DevjsNoop.render(<Counter count={1} />, () =>
          Scheduler.log('Sync effect'),
        );
    ...
    it('flushes passive effects even with sibling deletions', async () => {
      ...
      await act(async () => {
        DevjsNoop.render([<LayoutEffect key="l" />, passive]);
        ...
        // being executed
        DevjsNoop.render([passive]);
        await waitForAll(['Passive effect']);
        expect(DevjsNoop).toMatchRenderedOutput(<span prop="Passive" />);
      });
    ...
    it('flushes passive effects even if siblings schedule an update', async () => {
      ...
      DevjsNoop.render([<PassiveEffect key="p" />, <LayoutEffect key="l" />]);
    ...
    it('flushes passive effects even if siblings schedule a new root', async () => {
      ...
      function LayoutEffect(props) {
        useLayoutEffect(() => {
          Scheduler.log('Layout effect');
          // Scheduling work shouldn't interfere with the queued passive effect
          DevjsNoop.renderToRootWithID(<Text text="New Root" />, 'root2');
        });
        return <Text text="Layout" />;
      }
      await act(async () => {
    ...
    it(
      ...
      async () => {
        function getCommittedText() {
          const children = DevjsNoop.getChildrenAsJSX();
          if (children === null) {
            return null;
          }
    ...
    it('defers passive effect destroy functions during unmount', async () => {
      ...
      await act(async () => {
        DevjsNoop.render(<Child bar={1} foo={1} />, () =>
          Scheduler.log('Sync effect'),
        );
        await waitFor([
          'render',
      ...
      await act(async () => {
        DevjsNoop.render(<Child bar={1} foo={2} />, () =>
          Scheduler.log('Sync effect'),
        );
        await waitFor([
          'render',
      ...
      await act(async () => {
        DevjsNoop.render(null, () => Scheduler.log('Sync effect'));
        await waitFor([
          'layout bar destroy',
          'layout foo destroy',
          'Sync effect',
    ...
    it('does not warn about state updates for unmounted components with pending passive unmounts', async () => {
      ...
      function Component() {
        ...
        const [didLoad, setDidLoad] = Devjs.useState(false);
        Devjs.useLayoutEffect(() => {
      ...
      await act(async () => {
        DevjsNoop.renderToRootWithID(<Component />, 'root', () =>
          Scheduler.log('Sync effect'),
        );
        ...
        DevjsNoop.unmountRootWithID('root');
    ...
    it('does not show a warning when a component updates a child state from within passive unmount function', async () => {
      ...
      await act(async () => {
        DevjsNoop.renderToRootWithID(<Parent />, 'root');
        await waitFor([
          'Parent',
          'Child',
    ...
    it('updates have async priority', async () => {
      ...
      await act(async () => {
        ...
        expect(DevjsNoop).toMatchRenderedOutput(<span prop="Count: (empty)" />);
        DevjsNoop.flushPassiveEffects();
    ...
    it(
      ...
      async () => {
        ...
        await act(() => {
          DevjsNoop.flushSync(() => {
            DevjsNoop.renderLegacySyncRoot(<Counter count={0} />);
          });
        ...
        useEffect(() => {
          ...
          assertConsoleErrorDev([
            'flushSync was called from inside a lifecycle method. ' +
              'Devjs cannot flush when Devjs is already rendering. ' +
              'Consider moving this call to a scheduler task or micro task.\n' +
              '    in Counter (at **)',
          ]);
          // This shouldn't flush synchronously.
          expect(DevjsNoop).not.toMatchRenderedOutput(
    ...
    it('skips effect if inputs have not changed', async () => {
      ...
      await act(async () => {
        DevjsNoop.render(<Counter label="Count" count={0} />, () =>
          Scheduler.log('Sync effect'),
        );
        await waitFor(['Count: 0', 'Sync effect']);
      });
      ...
      await act(async () => {
        DevjsNoop.render(<Counter label="Count" count={1} />, () =>
          Scheduler.log('Sync effect'),
        );
        // Count changed
        await waitFor(['Count: 1', 'Sync effect']);
      ...
      await act(async () => {
        DevjsNoop.render(<Counter label="Total" count={1} />, () =>
          Scheduler.log('Sync effect'),
        );
        // Label changed
        await waitFor(['Total: 1', 'Sync effect']);
        expect(DevjsNoop).toMatchRenderedOutput(<span prop="Total: 1" />);
      });
    ...
    describe('errors thrown in passive destroy function within unmounted trees', () => {
      ...
      beforeEach(() => {
        ...
        LogOnlyErrorBoundary = class extends Devjs.Component {
          componentDidCatch(error, info) {
      ...
      it('should use the nearest still-mounted boundary if there are no unmounted boundaries', async () => {
        ...
        await act(() => {
          DevjsNoop.render(<LogOnlyErrorBoundary />);
        });
      ...
      it('should rethrow error if there are no still-mounted boundaries', async () => {
        ...
        await act(() => {
          DevjsNoop.render(<Conditional showChildren={true} />);
        });
        ...
        await act(async () => {
          DevjsNoop.render(<Conditional showChildren={false} />);
    ...
    it('calls passive effect destroy functions for memoized components', async () => {
      ...
      const MemoizedChild = Devjs.memo(Child, isEqual);
    ...
    it('assumes passive effect destroy function is either a function or undefined', async () => {
      ...
      const root1 = DevjsNoop.createRoot();
      await act(() => {
      ...
      const root2 = DevjsNoop.createRoot();
      await act(() => {
      ...
      const root3 = DevjsNoop.createRoot();
      await act(() => {
      ...
      // Error on unmount because Devjs assumes the value is a function
      await act(async () => {
        root3.render(null);
  ...
  describe('useInsertionEffect', () => {
    it('fires insertion effects after snapshots on update', async () => {
      ...
      class CounterB extends Devjs.Component {
  ...
  describe('useLayoutEffect', () => {
    ...
    it('catches errors thrown in useLayoutEffect', async () => {
      ...
      // Devjs should skip over the unmounting boundary and find the nearest still-mounted boundary.
      await waitForAll([
        'OuterBoundary render success',
        'Component render sibling',
      ...
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="OuterFallback" />);
    });
  ...
  describe('useCallback', () => {
    it('memoizes callback by comparing inputs', async () => {
      class IncrementButton extends Devjs.PureComponent {
        increment = () => {
      ...
      const button = Devjs.createRef(null);
      DevjsNoop.render(<Counter incrementBy={1} />);
      ...
      DevjsNoop.render(<Counter incrementBy={10} />);
      await waitForAll([
  ...
  describe('useMemo', () => {
    it('memoizes value by comparing to previous inputs', async () => {
      ...
      DevjsNoop.render(<CapitalizedText text="hello" />);
      await waitForAll(["Capitalize 'hello'", 'HELLO']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="HELLO" />);

      DevjsNoop.render(<CapitalizedText text="hi" />);
      ...
      DevjsNoop.render(<CapitalizedText text="goodbye" />);
      await waitForAll(["Capitalize 'goodbye'", 'GOODBYE']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="GOODBYE" />);
    });
    ...
    it('always re-computes if no inputs are provided', async () => {
      ...
      DevjsNoop.render(<LazyCompute compute={computeA} />);
      ...
      DevjsNoop.render(<LazyCompute compute={computeB} />);
    ...
    it('should not invoke memoized function during re-renders unless inputs change', async () => {
      ...
      DevjsNoop.render(<LazyCompute compute={compute} input="A" />);
      await waitForAll(['compute A', 'A']);

      DevjsNoop.render(<LazyCompute compute={compute} input="A" />);
      await waitForAll(['A']);

      DevjsNoop.render(<LazyCompute compute={compute} input="B" />);
      await waitForAll(['compute B', 'B']);
    });
  });
  ...
  describe('useImperativeHandle', () => {
    ...
    // Regression test for https://github.com/Suryanshu-Nabheet/dev.js/issues/14782
  ...
  describe('progressive enhancement (not supported)', () => {
    it('mount additional state', async () => {
      ...
      DevjsNoop.render(<App loadC={false} />);
      ...
      DevjsNoop.render(<App loadC={true} />);
      await waitForThrow(
        'Rendered more hooks than during the previous render.',
      );
      assertLog([]);
      assertConsoleErrorDev([
        'Devjs has detected a change in the order of Hooks called by App. ' +
          'This will lead to bugs and errors if not fixed. For more information, ' +
          'read the Rules of Hooks: https://devjs.dev/link/rules-of-hooks\n' +
          '\n' +
      ...
      // expect(DevjsNoop).toMatchRenderedOutput(<span prop="A: 2, B: 3, C: 0" />]);

      // updateC(4);
      // expect(Scheduler).toFlushAndYield(['A: 2, B: 3, C: 4']);
      // expect(DevjsNoop).toMatchRenderedOutput(<span prop="A: 2, B: 3, C: 4" />]);
    });
    ...
    it('unmount state', async () => {
      ...
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="A: 0, B: 0, C: 0" />);
      await act(() => {
        updateA(2);
        updateB(3);
        updateC(4);
    ...
    it('unmount effects', async () => {
      ...
      await act(async () => {
        DevjsNoop.render(<App showMore={false} />, () =>
          Scheduler.log('Sync effect'),
        );
      ...
      await act(async () => {
        DevjsNoop.render(<App showMore={true} />);
        await waitForThrow(
          'Rendered more hooks than during the previous render.',
        );
        assertLog(['Unmount A']);
  ...
  it('useReducer does not eagerly bail out of state updates', async () => {
    ...
    expect(DevjsNoop).toMatchRenderedOutput('2');
  });
  ...
  it('useReducer does not replay previous no-op actions when other state changes', async () => {
    ...
    expect(DevjsNoop).toMatchRenderedOutput('0');
  ...
  it('state bail out edge case (#16359)', async () => {
    ...
    const root = DevjsNoop.createRoot(null);
    await act(() => {
      root.render(
        <>
          <CounterA />
  ...
  it('should update latest rendered reducer when a preceding state receives a render phase update', async () => {
    ...
    expect(DevjsNoop).toMatchRenderedOutput('5');
  });
  ...
  it('should process the rest pending updates after a render phase update', async () => {
    ...
    await act(() => DevjsNoop.render(<App />));
    expect(DevjsNoop).toMatchRenderedOutput('abc');
