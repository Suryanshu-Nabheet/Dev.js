...
function prettyFormat(thing) {
  return prettyFormatPkg.format(thing, {
    plugins: [
      prettyFormatPkg.plugins.DevjsElement,
      prettyFormatPkg.plugins.DevjsTestComponent,
    ],
  });
}

describe('DevjsSuspenseFuzz', () => {
  beforeEach(() => {
    jest.resetModules();
    Devjs = require('devjs');
  ...
  function createFuzzer() {
    const {useState, useContext, useLayoutEffect} = Devjs;

    const ShouldSuspendContext = Devjs.createContext(true);
    ...
    function Container({children, updates}) {
      ...
      return <Devjs.Fragment key={step}>{children}</Devjs.Fragment>;
    }
    ...
    async function testResolvedOutput(unwrappedChildren) {
      ...
      resetCache();
      const expectedRoot = DevjsNoop.createRoot();
      await act(() => {
        expectedRoot.render(
      ...
      const concurrentRootThatSuspends = DevjsNoop.createRoot();
      await act(() => {
      ...
      if (!DevjsFeatureFlags.disableLegacyMode) {
        const legacyRootThatSuspends = DevjsNoop.createLegacyRoot();
        await act(() => {
    ...
    function generateTestCase(rand, numberOfElements) {
      ...
      function createRandomChild(hasSibling) {
        ...
        switch (action) {
          ...
          case 'container': {
            ...
            return Devjs.createElement(Container, {updates}, ...children);
          }
          case 'suspense': {
            remainingElements--;
            const children = createRandomChildren(3);
            ...
            return Devjs.createElement(Suspense, {fallback}, ...children);
          }
          case 'return':
          default:
            return null;
      ...
      return Devjs.createElement(Devjs.Fragment, null, ...children);
    }
