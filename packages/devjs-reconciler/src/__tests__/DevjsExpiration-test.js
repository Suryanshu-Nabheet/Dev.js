...
describe('DevjsExpiration', () => {
  ...
  it('increases priority of updates as time progresses', async () => {
    DevjsNoop.render(<Text text="Step 1" />);
    Devjs.startTransition(() => {
      DevjsNoop.render(<Text text="Step 2" />);
    });
    await waitFor(['Step 1']);

    expect(DevjsNoop).toMatchRenderedOutput('Step 1');
  ...
  it('two updates of like priority in the same event always flush within the same batch', async () => {
    ...
    // Schedule an update.
    Devjs.startTransition(() => {
      DevjsNoop.render(<TextClass text="A" />);
    });
    // Advance the timer.
    ...
    DevjsNoop.render(<TextClass text="B" />);
  ...
  it('cannot update at the same expiration time that is already rendering', async () => {
    ...
    const subscribers = [];
    class Connected extends Devjs.Component {
  ...
  it('should measure callback timeout relative to current time, not start-up time', async () => {
    // Corresponds to a bugfix: https://github.com/Suryanshu-Nabheet/dev.js/pull/15479
    ...
    await unstable_waitForExpired([]);
    expect(DevjsNoop).toMatchRenderedOutput('Hi');
  });
  ...
  it('idle work never expires', async () => {
    ...
    function App() {
      ...
      updateSyncPri = () => DevjsNoop.flushSync(() => setSyncPri(n => n + 1));
      updateIdlePri = () =>
