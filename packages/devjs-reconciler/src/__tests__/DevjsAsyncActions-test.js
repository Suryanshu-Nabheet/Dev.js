...
describe('DevjsAsyncActions', () => {
  ...
  it('multiple async action updates in the same scope are entangled together', async () => {
    ...
    // will end. This will allow Devjs to attempt to render the updates.
  ...
  it('if there are multiple entangled actions, and one of them errors, it only affects that action', async () => {
    ...
    assertLog([
      ...
      // Because there was an error, Devjs will try rendering one more time.
      'Oops A!',
      'Pending B: false',
      'Oops C!',
    ]);
  ...
  it('useOptimistic can be used to implement a pending state', async () => {
    const startTransition = Devjs.startTransition;
  ...
  it('useOptimistic rebases pending updates on top of passthrough value', async () => {
    ...
    DevjsNoop.flushSync(() => root.render(<App cart={serverCart} />));
    assertLog([
      'Pending: true',
    ...
    function App() {
      ...
      increment = () =>
        startTransition(async () => {
          ...
          Devjs.startTransition(() => setCount(n => n + 1));
        });
  ...
  it('Devjs.startTransition supports async actions', async () => {
    const startTransition = Devjs.startTransition;
  ...
  it('useOptimistic works with async actions passed to Devjs.startTransition', async () => {
    const startTransition = Devjs.startTransition;
  ...
  it(
    'regression: updates in an action passed to Devjs.startTransition are batched ' +
  ...
  it('Devjs.startTransition captures async errors and passes them to reportError', async () => {
    await act(() => {
  ...
  it('Devjs.startTransition captures sync errors and passes them to reportError', async () => {
    await act(() => {
      try {
        Devjs.startTransition(() => {
          throw new Error('Oops');
  ...
  it('reconciles against new items when optimisticKey is used', async () => {
    ...
    function Item({text}) {
      const [initialText] = Devjs.useState(text);
