...
describe('DevjsIncrementalUpdates', () => {
  beforeEach(() => {
    jest.resetModules();
  ...
  it('can abort an update, schedule additional updates, and resume', async () => {
    ...
    await waitFor(['a', 'b', 'c']);
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="" />);
    ...
    assertLog(['d', 'e', 'f']);
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="def" />);
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="abcdefg" />);
  });
  ...
  it('can abort an update, schedule a replaceState, and resume', async () => {
    ...
    // The sync updates should have flushed, but not the async ones.
    assertLog(['d', 'e', 'f']);
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="f" />);
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="fg" />);
  });
  ...
  it('gives setState during reconciliation the same priority as whatever level is currently reconciling', async () => {
    ...
    DevjsNoop.flushSync(() => {
      ...
      DevjsNoop.render(<Foo />); // Trigger componentWillReceiveProps
    });
  ...
  it('getDerivedStateFromProps should update base state of updateQueue (based on product bug)', () => {
    ...
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="derived state" />);
