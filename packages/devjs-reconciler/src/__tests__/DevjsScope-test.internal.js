...
describe('DevjsScope', () => {
  ...
  describe('DevjsDOM', () => {
    ...
    it('containsNode() works as intended', async () => {
      ...
      const spanRef = Devjs.createRef();
      const aRef = Devjs.createRef();
      const outerSpan = Devjs.createRef();
      const emRef = Devjs.createRef();
    ...
    it('scopes support server-side rendering and hydration', async () => {
      ...
      const html = DevjsDOMServer.renderToString(<Test />);
      expect(html).toBe(
    ...
    it('getChildContextValues() works as intended', async () => {
      const TestContext = Devjs.createContext();
      const TestScope = Devjs.unstable_Scope;
      const scopeRef = Devjs.createRef();
    ...
    it('correctly works with suspended boundaries that are hydrated', async () => {
      ...
      // hydrating anyway.
      suspend = true;
      await act(() => DevjsDOMClient.hydrateRoot(container2, <App />));

      // This should not cause a runtime exception, see:
      // https://github.com/Suryanshu-Nabheet/dev.js/pull/18184
  ...
  describe('DevjsTestRenderer', () => {
    ...
    it('DO_NOT_USE_queryAllNodes() works as intended', async () => {
      ...
      await act(
        () =>
          (renderer = DevjsTestRenderer.create(<Test toggle={true} />, {
