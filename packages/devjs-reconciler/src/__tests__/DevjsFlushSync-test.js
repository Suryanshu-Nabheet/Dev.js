...
// TODO: Migrate tests to Devjs DOM instead of Devjs Noop

describe('DevjsFlushSync', () => {
  beforeEach(() => {
    jest.resetModules();
  ...
  function getVisibleChildren(element: Element): Devjs$Node {
    const children = [];
    ...
    while (node) {
      if (node.nodeType === 1) {
        if (
          ...
          children.push(
            require('devjs').createElement(node.tagName.toLowerCase(), props),
          );
        }
  ...
  it('changes priority of updates in useEffect', async () => {
    function App() {
      ...
      useEffect(() => {
        if (syncState !== 1) {
          setState(1);
          DevjsDOM.flushSync(() => setSyncState(1));
        }
      }, [syncState, state]);
    ...
    await act(async () => {
      ...
      assertConsoleErrorDev([
        'flushSync was called from inside a lifecycle method. Devjs ' +
          'cannot flush when Devjs is already rendering. Consider moving this ' +
  ...
  it('flushes pending passive effects before scope is called in legacy mode', async () => {
    ...
    await act(() => {
      DevjsDOM.flushSync(() => {
        DevjsDOM.render(<App step={1} />, container);
      });
      assertLog([
        1,
        // Because we're in legacy mode, we shouldn't have flushed the passive
      ...
      DevjsDOM.flushSync(() => {
        ...
        DevjsDOM.render(<App step={currentStep + 1} />, container);
      });
  ...
  it('completely exhausts synchronous work queue even if something throws', async () => {
    ...
    // Because there were multiple errors, Devjs threw an AggregateError.
