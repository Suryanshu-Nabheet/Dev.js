...
describe('DevjsIncrementalReflection', () => {
  beforeEach(() => {
    jest.resetModules();
  ...
  it('finds no node before insertion and correct node before deletion', async () => {
    ...
    function findInstance(inst) {
      ...
      try {
        return DevjsNoop.findInstance(inst);
      } finally {
    ...
    expect(DevjsNoop.findInstance(classInstance)).toBe(hostSpan);
    ...
    expect(DevjsNoop.findInstance(classInstance)).toBe(hostDiv);
    ...
    // This should still be the host div since the deletion is not committed.
    expect(DevjsNoop.findInstance(classInstance)).toBe(null);

    // Render a div again
    DevjsNoop.render(<Foo step={4} />);
    await waitForAll([
