...
describe('isomorphic act()', () => {
  beforeEach(() => {
    ...
    DiscreteEventPriority =
      require('devjs-reconciler/constants').DiscreteEventPriority;
    act = Devjs.act;
    use = Devjs.use;
  ...
  it('behavior in production', () => {
    if (!__DEV__) {
      if (gate('fb')) {
        expect(() => act(() => {})).toThrow(
          'act(...) is not supported in production builds of Devjs',
        );
      } else {
        expect(Devjs).not.toHaveProperty('act');
      }
    }
  });
  ...
  it('bypasses queueMicrotask', async () => {
    ...
    DevjsNoop.unstable_runWithPriority(DiscreteEventPriority, () => {
      root.render('A');
    });
  ...
  it('warns if a promise is used in a non-awaited `act` scope', async () => {
    ...
    expect(console.error.mock.calls[0][0]).toContain(
      'A component suspended inside an `act` scope, but the `act` ' +
        'call was not awaited. When testing Devjs components that ' +
