...
describe('DevjsNewContext', () => {
  beforeEach(() => {
    jest.resetModules();

    Devjs = require('devjs');
    useContext = Devjs.useContext;
  ...
  function readContext(Context) {
    const dispatcher =
      Devjs.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE.H;
  ...
  sharedContextTests('useContext inside forwardRef component', Context =>
    Devjs.forwardRef(function Consumer(props, ref) {
      const contextValue = useContext(Context);
      const render = props.children;
      return render(contextValue);
    }),
  ...
  sharedContextTests('useContext inside memoized function component', Context =>
    Devjs.memo(function Consumer(props) {
      const contextValue = useContext(Context);
      const render = props.children;
      return render(contextValue);
    }),
  ...
  sharedContextTests(
    ...
    Context =>
      class Consumer extends Devjs.Component {
        render() {
  ...
  sharedContextTests(
    ...
    Context =>
      class Consumer extends Devjs.PureComponent {
        render() {
          const contextValue = readContext(Context);
          const render = this.props.children;
          return render(contextValue);
  ...
  function sharedContextTests(label, getConsumer) {
    describe(`reading context with ${label}`, () => {
      it('simple mount and update', async () => {
        const Context = Devjs.createContext(1);
        ...
        DevjsNoop.render(<App value={2} />);
        await waitForAll([]);
        expect(DevjsNoop).toMatchRenderedOutput(<span prop="Result: 2" />);

        // Update
        DevjsNoop.render(<App value={3} />);
        await waitForAll([]);
        expect(DevjsNoop).toMatchRenderedOutput(<span prop="Result: 3" />);
      });
      ...
      it('propagates through shouldComponentUpdate false', async () => {
        ...
        class Indirection extends Devjs.Component {
      ...
      it('nested providers', async () => {
        ...
        DevjsNoop.render(<App value={2} />);
        await waitForAll([]);
        expect(DevjsNoop).toMatchRenderedOutput(<span prop="Result: 8" />);

        // Update
        DevjsNoop.render(<App value={3} />);
        await waitForAll([]);
        expect(DevjsNoop).toMatchRenderedOutput(<span prop="Result: 12" />);
      });

      it('should provide the correct (default) values to consumers outside of a provider', async () => {
        const FooContext = Devjs.createContext({value: 'foo-initial'});
        const BarContext = Devjs.createContext({value: 'bar-initial'});
      ...
      it('compares context values with Object.is semantics', async () => {
        ...
        DevjsNoop.render(<App value={NaN} />);
        await waitForAll([
          'App',
          'Provider',
          'Indirection',
        ...
        expect(DevjsNoop).toMatchRenderedOutput(<span prop="Result: NaN" />);
      ...
      it('consumer bails out if value is unchanged and something above bailed out', async () => {
        ...
        class PureIndirection extends Devjs.PureComponent {
          render() {
      ...
      it("context consumer doesn't bail out inside hidden subtree", async () => {
        const Context = Devjs.createContext('dark');
        const Consumer = getConsumer(Context);
        ...
        DevjsNoop.render(<App theme="dark" />);
        ...
        DevjsNoop.render(<App theme="light" />);
      ...
      // This is a regression case for https://github.com/Suryanshu-Nabheet/dev.js/issues/12389.
      it('does not run into an infinite loop', async () => {
        ...
        DevjsNoop.render(<App reverse={false} />);
        await waitForAll([]);
        DevjsNoop.render(<App reverse={true} />);
        await waitForAll([]);
        DevjsNoop.render(<App reverse={false} />);
        await waitForAll([]);
      });

      // This is a regression case for https://github.com/Suryanshu-Nabheet/dev.js/issues/12686
      it('does not skip some siblings', async () => {
        ...
        class StaticContent extends Devjs.PureComponent {
          render() {
            return (
              <>
                <>
        ...
        class Indirection extends Devjs.PureComponent {
          render() {
            return (
              <ContextConsumer>
                {value => {
        ...
        // Initial mount
        let inst;
        DevjsNoop.render(<App ref={ref => (inst = ref)} />);
        await waitForAll(['App']);
        expect(DevjsNoop).toMatchRenderedOutput(
          <>
            <span prop="static 1" />
  ...
  describe('Context.Provider', () => {
    ...
    it('provider bails out if children and value are unchanged (like sCU)', async () => {
      ...
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Child" />);
    ...
    it('provider does not bail out if legacy context changed above', async () => {
      ...
      const legacyProviderRef = Devjs.createRef();
      const appRef = Devjs.createRef();
  ...
  describe('Context.Consumer', () => {
    it('warns if child is not a function', async () => {
      spyOnDev(console, 'error').mockImplementation(() => {});
      const Context = Devjs.createContext(0);
      DevjsNoop.render(<Context.Consumer />);
    ...
    it('can read other contexts inside consumer render prop', async () => {
      const FooContext = Devjs.createContext(0);
      const BarContext = Devjs.createContext(0);
      ...
      DevjsNoop.render(<App foo={1} bar={1} />);
      await waitForAll(['Foo: 1, Bar: 1']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Foo: 1, Bar: 1" />);

      // Update foo
      DevjsNoop.render(<App foo={2} bar={1} />);
      await waitForAll(['Foo: 2, Bar: 1']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Foo: 2, Bar: 1" />);

      // Update bar
      DevjsNoop.render(<App foo={2} bar={2} />);
      await waitForAll(['Foo: 2, Bar: 2']);
      expect(DevjsNoop).toMatchRenderedOutput(<span prop="Foo: 2, Bar: 2" />);
    });
    ...
    // https://github.com/Suryanshu-Nabheet/dev.js/pull/12470#issuecomment-376917711
    it('consumer does not bail out if there were no bailouts above it', async () => {
      ...
      // Initial mount
      let inst;
      DevjsNoop.render(<App value={1} ref={ref => (inst = ref)} />);
  ...
  describe('readContext', () => {
    ...
    it('can read the same context multiple times in the same function', async () => {
      const Context = Devjs.createContext({foo: 0, bar: 0, baz: 0}, (a, b) => {
        let result = 0;
        if (a.foo !== b.foo) {
          result |= 0b001;
        }
      ...
      DevjsNoop.render(<App foo={1} bar={1} baz={1} />);
      ...
      // Update only foo
      DevjsNoop.render(<App foo={2} bar={1} baz={1} />);
      await waitForAll(['Foo: 2, Bar: 1']);
      expect(DevjsNoop).toMatchRenderedOutput([
      ...
      // Update only bar
      DevjsNoop.render(<App foo={2} bar={2} baz={1} />);
      await waitForAll(['Foo: 2, Bar: 2']);
      expect(DevjsNoop).toMatchRenderedOutput([
      ...
      // Update only baz
      DevjsNoop.render(<App foo={2} bar={2} baz={2} />);
  ...
  it('unwinds after errors in complete phase', async () => {
    ...
    await waitForAll([]);
    expect(DevjsNoop).toMatchRenderedOutput(<span prop={10} />);
  });
  ...
  describe('fuzz test', () => {
    ...
    function ContextSimulator(maxDepth) {
      ...
      class ConsumerTree extends Devjs.Component {
        shouldComponentUpdate() {
          return false;
        }
        render() {
      ...
      function assertConsistentTree(expectedValues = {}) {
        const jsx = DevjsNoop.getChildrenAsJSX();
...
`);
  ...
  it('should treat Context as Context.Provider', async () => {
    ...
    await waitForAll([]);
    expect(DevjsNoop).toMatchRenderedOutput(<span prop="bar-updated" />);
  });
});
