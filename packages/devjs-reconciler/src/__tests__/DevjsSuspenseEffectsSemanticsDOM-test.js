...
describe('DevjsSuspenseEffectsSemanticsDOM', () => {
  beforeEach(() => {
    jest.resetModules();
  ...
  it('should not cause a cycle when combined with a render phase update', async () => {
    ...
    function App() {
      const [value, setValue] = Devjs.useState(true);
    ...
    function ComponentThatCausesBug({value}) {
      const [mirroredValue, setMirroredValue] = Devjs.useState(value);
      if (mirroredValue !== value) {
        setMirroredValue(value);
      }

      // eslint-disable-next-line no-unused-vars
      const [_, setRef] = Devjs.useState(null);
  ...
  it('does not destroy ref cleanup twice when hidden child is removed', async () => {
    ...
    function Parent({swap}) {
      return (
        <Devjs.Suspense fallback={<Text text="Loading..." />}>
  ...
  it('does not call componentWillUnmount twice when hidden child is removed', async () => {
    class ChildA extends Devjs.Component {
      componentDidMount() {
    ...
    class ChildB extends Devjs.Component {
      componentDidMount() {
        Scheduler.log('Did mount: ' + this.props.label);
      }
      componentWillUnmount() {
  ...
  it('regression: unmount hidden tree, in legacy mode', async () => {
    ...
    const LazySibling = Devjs.lazy(() => fakeImport(Sibling));
    ...
    // Initial render
    DevjsDOM.render(<App showMore={false} />, container);
    assertLog(['Child', 'Mount']);

    // Update that suspends, causing the existing tree to switches it to
    // a fallback.
    DevjsDOM.render(<App showMore={true} />, container);
    assertLog([
      'Child',
      'Loading...',
  ...
  it('does not call cleanup effects twice after a bailout', async () => {
    ...
    function App() {
      const [suspended, _setSuspended] = Devjs.useState(false);
      setSuspended = _setSuspended;
      const [letter, _setLetter] = Devjs.useState('A');
    ...
    function Child({letter}) {
      const [, _setStep] = Devjs.useState(0);
      setStep = _setStep;
