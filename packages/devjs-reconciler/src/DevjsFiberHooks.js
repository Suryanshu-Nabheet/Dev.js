...
import type {
  ...
  HookType,
  MemoCache,
} from './DevjsInternalTypes';
import type {Lanes, Lane} from './DevjsFiberLane';
import type {HookFlags} from './DevjsHookEffectTags';
import type {Flags} from './DevjsFiberFlags';
import type {TransitionStatus} from './DevjsFiberConfig';
import type {ScheduledGesture} from './DevjsFiberGestureScheduler';
...
import {readContext, checkIfContextChanged} from './DevjsFiberNewContext';
import {HostRoot, CacheComponent, HostComponent} from './DevjsWorkTags';
import {
...
import {createCache} from './DevjsFiberCacheComponent';
import {
  createUpdate as createLegacyQueueUpdate,
  enqueueUpdate as enqueueLegacyQueueUpdate,
  entangleTransitions as entangleLegacyQueueTransitions,
} from './DevjsFiberClassUpdateQueue';
import {
  enqueueConcurrentHookUpdate,
  enqueueConcurrentHookUpdateAndEagerlyBailout,
  enqueueConcurrentRenderForLane,
} from './DevjsFiberConcurrentUpdates';
import {getTreeId} from './DevjsFiberTreeContext';
...
import type {ThenableState} from './DevjsFiberThenable';
import type {Transition} from 'devjs/src/DevjsStartTransition';
import {
  peekEntangledActionLane,
  peekEntangledActionThenable,
  chainThenableValue,
} from './DevjsFiberAsyncAction';
import {requestTransitionLane} from './DevjsFiberRootScheduler';
import {isCurrentTreeHidden} from './DevjsFiberHiddenContext';
import {requestCurrentTransition} from './DevjsFiberTransition';

import {callComponentInDEV} from './DevjsFiberCallUserSpace';

import {scheduleGesture} from './DevjsFiberGestureScheduler';
  ...
  if (__DEV__) {
    ...
    if (!didWarnAboutMismatchedHooksForComponent.has(componentName)) {
      ...
      if (hookTypesDev !== null) {
        ...
        console.error(
          'Devjs has detected a change in the order of Hooks called by %s. ' +
            'This will lead to bugs and errors if not fixed. ' +
            'For more information, read the Rules of Hooks: https://devjs.dev/link/rules-of-hooks\n\n' +
...
function warnOnUseFormStateInDev(): void {
  if (__DEV__) {
    ...
    if (!didWarnAboutUseFormState.has(componentName)) {
      ...
      console.error(
        'DevjsDOM.useFormState has been renamed to Devjs.useActionState. ' +
          'Please update %s to use Devjs.useActionState.',
        componentName,
      );
    }
  }
...
export function renderWithHooks<Props, SecondArg>(
  ...
  // This is tricky because it's valid for certain types of components (e.g. Devjs.lazy)
  ...
  if (__DEV__) {
    if (current !== null && current.memoizedState !== null) {
      DevjsSharedInternals.H = HooksDispatcherOnUpdateInDEV;
      ...
      DevjsSharedInternals.H = HooksDispatcherOnMountWithHookTypesInDEV;
    } else {
      DevjsSharedInternals.H = HooksDispatcherOnMountInDEV;
    }
  } else {
    DevjsSharedInternals.H =
  ...
  // We need to be able to memoize the devjsive inputs to the `use` call using
  // a hook (i.e. `useMemo`), which means, the devjsive inputs to `use` must
...
function finishRenderingHooks<Props, SecondArg>(
  ...
  if (__DEV__) {
    ...
    if (
      ...
      console.error(
        'Internal Devjs error: Expected static flag was missing. Please ' +
          'notify the Devjs team.',
      );
    }
  }
...
function renderWithHooksAgain<Props, SecondArg>(
  ...
  do {
    ...
    DevjsSharedInternals.H = __DEV__
...
export function TransitionAwareHostComponent(): TransitionStatus {
  const dispatcher: any = DevjsSharedInternals.H;
...
function use<T>(usable: Usable<T>): T {
  if (usable !== null && typeof usable === 'object') {
    ...
    } else if (usable.$$typeof === devjs_CONTEXT_TYPE) {
      const context: DevjsContext<T> = (usable: any);
...
function useMemoCache(size: number): Array<mixed> {
      ...
      if (currentUpdateQueue !== null) {
        ...
        if (currentMemoCache != null) {
          memoCache = {
            ...
            // from the previous attempt. We can do this because the Devjs
  ...
  if (data === undefined || (__DEV__ && ignorePreviousDependencies)) {
    ...
    for (let i = 0; i < size; i++) {
      data[i] = devjs_MEMO_CACHE_SENTINEL;
    }
...
function updateReducerImpl<S, A>(
  ...
  if (queue === null) {
    throw new Error(
      'Should have a queue. You are likely calling Hooks conditionally, ' +
        'which is not allowed. (https://devjs.dev/link/invalid-hook-call)',
    );
  }
  ...
  if (baseQueue === null) {
    ...
    // baseState is derived from other devjsive values.
  } else {
      ...
      if (enableGestureTransition && updateLane === GestureLane) {
        ...
        if (scheduledGesture !== null) {
          if (scheduledGesture.count === 0 && !scheduledGesture.committing) {
            ...
            if (root === null) {
              throw new Error(
                'Expected a work-in-progress root. This is a bug in Devjs. Please file an issue.',
              );
            }
...
function mountSyncExternalStore<T>(
  ...
  // normal rules of Devjs, and only works because store updates are
  // always synchronous.
...
function notifyActionListeners<S, P>(actionNode: ActionStateQueueNode<S, P>) {
  // Notify Devjs that the action has finished.
  const listeners = actionNode.listeners;
  for (let i = 0; i < listeners.length; i++) {
    // This is always a Devjs internal listener, so we don't need to worry
    // about it throwing.
      ...
      if (!refObject.hasOwnProperty('current')) {
        console.error(
          'Expected useImperativeHandle() first argument to either be a ' +
            'ref callback or Devjs.createRef() object. Instead received: %s.',
...
function mountDebugValue<T>(value: T, formatterFn: ?(value: T) => mixed): void {
  ...
  // The devjs-debug-hooks package injects its own implementation
  ...
  if (isRenderPhaseUpdate(fiber)) {
    ...
    if (
      ...
      if (lastRenderedReducer !== null) {
        ...
        if (__DEV__) {
          prevDispatcher = DevjsSharedInternals.H;
          DevjsSharedInternals.H = InvalidNestedHooksDispatcherOnUpdateInDEV;
        }
        try {
          ...
          if (is(eagerState, currentState)) {
            // Fast path. We can bail out without scheduling Devjs to re-render.
...
// TODO: Move to DevjsFiberConcurrentUpdates?
...
if (__DEV__) {
  ...
  const warnInvalidHookAccess = () => {
    console.error(
      'Do not call Hooks inside useEffect(...), useMemo(...), or other built-in Hooks. ' +
        ...
        'https://devjs.dev/link/rules-of-hooks',
    );
  };

  HooksDispatcherOnMountInDEV = {
    readContext<T>(context: DevjsContext<T>): T {
      return readContext(context);
    },
    use,
    ...
    useContext<T>(context: DevjsContext<T>): T {
    ...
    useMemo<T>(create: () => T, deps: Array<mixed> | void | null): T {
      ...
      DevjsSharedInternals.H = InvalidNestedHooksDispatcherOnMountInDEV;
      try {
  ...
  HooksDispatcherOnRerenderInDEV = {
    ...
    useMemo<T>(create: () => T, deps: Array<mixed> | void | null): T {
      ...
      updateHookTypesDev();
      const prevDispatcher = DevjsSharedInternals.H;
      DevjsSharedInternals.H = InvalidNestedHooksDispatcherOnRerenderInDEV;
      try {
        return updateMemo(create, deps);
      } finally {
        DevjsSharedInternals.H = prevDispatcher;
