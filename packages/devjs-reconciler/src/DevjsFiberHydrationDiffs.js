...
import {enableSrcObject} from 'shared/DevjsFeatureFlags';
...
function describeValue(value: mixed, maxLength: number): string {
  switch (typeof value) {
    ...
    case 'object': {
      ...
      if ((value: any).$$typeof === devjs_ELEMENT_TYPE) {
  ...
  if (node.fiber.tag === HostText) {
    ...
    if (type !== null) {
      ...
      if (node.serverProps === undefined) {
        ...
        if (__DEV__) {
          console.error(
            'Should not have matched a non HostText fiber to a Text node. This is a bug in Devjs.',
          );
        }
      } else {
