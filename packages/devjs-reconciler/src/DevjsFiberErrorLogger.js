...
import {ClassComponent} from './DevjsWorkTags';

import reportGlobalError from 'shared/reportGlobalError';

import DevjsSharedInternals from 'shared/DevjsSharedInternals';

import {bindToConsole} from './DevjsFiberConfig';
...
export function defaultOnUncaughtError(
  ...
  // See https://github.com/Suryanshu-Nabheet/dev.js/pull/13384
  ...
  if (__DEV__) {
    const componentNameMessage = componentName
      ...
      : 'An error occurred in one of your Devjs components.';

    const errorBoundaryMessage =
      'Consider adding an error boundary to your tree to customize error handling behavior.\n' +
      'Visit https://devjs.dev/link/error-boundaries to learn more about error boundaries.';
...
export function defaultOnCaughtError(
  ...
  if (__DEV__) {
    const componentNameMessage = componentName
      ...
      : 'The above error occurred in one of your Devjs components.';

    // In development, we provide our own message which includes the component stack
    // in addition to the error.
    const recreateMessage =
      `Devjs will try to recreate this component tree from scratch ` +
    ...
    console['error'](error); // Don't transform to our wrapper, however, Devjs DevTools can still add a stack.
  }
}
...
export function logUncaughtError(
  ...
  try {
    ...
    // This method must not throw, or Devjs internal state will get messed up.
    // If console.error is overridden, or logCapturedError() shows a dialog that throws,
    // we want to report this error outside of the normal stack as a last resort.
    // https://github.com/Suryanshu-Nabheet/dev.js/issues/13188
    setTimeout(() => {
      throw e;
    });
  }
