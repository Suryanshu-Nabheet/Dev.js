...
import type {ViewTransitionProps} from 'shared/DevjsTypes';

import type {Fiber, FiberRoot} from './DevjsInternalTypes';

import type {Instance, TextInstance, Props} from './DevjsFiberConfig';

import type {OffscreenState} from './DevjsFiberOffscreenComponent';
...
import {trackAnimatingTask} from './DevjsProfilerTimer';
import {scheduleGestureTransitionEvent} from './DevjsFiberWorkLoop';
  ...
  if ((child.flags & ViewTransitionNamedStatic) !== NoFlags) {
    ...
    if (state.paired) {
      ...
      if (props.name == null || props.name === 'auto') {
        throw new Error(
          'Found a pair with an auto name. This is a bug in Devjs.',
        );
      }
      const name = props.name;
