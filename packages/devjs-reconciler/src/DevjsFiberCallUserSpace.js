...
import type {Effect} from './DevjsFiberHooks';
import type {CapturedValue} from './DevjsCapturedValue';

import {isRendering, setIsRendering} from './DevjsCurrentFiber';
import {captureCommitPhaseError} from './DevjsFiberWorkLoop';

// These indirections exists so we can exclude its stack frame in DEV (and anything below it).
// TODO: Consider marking the whole bundle instead of these boundaries.

const callComponent = {
  devjs_stack_bottom_frame: function <Props, Arg, R>(
...
const callRender = {
  devjs_stack_bottom_frame: function <R>(instance: ClassInstance<R>): R {
    const wasRendering = isRendering;
    setIsRendering(true);
    try {
...
export const callRenderInDEV: <R>(instance: ClassInstance<R>) => R => R =
  __DEV__
    ? // We use this technique to trick minifiers to preserve the function name.
      (callRender.devjs_stack_bottom_frame.bind(callRender): any)
    : (null: any);
...
export const callComponentDidMountInDEV: (
  ...
  ? // We use this technique to trick minifiers to preserve the function name.
    (callComponentDidMount.devjs_stack_bottom_frame.bind(
...
export const callComponentDidUpdateInDEV: (
  ...
  ? // We use this technique to trick minifiers to preserve the function name.
    (callComponentDidUpdate.devjs_stack_bottom_frame.bind(
...
export const callComponentDidCatchInDEV: (
  ...
  ? // We use this technique to trick minifiers to preserve the function name.
    (callComponentDidCatch.devjs_stack_bottom_frame.bind(
...
export const callComponentWillUnmountInDEV: (
  ...
  ? // We use this technique to trick minifiers to preserve the function name.
    (callComponentWillUnmount.devjs_stack_bottom_frame.bind(
...
export const callCreateInDEV: (effect: Effect) => (() => void) | void = __DEV__
  ? // We use this technique to trick minifiers to preserve the function name.
    (callCreate.devjs_stack_bottom_frame.bind(callCreate): any)
  : (null: any);
...
export const callDestroyInDEV: (
  ...
  ? // We use this technique to trick minifiers to preserve the function name.
    (callDestroy.devjs_stack_bottom_frame.bind(callDestroy): any)
  : (null: any);
