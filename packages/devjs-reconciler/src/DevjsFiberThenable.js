...
import {callLazyInitInDEV} from './DevjsFiberCallUserSpace';

import {getWorkInProgressRoot} from './DevjsFiberWorkLoop';

import DevjsSharedInternals from 'shared/DevjsSharedInternals';

import {enableAsyncDebugInfo} from 'shared/DevjsFeatureFlags';
...
export const noopSuspenseyCommitThenable = {
  then() {
    if (__DEV__) {
      console.error(
        'Internal Devjs error: A listener was unexpectedly attached to a ' +
          '"noop" thenable. This is a bug in Devjs. Please file an issue.',
      );
    }
  },
};
...
export function trackUsedThenable<T>(
  ...
  if (__DEV__ && DevjsSharedInternals.actQueue !== null) {
    DevjsSharedInternals.didUsePromise = true;
  }
    ...
    if (previous !== thenable) {
      ...
      if (__DEV__) {
        ...
        if (!thenableStateDev.didWarnAboutUncachedPromise) {
          ...
          // promise-as-Devjs-node, the owner component is likely different from
...
export function getSuspendedThenable(): Thenable<mixed> {
  ...
  if (suspendedThenable === null) {
    throw new Error(
      'Expected a suspended thenable. This is a bug in Devjs. Please file ' +
        'an issue.',
    );
  }
