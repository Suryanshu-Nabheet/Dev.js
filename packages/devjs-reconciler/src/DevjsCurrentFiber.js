...
import {getOwnerStackByFiberInDev} from './DevjsFiberComponentStack';
...
export function runWithFiberInDEV<A0, A1, A2, A3, A4, T>(
  ...
  throw new Error(
    'runWithFiberInDEV should never be called in production. This is a bug in Devjs.',
  );
}

export function resetCurrentFiber() {
  if (__DEV__) {
    DevjsSharedInternals.getCurrentStack = null;
    isRendering = false;
  }
  current = null;
}
...
export function setCurrentFiber(fiber: Fiber | null) {
  if (__DEV__) {
    DevjsSharedInternals.getCurrentStack =
