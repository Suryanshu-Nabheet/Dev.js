...
const run = async ({cwd, packages, version, ci}, versionsMap) => {
  ...
  const updateDependencies = async (targetPackageJSON, key) => {
    ...
    if (targetDependencies) {
      ...
      for (let i = 0; i < packages.length; i++) {
        ...
        if (targetDependency) {
          // For example, say we're updating devjs-dom's dependency on scheduler.
          ...
          // and the dependency constraint in the local version of devjs-dom (e.g. scheduler@^0.11.0).
          ...
          // Otherwise we leave the constraint alone (e.g. devjs@^16.0.0 doesn't change between releases).
  ...
  if (packages.includes('devjs')) {
    ...
    console.log(
      theme`Skipping Devjs renderer version update because Devjs is not included in the release.`
    );
  }
