...
const run = async ({cwd, packages, tags}) => {
  // Tags are named after the devjs version.
  const {version} = readJsonSync(
    `${cwd}/build/node_modules/devjs/package.json`
  );
  ...
  if (tags.length === 1 && tags[0] === 'next') {
    ...
    if (tags.includes('latest')) {
      ...
      const status = await execRead(
        'git diff packages/shared/DevjsVersion.js',
        {cwd}
      );
      if (status) {
        console.log(theme.path`• packages/shared/DevjsVersion.js`);
      }
      ...
      console.log(
        theme.link`https://github.com/Suryanshu-Nabheet/dev.js/releases/tag/v%s`,
        version
      );
      console.log(
      ...
      // Update devjsjs.org so the Devjs version shown in the header is up to date.
      console.log();
      console.log(
