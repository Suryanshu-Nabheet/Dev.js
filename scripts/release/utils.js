...
const {stablePackages, experimentalPackages} = require('../../DevjsVersions');
...
const getBuildInfo = async () => {
  ...
  // Devjs version is stored explicitly, separately for DevTools support.
  // See updateVersionsForNext() below for more info.
  const packageJSON = await readJson(
    join(cwd, 'packages', 'devjs', 'package.json')
  );
  const devjsVersion = isExperimental
    ? `${packageJSON.version}-experimental-${commit}-${dateString}`
    : `${packageJSON.version}-${commit}-${dateString}`;

  return {branch, checksum, commit, devjsVersion, version};
};
...
// As well as the embedded renderer version in "packages/shared/DevjsVersion".
// Canaries version numbers use the format of 0.0.0-<sha>-<date> to be easily recognized (e.g. 0.0.0-01974a867-20200129).
// A separate "Devjs version" is used for the embedded renderer version to support DevTools,
// since it needs to distinguish between different version ranges of Devjs.
// It is based on the version of Devjs in the local package.json (e.g. 16.12.0-01974a867-20200129).
// Both numbers will be replaced if the "next" release is promoted to a stable release.
const updateVersionsForNext = async (cwd, devjsVersion, version) => {
  const isExperimental = devjsVersion.includes('experimental');
  const packages = getPublicPackages(isExperimental);
  const packagesDir = join(cwd, 'packages');

  // Update the shared Devjs version source file.
  // This is bundled into built renderers.
  // The promote script will replace this with a final version later.
  const sourceDevjsVersionPath = join(cwd, 'packages/shared/DevjsVersion.js');
  const sourceDevjsVersion = readFileSync(
    sourceDevjsVersionPath,
    'utf8'
  ).replace(/export default '[^']+';/, `export default '${devjsVersion}';`);
  writeFileSync(sourceDevjsVersionPath, sourceDevjsVersion);
