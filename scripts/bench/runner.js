...
async function runBenchmarks(devjsPath) {
  ...
  for (let i = 0; i < benchmarkNames.length; i++) {
    ...
    if (
      ...
      await buildBenchmark(devjsPath, benchmarkName);
      console.log(
...
// from remote main (default devjs repo)
async function benchmarkRemoteMaster() {
  console.log(chalk.gray(`- Building devjs bundles...`));
...
// get the performance benchmark results
// of the local devjs repo
async function benchmarkLocal(devjsPath) {
  console.log(chalk.gray(`- Building devjs bundles...`));
  await builddevjsBundles(devjsPath, skipBuild);
  return {
    benchmarks: await runBenchmarks(devjsPath),
  };
}
