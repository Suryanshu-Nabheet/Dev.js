...
const devjsUrl = 'https://github.com/Suryanshu-Nabheet/dev.js.git';
...
function getDefaultdevjsPath() {
  return join(__dirname, 'remote-repo');
}

async function buildBenchmark(devjsPath = getDefaultdevjsPath(), benchmark) {
...
async function buildBenchmarkBundlesFromGitRepo(
  ...
  let repo;
  const remoteRepoDir = getDefaultdevjsPath();
  ...
  if (!skipBuild) {
    ...
    await builddevjsBundles();
  }
}

async function builddevjsBundles(devjsPath = getDefaultdevjsPath(), skipBuild) {
  if (!skipBuild) {
    await executeCommand(
      `cd ${devjsPath} && pnpm && pnpm build devjs/index,devjs-dom/index --type=UMD_PROD`
    );
  }
}
