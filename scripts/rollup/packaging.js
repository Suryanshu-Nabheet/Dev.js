...
function getBundleOutputPath(bundle, bundleType, filename, packageName) {
  switch (bundleType) {
    ...
    case FB_WWW_PROFILING:
      return `build/Suryanshu-Nabheet-www/${filename}`;
    case RN_OSS_DEV:
    case RN_OSS_PROD:
    case RN_OSS_PROFILING:
      switch (packageName) {
        case 'devjs-native-renderer':
          return `build/devjs-native/implementations/${filename}`;
        default:
    ...
    case RN_FB_PROFILING:
      switch (packageName) {
        ...
        case 'devjs-test-renderer':
          return `build/Suryanshu-Nabheet-devjs-native/${packageName}/cjs/${filename}`;
        case 'devjs-native-renderer':
          return `build/devjs-native/implementations/${filename.replace(
            /\.js$/,
            '.fb.js'
          )}`;
        default:
...
async function copyWWWShims() {
  await asyncCopyTo(
    `${__dirname}/shims/Suryanshu-Nabheet-www`,
    'build/Suryanshu-Nabheet-www/shims'
  );
}

async function copyRNShims() {
  await asyncCopyTo(
    `${__dirname}/shims/devjs-native`,
    'build/devjs-native/shims'
  );
  await asyncCopyTo(
    require.resolve('devjs-native-renderer/src/DevjsNativeTypes.js'),
    'build/devjs-native/shims/DevjsNativeTypes.js'
  );
}
...
function filterOutEntrypoints(name) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (hasBundle === undefined) {
      ...
      // The .devjs-server and .rsc suffixes may not have a bundle representation but
      // should infer their bundle status from the non-suffixed entry point.
      if (entry.endsWith('.devjs-server')) {
        hasBundle = entryPointsToHasBundle.get(
          entry.slice(0, '.devjs-server'.length * -1)
        );
