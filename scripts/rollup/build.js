...
function getPlugins(
  ...
  try {
    ...
    const needsMinifiedByClosure =
      bundleType !== ESM_PROD &&
      bundleType !== ESM_DEV &&
      // TODO(@poteto) figure out ICE in closure compiler for eslint-plugin-devjs-hooks (ts)
    ...
    return [
      ...
      // Remove 'use strict' from individual source files. We skip eslint-plugin-devjs-hooks because
      // it bundles compiler-type code that may examine "use strict" used outside of a directive
      // context, e.g. as a StringLiteral.
      bundle.name !== 'eslint-plugin-devjs-hooks'
        ? {
      ...
      needsMinifiedByClosure &&
        closure({
          ...
          // https://github.com/Suryanshu-Nabheet/dev.js/issues/10909
...
function shouldSkipBundle(bundle, bundleType) {
  ...
  if (requestedBundleNames.length > 0) {
    ...
    // entry ends in something. Such as `devjs-dom/index` only matches
    // `devjs-dom` but not `devjs-dom/server`. Everything else is fuzzy
    // search.
...
async function buildEverything() {
  ...
  if (syncFBSourcePath) {
    await Sync.syncDevjsNative(syncFBSourcePath);
  } else if (syncWWWPath) {
    await Sync.syncDevjsDom('build/Suryanshu-Nabheet-www', syncWWWPath);
  }
