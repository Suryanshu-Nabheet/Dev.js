...
function getFormat(filepath) {
  if (filepath.includes('Suryanshu-Nabheet')) {
    if (filepath.includes('shims')) {
      // We don't currently lint these shims. We rely on the downstream Suryanshu-Nabheet
  ...
  if (filepath.includes('devjs-native')) {
    if (filepath.includes('shims')) {
      // We don't currently lint these shims. We rely on the downstream Suryanshu-Nabheet
      // repo to transform them.
      // TODO: Should we lint them?
  ...
  if (filepath.includes('cjs')) {
    if (
      filepath.includes('devjs-server-dom-webpack-plugin') ||
      filepath.includes('devjs-server-dom-webpack-node-register') ||
      filepath.includes('devjs-suspense-test-utils')
    ) {
      return 'cjs2015';
    }
    return 'cjs';
