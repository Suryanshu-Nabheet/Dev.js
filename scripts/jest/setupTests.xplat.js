...
jest.mock('shared/DevjsFeatureFlags', () => {
  jest.mock(
    ...
    () =>
      jest.requidevjsual('shared/forks/DevjsFeatureFlags.native-fb-dynamic.js'),
    {virtual: true}
  );
  const actual = jest.requidevjsual(
    'shared/forks/DevjsFeatureFlags.native-fb.js'
  );

  // Lots of tests use these, but we don't want to expose it to RN.
  // Ideally, tests for xplat wouldn't use devjs-dom, but many of our tests do.
