...
const argv = yargs
  ...
  .options({
    ...
    devjsVersion: {
      describe: 'DevTools testing for specific version of Devjs',
      requiresArg: true,
      type: 'string',
    },
    sourceMaps: {
...
function validateOptions() {
  ...
  if (argv.project === 'devtools') {
    ...
    if (argv.devjsVersion && !semver.validRange(argv.devjsVersion)) {
      success = false;
      logError('please specify a valid version range for --devjsVersion');
    }
  } else {
    ...
    if (argv.devjsVersion) {
      logError('Only DevTools tests supports the --devjsVersion flag.');
      success = false;
    }
  }
  ...
  if (argv.build) {
    ...
    if (!fs.existsSync(buildDir)) {
      logError(
        'Build directory does not exist, please run `pnpm build` or remove the --build option.'
      );
      success = false;
    } else if (Date.now() - fs.statSync(buildDir).mtimeMs > 1000 * 60 * 15) {
      logError(
        'Warning: Running a build test with a build directory older than 15 minutes.\nPlease remember to run `pnpm build` when using --build.'
      );
    }
  }
...
function getEnvars() {
  ...
  if (argv.devjsVersion) {
    envars.devjs_VERSION = semver.coerce(argv.devjsVersion);
  }
