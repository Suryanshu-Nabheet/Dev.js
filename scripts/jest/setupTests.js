...
if (process.env.devjs_CLASS_EQUIVALENCE_TEST) {
  ...
  global.__devjs_ROOT_PATH_TEST__ = path.resolve(__dirname, '../..');
  ...
  expect.extend({
    ...require('./matchers/devjsTestMatchers'),
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    const decodeErrorMessage = function (message) {
      ...
      const re = /devjs.dev\/errors\/(\d+)?\??([^\s]*)/;
      let matches = message.match(re);
      if (!matches || matches.length !== 3) {
        // Some tests use Devjs 17, when the URL was different.
  ...
  // mock in every file.
  jest.mock('jsdom', () => {
    return require('internal-test-utils/DevjsJSDOM.js');
  });
}
...
jest.mock('async_hooks', () => {
  const actual = jest.requidevjsual('async_hooks');
  return {
    ...actual,
    createHook(config) {
      if (installedHook) {
