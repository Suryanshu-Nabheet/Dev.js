...
function readChangelogSnippet(preferredPackage) {
  const cacheKey =
    preferredPackage === 'eslint-plugin-devjs-hooks'
      ? preferredPackage
      : 'root';
...
async function summarizePackageCommits({
  ...
  const hooksStyle = readChangelogSnippet('eslint-plugin-devjs-hooks');
  ...
  const payload = commits.map(commit => {
    ...
    return {
      ...
      style: usesHooksStyle ? 'eslint-plugin-devjs-hooks' : 'root',
  ...
  const promptParts = [
    ...
    'Use `"root"` style unless the payload specifies `"eslint-plugin-devjs-hooks"`, in which case use that style sample.',
    '',
    '--- STYLE: root ---',
    rootStyle,
    '--- END STYLE ---',
    '',
    '--- STYLE: eslint-plugin-devjs-hooks ---',
    hooksStyle,
    '--- END STYLE ---',
    '',
...
function stripClaudeBanner(text) {
  return text
    ...
    .filter(
      line =>
        line.trim() !==
        'Claude Code at Meta (https://github.com/Suryanshu-Nabheet/dev.js
    )
    .join('\n')
    .trim();
}
