...
const {stablePackages} = require('../../../DevjsVersions');
const {isCommandAvailable} = require('./utils');

function parseArgs(argv) {
  const parser = yargs(argv)
    .usage(
      'Usage: pnpm generate-changelog [--codex|--claude] [--debug] [--format <text|csv|json>] [<pkg@version> ...]'
    )
    .example(
      '$0 --codex eslint-plugin-devjs-hooks@7.0.1',
      'Generate changelog for a single package using Codex.'
    )
    .example(
      '$0 --claude devjs@19.3 devjs-dom@19.3',
