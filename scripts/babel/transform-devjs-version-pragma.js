...
const GATE_VERSION_STR = '@devjsVersion ';
const devjs_VERSION_ENV = process.env.devjs_VERSION;

function transform(babel) {
  const {types: t} = babel;

  // Runs tests conditionally based on the version of devjs (semver range) we are running
  // Input:
  //   @devjsVersion >= 17.0
  //   test('some test', () => {/*...*/})
  //
  // Output:
  //    @devjsVersion >= 17.0
  //   _test_devjs_version('>= 17.0', 'some test', () => {/*...*/});
  //
  ...
  return {
    name: 'transform-devjs-version-pragma',
    visitor: {
            ...
            case 'Identifier': {
              if (
                ...
                if (condition !== null) {
                  callee.name =
                    callee.name === 'fit'
                      ? '_test_devjs_version_focus'
                      : '_test_devjs_version';
                  expression.arguments = [condition, ...expression.arguments];
                } else if (devjs_VERSION_ENV) {
                  callee.name = '_test_ignore_for_devjs_version';
                }
              }
              break;
            }
              ...
              if (
                ...
                if (condition !== null) {
                  statement.expression = t.callExpression(
                    t.identifier('_test_devjs_version_focus'),
                    [condition, ...expression.arguments]
                  );
                } else if (devjs_VERSION_ENV) {
                  statement.expression = t.callExpression(
                    t.identifier('_test_ignore_for_devjs_version'),
