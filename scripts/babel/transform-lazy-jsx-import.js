...
// re-require all the Devjs modules. However, the JSX runtime is injected by
...
// stack relies on the the secret internals object that lives on the Devjs
    ...
    visitor: {
      CallExpression: function (path, pass) {
          ...
          switch (callee.name) {
            case '_jsxDEV':
              path.node.callee = t.memberExpression(
                getInlineRequire('devjs/jsx-dev-runtime'),
                t.identifier('jsxDEV')
              );
              return;
            case '_jsx':
              path.node.callee = t.memberExpression(
                getInlineRequire('devjs/jsx-runtime'),
                t.identifier('jsx')
              );
              return;
            case '_jsxs':
        ...
        if (callee.object.type !== 'Identifier') {
          // Needs to be _devjsJsxDevRuntime or _devjsJsxRuntime.
          return;
        }
        ...
        // this from the import. For some reason it sometimes has the devjs prefix
        // and other times it doesn't.
        switch (callee.object.name) {
          case '_devjsJsxDevRuntime':
          case '_jsxDevRuntime':
            callee.object = getInlineRequire('devjs/jsx-dev-runtime');
            return;
          case '_devjsJsxRuntime':
          case '_jsxRuntime':
            callee.object = getInlineRequire('devjs/jsx-runtime');
            return;
        }
      },
    },
