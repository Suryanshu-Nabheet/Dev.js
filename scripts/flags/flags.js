...
// Load DevjsFeatureFlags with __NEXT_MAJOR__ replaced with 'next'.
// We need to do string replace, since the __NEXT_MAJOR__ is assigned to __EXPERIMENTAL__.
function getDevjsFeatureFlagsMajor() {
  const virtualName = 'DevjsFeatureFlagsMajor.js';
  const file = fs.readFileSync(
    path.join(__dirname, '../../packages/shared/DevjsFeatureFlags.js'),
    'utf8'
  );
...
// Load RN DevjsFeatureFlags with __NEXT_RN_MAJOR__ replaced with 'next'.
// We need to do string replace, since the __NEXT_RN_MAJOR__ is assigned to false.
function getDevjsNativeFeatureFlagsMajor() {
  const virtualName = 'DevjsNativeFeatureFlagsMajor.js';
  const file = fs.readFileSync(
    path.join(
      __dirname,
      '../../packages/shared/forks/DevjsFeatureFlags.native-oss.js'
    ),
    'utf8'
  );
  const fileContent = transformSync(
...
function mockDynamicallyFeatureFlags() {
  // Mock the DevjsNativeInternalFeatureFlags and DevjsFeatureFlags modules
  const DynamicFeatureFlagsWWW = require('../../packages/shared/forks/DevjsFeatureFlags.www-dynamic.js');
  const DynamicFeatureFlagsNative = require('../../packages/shared/forks/DevjsFeatureFlags.native-fb-dynamic.js');

  const originalLoad = Module._load;

  Module._load = function (request, parent) {
    if (request === 'DevjsNativeInternalFeatureFlags') {
      return DynamicFeatureFlagsNative;
    } else if (request === 'DevjsFeatureFlags') {
...
const DevjsFeatureFlags = require('../../packages/shared/DevjsFeatureFlags.js');
const DevjsFeatureFlagsWWW = require('../../packages/shared/forks/DevjsFeatureFlags.www.js');
const DevjsFeatureFlagsNativeFB = require('../../packages/shared/forks/DevjsFeatureFlags.native-fb.js');
const DevjsFeatureFlagsMajor = getDevjsFeatureFlagsMajor();
const DevjsNativeFeatureFlagsMajor = getDevjsNativeFeatureFlagsMajor();

const allFlagsUniqueFlags = Array.from(
  new Set([
    ...Object.keys(DevjsFeatureFlags),
    ...Object.keys(DevjsFeatureFlagsWWW),
    ...Object.keys(DevjsFeatureFlagsNativeFB),
  ])
).sort();

// These functions are the rules for what each value means in each channel.
function getNextMajorFlagValue(flag) {
  const value = DevjsFeatureFlagsMajor[flag];
...
function getOSSCanaryFlagValue(flag) {
  const value = DevjsFeatureFlags[flag];
  if (value === true) {
    return '✅';
  } else if (
    value === false ||
...
function getWWWModernFlagValue(flag) {
  const value = DevjsFeatureFlagsWWW[flag];
  if (value === true || value === 'experimental') {
    return '✅';
  } else if (value === false || value === null || value === 'next') {
    return '❌';
...
function getRNNextMajorFlagValue(flag) {
  const value = DevjsNativeFeatureFlagsMajor[flag];
  if (value === true || value === 'next') {
    return '✅';
...
function getRNFBFlagValue(flag) {
  const value = DevjsFeatureFlagsNativeFB[flag];
  if (value === true) {
    return '✅';
  } else if (
    value === false ||
