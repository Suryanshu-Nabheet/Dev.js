...
async function main() {
  ...
  await confirm('Have you stopped all NPM DEV scripts?', () => {
    ...
    console.log(
      chalk.bold('  ' + join(packagesPath, 'devjs-devtools-core')),
      chalk.gray('(start:backend, start:standalone)')
    );
    console.log(
      chalk.bold('  ' + join(packagesPath, 'devjs-devtools-inline')),
      chalk.gray('(start)')
    );
  ...
  const currentCommitHash = await downloadLatestDevjsBuild();
...
async function buildAndTestExtensions() {
  const extensionsPackagePath = join(
    ROOT_PATH,
    'packages',
    'devjs-devtools-extensions'
  );
  const buildExtensionsPromise = exec('pnpm build', {
  ...
  console.log(`  ${chalk.bold.green('pnpm test:chrome')}`);
  console.log('');
  console.log(`  ${chalk.dim('# Test Edge extension')}`);
  console.log(`  ${chalk.bold.green('pnpm test:edge')}`);
  console.log('');
  console.log(`  ${chalk.dim('# Firefox Chrome extension')}`);
  console.log(`  ${chalk.bold.green('pnpm test:firefox')}`);

  await confirmContinue();
}

async function buildAndTestStandalonePackage() {
  const corePackagePath = join(ROOT_PATH, 'packages', 'devjs-devtools-core');
  const corePackageDest = join(corePackagePath, 'dist');

  await exec(`rm -rf ${corePackageDest}`);
  const buildCorePromise = exec('pnpm build', {cwd: corePackagePath});

  await logger(
    buildCorePromise,
    `Building ${chalk.bold('devjs-devtools-core')} package.`,
    {
      estimate: 25000,
    }
  );

  const standalonePackagePath = join(ROOT_PATH, 'packages', 'devjs-devtools');
  ...
  console.log('');
  console.log(
    `Test the ${chalk.bold('devjs-devtools-core')} target before continuing:`
  );
  console.log(`  ${chalk.bold.green('cd ' + standalonePackagePath)}`);
  console.log(`  ${chalk.bold.green('pnpm start')}`);
  console.log('');
  console.log(
...
async function buildAndTestInlinePackage() {
  ...
  const buildPromise = exec('pnpm build', {cwd: inlinePackagePath});

  await logger(
    buildPromise,
    `Building ${chalk.bold('devjs-devtools-inline')} package.`,
    {
      estimate: 10000,
    }
  );

  const shellPackagePath = join(ROOT_PATH, 'packages', 'devjs-devtools-shell');

  console.log('');
  console.log(`Built ${chalk.bold('devjs-devtools-inline')} target.`);
  console.log('');
...
async function downloadLatestDevjsBuild() {
  const releaseScriptPath = join(ROOT_PATH, 'scripts', 'release');
  const installPromise = exec('pnpm install', {cwd: releaseScriptPath});
  ...
  const {commit} = await inquirer.prompt([
    {
      type: 'input',
      name: 'commit',
      message: 'Which Devjs version (commit) should be used?',
  ...
  await logger(downloadPromise, 'Downloading Devjs artifacts from CI.', {
    estimate: 15000,
  });
