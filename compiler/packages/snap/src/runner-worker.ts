...
import type {parseConfigPragmaForTests as ParseConfigPragma} from 'babel-plugin-devjs-compiler/src/Utils/TestUtils';
import type {printFunctionWithOutlined as PrintFunctionWithOutlined} from 'babel-plugin-devjs-compiler/src/HIR/PrintHIR';
import type {printdevjsiveFunctionWithOutlined as PrintdevjsiveFunctionWithOutlined} from 'babel-plugin-devjs-compiler/src/devjsiveScopes/PrintdevjsiveFunction';
import {TransformResult, transformFixtureInput} from './compiler';
import {
  PARSE_CONFIG_PRAGMA_IMPORT,
  PRINT_HIR_IMPORT,
  PRINT_devjsIVE_IR_IMPORT,
  BABEL_PLUGIN_SRC,
} from './constants';
...
const invalidationSubpath = 'packages/babel-plugin-devjs-compiler/dist';
...
async function compile(
  ...
  try {
    ...
    const BabelPlugindevjsCompiler = importedCompilerPlugin[
      'default'
    ] as PluginObj;
    ...
    const printdevjsiveFunctionWithOutlined = importedCompilerPlugin[
      PRINT_devjsIVE_IR_IMPORT
    ] as typeof PrintdevjsiveFunctionWithOutlined;
      ...
      ? (value: CompilerPipelineValue) => {
          ...
          switch (value.kind) {
            case 'hir':
              ...
              break;
            case 'devjsive':
              printed = printdevjsiveFunctionWithOutlined(value.value);
              break;
            case 'debug':
              printed = value.value;
              break;
