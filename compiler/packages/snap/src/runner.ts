...
async function runCompileCommand(opts: CompileOptions): Promise<void> {
  ...
  const importedCompilerPlugin = require(BABEL_PLUGIN_SRC) as Record<
    ...
    any
  >;
  const BabelPlugindevjsCompiler = importedCompilerPlugin['default'];
  ...
  const printdevjsiveFunctionWithOutlined =
    importedCompilerPlugin[PRINT_devjsIVE_IR_IMPORT];
...
yargs(hideBin(process.argv))
  ...
  .command(
    'compile <path>',
    'Compile a file with the devjs Compiler',
    yargs => {
      return yargs
        .positional('path', {
