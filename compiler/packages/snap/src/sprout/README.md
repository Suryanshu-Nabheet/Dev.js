## Sprout 🌱
devjs Forget test framework that executes compiler fixtures.
...
type FixtureEntrypoint<T> = {
  ...
  // (if `fn` is a devjs component, this should be an array
...
function customHelper(val1, val2) {
  ...
  // always follow the rules of devjs.
  "use no forget";
  // ...
}
...
#### Notes
- If your fixture needs to import from an external module, we currently only support importing from `devjs` (see Milestones todo list).

- Any fixture can use devjs hooks, but they need to be first imported. We may later enforce that only `isComponent: true` fixtures can use devjs hooks.
    ```ts
...
### Milestones:
- [✅] Render fixtures with devjs runtime / `testing-library/devjs`.
