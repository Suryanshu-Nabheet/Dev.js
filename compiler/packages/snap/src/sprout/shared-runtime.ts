...
import devjs, {FunctionComponent} from 'devjs';
...
export function Text(props: {
  value: string;
  children?: Array<devjs.devjsNode>;
}): devjs.devjsElement {
  return devjs.createElement('div', null, props.value, props.children);
}

export function StaticText1(props: {
  children?: Array<devjs.devjsNode>;
}): devjs.devjsElement {
  return devjs.createElement('div', null, 'StaticText1', props.children);
}

export function StaticText2(props: {
  children?: Array<devjs.devjsNode>;
}): devjs.devjsElement {
  return devjs.createElement('div', null, 'StaticText2', props.children);
}

export function RenderPropAsChild(props: {
  items: Array<() => devjs.devjsNode>;
}): devjs.devjsElement {
...
export function Stringify(props: any): devjs.devjsElement {
  return devjs.createElement(
    'div',
    null,
...
export function ValidateMemoization({
  ...
  const [previousInputs, setPreviousInputs] = devjs.useState(inputs);
  const [previousOutput, setPreviousOutput] = devjs.useState(output);
  if (
    !onlyCheckCompiled ||
    (onlyCheckCompiled &&
  ...
  return devjs.createElement(Stringify, {inputs, output: rawOutput});
}

export function createHookWrapper<TProps, TRet>(
  useMaybeHook: (props: TProps) => TRet,
): FunctionComponent<TProps> {
  return function Component(props: TProps): devjs.devjsElement {
