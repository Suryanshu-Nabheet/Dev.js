...
import {render} from '@testing-library/devjs';
import {JSDOM} from 'jsdom';
import devjs, {MutableRefObject} from 'devjs';
 ...
 */
class WrapperTestComponentWithErrorBoundary extends devjs.Component<
...
> {
  ...
  override render() {
    ...
    return devjs.createElement(WrapperTestComponent, this.props);
  }
}
...
function WrapperTestComponent(props: {fn: any; params: Array<any>}) {
  ...
  // needs to passed through to devjs's runtime as-is) or a non-jsx value
...
function renderComponentSequentiallyForEachProps(
  ...
  const {rerender, container} = render(
    devjs.createElement(WrapperTestComponentWithErrorBoundary, {
      fn,
      params: [initialProps],
    }),
  );
...
(globalThis as any).evaluateFixtureExport = function (
  ...
  if (entrypoint.sequentialRenders !== null) {
    ...
    // Try to run fixture as a devjs component. This is necessary because not
    // all components are functions (some are ForwardRef or Memo objects).
    const result = render(
      devjs.createElement(entrypoint.fn as any, entrypoint.params[0]),
    ).container.innerHTML;
    ...
    const result = render(devjs.createElement(WrapperTestComponent, entrypoint))
      .container.innerHTML;
  ...
  (globalThis.console as any) = {
    ...
    error: (...args: Array<any>) => {
      if (
        ...
        args[0].includes('devjsDOMTestUtils.act` is deprecated')
      ) {
        // remove this once @testing-library/devjs is upgraded to devjs 19.
        return;
      }

      const stack = new Error().stack?.split('\n', 5) ?? [];
      for (const stackFrame of stack) {
        // devjs warns on exceptions thrown during render, we avoid printing
        // here to reduce noise in test fixture outputs.
        if (
          (stackFrame.includes('at logCaughtError') &&
            stackFrame.includes('devjs-dom-client.development.js')) ||
