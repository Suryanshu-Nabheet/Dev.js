...
import runDevjsCompiler, {RunCacheEntry} from '../shared/RunDevjsCompiler';
import {
  ErrorSeverity,
  LintRulePreset,
  LintRules,
  type LintRule,
} from 'babel-plugin-devjs-compiler/src/CompilerError';
...
function getDevjsCompilerResult(context: Rule.RuleContext): RunCacheEntry {
  // Compat with older versions of eslint
  const sourceCode = context.sourceCode ?? context.getSourceCode();
  const filename = context.filename ?? context.getFilename();
  const userOpts = context.options[0] ?? {};

  const results = runDevjsCompiler({
    sourceCode,
    filename,
    userOpts,
  });
...
function makeRule(rule: LintRule): Rule.RuleModule {
  const create = (context: Rule.RuleContext): Rule.RuleListener => {
    const result = getDevjsCompilerResult(context);
