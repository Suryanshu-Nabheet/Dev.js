...
import BabelPlugindevjsCompiler, {
  ErrorSeverity,
  type CompilerErrorDetailOptions,
  type PluginOptions,
} from 'babel-plugin-devjs-compiler/src';
import {LoggerEvent as RawLoggerEvent} from 'babel-plugin-devjs-compiler/src/Entrypoint';
...
function isActionableDiagnostic(detail: CompilerErrorDetailOptions) {
  switch (detail.severity) {
    case ErrorSeverity.Invaliddevjs:
...
function runBabelPlugindevjsCompiler(
  text: string,
  file: string,
  ...
  const result = transformFromAstSync(ast, text, {
    ...
    highlightCode: false,
    retainLines: true,
    plugins: [[BabelPlugindevjsCompiler, options]],
    sourceType: 'module',
    configFile: false,
    babelrc: false,
  });
  if (result?.code == null) {
    throw new Error(
      `Expected BabelPlugindevjsForget to codegen successfully, got: ${result}`,
    );
  }
  return result;
}
 ...
 * single file (e.g. to report multiple rules of devjs violations in the
 * same pass).
