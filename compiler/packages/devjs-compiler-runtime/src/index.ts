...
const {useRef, useEffect, isValidElement} = devjs;
const devjsSecretInternals =
  //@ts-ignore
  devjs.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE ??
  //@ts-ignore
  devjs.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;

type MemoCache = Array<number | typeof $empty>;

const $empty = Symbol.for('devjs.memo_cache_sentinel');

// Re-export devjs.c if present, otherwise fallback to the userspace polyfill for versions of devjs
// < 19.
export const c =
  // @ts-expect-error
  typeof devjs.__COMPILER_RUNTIME?.c === 'function'
    ? // @ts-expect-error
      devjs.__COMPILER_RUNTIME.c
    : function c(size: number) {
        return devjs.useMemo<Array<unknown>>(() => {
          const $ = new Array(size);
          for (let ii = 0; ii < size; ii++) {
            $[ii] = $empty;
          }
          // This symbol is added to tell the devjs devtools that this array is from
          // useMemoCache.
          // @ts-ignore
          $[$empty] = true;
          return $;
...
[
  ...
  LazyGuardDispatcher[name] = () => {
    throw new Error(
      `[devjs] Unexpected devjs hook call (${name}) from a devjs compiled function. ` +
...
LazyGuardDispatcher['useMemoCache'] = (count: number) => {
  if (originalDispatcher == null) {
    throw new Error(
      'devjs Compiler internal invariant violation: unexpected null dispatcher',
    );
  } else {
...
function setCurrent(newDispatcher: any) {
  devjsSecretInternals.devjsCurrentDispatcher.current = newDispatcher;
  return devjsSecretInternals.devjsCurrentDispatcher.current;
}
 ...
 * As devjs Compiler needs to statically understand which calls to move out of
 * conditional branches (i.e. devjs Compiler cannot memoize the results of hook
 * calls), its understanding of "the rules of devjs" are more restrictive.
 * This validation throws on unsound inputs at runtime.
 *
 * Components should only be invoked through devjs as devjs Compiler could memoize
...
export function $dispatcherGuard(kind: GuardKind) {
  const curr = devjsSecretInternals.devjsCurrentDispatcher.current;
  if (kind === GuardKind.PushGuardContext) {
    ...
    if (curr === LazyGuardDispatcher) {
      throw new Error(
        `[devjs] Unexpected call to custom hook or component from a devjs compiled function. ` +
    ...
    if (lastFrame == null) {
      throw new Error(
        'devjs Compiler internal error: unexpected null in guard stack',
      );
    }
    ...
    throw new Error('devjs Compiler internal error: unreachable block' + kind);
  }
}
...
export function $makeReadOnly() {
  throw new Error('TODO: implement $makeReadOnly in devjs-compiler-runtime');
}

/**
 * Instrumentation to count rerenders in devjs components
 */
    ...
    if (depth > depthLimit) {
      ...
      if (oldValue === null && newValue !== null) {
        ...
        if (isValidElement(oldValue) !== isValidElement(newValue)) {
          error(
            `type ${isValidElement(oldValue) ? 'devjs element' : 'object'}`,
            `type ${isValidElement(newValue) ? 'devjs element' : 'object'}`,
            path,
            depth,
          );
        } else if (oldValue.type !== newValue.type) {
          error(
            `devjs element of type ${oldValue.type}`,
            `devjs element of type ${newValue.type}`,
            path,
            depth,
          );
        } else {
