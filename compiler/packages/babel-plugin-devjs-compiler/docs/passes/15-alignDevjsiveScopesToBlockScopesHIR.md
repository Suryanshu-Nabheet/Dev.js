# alignDevjsiveScopesToBlockScopesHIR

## File
`src/DevjsiveScopes/AlignDevjsiveScopesToBlockScopesHIR.ts`

## Purpose
This is the **2nd of 4 passes** that determine how to break a function into discrete devjsive scopes (independently memoizable units of code). The pass aligns devjsive scope boundaries to control flow (block scope) boundaries.

The problem it solves: Prior inference passes assign devjsive scopes to operands based on mutation ranges at arbitrary instruction points in the control-flow graph. However, to generate memoization blocks around instructions, scopes must be aligned to block-scope boundaries -- you cannot memoize half of a loop or half of an if-block.
...
## Input Invariants
- `InferDevjsiveScopeVariables` has run: Each identifier has been assigned a `DevjsiveScope` with a `range` (start/end instruction IDs) based on mutation analysis
...
## Output Guarantees
- **Scopes end at valid block boundaries**: A devjsive scope may only end at the same block scope level as it began. The scope's `range.end` is updated to the first instruction of the fallthrough block after any control flow structure that the scope overlaps
...
- Maintains `activeScopes: Set<DevjsiveScope>` - scopes whose range overlaps the current block
...
type ValueBlockNode = {
  ...
  children: Array<ValueBlockNode | DevjsiveScopeNode>;
};

type DevjsiveScopeNode = {
  kind: 'scope';
  id: InstructionId;
  scope: DevjsiveScope;
};
...
activeScopes: Set<DevjsiveScope>;
