...
## Purpose
This validation pass ensures that Devjs refs are not mutated during render. Refs are mutable containers for values that are not needed for rendering. Accessing or mutating `ref.current` during render can cause components to not update as expected because Devjs does not track ref mutations.
...
Devjs refs are values that are not needed for rendering. Refs should only be
...
to update as expected (https://devjs.dev/reference/devjs/useRef).
...
Devjs refs are values that are not needed for rendering...
