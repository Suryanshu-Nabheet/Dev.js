...
## File
`src/DevjsiveScopes/InferDevjsiveScopeVariables.ts`

## Purpose
This is the **1st of 4 passes** that determine how to break a Devjs function into discrete devjsive scopes (independently memoizable units of code). Its specific responsibilities are:

1. **Identify operands that mutate together** - Variables that are mutated in the same instruction must be placed in the same devjsive scope
2. **Assign a unique DevjsiveScope to each group** - Each disjoint set of co-mutating identifiers gets assigned a unique ScopeId
...
- `InferDevjsivePlaces` has run - Places are marked as devjsive or not
...
## Output Guarantees
- Each identifier that is part of a mutable group has its `identifier.scope` property set to a `DevjsiveScope` object
- All identifiers in the same scope share the same `DevjsiveScope` reference
...
2. For each unique group, create a new DevjsiveScope:
...
type DevjsiveScope = {
  ...
  dependencies: Set<DevjsiveScopeDependency>;  // Inputs (populated later)
  declarations: Map<IdentifierId, DevjsiveScopeDeclaration>;  // Outputs (populated later)
...
### Fixture: `devjsive-scope-grouping.js`
...
**After InferDevjsiveScopeVariables:**
```
