...
## File
`src/HIR/BuildDevjsiveScopeTerminalsHIR.ts`

## Purpose
This pass transforms the HIR by inserting `DevjsiveScopeTerminal` nodes to explicitly demarcate the boundaries of devjsive scopes within the control flow graph. It converts the implicit scope ranges (stored on identifiers as `identifier.scope.range`) into explicit control flow structure by:

1. Inserting a `scope` terminal at the **start** of each devjsive scope
2. Inserting a `goto` terminal at the **end** of each devjsive scope
...
## Input Invariants
- **Properly nested scopes and blocks**: The pass assumes `assertValidBlockNesting` has passed, meaning all program blocks and devjsive scopes form a proper tree hierarchy
- **Aligned scope ranges**: Devjsive scope ranges have been correctly aligned and merged by previous passes
- **Valid instruction IDs**: All instructions have sequential IDs that define the scope boundaries
- **Scopes attached to identifiers**: Devjsive scopes are found by traversing all `Place` operands and collecting unique non-empty scopes

## Output Guarantees
- **Explicit scope terminals**: Each devjsive scope is represented in the CFG as a `DevjsiveScopeTerminal` with:
...
```
for each devjsive scope (in range pre-order):
...
type TerminalRewriteInfo =
  | {
      ...
      scope: DevjsiveScope;    // The scope being created
    }
  | {
      kind: 'EndScope';
      instrId: InstructionId;  // Where to insert
...
### Fixture: `devjsive-scopes-if.js`

**Before BuildDevjsiveScopeTerminalsHIR:**
```
bb0 (block):
...
**After BuildDevjsiveScopeTerminalsHIR:**
```
bb0 (block):
