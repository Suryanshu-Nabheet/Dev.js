...
## Input Invariants
- Operates on DevjsiveFunction (post-devjsive scope inference)
...
Validates that the memoized value actually ends up in a devjsive scope:
```
...
### State Management
The visitor tracks:
- `scopes: Set<ScopeId>` - All completed devjsive scopes
...
### Fixture: `error.preserve-use-memo-ref-missing-devjsive.ts`
...
Devjs Compiler has skipped optimizing this component because the existing manual
...
error.preserve-use-memo-ref-missing-devjsive.ts:9:21
