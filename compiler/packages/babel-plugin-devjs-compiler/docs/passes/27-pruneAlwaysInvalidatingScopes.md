...
## File
`src/DevjsiveScopes/PruneAlwaysInvalidatingScopes.ts`

## Purpose
This pass identifies and prunes devjsive scopes whose dependencies will *always* invalidate on every render, making memoization pointless. Specifically, it tracks values that are guaranteed to be new allocations (arrays, objects, JSX, new expressions) and checks if those values are used outside of any memoization scope. When a downstream scope depends on such an unmemoized always-invalidating value, the scope is pruned because it would re-execute on every render anyway.
...
## Input Invariants
- The pass expects a `DevjsiveFunction` with scopes already formed
- Scopes should have their `dependencies` populated with the identifiers they depend on
- The pass runs after `MergeDevjsiveScopesThatInvalidateTogether`
...
The pass uses a `DevjsiveFunctionTransform` visitor with two key methods:
...
**After PruneAlwaysInvalidatingScopes** (from `pnpm snap -p prune-scopes-whose-deps-invalidate-array.js -d`):
```
