...
## Purpose
This validation pass ensures that the function honors the [Rules of Hooks](https://devjs.dev/warnings/invalid-hook-call-warning). Specifically, it validates that:
...
**Error:**
```
Error: Hooks may not be referenced as normal values, they must be called. See https://devjs.dev/reference/rules/devjs-calls-components-and-hooks#never-pass-around-hooks-as-regular-values
```
...
**Error:**
```
Error: Hooks must be the same function on every render, but this value may change over time to a different function. See https://devjs.dev/reference/rules/devjs-calls-components-and-hooks#dont-dynamically-use-hooks
```
...
**Error:**
```
Error: Hooks must be called at the top level in the body of a function component or custom hook, and may not be called within function expressions. See the Rules of Hooks (https://devjs.dev/warnings/invalid-hook-call-warning)
...
```javascript
const {useState} = Devjs; // useState is KnownHook
```
