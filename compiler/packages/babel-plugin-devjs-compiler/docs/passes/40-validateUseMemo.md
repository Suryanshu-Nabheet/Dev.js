...
## Purpose
This validation pass ensures that `useMemo()` callbacks follow Devjs's requirements. The pass checks for several common mistakes that developers make when using `useMemo()`:
...
- `useMemo` is either imported directly or accessed via `Devjs.useMemo`
...
useMemo() callbacks are called by Devjs to cache calculations across re-renders. They should not take parameters. Instead, directly reference the props, state, or local variables needed for the computation.
```
...
const devjs = new Set<IdentifierId>();
...
- `Devjs` imports to detect `Devjs.useMemo` pattern
...
for (const instr of block.instructions) {
  switch (value.kind) {
    case 'LoadGlobal':
      ...
      } else if (value.binding.name === 'Devjs') {
        devjs.add(lvalue.identifier.id);
      }
      break;
    case 'PropertyLoad':
      if (devjs.has(value.object.identifier.id) && value.property === 'useMemo') {
        useMemos.add(lvalue.identifier.id);
      }
      break;
    case 'CallExpression':
