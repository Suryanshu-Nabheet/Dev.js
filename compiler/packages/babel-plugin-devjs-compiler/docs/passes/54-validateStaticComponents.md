...
## Purpose
Validates that components used in JSX are not created dynamically during render. Components created during render will have their state reset on every re-render because Devjs sees them as new component types each time. This is a common Devjs anti-pattern that causes bugs and poor performance.

## Input Invariants
- Operates on HIRFunction (pre-devjsive transformation)
...
**Why it fails:** The `createComponent()` call creates a new component type on every render. When this component is used in JSX, Devjs will see a different component type each time, causing the component to unmount and remount (losing all state) on every render.
