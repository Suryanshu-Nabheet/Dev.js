...
## File
`src/DevjsiveScopes/PromoteUsedTemporaries.ts`
...
2. Variables referenced across devjsive scope boundaries are named
...
## Input Invariants
- The DevjsiveFunction has undergone scope construction and dependency propagation
...
class PromoteInterposedTemporaries {
  ...
  override visitInstruction(instruction: DevjsiveInstruction, state: InterState): void {
