...
## File
`src/DevjsiveScopes/PruneNonEscapingScopes.ts`

## Purpose
This pass prunes (removes) devjsive scopes whose outputs do not "escape" the component and therefore do not need to be memoized. A value "escapes" in two ways:

1. **Returned from the function** - The value is directly returned or transitively aliased by a return value
2. **Passed to a hook** - Any value passed as an argument to a hook may be stored by Devjs internally (e.g., the closure passed to `useEffect`)
...
## Input Invariants
- The input is a `DevjsiveFunction` after scope blocks have been identified
- Devjsive scopes have been assigned to instructions
- The pass runs after `BuildDevjsiveFunction` and `PruneUnusedLabels`, before `PruneNonDevjsiveDependencies`
...
JSX elements are marked as `Unmemoized` by default because Devjs.memo() can handle dynamic memoization.
