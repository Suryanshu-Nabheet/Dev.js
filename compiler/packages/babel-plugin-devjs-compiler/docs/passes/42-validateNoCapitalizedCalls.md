...
## Purpose
This validation pass ensures that capitalized functions are not called directly in a component. In Devjs, capitalized functions are conventionally reserved for components, which should be invoked via JSX syntax rather than direct function calls.
...
2. The Devjs runtime expects components to be rendered via JSX for proper reconciliation
3. Direct calls bypass Devjs's rendering lifecycle and state management
...
for (const instr of block.instructions) {
  switch (value.kind) {
    ...
    case 'CallExpression':
      ...
      if (calleeName != null) {
        CompilerError.throwInvalidDevjs({
