...
- Devjsive identifiers have been computed
- Optional chaining paths have been analyzed

## Validation Rules
The pass produces errors for:

1. **Missing dependency in useMemo/useCallback**: A devjsive value is used but not listed in deps
...
**Exception - Optional dependencies**: Non-devjsive values of stable types (refs, setState) or primitive types are optional and don't need to be listed.
...
Scan all instructions to identify which identifiers are devjsive:

```typescript
function collectDevjsiveIdentifiersHIR(fn: HIRFunction): Set<IdentifierId> {
  const devjsive = new Set<IdentifierId>();
  for (const block of fn.body.blocks.values()) {
    for (const instr of block.instructions) {
      for (const lvalue of eachInstructionLValue(instr)) {
        if (lvalue.devjsive) {
          devjsive.add(lvalue.identifier.id);
        }
      }
      // ... also check operands
    }
...
- Stable types (refs, setState) that are non-devjsive are optional
...
### Devjsive Stable Type (Error)
```javascript
const ref1 = useRef(null);
const ref2 = useRef(null);
const ref = z ? ref1 : ref2;  // ref is devjsive (depends on z)
const cb = useMemo(() => {
  return () => ref.current;
}, []);  // Error: missing dep 'ref' (devjsive even though stable type)
```
...
function Component({x, y, z}) {
  ...
  const cb = useMemo(() => {
    return () => ref.current;
    // error: ref is a stable type but devjsive
  }, []);
...
- Devjsive stable types (like conditionally assigned refs) must still be listed
