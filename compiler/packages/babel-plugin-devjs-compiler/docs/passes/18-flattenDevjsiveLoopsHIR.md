# flattenDevjsiveLoopsHIR

## File
`src/DevjsiveScopes/FlattenDevjsiveLoopsHIR.ts`

## Purpose
This pass **prunes devjsive scopes that are nested inside loops** (for, for-in, for-of, while, do-while). The compiler does not yet support memoization within loops because:
...
When a devjsive scope is found inside a loop body, the pass converts its terminal from `scope` to `pruned-scope`. A `pruned-scope` terminal is later treated specially during codegen - its instructions are emitted inline without any memoization guards.

## Input Invariants
- The HIR has been through `buildDevjsiveScopeTerminalsHIR`, which creates `scope` terminal nodes for devjsive scopes
...
**Before FlattenDevjsiveLoopsHIR:**
```
...
function useHook(nodeID, condition) {
  ...
  if ($[2] !== condition || $[3] !== node || $[4] !== t0) {
    ...
    t1 = Symbol.for("devjs.early_return_sentinel");
  ...
  if ($[6] === Symbol.for("devjs.memo_cache_sentinel")) {
    t2 = new Class();
    $[6] = t2;
  }
  return t2;
