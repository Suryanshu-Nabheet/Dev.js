...
2. Each `DevjsiveScope.declarations` contains identifiers that:
...
2. **collectTemporariesSidemap**: Creates a mapping from temporary IdentifierIds to their source `DevjsiveScopeDependency`. For example:
   ```
   $0 = LoadLocal 'a'
   $1 = PropertyLoad $0.'b'
   ```
...
For each scope, use `DevjsiveScopeDependencyTreeHIR` to:
...
```typescript
type DevjsiveScopeDependency = {
  identifier: Identifier;        // Root identifier
  devjsive: boolean;             // Whether the value is devjsive
...
### Fixture: `devjsive-control-dependency-if.js`
...
- Even though `x` is assigned to constants (1 or 2), its value depends on the devjsive control flow condition `props.cond`
