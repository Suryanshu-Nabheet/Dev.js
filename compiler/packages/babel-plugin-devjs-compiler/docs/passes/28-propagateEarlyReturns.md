...
## File
`src/DevjsiveScopes/PropagateEarlyReturns.ts`

## Purpose
The `propagateEarlyReturns` pass ensures that devjsive scopes (memoization blocks) correctly honor the control flow behavior of the original code, particularly when a function returns early from within a devjsive scope. Without this transformation, if a component returned early on the previous render and the inputs have not changed, the cached memoization block would be skipped entirely, but the early return would not occur, causing incorrect behavior.

The pass solves this by transforming `return` statements inside devjsive scopes into assignments to a temporary variable followed by a labeled `break`. After the devjsive scope completes, generated code checks whether the early return sentinel value was replaced with an actual return value; if so, the function returns that value.

## Input Invariants
1. **DevjsiveFunction structure**: The input must be a `DevjsiveFunction` with scopes already inferred (devjsive scope blocks are already established)
2. **Scope earlyReturnValue not set**: The pass expects `scopeBlock.scope.earlyReturnValue === null` for scopes it processes
3. **Return statements within devjsive scopes**: The pass specifically targets `return` terminal statements that appear within a `withinDevjsiveScope` context

## Output Guarantees
1. **Labeled scope blocks**: Top-level devjsive scopes containing early returns are wrapped in a labeled block (e.g., `bb14: { ... }`)
2. **Sentinel initialization**: At the start of each such scope, a temporary variable is initialized to `Symbol.for("devjs.early_return_sentinel")`
...
The pass uses a visitor pattern with a `DevjsiveFunctionTransform` that tracks two pieces of state:

```typescript
type State = {
  withinDevjsiveScope: boolean;  // Are we inside a devjsive scope?
  earlyReturnValue: DevjsiveScope['earlyReturnValue'];  // Bubble up early return info
};
```

### Key Steps:

1. **visitScope** - When entering a devjsive scope:
   - Create an inner state with `withinDevjsiveScope: true`
   - Traverse the scope's contents
   - If any early returns were found (`earlyReturnValue !== null`):
     - If this is the **outermost** scope (parent's `withinDevjsiveScope` is false):
...
2. **transformTerminal** - When encountering a `return` inside a devjsive scope:
...
let t0 = Symbol.for("devjs.early_return_sentinel");
```

## Edge Cases

### Nested Devjsive Scopes
...
### Returns Outside Devjsive Scopes
The pass only transforms returns where `state.withinDevjsiveScope === true`. Returns outside scopes are left unchanged.
...
### Fixture: `early-return-within-devjsive-scope.js`
...
**After PropagateEarlyReturns** (from `pnpm snap -p early-return-within-devjsive-scope.js -d`):
```
scope @0 [...] earlyReturn={id: #t34$34, label: 14} {
  [0] $36 = LoadGlobal(global) Symbol
  [0] $37 = PropertyLoad $36.for
  [0] $38 = "devjs.early_return_sentinel"
  [0] $35 = MethodCall $36.$37($38)
  [0] StoreLocal Let #t34$34{devjsive} = $35    // Initialize sentinel
  bb14: {
    [2] $19_@0 = Array []
    [3] StoreLocal Const x$20_@0 = $19_@0
    [4] $22{devjsive} = LoadLocal props$18
    [5] $23{devjsive} = PropertyLoad $22.cond
    [6] if ($23) {
...
function Component(props) {
  ...
  if ($[0] !== props.a || $[1] !== props.b || $[2] !== props.cond) {
    t0 = Symbol.for("devjs.early_return_sentinel");
    bb0: {
      const x = [];
      if (props.cond) {
        x.push(props.a);
  ...
  if (t0 !== Symbol.for("devjs.early_return_sentinel")) {
    return t0;
  }
}
```
