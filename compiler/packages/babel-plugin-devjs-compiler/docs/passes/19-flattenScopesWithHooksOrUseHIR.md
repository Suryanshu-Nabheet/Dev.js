...
## File
`src/DevjsiveScopes/FlattenScopesWithHooksOrUseHIR.ts`

## Purpose
This pass removes (flattens) devjsive scopes that transitively contain hook calls or `use()` operator calls. The key insight is that:
...
2. **The `use()` operator** - while it can be called conditionally in source code, Devjs requires it to be called consistently if the component needs the returned value. Memoizing a scope containing `use()` would also make it conditionally called.

By running devjsive scope inference first (agnostic of hooks), the compiler knows which values "construct together" in the same scope. The pass then removes ALL memoization for scopes containing hook/use calls to ensure they are always executed unconditionally.

## Input Invariants
- HIR must have devjsive scope terminals already built (pass runs after `BuildDevjsiveScopeTerminalsHIR`)
...
1. Maintain a stack `activeScopes` of currently "open" devjsive scopes
...
DevjsiveScopeTerminal: {kind: 'scope', block, fallthrough, scope, id, loc}
```
