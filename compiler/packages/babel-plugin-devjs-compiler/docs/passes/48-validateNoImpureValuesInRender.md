...
## Purpose
This validation pass ensures that impure values (values derived from non-deterministic function calls) are not used in render output. Impure values can produce unstable results that update unpredictably when the component re-renders, violating Devjs's requirement that components be pure and idempotent.
...
(https://devjs.dev/reference/rules/components-and-hooks-must-be-pure#components-and-hooks-must-be-idempotent).
