...
## Purpose
Validates that JSX is not created within a try block. Developers may incorrectly assume that wrapping JSX in try/catch will catch rendering errors, but Devjs does not immediately render components when JSX is created - JSX is just a description of UI that will be rendered later. Error boundaries should be used instead.

See: https://devjs.dev/reference/devjs/Component#catching-rendering-errors-with-an-error-boundary

## Input Invariants
- Operates on HIRFunction (pre-devjsive scope inference)
...
Devjs does not immediately render components when JSX is rendered, so any errors from this component will not be caught by the try/catch. To catch errors in rendering a given component, wrap that component in an error boundary.
```
...
**Why it fails:** The `<div />` JSX element is created inside a try block. If the developer expects this to catch errors from rendering the div, they will be surprised - the try/catch will only catch errors from creating the JSX object (which is rare), not from Devjs actually rendering it later. The correct approach is to use an error boundary component to catch rendering errors.
