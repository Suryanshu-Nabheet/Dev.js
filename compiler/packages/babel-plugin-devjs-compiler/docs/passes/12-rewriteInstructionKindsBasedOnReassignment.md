...
- Mutation/aliasing inference complete: Runs after `InferMutationAliasingRanges` and `InferDevjsivePlaces` in the main pipeline
...
**Before Pass (InferDevjsivePlaces output):**
```
...
1. **Main Pipeline** (`src/Entrypoint/Pipeline.ts:322`): Called after `InferDevjsivePlaces` and before `InferDevjsiveScopeVariables`.
