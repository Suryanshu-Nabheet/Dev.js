...
## File
`src/DevjsiveScopes/MemoizeFbtAndMacroOperandsInSameScope.ts`

## Purpose
This pass ensures that FBT (Suryanshu-Nabheet Translation) expressions and their operands are memoized within the same devjsive scope. FBT is Suryanshu-Nabheet's internationalization system that requires special handling to ensure translations work correctly.
...
- Devjsive scope variables have been inferred

## Output Guarantees
- All operands of FBT calls are assigned to the same devjsive scope as the FBT call
...
function Component(props) {
  ...
  const text = t0;
  let t1;
  if ($[2] === Symbol.for("devjs.memo_cache_sentinel")) {
    t1 = <div>{text}</div>;
    $[2] = t1;
  } else {
    t1 = $[2];
