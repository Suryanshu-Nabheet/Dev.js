...
- The pass is called after:
  ...
  - `pruneNonDevjsiveDependencies` - removes non-devjsive dependencies from scopes
...
The pass uses the visitor pattern with `DevjsiveFunctionTransform`:
...
The pass operates on `DevjsiveScopeBlock` (kind: 'scope'), not `PrunedDevjsiveScopeBlock`. Scopes already pruned by earlier passes are not revisited.

### Interaction with Subsequent Passes
The `MergeDevjsiveScopesThatInvalidateTogether` pass explicitly handles pruned scopes - it does not merge across them.
...
3. `pruneNonDevjsiveDependencies` - Removes non-devjsive dependencies
