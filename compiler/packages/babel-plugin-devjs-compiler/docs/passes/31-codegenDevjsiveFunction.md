# codegenDevjsiveFunction

## File
`src/DevjsiveScopes/CodegenDevjsiveFunction.ts`

## Purpose
This is the final pass that converts the DevjsiveFunction representation back into a Babel AST. It generates the memoization code that makes Devjs components and hooks efficient by:
...
## Input Invariants
- The DevjsiveFunction has been through all prior passes
- All identifiers that need names have been promoted and renamed
- Devjsive scopes have finalized `dependencies`, `declarations`, and `reassignments`
...
- All devjsive scopes become if-else blocks checking dependencies
...
```typescript
export function codegenFunction(fn: DevjsiveFunction): Result<CodegenFunction, CompilerError> {
  ...
  const compiled = codegenDevjsiveFunction(cx, fn);
...
### codegenDevjsiveScope
The core of memoization code generation:

```typescript
function codegenDevjsiveScope(cx: Context, statements: Array<t.Statement>,
                              scope: DevjsiveScope, block: DevjsiveBlock): void {
  ...
  if (testCondition === null) {
    testCondition = t.binaryExpression('===',
      $[firstOutputIndex],
      t.callExpression(Symbol.for, ['devjs.memo_cache_sentinel'])
    );
  }
...
if ($[0] !== "source_hash_abc123") {
  for (let $i = 0; $i < cacheCount; $i++) {
    $[$i] = Symbol.for("devjs.memo_cache_sentinel");
  }
...
type CodegenFunction = {
  ...
  memoBlocks: number;        // Number of devjsive scopes
