...
### Visitor Pattern
```typescript
class Visitor extends DevjsiveFunctionVisitor<Scopes> {
  override visitBlock(block: DevjsiveBlock, state: Scopes): void {
    state.enter(() => {
      this.traverseBlock(block, state);
    });
  }

  override visitScope(scope: DevjsiveScopeBlock, state: Scopes): void {
