...
## File
`src/DevjsiveScopes/PruneUnusedLabels.ts`
...
## Input Invariants
- The input is a `DevjsiveFunction` (after conversion from HIR)
...
- Each `DevjsiveTerminalStatement` has an optional `label` field containing `id` and `implicit`
