...
## File
`src/DevjsiveScopes/AlignMethodCallScopes.ts`
...
1. Both the MethodCall lvalue and the property have the **same** devjsive scope
2. **Neither** has a devjsive scope
...
- `inferDevjsiveScopeVariables` has already run, assigning initial devjsive scopes to identifiers based on mutation analysis
- Each instruction's lvalue has an `identifier.scope` that is either a `DevjsiveScope` or `null`
