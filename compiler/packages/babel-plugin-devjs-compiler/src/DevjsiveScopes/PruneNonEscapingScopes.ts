...
import {getPlaceScope, DevjsiveScope} from '../HIR/HIR';
import {
...
/*
 * This pass prunes devjsive scopes that are not necessary to bound downstream computation.
 ...
 *    have its referenced ultimately stored by Devjs (ie, be aliased by an external value).
 ...
 *    memoization with `Devjs.memo()`. Static memoization may be JIT'd and can look at
 *    the precise props w/o dynamic iteration, but incurs potentially large code-size
 *    overhead. Dynamic memoization with `Devjs.memo()` incurs potentially increased
 ...
 */
export function pruneNonEscapingScopes(fn: DevjsiveFunction): void {
  /*
  ...
  visitDevjsiveFunction(fn, new CollectDependenciesVisitor(fn.env, state), []);
  ...
  visitDevjsiveFunction(fn, new PruneScopesTransform(), memoized);
}
 ...
 */
class CollectDependenciesVisitor extends DevjsiveFunctionVisitor<
  Array<DevjsiveScope>
> {
  env: Environment;
  state: State;
    ...
    switch (value.kind) {
      ...
      case 'UnaryExpression': {
        if (options.forceMemoizePrimitives) {
          ...
          return {
            lvalues: lvalue !== null ? [{place: lvalue, level}] : [],
            rvalues: [...eachDevjsiveValueOperand(value)],
          };
        }
      ...
      case 'TaggedTemplateExpression': {
        ...
        const operands = [...eachDevjsiveValueOperand(value)];
        lvalues.push(
          ...operands
  ...
  visitValueForMemoization(
    ...
    if (value.kind === 'LoadLocal' && lvalue !== null) {
      ...
      if (getHookKind(state.env, callee.identifier) != null) {
         ...
         * to devjsive scopes in the hook. However if the hook is annotated as
  ...
  override visitTerminal(
    stmt: DevjsiveTerminalStatement<DevjsiveTerminal>,
    scopes: Array<DevjsiveScope>,
  ): void {
...
// Prune devjsive scopes that do not have any memoized outputs
class PruneScopesTransform extends DevjsiveFunctionTransform<
  Set<DeclarationId>
> {
