 ...
 */
class PromoteTemporaries extends DevjsiveFunctionVisitor<State> {
  override visitScope(scopeBlock: DevjsiveScopeBlock, state: State): void {
    ...
    /*
     * This is technically optional. We could prune DevjsiveScopes
 ...
 */
class PromoteAllInstancedOfPromotedTemporaries extends DevjsiveFunctionVisitor<State> {
  ...
  traverseScopeIdentifiers(scope: DevjsiveScope, state: State): void {
  ...
  override visitScope(scope: DevjsiveScopeBlock, state: State): void {
 ...
 */
class CollectPromotableTemporaries extends DevjsiveFunctionVisitor<State> {
...
class PromoteInterposedTemporaries extends DevjsiveFunctionVisitor<InterState> {
  #promotable: State;
...
export function promoteUsedTemporaries(fn: DevjsiveFunction): void {
  const state: State = {
    tags: new Set(),
    promoted: new Set(),
    pruned: new Map(),
  };
  visitDevjsiveFunction(fn, new CollectPromotableTemporaries(), state);
  for (const operand of fn.params) {
    const place = operand.kind === 'Identifier' ? operand : operand.place;
    if (place.identifier.name === null) {
      promoteIdentifier(place.identifier, state);
  ...
  visitDevjsiveFunction(fn, new PromoteTemporaries(), state);
