 ...
 * 1. InferDevjsiveScopeVariables (this pass, on HIR) determines operands that mutate
 *     together and assigns them a unique devjsive scope.
 * 2. AlignDevjsiveScopesToBlockScopes (on DevjsiveFunction) aligns devjsive scopes
 *     to block scopes.
 * 3. MergeOverlappingDevjsiveScopes (on DevjsiveFunction) ensures that devjsive
 ...
 *
 * For each mutable variable, infers a devjsive scope which will construct that
 * variable. Variables that co-mutate are assigned to the same devjsive scope.
 ...
 * ```
 */
export function inferDevjsiveScopeVariables(fn: HIRFunction): void {
  /*
   * Represents the set of devjsive scopes as disjoint sets of identifiers
  ...
  const scopes: Map<Identifier, DevjsiveScope> = new Map();
  ...
  for (const [, scope] of scopes) {
    if (
      ...
      fn.env.logger?.debugLogIRs?.({
        kind: 'hir',
        name: 'InferDevjsiveScopeVariables (invalid scope)',
        value: fn,
      });
