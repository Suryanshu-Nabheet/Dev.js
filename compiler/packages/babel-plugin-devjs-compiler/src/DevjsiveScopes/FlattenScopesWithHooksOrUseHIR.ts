 ...
 * - They can introduce devjsivity even when their arguments are non-devjsive (accounted for in InferDevjsivePlaces)
 * - They cannot be called conditionally
 *
 * The `use` operator is similar:
 * - It can access context, and therefore introduce devjsivity
 * - It can be called conditionally, but _it must be called if the component needs the return value_. This is because
 *   Devjs uses the fact that use was called to remember that the component needs the value, and that changes to the
 ...
 * the devjsive scope inference first, agnostic of hooks, we know that the devjsive scopes accurately
