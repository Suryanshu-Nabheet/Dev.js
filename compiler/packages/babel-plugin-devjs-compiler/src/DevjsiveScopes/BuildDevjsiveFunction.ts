...
/*
 * Converts from HIR (lower-level CFG) to DevjsiveFunction, a tree representation
 ...
 */
export function buildDevjsiveFunction(fn: HIRFunction): DevjsiveFunction {
...
class Driver {
  ...
  wrapWithSequence(
    ...
    loc: SourceLocation,
  ): {block: BlockId; value: DevjsiveValue; place: Place; id: InstructionId} {
    if (instructions.length === 0) {
      return continuation;
    }
    const sequence: DevjsiveSequenceValue = {
   ...
   * value blocks as compound values in DevjsiveFunction (no phis). However,
  ...
  extractValueBlockResult(
    ...
    blockId: BlockId,
    loc: SourceLocation,
  ): {block: BlockId; place: Place; value: DevjsiveValue; id: InstructionId} {
    ...
    let value: DevjsiveValue = instr.value;
    if (
  ...
  valueBlockResultToSequence(
    ...
    const instructions: Array<DevjsiveInstruction> = [];
    let innerValue: DevjsiveValue = result.value;
  ...
  traverseBlock(block: BasicBlock): DevjsiveBlock {
    const blockValue: DevjsiveBlock = [];
    this.visitBlock(block, blockValue);
    return blockValue;
  }

  visitBlock(block: BasicBlock, blockValue: DevjsiveBlock): void {
    ...
    switch (terminal.kind) {
      ...
      case 'if': {
        ...
        let consequent: DevjsiveBlock | null = null;
        ...
        let alternate: DevjsiveBlock | null = null;
        if (alternateId !== null) {
      ...
      case 'maybe-throw': {
        /*
         * DevjsiveFunction does not explicit model maybe-throw semantics,
  ...
  visitValueBlock(
    ...
    if (block.terminal.kind === 'branch') {
      ...
      /*
       * DevjsiveFunction does not explicitly model maybe-throw semantics,
  ...
  visitValueBlockTerminal(terminal: Terminal): {
    ...
    switch (terminal.kind) {
      ...
      case 'optional': {
        ...
        const call: DevjsiveSequenceValue = {
          kind: 'SequenceExpression',
          instructions: [
            {
              id: test.id,
      ...
      case 'logical': {
        ...
        const left: DevjsiveSequenceValue = {
          kind: 'SequenceExpression',
          instructions: [
            {
              id: test.id,
        ...
        const value: DevjsiveLogicalValue = {
      ...
      case 'ternary': {
        ...
        const value: DevjsiveTernaryValue = {
  ...
  visitBreak(
    ...
    id: InstructionId,
    loc: SourceLocation,
  ): DevjsiveTerminalStatement<DevjsiveBreakTerminal> | null {
    ...
    if (this.cx.scopeFallthroughs.has(target.block)) {
      CompilerError.invariant(target.type === 'implicit', {
        reason: 'Expected devjsive scope to implicitly break to fallthrough',
        loc,
      });
      return null;
    }
  ...
  visitContinue(
    ...
    id: InstructionId,
    loc: SourceLocation,
  ): DevjsiveTerminalStatement<DevjsiveContinueTerminal> {
...
class Context {
  ...
  getContinueTarget(
    block: BlockId,
  ): {block: BlockId; type: DevjsiveTerminalTargetKind} | null {
    let hasPrecedingLoop = false;
    for (let i = this.#controlFlowStack.length - 1; i >= 0; i--) {
      const target = this.#controlFlowStack[i]!;
