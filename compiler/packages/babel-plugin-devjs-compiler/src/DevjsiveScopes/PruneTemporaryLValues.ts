 ...
 */
export function pruneUnusedLValues(fn: DevjsiveFunction): void {
  const lvalues = new Map<DeclarationId, DevjsiveInstruction>();
  visitDevjsiveFunction(fn, new Visitor(), lvalues);
 ...
 * (ternary, logical, optional) in DevjsiveFunction may not be the same as the IdentifierId
 ...
 */
type LValues = Map<DeclarationId, DevjsiveInstruction>;

class Visitor extends DevjsiveFunctionVisitor<LValues> {
