...
import {printDevjsiveScopeSummary} from './PrintDevjsiveFunction';
import {
  DevjsiveFunctionTransform,
  DevjsiveFunctionVisitor,
  Transformed,
 ...
 * scopes that are in the same (devjsive) block, ie we don't merge across
 ...
 * better to flatten.
 */
export function mergeDevjsiveScopesThatInvalidateTogether(
  fn: DevjsiveFunction,
): void {
  const lastUsageVisitor = new FindLastUsageVisitor();
  visitDevjsiveFunction(fn, lastUsageVisitor, undefined);
  visitDevjsiveFunction(fn, new Transform(lastUsageVisitor.lastUsage), null);
}
...
class Transform extends DevjsiveFunctionTransform<DevjsiveScopeDependencies | null> {
  ...
  override visitBlock(
    ...
    if (merged.length) {
      ...
      for (const entry of merged) {
        log(
          printDevjsiveScopeSummary(entry.block.scope) +
...
function canMergeScopes(
  ...
  log(
    `  ${printDevjsiveScopeSummary(current.scope)} ${[...current.scope.declarations.values()].map(decl => decl.identifier.declarationId)}`,
  );
  log(
    `  ${printDevjsiveScopeSummary(next.scope)} ${[...next.scope.dependencies].map(dep => `${dep.identifier.declarationId} ${temporaries.get(dep.identifier.declarationId) ?? dep.identifier.declarationId}`)}`,
  );
  return false;
}
...
function areEqualDependencies(
  a: Set<DevjsiveScopeDependency>,
  b: Set<DevjsiveScopeDependency>,
): boolean {
  if (a.size !== b.size) {
    return false;
  }
 ...
 */
function scopeIsEligibleForMerging(scopeBlock: DevjsiveScopeBlock): boolean {
