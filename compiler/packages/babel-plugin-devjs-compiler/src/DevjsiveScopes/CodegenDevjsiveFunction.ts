...
import {buildDevjsiveFunction} from './BuildDevjsiveFunction';
import {SINGLE_CHILD_FBT_TAGS} from './MemoizeFbtAndMacroOperandsInSameScope';
import {DevjsiveFunctionVisitor, visitDevjsiveFunction} from './visitors';
import {DevjsFunctionType} from '../HIR/Environment';
import {ProgramContext} from '../Entrypoint';

export const MEMO_CACHE_SENTINEL = 'devjs.memo_cache_sentinel';
export const EARLY_RETURN_SENTINEL = 'devjs.early_return_sentinel';
...
export type CodegenFunction = {
  ...
  /*
   * Number of memo *blocks* (devjsive scopes) regardless of
   * how many inputs/outputs each block has
   */
  memoBlocks: number;

  /**
   * Number of memoized values across all devjsive scopes
   */
  memoValues: number;

  /**
   * The number of devjsive scopes that were created but had to be discarded
...
export function codegenFunction(
  ...
  const compileResult = codegenDevjsiveFunction(cx, fn);
  ...
  for (const {fn: outlinedFunction, type} of cx.env.getOutlinedFunctions()) {
    const devjsiveFunction = buildDevjsiveFunction(outlinedFunction);
    pruneUnusedLabels(devjsiveFunction);
    pruneUnusedLValues(devjsiveFunction);
    pruneHoistedContexts(devjsiveFunction);

    const identifiers = renameVariables(devjsiveFunction);
    const codegen = codegenDevjsiveFunction(
      new Context(
        cx.env,
        devjsiveFunction.id ?? '[[ anonymous ]]',
        identifiers,
        cx.fbtOperands,
      ),
      devjsiveFunction,
...
function codegenDevjsiveFunction(
  cx: Context,
  fn: DevjsiveFunction,
): Result<CodegenFunction, CompilerError> {
  ...
  visitDevjsiveFunction(fn, countMemoBlockVisitor, undefined);
...
class CountMemoBlockVisitor extends DevjsiveFunctionVisitor<void> {
  env: Environment;
  memoBlocks: number = 0;
  memoValues: number = 0;
  ...
  override visitScope(scopeBlock: DevjsiveScopeBlock, state: void): void {
    this.memoBlocks += 1;
...
function codegenBlock(cx: Context, block: DevjsiveBlock): t.BlockStatement {
...
function codegenBlockNoReset(
  ...
  for (const item of block) {
    switch (item.kind) {
      ...
      case 'scope': {
        const temp = new Map(cx.temp);
        codegenDevjsiveScope(cx, statements, item.scope, item.instructions);
        cx.temp = temp;
        break;
      }
      case 'terminal': {
...
function codegenDependency(
  cx: Context,
  dependency: DevjsiveScopeDependency,
): t.Expression {
...
function codegenInstructionValue(
  ...
  switch (instrValue.kind) {
        ...
        if (property.kind === 'ObjectProperty') {
          ...
          switch (property.type) {
            ...
            case 'method': {
              ...
              const devjsiveFunction = buildDevjsiveFunction(loweredFunc.func);
              pruneUnusedLabels(devjsiveFunction);
              pruneUnusedLValues(devjsiveFunction);
              const fn = codegenDevjsiveFunction(
                new Context(
                  cx.env,
                  devjsiveFunction.id ?? '[[ anonymous ]]',
    ...
    case 'FunctionExpression': {
      ...
      const devjsiveFunction = buildDevjsiveFunction(loweredFunc);
      pruneUnusedLabels(devjsiveFunction);
      pruneUnusedLValues(devjsiveFunction);
      pruneHoistedContexts(devjsiveFunction);
      const fn = codegenDevjsiveFunction(
      ...
      const expressions = body.map(stmt => {
        if (stmt.type === 'ExpressionStatement') {
          ...
          if (t.isVariableDeclaration(stmt)) {
            ...
            cx.errors.push({
              reason: `(CodegenDevjsiveFunction::codegenInstructionValue) Cannot declare variables in a value block, tried to declare '${
            ...
            cx.errors.push({
              reason: `(CodegenDevjsiveFunction::codegenInstructionValue) Handle conversion of ${stmt.type} to expression`,
              category: ErrorCategory.Todo,
              loc: stmt.loc ?? null,
              suggestions: null,
            });
    ...
    case 'JsxAttribute': {
      ...
      switch (innerValue.type) {
        ...
        default: {
           ...
           * spec: https://github.com/Suryanshu-Nabheet/jsx/blob/main/AST.md#jsx-attributes
           */
...
function compareScopeDependency(
  a: DevjsiveScopeDependency,
  b: DevjsiveScopeDependency,
): number {
  CompilerError.invariant(
...
function compareScopeDeclaration(
  a: DevjsiveScopeDeclaration,
  b: DevjsiveScopeDeclaration,
): number {
  CompilerError.invariant(
    a.identifier.name?.kind === 'named' && b.identifier.name?.kind === 'named',
    {
