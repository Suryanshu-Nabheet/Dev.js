 ...
 * - declared
 *
 * This is because Devjs Compiler converts a `function foo()` function declaration to
 * 1. a `let foo;` declaration before devjsive memo blocks
 ...
 */
export function pruneHoistedContexts(fn: DevjsiveFunction): void {
  visitDevjsiveFunction(fn, new Visitor(), {
    activeScopes: empty(),
...
class Visitor extends DevjsiveFunctionTransform<VisitorState> {
  override visitScope(scope: DevjsiveScopeBlock, state: VisitorState): void {
