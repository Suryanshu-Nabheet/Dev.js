 ...
 * devjsive scopes (independently memoizeable units of code):
 * 1. InferDevjsiveScopeVariables (on HIR) determines operands that mutate together and assigns
 *     them a unique devjsive scope.
 * 2. AlignDevjsiveScopesToBlockScopes (this pass, on DevjsiveFunction) aligns devjsive scopes
 *     to block scopes.
 * 3. MergeOverlappingDevjsiveScopes (on DevjsiveFunction) ensures that devjsive scopes do not
 *     overlap, merging any such scopes.
 * 4. BuildDevjsiveBlocks (on DevjsiveFunction) groups the statements for each scope into
 *     a DevjsiveScopeBlock.
 *
 * Prior inference passes assign a devjsive scope to each operand, but the ranges of these
 ...
 *
 * This pass updates devjsive scope boundaries to align to control flow boundaries, for
 * example:
 *
 * ```javascript
 * function foo(cond, a) {
 ...
 *
 * The more general rule is that a devjsive scope may only end at the same block scope as it
 ...
 */
export function alignDevjsiveScopesToBlockScopesHIR(fn: HIRFunction): void {
  const activeBlockFallthroughRanges: Array<{
    range: InstructionRange;
    fallthrough: BlockId;
  }> = [];
  const activeScopes = new Set<DevjsiveScope>();
  const seen = new Set<DevjsiveScope>();
  const valueBlockNodes = new Map<BlockId, ValueBlockNode>();
  const placeScopes = new Map<Place, DevjsiveScope>();
  ...
  for (const [, block] of fn.body.blocks) {
    ...
    mapTerminalSuccessors(terminal, successor => {
      ...
      if (successorBlock.kind === 'block' || successorBlock.kind === 'catch') {
         ...
         *
         * For compatibility with the previous DevjsiveFunction-based scope merging logic,
...
function _printNode(
  node: ValueBlockNode | DevjsiveScopeNode,
  out: Array<string>,
  depth: number = 0,
): void {
