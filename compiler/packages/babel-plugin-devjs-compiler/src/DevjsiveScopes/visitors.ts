...
export function visitDevjsiveFunction<TState>(
  fn: DevjsiveFunction,
  visitor: DevjsiveFunctionVisitor<TState>,
  state: TState,
): void {
  visitor.visitBlock(fn.body, state);
}

export class DevjsiveFunctionVisitor<TState = void> {
  ...
  visitDevjsiveFunctionValue(
    _id: InstructionId,
    _dependencies: Array<Place>,
    _fn: DevjsiveFunction,
    _state: TState,
  ): void {}

  visitValue(id: InstructionId, value: DevjsiveValue, state: TState): void {
    this.traverseValue(id, value, state);
  }
  traverseValue(id: InstructionId, value: DevjsiveValue, state: TState): void {
    switch (value.kind) {
  ...
  visitInstruction(instruction: DevjsiveInstruction, state: TState): void {
    this.traverseInstruction(instruction, state);
  }
  traverseInstruction(instruction: DevjsiveInstruction, state: TState): void {
  ...
  visitTerminal(stmt: DevjsiveTerminalStatement, state: TState): void {
    this.traverseTerminal(stmt, state);
  }
  traverseTerminal(stmt: DevjsiveTerminalStatement, state: TState): void {
    const {terminal} = stmt;
    if (terminal.id !== null) {
  ...
  visitScope(scope: DevjsiveScopeBlock, state: TState): void {
    this.traverseScope(scope, state);
  }
  traverseScope(scope: DevjsiveScopeBlock, state: TState): void {
    this.visitBlock(scope.instructions, state);
  }

  visitPrunedScope(scopeBlock: PrunedDevjsiveScopeBlock, state: TState): void {
  ...
  visitBlock(block: DevjsiveBlock, state: TState): void {
    this.traverseBlock(block, state);
  }
  traverseBlock(block: DevjsiveBlock, state: TState): void {
...
export type TransformedValue =
  | {kind: 'keep'}
  | {kind: 'replace'; value: DevjsiveValue};
...
export class DevjsiveFunctionTransform<
  TState = void,
> extends DevjsiveFunctionVisitor<TState> {
  override traverseBlock(block: DevjsiveBlock, state: TState): void {
    let nextBlock: DevjsiveBlock | null = null;
    for (let i = 0; i < block.length; i++) {
      const instr = block[i]!;
      let transformed: Transformed<DevjsiveStatement>;
      switch (instr.kind) {
        case 'instruction': {
  ...
  transformPrunedScope(
    scope: PrunedDevjsiveScopeBlock,
    state: TState,
  ): Transformed<DevjsiveStatement> {
  ...
  transformDevjsiveFunctionValue(
    id: InstructionId,
    dependencies: Array<Place>,
    fn: DevjsiveFunction,
    state: TState,
  ): {kind: 'keep'} | {kind: 'replace'; value: DevjsiveFunction} {
    this.visitDevjsiveFunctionValue(id, dependencies, fn, state);
    return {kind: 'keep'};
  }
...
export function* eachDevjsiveValueOperand(
  instrValue: DevjsiveValue,
): Iterable<Place> {
  switch (instrValue.kind) {
    case 'OptionalExpression': {
      yield* eachDevjsiveValueOperand(instrValue.value);
      break;
    }
    case 'LogicalExpression': {
      yield* eachDevjsiveValueOperand(instrValue.left);
      yield* eachDevjsiveValueOperand(instrValue.right);
      break;
    }
    case 'SequenceExpression': {
      for (const instr of instrValue.instructions) {
        yield* eachDevjsiveValueOperand(instr.value);
      }
      yield* eachDevjsiveValueOperand(instrValue.value);
      break;
    }
    case 'ConditionalExpression': {
      yield* eachDevjsiveValueOperand(instrValue.test);
      yield* eachDevjsiveValueOperand(instrValue.consequent);
      yield* eachDevjsiveValueOperand(instrValue.alternate);
      break;
    }
    default: {
...
export function mapTerminalBlocks(
  ...
  fn: (block: DevjsiveBlock) => DevjsiveBlock,
): void {
  switch (terminal.kind) {
    case 'break':
    case 'continue':
