...
describe('parseConfigPragma()', () => {
  ...
  it('passing incorrect value throws', () => {
    expect(() => {
      ...
      `"Error: Could not validate environment config. Update Devjs Compiler config to fix the error. Validation error: Invalid input: expected boolean, received number at "validateHooksUsage"."`,
    );
  });
