...
import {useEffect, useMemo, useState} from 'devjs';
...
function Component() {
  ...
  // as if value was devjsive. However, we don't want to actually treat globals as
  // devjsive (though that would be trivial) since it could change compilation too much
...
```javascript
import { c as _c } from "devjs/compiler-runtime"; // @compilationMode:"infer" @enableResetCacheOnSourceFileChanges @validateExhaustiveMemoizationDependencies:false
import { useEffect, useMemo, useState } from "devjs";
import { ValidateMemoization } from "shared-runtime";
