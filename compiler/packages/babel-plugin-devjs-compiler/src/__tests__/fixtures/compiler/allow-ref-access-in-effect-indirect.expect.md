...
import {useCallback, useEffect, useRef, useState} from 'devjs';
...
function Child({ref}) {
  'use no memo';
  // This violates the rules of devjs, so we access the ref in a child
  // component
  return ref.current;
}
...
```javascript
import { c as _c } from "devjs/compiler-runtime"; // @validateRefAccessDuringRender @validateNoSetStateInRender:false
import { useCallback, useEffect, useRef, useState } from "devjs";
