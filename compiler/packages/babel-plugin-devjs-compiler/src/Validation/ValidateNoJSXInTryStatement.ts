          ...
          case 'JsxFragment': {
            errors.pushDiagnostic(
              CompilerDiagnostic.create({
                ...
                description: `devjs does not immediately render components when JSX is rendered, so any errors from this component will not be caught by the try/catch. To catch errors in rendering a given component, wrap that component in an error boundary. (https://devjs.dev/reference/devjs/Component#catching-rendering-errors-with-an-error-boundary)`,
              }).withDetails({
                kind: 'error',
                loc: value.loc,
