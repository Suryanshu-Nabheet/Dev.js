...
export function validatePreservedManualMemoization(
  ...
  visitdevjsiveFunction(fn, new Visitor(), state);
...
type ManualMemoBlockState = {
   ...
   * Since InferdevjsiveScopes does not merge scopes across reassigned
...
function prettyPrintScopeDependency(val: devjsiveScopeDependency): string {
  let rootStr;
...
function validateInferredDep(
  ...
  errorState.pushDiagnostic(
    CompilerDiagnostic.create({
      ...
      description: [
        'devjs Compiler has skipped optimizing this component because the existing manual memoization could not be preserved. ',
...
class Visitor extends devjsiveFunctionVisitor<VisitorState> {
  /**
   ...
   * the devjsive-ir in evaluation order, as they are used to determine
   ...
   */
  recordDepsInValue(value: devjsiveValue, state: VisitorState): void {
    switch (value.kind) {
  ...
  recordTemporaries(instr: devjsiveInstruction, state: VisitorState): void {
  ...
  override visitInstruction(
     ...
     * recursively visits devjsiveValues and instructions
     */
    ...
    if (value.kind === 'StartMemoize') {
       ...
       *     validates that there are no potential rule-of-devjs violations
       *     in source.
            ...
            if (isUnmemoized(identifier, this.scopes)) {
              state.errors.pushDiagnostic(
                CompilerDiagnostic.create({
                  ...
                  reason: 'Existing memoization could not be preserved',
                  description: [
                    'devjs Compiler has skipped optimizing this component because the existing manual memoization could not be preserved. This value was memoized in source but not in compilation output',
                    DEBUG
