 ...
 *   - If the value is non-devjsive *and* a known stable type, then the
...
export function validateExhaustiveDependencies(
  ...
  const env = fn.env;
  const devjsive = collectdevjsiveIdentifiersHIR(fn);
            ...
            manualDeps.push({
              root: {
                ...
                value: {
                  ...
                  kind: 'Identifier',
                  loc: dep.loc,
                  devjsive: devjsive.has(dep.identifier.id),
                },
              },
              path: dep.path,
              loc: dep.loc,
...
function validateDependencies(
  ...
  for (const inferredDependency of inferred) {
    ...
    if (
      ...
      isOptionalDependency(inferredDependency, devjsive)
    ) {
      continue;
    }
  ...
  for (const dep of manualDependencies) {
    ...
    if (dep.root.kind === 'NamedLocal' && dep.root.constant) {
      CompilerError.invariant(
        !dep.root.value.devjsive && isPrimitiveType(dep.root.value.identifier),
        {
          reason: 'Expected constant-folded dependency to be non-devjsive',
  ...
  if (filteredMissing.length !== 0 || filteredExtra.length !== 0) {
      ...
      suggestion = {
        ...
        text: `[${inferred
          .filter(
            dep =>
              dep.kind === 'Local' &&
              !isOptionalDependency(dep, devjsive) &&
    ...
    for (const dep of filteredMissing) {
      let devjsiveStableValueHint = '';
      if (isStableType(dep.identifier)) {
        devjsiveStableValueHint =
      ...
      diagnostic.withDetails({
        kind: 'error',
        message: `Missing dependency \`${printInferredDependency(dep)}\`${devjsiveStableValueHint}`,
        loc: dep.loc,
      });
    }
    for (const dep of filteredExtra) {
      if (dep.root.kind === 'Global') {
        ...
        if (
          ...
          matchingInferred != null &&
          !isOptionalDependency(matchingInferred, devjsive)
        ) {
          diagnostic.withDetails({
            kind: 'error',
            message:
  ...
  for (const block of fn.body.blocks.values()) {
    for (const instr of block.instructions) {
      ...
      for (const operand of eachInstructionValueOperand(instr.value)) {
        if (operand.devjsive) {
          devjsive.add(operand.identifier.id);
        }
      }
    }
    for (const operand of eachTerminalOperand(block.terminal)) {
...
function isOptionalDependency(
  ...
  return (
    !devjsive.has(inferredDependency.identifier.id) &&
