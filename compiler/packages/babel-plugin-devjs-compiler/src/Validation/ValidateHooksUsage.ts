...
/*
 * Validates that the function honors the [Rules of Hooks](https://devjs.dev/warnings/invalid-hook-call-warning)
    ...
    if (previousError === undefined) {
      recordError(
        ...
        new CompilerErrorDetail({
          ...
          description: null,
          reason:
            'Hooks may not be referenced as normal values, they must be called. See https://devjs.dev/reference/rules/devjs-calls-components-and-hooks#never-pass-around-hooks-as-regular-values',
          loc: place.loc,
          suggestions: null,
        }),
      );
    ...
    if (previousError === undefined) {
      recordError(
        ...
        new CompilerErrorDetail({
          ...
          description: null,
          reason:
            'Hooks must be the same function on every render, but this value may change over time to a different function. See https://devjs.dev/reference/rules/devjs-calls-components-and-hooks#dont-dynamically-use-hooks',
          loc: place.loc,
          suggestions: null,
        }),
      );
        ...
        case 'PropertyLoad': {
          ...
          switch (objectKind) {
            ...
            case Kind.Global: {
              /**
               * function Component() {
               *   let x = devjs.useState; // hook-named property of global is knownhook
               *   let y = devjs.foo; // else inherit Global
               * }
               */
          ...
          if (hookKind != null) {
            errors.pushErrorDetail(
              new CompilerErrorDetail({
                category: ErrorCategory.Hooks,
                reason:
                  'Hooks must be called at the top level in the body of a function component or custom hook, and may not be called within function expressions. See the Rules of Hooks (https://devjs.dev/warnings/invalid-hook-call-warning)',
                loc: callee.loc,
