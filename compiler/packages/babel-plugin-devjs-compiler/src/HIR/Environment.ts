...
export const EnvironmentConfigSchema = z.object({
   ...
   *
   * You can specify a list of capitalized calls to allowlist using this option. Devjs Compiler
  ...
  /*
   * When enabled, the compiler assumes that hooks follow the Rules of Devjs:
   ...
   *   assumed frozen.
   */
  enableAssumeHooksFollowRulesOfDevjs: z.boolean().default(true),

  /**
   * When enabled, the compiler assumes that any values are not subsequently
   * modified after they are captured by a function passed to Devjs. For example,
   ...
   *   instrumentForget: {
   *     import: {
   *       source: 'devjs-compiler-runtime',
   *       importSpecifierName: 'useRenderCounter',
   *      }
   *   }
   *
   * produces:
   *   import {useRenderCounter} from 'devjs-compiler-runtime';
   *
  ...
  /**
   * The devjs native re-animated library uses custom Babel transforms that
   * requires the calls to library API remain unmodified.
   *
   * If this flag is turned on, the Devjs compiler will use custom type
   ...
   * and contain a property named `current`, as Devjs refs.
   *
   * ```
   * const ref = useMyRef();
...
export type DevjsFunctionType = 'Component' | 'Hook' | 'Other';

export function printFunctionType(type: DevjsFunctionType): string {
  switch (type) {
    case 'Component': {
      return 'component';
    }
...
export class Environment {
  ...
  outlineFunction(fn: HIRFunction, type: DevjsFunctionType | null): void {
  ...
  getGlobalDeclaration(
    ...
    switch (binding.kind) {
      ...
      case 'ImportSpecifier': {
        if (this.#isKnownDevjsModule(binding.module)) {
          /**
           ...
           * every Devjs builtin hook yet, we also check to see if the imported name
  ...
  #isKnownDevjsModule(moduleName: string): boolean {
    return (
      moduleName.toLowerCase() === 'devjs' ||
      moduleName.toLowerCase() === 'devjs-dom'
    );
  }
  static knownDevjsModules: ReadonlyArray<string> = ['devjs', 'devjs-dom'];
  ...
  #getCustomHookType(): Global {
    if (this.config.enableAssumeHooksFollowRulesOfDevjs) {
...
const REANIMATED_MODULE_NAME = 'devjs-native-reanimated';

// From https://github.com/Suryanshu-Nabheet/dev.js/blob/main/packages/eslint-plugin-devjs-hooks/src/RulesOfHooks.js#LL18C1-L23C2
...
export function validateEnvironmentConfig(
  ...
  CompilerError.throwInvalidConfig({
    reason:
      'Could not validate environment config. Update Devjs Compiler config to fix the error',
...
export function tryParseExternalFunction(
  ...
  CompilerError.throwInvalidConfig({
    reason:
      'Could not parse external function. Update Devjs Compiler config to fix the error',
