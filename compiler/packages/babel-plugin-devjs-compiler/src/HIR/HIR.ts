...
import {Environment, DevjsFunctionType} from './Environment';
...
// AST -> (lowering) -> HIR -> (analysis) -> Devjsive Scopes -> (codegen) -> AST
...
/*
 * A Devjs function defines a computation that takes some set of devjsive inputs
 ...
 *      DevjsiveBlockScope*
 *       Place* (dependencies)
 *       (DevjsiveInstruction | DevjsiveTerminal)*
 *
 * Where DevjsiveTerminal may recursively contain zero or more DevjsiveBlocks.
 *
 * Each DevjsiveBlockScope describes a set of dependencies as well as the instructions (and terminals)
 * within that scope.
 */
export type DevjsiveFunction = {
  loc: SourceLocation;
...
export type DevjsiveScopeBlock = {
  kind: 'scope';
  scope: DevjsiveScope;
  instructions: DevjsiveBlock;
};

export type PrunedDevjsiveScopeBlock = {
  kind: 'pruned-scope';
  scope: DevjsiveScope;
  instructions: DevjsiveBlock;
};

export type DevjsiveBlock = Array<DevjsiveStatement>;

export type DevjsiveStatement =
  | DevjsiveInstructionStatement
  | DevjsiveTerminalStatement
  | DevjsiveScopeBlock
  | PrunedDevjsiveScopeBlock;

export type DevjsiveInstructionStatement = {
  kind: 'instruction';
  instruction: DevjsiveInstruction;
};

export type DevjsiveTerminalStatement<
  Tterminal extends DevjsiveTerminal = DevjsiveTerminal,
> = {
  kind: 'terminal';
  terminal: Tterminal;
  label: {
...
export type DevjsiveInstruction = {
  id: InstructionId;
  lvalue: Place | null;
  value: DevjsiveValue;
...
export type DevjsiveValue =
  | InstructionValue
  | DevjsiveLogicalValue
  | DevjsiveSequenceValue
  | DevjsiveTernaryValue
  | DevjsiveOptionalCallValue;

export type DevjsiveLogicalValue = {
...
export type DevjsiveTernaryValue = {
  kind: 'ConditionalExpression';
  test: DevjsiveValue;
  consequent: DevjsiveValue;
  alternate: DevjsiveValue;
  loc: SourceLocation;
};

export type DevjsiveSequenceValue = {
  kind: 'SequenceExpression';
  instructions: Array<DevjsiveInstruction>;
  id: InstructionId;
  value: DevjsiveValue;
  loc: SourceLocation;
};

export type DevjsiveOptionalCallValue = {
...
export type DevjsiveTerminal =
  | DevjsiveBreakTerminal
  | DevjsiveContinueTerminal
  | DevjsiveReturnTerminal
  | DevjsiveThrowTerminal
  | DevjsiveSwitchTerminal
  | DevjsiveDoWhileTerminal
  | DevjsiveWhileTerminal
  | DevjsiveForTerminal
  | DevjsiveForOfTerminal
  | DevjsiveForInTerminal
  | DevjsiveIfTerminal
  | DevjsiveLabelTerminal
  | DevjsiveTryTerminal;

function _staticInvariantDevjsiveTerminalHasLocation(
  terminal: DevjsiveTerminal,
): SourceLocation {
  // If this fails, it is because a variant of DevjsiveTerminal is missing a .loc - add it!
  return terminal.loc;
}

function _staticInvariantDevjsiveTerminalHasInstructionId(
  terminal: DevjsiveTerminal,
): InstructionId {
  // If this fails, it is because a variant of DevjsiveTerminal is missing a .id - add it!
  return terminal.id;
}

export type DevjsiveTerminalTargetKind = 'implicit' | 'labeled' | 'unlabeled';
export type DevjsiveBreakTerminal = {
  kind: 'break';
  target: BlockId;
  id: InstructionId;
  targetKind: DevjsiveTerminalTargetKind;
  loc: SourceLocation;
};
export type DevjsiveContinueTerminal = {
  kind: 'continue';
  target: BlockId;
  id: InstructionId;
  targetKind: DevjsiveTerminalTargetKind;
...
export type DevjsiveReturnTerminal = {
  kind: 'return';
  value: Place;
  id: InstructionId;
  loc: SourceLocation;
};
export type DevjsiveThrowTerminal = {
  kind: 'throw';
  value: Place;
  id: InstructionId;
  loc: SourceLocation;
};
export type DevjsiveSwitchTerminal = {
  kind: 'switch';
  test: Place;
  cases: Array<{
    test: Place | null;
    block: DevjsiveBlock | void;
  }>;
  id: InstructionId;
  loc: SourceLocation;
};
export type DevjsiveDoWhileTerminal = {
  kind: 'do-while';
  loop: DevjsiveBlock;
  test: DevjsiveValue;
  id: InstructionId;
...
export type DevjsiveWhileTerminal = {
  kind: 'while';
  test: DevjsiveValue;
  loop: DevjsiveBlock;
  id: InstructionId;
...
export type DevjsiveForTerminal = {
  kind: 'for';
  init: DevjsiveValue;
  test: DevjsiveValue;
  update: DevjsiveValue | null;
  loop: DevjsiveBlock;
  id: InstructionId;
  loc: SourceLocation;
};
export type DevjsiveForOfTerminal = {
  kind: 'for-of';
  init: DevjsiveValue;
  test: DevjsiveValue;
  loop: DevjsiveBlock;
...
export type DevjsiveForInTerminal = {
  kind: 'for-in';
  init: DevjsiveValue;
  loop: DevjsiveBlock;
  id: InstructionId;
...
export type DevjsiveIfTerminal = {
  kind: 'if';
  test: Place;
  consequent: DevjsiveBlock;
  alternate: DevjsiveBlock | null;
  id: InstructionId;
  loc: SourceLocation;
};
export type DevjsiveLabelTerminal = {
  kind: 'label';
  block: DevjsiveBlock;
  id: InstructionId;
  loc: SourceLocation;
};
export type DevjsiveTryTerminal = {
  kind: 'try';
  block: DevjsiveBlock;
  handlerBinding: Place | null;
  handler: DevjsiveBlock;
  id: InstructionId;
  loc: SourceLocation;
};
...
/*
 * Each devjsive scope may have its own control-flow, so the instructions form
 ...
 * can occur pervasively and Devjs runtime is responsible for resetting state when
...
export type DevjsiveScopeTerminal = {
  kind: 'scope';
  fallthrough: BlockId;
  block: BlockId;
  scope: DevjsiveScope;
 ...
 * and enforced by the `devjs-hooks/exhaustive-deps` rule.
 *
...
export type Identifier = {
  ...
  /*
   * The ID of the devjsive scope which will compute this value. Multiple
   * variables may have the same scope id.
   */
  scope: DevjsiveScope | null;
  type: Type;
  loc: SourceLocation;
};
...
export enum ValueReason {
  /**
   * Defined outside the Devjs function.
   */
  Global = 'global',
   ...
   */
  DevjsiveFunctionArgument = 'devjsive-function-argument',
...
export const ValueReasonSchema = z.enum([
  ...
  ValueReason.Other,
  ValueReason.DevjsiveFunctionArgument,
...
export type DevjsiveScope = {
  id: ScopeId;
  range: MutableRange;

  /**
   * The inputs to this devjsive scope
   */
  dependencies: DevjsiveScopeDependencies;

  /**
   * The set of values produced by this scope. This may be empty
   * for scopes that produce reassignments only.
   */
  declarations: Map<IdentifierId, DevjsiveScopeDeclaration>;
  ...
  /**
   * Devjsive scopes may contain a return statement, which needs to be replayed
   * whenever the inputs to the scope have not changed since the previous execution.
   * If the devjsive scope has an early return, this variable stores the temporary
   * identifier to which the return value will be assigned. See PropagateEarlyReturns
   * for more about how early returns in devjsive scopes are compiled and represented.
   *
...
export type DevjsiveScopeDependencies = Set<DevjsiveScopeDependency>;

export type DevjsiveScopeDeclaration = {
  identifier: Identifier;
  scope: DevjsiveScope; // the scope in which the variable was originally declared
};
...
export type DevjsiveScopeDependency = {
  identifier: Identifier;
  /**
   * Reflects whether the base identifier is devjsive. Note that some devjsive
   * objects may have non-devjsive properties, but we do not currently track
   * this.
   *
   * ```js
   * // Technically, result[0] is devjsive and result[1] is not.
   * // Currently, both dependencies would be marked as devjsive.
...
function isScopeActive(scope: DevjsiveScope, id: InstructionId): boolean {
