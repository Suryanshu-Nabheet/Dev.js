...
import {isMutable} from '../DevjsiveScopes/InferDevjsiveScopeVariables';
...
/**
 * While previous passes ensure that devjsive scopes span valid sets of program
 * blocks, pairs of devjsive scopes may still be inconsistent with respect to
 * each other.
 *
 * (a) Devjsive scopes ranges must form valid blocks in the resulting javascript
 ...
 * For each devjsive scope, we currently produce exactly one if-block which
 ...
 *   of a devjsive scope from disconnected instruction ranges.
 */

export function mergeOverlappingDevjsiveScopesHIR(fn: HIRFunction): void {
  /**
   * Collect all scopes eagerly because some scopes begin before the first
   * instruction that references them (due to alignDevjsiveScopesToBlocks)
   */
   ...
   */
  const joinedScopes = getOverlappingDevjsiveScopes(fn, scopesInfo);
...
type TraversalState = {
  joined: DisjointSet<DevjsiveScope>;
  activeScopes: Array<DevjsiveScope>;
};

function collectScopeInfo(fn: HIRFunction): ScopeInfo {
  const scopeStarts: Map<InstructionId, Set<DevjsiveScope>> = new Map();
  const scopeEnds: Map<InstructionId, Set<DevjsiveScope>> = new Map();
  const placeScopes: Map<Place, DevjsiveScope> = new Map();
...
function visitInstructionId(
  ...
  if (scopeStartTop != null && scopeStartTop.id <= id) {
     ...
     * as they end up with the same devjsive block
     */
...
function getOverlappingDevjsiveScopes(
  fn: HIRFunction,
  context: ScopeInfo,
): DisjointSet<DevjsiveScope> {
  const state: TraversalState = {
    joined: new DisjointSet<DevjsiveScope>(),
    activeScopes: [],
  };
