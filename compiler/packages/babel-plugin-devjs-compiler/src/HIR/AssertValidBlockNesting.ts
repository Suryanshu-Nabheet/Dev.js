 ...
 * ```
 *
 * Scope = devjsive scope whose range has been correctly aligned and merged.
 */
type Block =
  | ({
...
export function getScopes(fn: HIRFunction): Set<DevjsiveScope> {
  const scopes: Set<DevjsiveScope> = new Set();
