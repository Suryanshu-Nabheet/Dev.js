...
import {printDevjsiveScopeSummary} from '../DevjsiveScopes/PrintDevjsiveFunction';
...
export function printInstruction(instr: DevjsiveInstruction): string {
  const id = `[${instr.id}]`;
...
export function printTerminal(terminal: Terminal): Array<string> | string {
  ...
  switch (terminal.kind) {
    ...
    case 'scope': {
      value = `[${terminal.id}] Scope ${printDevjsiveScopeSummary(
        terminal.scope,
      )} block=bb${terminal.block} fallthrough=bb${terminal.fallthrough}`;
      break;
    }
    case 'pruned-scope': {
      value = `[${terminal.id}] <pruned> Scope ${printDevjsiveScopeSummary(
        terminal.scope,
      )} block=bb${terminal.block} fallthrough=bb${terminal.fallthrough}`;
      break;
    }
...
export function printInstructionValue(instrValue: DevjsiveValue): string {
  let value = '';
...
function printScope(scope: DevjsiveScope | null): string {
