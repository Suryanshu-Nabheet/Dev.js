 ...
 */
export class DevjsiveScopeDependencyTreeHIR {
  /**
   ...
   */
  #hoistableObjects: Map<Identifier, HoistableNode & {devjsive: boolean}> =
    new Map();
  #deps: Map<Identifier, DependencyNode & {devjsive: boolean}> = new Map();
   ...
   */
  constructor(hoistableObjects: Iterable<DevjsiveScopeDependency>) {
    for (const {path, identifier, devjsive, loc} of hoistableObjects) {
      let currNode = DevjsiveScopeDependencyTreeHIR.#getOrCreateRoot(
        identifier,
        devjsive,
  ...
  static #getOrCreateRoot<T extends string>(
    ...
    devjsive: boolean,
    roots: Map<Identifier, TreeNode<T> & {devjsive: boolean}>,
    defaultAccessType: T,
    loc: SourceLocation,
  ): TreeNode<T> {
    ...
    if (rootNode === undefined) {
      ...
      CompilerError.invariant(devjsive === rootNode.devjsive, {
        reason: '[DeriveMinimalDependenciesHIR] Conflicting devjsive root flag',
   ...
   */
  addDependency(dep: DevjsiveScopeDependency): void {
    const {identifier, devjsive, path, loc} = dep;
    let depCursor = DevjsiveScopeDependencyTreeHIR.#getOrCreateRoot(
      identifier,
      devjsive,
      this.#deps,
  ...
  deriveMinimalDependencies(): Set<DevjsiveScopeDependency> {
    const results = new Set<DevjsiveScopeDependency>();
...
function collectMinimalDependenciesInSubtree(
  ...
  results: Set<DevjsiveScopeDependency>,
): void {
  if (isDependency(node.accessType)) {
    results.add({identifier: rootIdentifier, devjsive, path, loc: node.loc});
  } else {
