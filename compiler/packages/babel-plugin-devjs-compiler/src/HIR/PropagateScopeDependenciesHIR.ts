...
import {DevjsiveScopeDependencyTreeHIR} from './DeriveMinimalDependenciesHIR';
...
export function propagateScopeDependenciesHIR(fn: HIRFunction): void {
  ...
  for (const [scope, deps] of scopeDeps) {
     ...
     */
    const tree = new DevjsiveScopeDependencyTreeHIR(
 ...
 *
 * If a LoadLocal or PropertyLoad instruction is within the devjsive scope range
...
): ReadonlyMap<IdentifierId, DevjsiveScopeDependency> {
...
function collectTemporariesSidemapImpl(
  fn: HIRFunction,
  usedOutsideDeclaringScope: ReadonlySet<DeclarationId>,
  temporaries: Map<IdentifierId, DevjsiveScopeDependency>,
...
function getProperty(
  ...
  optional: boolean,
  loc: SourceLocation,
  temporaries: ReadonlyMap<IdentifierId, DevjsiveScopeDependency>,
): DevjsiveScopeDependency {
  /*
   ...
   */
  let property: DevjsiveScopeDependency;
  if (resolvedDependency == null) {
    property = {
      identifier: object.identifier,
      devjsive: object.devjsive,
    ...
    property = {
      ...
      devjsive: resolvedDependency.devjsive,
      path: [
...
export class DependencyCollectionContext {
  ...
  #scopes: Stack<DevjsiveScope> = empty();
  // Devjsive dependencies used in the current devjsive scope.
  #dependencies: Stack<Array<DevjsiveScopeDependency>> = empty();
  deps: Map<DevjsiveScope, Array<DevjsiveScopeDependency>> = new Map();
  ...
  #checkValidDependency(maybeDependency: DevjsiveScopeDependency): boolean {
  ...
  #isScopeActive(scope: DevjsiveScope): boolean {
    if (this.#scopes === null) {
      return false;
    }
  ...
  get currentScope(): Stack<DevjsiveScope> {
    return this.#scopes;
  }
  ...
  visitDependency(maybeDependency: DevjsiveScopeDependency): void {
    /*
    ...
    if (
      ...
      maybeDependency = {
        identifier: maybeDependency.identifier,
        devjsive: maybeDependency.devjsive,
        path: [],
   ...
   * current one as a {@link DevjsiveScope.reassignments}
   */
...
export function handleInstruction(
  ...
  if (value.kind === 'PropertyLoad') {
     ...
     * (and hoist) these declarations if they are included in a devjsive scope.
...
function collectDependencies(
  ...
  temporaries: ReadonlyMap<IdentifierId, DevjsiveScopeDependency>,
  processedInstrsInOptional: ReadonlySet<Instruction | Terminal>,
): Map<DevjsiveScope, Array<DevjsiveScopeDependency>> {
