...
import {inRange} from '../DevjsiveScopes/InferDevjsiveScopeVariables';
import {printDependency} from '../DevjsiveScopes/PrintDevjsiveFunction';
import {
  Set_equal,
  Set_filter,
  Set_intersect,
...
export function collectHoistablePropertyLoads(
  ...
  hoistableFromOptionals: ReadonlyMap<BlockId, DevjsiveScopeDependency>,
...
type RootNode = {
  ...
  fullPath: DevjsiveScopeDependency;
  hasOptional: boolean;
  root: IdentifierId;
};
...
class PropertyPathRegistry {
  ...
  getOrCreateIdentifier(
    ...
    if (rootNode === undefined) {
      ...
      CompilerError.invariant(devjsive === rootNode.fullPath.devjsive, {
        reason:
          '[HoistablePropertyLoads] Found inconsistencies in `devjsive` flag when deduping identifier reads within the same scope',
        loc: identifier.loc,
      });
    }
    return rootNode;
    ...
    if (child == null) {
      child = {
        ...
        fullPath: {
          ...
          devjsive: parent.fullPath.devjsive,
  ...
  getOrCreateProperty(n: DevjsiveScopeDependency): PropertyPathNode {
    /**
     * We add DevjsiveScopeDependencies according to instruction ordering,
     * so all subpaths of a PropertyLoad should already exist
     * (e.g. a.b is added before a.b.c),
     */
    let currNode = this.getOrCreateIdentifier(n.identifier, n.devjsive, n.loc);
    if (n.path.length === 0) {
      return currNode;
    }
...
function isImmutableAtInstr(
  ...
  if (context.nestedFnImmutableContext != null) {
    ...
    /**
     * Since this runs *after* buildDevjsiveScopeTerminals, identifier mutable ranges
     * are not valid with respect to current instruction id numbering.
     * We use attached devjsive scope ranges as a proxy for mutable range, but this
          ...
          if (dep.root.kind === 'NamedLocal') {
            ...
            for (let i = 0; i < dep.path.length; i++) {
              ...
              const depNode = context.registry.getOrCreateProperty({
                ...
                devjsive: dep.root.value.devjsive,
                loc: dep.loc,
              });
  ...
  for (const block of fn.body.blocks.values()) {
    for (const {lvalue, value} of block.instructions) {
      if (value.kind === 'CallExpression') {
         ...
         * or pass already declared functions to devjs (e.g. as JSXAttributes).
         *
