...
export function inferMutationAliasingEffects(
  ...
  const paramKind: AbstractValue = isFunctionExpression
    ...
    : {
        ...
        reason: new Set([ValueReason.devjsiveFunctionArgument]),
      };

  if (fn.fnType === 'Component') {
    CompilerError.invariant(fn.params.length <= 2, {
      reason:
        'Expected devjs component to have not more than two parameters: one for props and for ref',
      loc: fn.loc,
    });
    ...
    switch (variant) {
      ...
      case 'MutateTransitive': {
        switch (kind) {
          ...
          case ValueKind.Primitive: {
            // technically an error, but it's not devjs specific
            return 'none';
          }
          case ValueKind.Frozen: {
    ...
    case 'StoreGlobal': {
      ...
      effects.push({
        ...
        error: CompilerDiagnostic.create({
          ...
          description: `Variable ${variable} is declared outside of the component/hook. Reassigning this value during render is a form of side effect, which can cause unpredictable behavior depending on when the component happens to re-render. If this variable is used in rendering, use useState instead. Otherwise, consider updating it in an effect. (https://devjs.dev/reference/rules/components-and-hooks-must-be-pure#side-effects-must-run-outside-of-render)`,
        }).withDetails({
          kind: 'error',
          loc: instr.loc,
      ...
      error: CompilerDiagnostic.create({
        ...
        description:
          (signature.canonicalName != null
            ...
            : '') +
          'Calling an impure function can produce unstable results that update unpredictably when the component happens to re-render. (https://devjs.dev/reference/rules/components-and-hooks-must-be-pure#components-and-hooks-must-be-idempotent)',
      }).withDetails({
        kind: 'error',
        loc,
      ...
      CompilerDiagnostic.create({
        ...
        description: [
          'This API returns functions which cannot be memoized without leading to stale UI. ' +
            'To prevent this, by default devjs Compiler will skip memoizing this component/hook. ' +
...
export function getWriteErrorReason(abstractValue: AbstractValue): string {
  if (abstractValue.reason.has(ValueReason.Global)) {
  ...
  } else if (abstractValue.reason.has(ValueReason.devjsiveFunctionArgument)) {
    ...
    return 'This modifies a variable that devjs considers immutable';
  }
}
