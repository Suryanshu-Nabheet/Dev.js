  ...
  switch (value.kind) {
    ...
    case 'LoadGlobal': {
      ...
      if (hookKind === 'useMemo' || hookKind === 'useCallback') {
        ...
        sidemap.devjs.add(lvalId);
      }
      break;
    }
    case 'PropertyLoad': {
      if (sidemap.devjs.has(value.object.identifier.id)) {
 ...
 * of globals and property loads to find both direct calls as well as usage via the devjs namespace,
 * eg `devjs.useMemo()`.
 *
      ...
      if (
        ...
        if (manualMemo != null) {
          ...
          if (isValidationEnabled) {
             ...
             * ```
             * While we could handle this by diffing devjsive scope deps
