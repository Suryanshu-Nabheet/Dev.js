...
const PanicThresholdOptionsSchema = z.enum([
   ...
   * If Forget is invoked through `BabelPlugindevjsCompiler`, this will at the least
...
export type PluginOptions = Partial<{
   ...
   * For example:
   *   gating: {
   *     source: 'devjsForgetFeatureFlag',
   *     importSpecifierName: 'isForgetEnabled_Pokes',
   *   }
   *
   * produces:
   *   import {isForgetEnabled_Pokes} from 'devjsForgetFeatureFlag';
   *
   *   Foo_forget()   {}
   *
   *   Foo_uncompiled() {}
  ...
  /**
   * By default devjs Compiler will skip compilation of code that suppresses the default
   * devjs ESLint rules, since this is a strong indication that the code may be breaking devjs rules
   * in some way.
   *
  ...
  /**
   * The compiler has customized support for devjs-native-reanimated, intended as a temporary workaround.
   * Set this flag (on by default) to automatically check for this library and activate the support.
   */
  enableReanimatedCheck: boolean;

  /**
   * The minimum major version of devjs that the compiler should emit code for. If the target is 19
   * or higher, the compiler emits direct imports of devjs runtime APIs needed by the compiler. On
   * versions prior to 19, an extra runtime package devjs-compiler-runtime is necessary to provide
...
const CompilerdevjsTargetSchema = z.union([
  z.literal('17'),
  z.literal('18'),
  z.literal('19'),
  /**
   * Used exclusively for Meta apps which are guaranteed to have compatible
   * devjs runtime and compiler versions. Note that only the FB-internal bundles
   * re-export useMemoCache (see
   * https://github.com/Suryanshu-Nabheet/dev.js/blob/5b0ef217ef32333a8e56f39be04327c89efa346f/packages/devjs/index.fb.js#L68-L70),
   * so this option is invalid / creates runtime errors for open-source users.
   */
  z.object({
    kind: z.literal('donotuse_meta_internal'),
    runtimeModule: z.string().default('devjs'),
  }),
]);
export type CompilerdevjsTarget = z.infer<typeof CompilerdevjsTargetSchema>;
      ...
      switch (key) {
        ...
        case 'dynamicGating': {
          if (value == null) {
            ...
            if (result.success) {
              ...
              CompilerError.throwInvalidConfig({
                reason:
                  'Could not parse dynamic gating. Update devjs Compiler config to fix the error',
        ...
        case 'customOptOutDirectives': {
          ...
          if (result.success) {
            ...
            CompilerError.throwInvalidConfig({
              reason:
                'Could not parse custom opt out directives. Update devjs Compiler config to fix the error',
              description: `${fromZodError(result.error)}`,
              loc: null,
              suggestions: null,
            });
...
export function parseTargetConfig(value: unknown): CompilerdevjsTarget {
  const parsed = CompilerdevjsTargetSchema.safeParse(value);
  if (parsed.success) {
    return parsed.data;
  } else {
    CompilerError.throwInvalidConfig({
