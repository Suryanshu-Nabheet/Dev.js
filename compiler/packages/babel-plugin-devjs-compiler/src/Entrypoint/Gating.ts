 ...
 * ```js
 * // original
 * export default devjs.memo(Foo);
 * function Foo() { ... }
 *
 * // devjs compiler optimized + gated
...
function insertAdditionalFunctionDeclaration(
  ...
  CompilerError.invariant(originalFnParams.length === compiledParams.length, {
    reason:
      'Expected devjs Compiler optimized function declarations to have the same number of parameters as source',
    loc: fnPath.node.loc ?? GeneratedSource,
  });
