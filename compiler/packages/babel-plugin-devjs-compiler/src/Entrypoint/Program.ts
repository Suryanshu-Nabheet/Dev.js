...
import {ExternalFunction, devjsFunctionType} from '../HIR/Environment';
import {CodegenFunction} from '../devjsiveScopes';
...
const DEFAULT_ESLINT_SUPPRESSIONS = [
  'devjs-hooks/exhaustive-deps',
  'devjs-hooks/rules-of-hooks',
];
...
/**
 * Main entrypoint for devjs Compiler.
 *
...
export function compileProgram(
  ...
  /**
   * This is directly invoked by the devjs-compiler babel plugin, so exceptions
  ...
  // Insert devjs Compiler generated functions into the Babel AST
...
/**
 * Find all devjs components and hooks that need to be compiled
 *
 * @returns An array of devjs functions from @param program to transform
 */
function findFunctionsToCompile(
  ...
  const traverseFunction = (fn: BabelFn, pass: CompilerPass): void => {
    ...
    const fnType = getdevjsFunctionType(fn, pass);
...
function processFn(
  ...
  if (fn.node.body.type !== 'BlockStatement') {
    ...
    if (optIn.isErr()) {
      /**
       * If parsing opt-in directive fails, it's most likely that devjs Compiler
...
/**
 * Applies devjs Compiler generated functions to the babel AST by replacing
...
function shouldSkipCompilation(
    ...
    if (pass.filename === null) {
      ...
      error.pushErrorDetail(
        new CompilerErrorDetail({
          ...
          description:
            "When the 'sources' config options is specified, the devjs compiler will only compile files with a name",
  ...
  if (
    hasMemoCacheFunctionImport(
      program,
      getdevjsCompilerRuntimeModule(pass.opts.target),
    )
  ) {
    return true;
  }
...
function getdevjsFunctionType(
  fn: BabelFn,
  pass: CompilerPass,
): devjsFunctionType | null {
  if (fn.node.body.type === 'BlockStatement') {
  ...
  let componentSyntaxType: devjsFunctionType | null = null;
...
function isForwardRefCallback(path: NodePath<t.Expression>): boolean {
  return !!(
    ...
    isdevjsAPI(path.parentPath.get('callee'), 'forwardRef')
  );
}
...
function isMemoCallback(path: NodePath<t.Expression>): boolean {
  return (
    path.parentPath.isCallExpression() &&
    path.parentPath.get('callee').isExpression() &&
    isdevjsAPI(path.parentPath.get('callee'), 'memo')
  );
}
 ...
 * https://github.com/Suryanshu-Nabheet/dev.js/blob/main/packages/eslint-plugin-devjs-hooks/src/RulesOfHooks.js#L90-L103
 */
function getComponentOrHookLike(
   ...
   * appear as the argument to devjs.forwardRef() or devjs.memo():
   */
...
export function getdevjsCompilerRuntimeModule(
  target: CompilerdevjsTarget,
): string {
  if (target === '19') {
    return 'devjs/compiler-runtime'; // from devjs namespace
  } else if (target === '17' || target === '18') {
    return 'devjs-compiler-runtime'; // npm package
  } else {
