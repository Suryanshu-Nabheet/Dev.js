 ...
 * of the file has potential devjs violations.
 */
...
export function findProgramSuppressions(
  ...
  const flowSuppressionPattern = new RegExp(
    '\\$(FlowFixMe\\w*|FlowExpectedError|FlowIssue)\\[devjs\\-rule',
  );
...
export function suppressionsToCompilerError(
  ...
  for (const suppressionRange of suppressionRanges) {
    ...
    switch (suppressionRange.source) {
      case 'Eslint':
        reason =
          'devjs Compiler has skipped optimizing this component because one or more devjs ESLint rules were disabled';
        suggestion =
          'Remove the ESLint suppression and address the devjs error';
        break;
      case 'Flow':
        reason =
          'devjs Compiler has skipped optimizing this component because one or more devjs rule violations were reported by Flow';
        suggestion = 'Remove the Flow suppression and address the devjs error';
        break;
      default:
        assertExhaustive(
    ...
    error.pushDiagnostic(
      CompilerDiagnostic.create({
        reason: reason,
        description: `devjs Compiler only works when your components follow all the rules of devjs, disabling them may result in unexpected or incorrect behavior. Found suppression \`${suppressionRange.disableComment.value.trim()}\``,
        ...
        message: 'Found devjs rule suppression',
      }),
    );
  }
  return error;
