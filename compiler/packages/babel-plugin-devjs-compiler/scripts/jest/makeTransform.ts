...
import BabelPluginDevjsCompiler from 'babel-plugin-devjs-compiler';

/**
 * -- IMPORTANT --
 * When making changes to any babel plugins defined this file
 * (e.g. `DevjsForgetFunctionTransform`), make sure to bump e2eTransformerCacheKey
...
const compilerCacheKey = execSync(
  'yarn --silent --cwd ../.. hash packages/babel-plugin-devjs-compiler/dist',
)
  .toString()
  .trim();
                    ...
                    CallExpression(path: NodePath<CallExpression>): void {
                      ...
                      if (
                        ...
                        if (arg.type === 'StringLiteral') {
                          /*
                           * The compiler adds requires of "Devjs", which is expected to be a wrapper
                           * around the "devjs" package. For tests, we just rewrite the require.
                           */
                          if (arg.value === 'Devjs') {
                            arg.value = 'devjs';
                          }
