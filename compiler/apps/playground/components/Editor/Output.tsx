...
} from '@heroicons/devjs/outline';
import MonacoEditor, {DiffEditor} from '@monaco-editor/devjs';
import {
  CompilerErrorDetail,
  CompilerDiagnostic,
  type CompilerError,
...
const tabifyCache = new LRUCache<Store, Promise<Map<string, devjsNode>>>({
  max: 5,
});
...
export type PrintedCompilerPipelineValue =
  ...
  | {kind: 'devjsive'; name: string; fnName: string | null; value: string}
...
async function tabify(
...
): Promise<Map<string, devjsNode>> {
  const tabs = new Map<string, devjs.devjsNode>();
  const reorderedTabs = new Map<string, devjs.devjsNode>();
  ...
  let nonDiffPasses = ['HIR', 'BuilddevjsiveFunction', 'EnvironmentConfig'];
...
devjs Compiler compiled this function successfully, but there are lint errors that indicate potential issues with the original code.
...
function OutputContent({store, compilerOutput}: Props): JSX.Element {
  ...
  for (const [passName, results] of compilerOutput.results) {
    for (const result of results) {
      let currResult = '';
      if (result.kind === 'hir' || result.kind === 'devjsive') {
