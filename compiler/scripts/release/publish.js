 ...
 * be.
 *
 * Please run this first (ie `pnpm npm:publish`) and double check the contents of the files that
 * will be pushed to npm.
 *
 * If it looks good, you can run `pnpm npm:publish --for-real` to really publish to npm. You must
 ...
 *
 * Optionally, you can add the `--debug` flag to `pnpm npm:publish --debug --for-real` to run all
