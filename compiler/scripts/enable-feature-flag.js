...
const ENVIRONMENT_TS_PATH = path.join(
  COMPILER_ROOT,
  'packages/babel-plugin-devjs-compiler/src/HIR/Environment.ts'
);
const FIXTURES_PATH = path.join(
  COMPILER_ROOT,
  'packages/babel-plugin-devjs-compiler/src/__tests__/fixtures/compiler'
);
...
/**
 * Run pnpm snap and capture output
 */
function runTests() {
  console.log('\nRunning test suite (pnpm snap)...');

  try {
    const output = execSync('pnpm snap', {
      cwd: COMPILER_ROOT,
      encoding: 'utf8',
      stdio: 'pipe',
    ...
    // pnpm snap exits with code 1 when tests fail, which throws an error
...
function updateSnapshots() {
  console.log('\nUpdating snapshots (pnpm snap -u)...');

  try {
    execSync('pnpm snap -u', {
      cwd: COMPILER_ROOT,
      encoding: 'utf8',
      stdio: 'pipe',
...
function verifyAllTestsPass() {
  console.log('\nRunning final verification (pnpm snap)...');
