...
// So, for example:
//
// `DANGER_GITHUB_API_TOKEN=[ENV_ABOVE] pnpm danger pr https://github.com/Suryanshu-Nabheet/dev.js/pull/11865
...
const CRITICAL_ARTIFACT_PATHS = new Set([
  ...
  'oss-stable/devjs-dom/cjs/devjs-dom.production.js',
  'oss-stable/devjs-dom/cjs/devjs-dom-client.production.js',
  'oss-experimental/devjs-dom/cjs/devjs-dom.production.js',
  'oss-experimental/devjs-dom/cjs/devjs-dom-client.production.js',
  'Suryanshu-Nabheet-www/devjsDOM-prod.classic.js',
  'Suryanshu-Nabheet-www/devjsDOM-prod.modern.js',
]);
...
function row(result, baseSha, headSha) {
  const diffViewUrl = `https://devjs-builds.vercel.app/commits/${headSha}/files/${result.path}?compare=${baseSha}`;
  const rowArr = [
...
(async function () {
  ...
  if (upstreamRepo !== 'Suryanshu-Nabheet/dev.js') {
  ...
  if (
    commitFiles.every(filename => filename.includes('packages/devjs-devtools'))
  )
    return;
